from midi_to_dataframe.midi.containers import Track, Pattern
from midi_to_dataframe.midi.events import NoteOnEvent, NoteOffEvent, EndOfTrackEvent, SetTempoEvent, ProgramChangeEvent
from midi_to_dataframe.midi.fileio import write_midifile
...
class MidiWriter(object):
    ...
    def convert_to_midi(self, dataframe, save_to_path):
            ...
            for note_word in note_words:
                if note_word != REST:
                    ...
                    if midi_track is not None:
                        ...
                        on_event = NoteOnEvent(velocity=127, pitch=key, channel=track_num)
                        ...
                        off_event = NoteOffEvent(pitch=key, channel=track_num)
        ...
        for track_num, midi_track in self._midi_tracks_by_track_num.items():
            midi_track.append(EndOfTrackEvent(tick=1))

        # Write MIDI file to disk
        write_midifile(save_to_path, pattern)
    ...
    def _add_track(self, track_index, program_name, pattern):
        ...
        if program_name == PERCUSSION:
            ...
            # Set tempo (BPM)
            tempo_event = SetTempoEvent(tick=0, bpm=int(self._bpm))
            track.append(tempo_event)

            # Set drum channel
            track.append(ProgramChangeEvent(tick=0, channel=PERCUSSION_CHANNEL))
        else:
            program_num = self._note_mapper.get_program_number(program_name)
            program_change_event = ProgramChangeEvent(tick=0, data=[program_num])
