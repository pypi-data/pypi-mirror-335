...
from midi_to_dataframe.midi.events import ProgramChangeEvent, SetTempoEvent, TimeSignatureEvent, NoteOffEvent, NoteOnEvent
from midi_to_dataframe.midi.fileio import read_midifile
...
class MidiReader(object):
    ...
    def _process_midi_event(self, event, program):
        ...
        if (type(event) == NoteOnEvent or type(event) == NoteOffEvent) and event.channel == MIDI_DRUM_CHANNEL:
            program = DEFAULT_MIDI_PROGRAM_NUM
        elif (type(event) == NoteOnEvent or type(event) == NoteOffEvent) and program is None:
            # If program was never set, default to 1 (Piano)
            program = 1

        # True Note On events have positive velocity
        if type(event) == NoteOnEvent and event.velocity > 0:
