...
class TestNoAgg(unittest.TestCase):
     def test_node_edge_aggregation_both(self):
         """Provide a small in-memory HyGraph with a couple of edges for node-level aggregation."""
         self.hy = HyGraph()

         # Create three stations
         self.hy.add_pgnode("N1", label="Station", start_time=datetime(2024, 1, 1))
         self.hy.add_pgnode("N2", label="Station", start_time=datetime(2024, 1, 1))
         self.hy.add_pgnode("N3", label="Station", start_time=datetime(2024, 1, 1))
         self.hy.add_pgnode("N4", label="X", start_time=datetime(2024, 1, 1))

         # Create edges labeled 'trip'
         self.hy.add_pgedge("E1", "N1", "N2", "trip", start_time=datetime(2024, 1, 1))
         self.hy.add_pgedge("E2", "N2", "N3", "trip", start_time=datetime(2024, 1, 1))
         self.hy.add_pgedge("E3", "N1", "N3", "trip", start_time=datetime(2024, 1, 2))
         self.hy.add_pgedge("E4", "N1", "N3", "tRTrip", start_time=datetime(2024, 1, 2))
         def active_ride_aggregator(hy, element_type, oid, attribute, dt):
             ...
             if et and dt >= et:
                 return 0
             return 1
         # We want to store for each node the sum of in+out edges at each day
         query_node_agg = {
             'element_type': 'node',
             'node_filter': lambda n: n['data'].oid=="N1",
             'node_settings': {
             'include_edges': ['trip']  # Only edges with label='trip'
               },
             'time_series_config': {
                 'start_date': datetime(2024, 1, 1),
                 'end_date': datetime(2024, 1, 3),
                 'freq': 'D',
                 'attribute': 'node_edge_sum',
                 'aggregate_function': active_ride_aggregator,
                 'direction': 'both',  # <--- gather from in+out edges
                 'use_actual_timestamps': False
             }
         }

         self.hy.create_time_series_from_graph(query_node_agg)

         # Now each node should have a new property: node_edge_sum
         # and we can check the time series length for each node
         pgnodes = self.hy.get_all_pgnodes()
         print(pgnodes)
         assert len(pgnodes) == 3, "We have 3 PGNodes."
         print(pgnodes)
         for (node_id, node_data) in pgnodes:
             node_obj = node_data['data']
             # Check property
             assert 'node_edge_sum' in node_obj.temporal_properties, f"Missing TS property on node {node_id}"
             ts_prop = node_obj.temporal_properties['node_edge_sum'].get_time_series()
             # We expect 3 daily points from 2024-01-01 to 2024-01-03
             assert ts_prop.data.shape[0] == 3, "Should have 3 daily points."
