...
from application.epex_sftp.env_vars import *

class EpexSftpDataSourcePlugin(DataSource):
    ...
    def read_data(self) -> None:
        start_date_formatted = date_parser.parse(start_date).date()
        if self.table_name == "intradayconhistorical":
            ...
            source_path, file_name = self._adjust_epex_source_path_filename(start_date_formatted)
        ...
        for sp in source_path_list:
            ...
            if self.table_name in ['paneuropeanprices', 'paneuropeanvolumes']:
                ...
                sftp_files_list = self.sftp.pull_obj_ids_between(sp, start_date, end_date)
    ...
    def transform_data(self) -> None:
        for key, df in self.dfs.items():
            if self.table in ['intradayauctionpriceshh', 'intradayauctionpricesqh', 'intradayauctionpriceshourly', 'auctionspotprices', 'auctionspotpriceshh', 'paneuropeanprices']:
                currency_code = description_line.split(" ")[-1]
    ...
    def _get_dates_between(self):
        ...
        start_date = datetime.strptime(parser.parse(env_start_date).strftime('%d/%m/%Y'), '%d/%m/%Y')
        logging.info(f'START DATE: {start_date}')
        end_date = datetime.strptime(parser.parse(env_end_date).strftime('%d/%m/%Y'), '%d/%m/%Y')
