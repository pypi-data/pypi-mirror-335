...
class EpexSftpPlugin(DataSource):
    ...
    def read_data(self) -> None:
        ...
        if self.table_name == "intradayconhistorical":
            ...
            self.source_path, self.file_name = self._adjust_epex_source_path_filename(start_date_formatted)
        init_ValidateObject = ValidateObject()
        delimiter = init_ValidateObject.get_delimiter(self.pipeline_conf['ingestion']['file']['delimiter'])
        ...
        for sp in source_path_list:
            ...
            valid_sftp_list, _ = init_ValidateObject.validate_file_list(sftp_files_list, self.file_extension)
            download_f = [_ for _ in valid_sftp_list if self.file_name.split(';')[0] in _]
            logging.info(valid_sftp_list)
            logging.info(sp)
            for key, response in self.sftp.read_sftp_files_as_strings(download_f, sp).items():


                lines_in_file = response.split("\n")

                if self.table_name == 'intradaycon' or self.table_name == 'intradayconhistorical':
                    country_name = key.split("-")[1]
                elif self.table_name in ['paneuropeanvolumes', 'paneuropeanprices']:
                    country_name = key.split("_")[-3]
                else:
                    country_name = key.split("_")[-2]
                    if country_name == "luxembourg" and (self.table_name == 'auctionspotprices' or self.table_name == 'auctionspotvolumes'):
                ...
                self.matcher[key] = description_line
        ...
        for key, df in self.dfs.items():
            if self.table_name == 'intradaycon' or self.table_name == 'intradayconhistorical':
                ...
                if country_name == "luxembourg" and (self.table_name == 'auctionspotprices' or self.table_name == 'auctionspotvolumes'):
                    ...
                    country_name = "germany_" + country_name
            logging.info(f'Country Name: {country_name}')
            if self.table_name in ['intradayauctionpriceshh', 'intradayauctionpricesqh', 'intradayauctionpriceshourly', 'auctionspotprices', 'auctionspotpriceshh', 'paneuropeanprices']:
                currency_code = self.matcher[key].split(" ")[-1]
                logging.info(f'Currency Code: {currency_code}')
                df["currency"] = currency_code
            if self.table_name in ['paneuropeanvolumes', 'paneuropeanprices']:
                df["ida_code"] = ida_code
            if self.table_name == 'intradayauctionpriceshh' and country_name != 'france':
                df["Off-Peak 1"] = ""
                df["Off-Peak 2"] = ""
                df["Sun Peak"] = ""
            ...
            if self.table_name == 'intradayauctionpriceshourly' and country_name != 'ch-ida1':
                df["Peakload"] = ""
                df["Off-Peak"] = ""
                df["Off-Peak 1"] = ""
            ...
            if self.table_name == 'auctionspotprices' and country_name == 'great-britain':
                df["Middle-Night"] = ""
                df["Early Morning"] = ""
                df["Early Afternoon"] = ""
                df["Late Morning"] = ""
                ...
                df = df.reindex(columns=header_ordered)
            if self.table_name in ['auctionspotprices', 'paneuropeanvolumes', 'paneuropeanprices']:
                df = self._extract_auction_spot_data_date(df)
                if self.table_name == 'paneuropeanvolumes':
                    df = self._epex_transpose(df, 'volume', key)
                elif self.table_name == 'paneuropeanprices':
