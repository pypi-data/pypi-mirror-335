...
def initialize_manual_service() -> ManualService:
    ...
    try:
        ...
        return ManualService(
            ...
            root_page_id=os.environ.get("ZCP_ROOT_PAGE_ID"),
...
@router.get("/manuals", response_model=ManualList)
async def get_manuals():
    """Get hierarchical list of manuals and folders"""
    try:
        items = await manual_service.get_manuals()
        return ManualList(items=items)
    except Exception as e:
...
async def publish_manual(
    ...
    try:
        ...
        # Generate job ID and create initial status
        ...
        # Add the publication process to background tasks
        background_tasks.add_task(
            manual_service.publish_manual,
