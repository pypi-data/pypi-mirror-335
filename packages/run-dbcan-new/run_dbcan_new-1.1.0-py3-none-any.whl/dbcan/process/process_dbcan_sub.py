...
from dbcan.constants import dbCAN_sub_COLUMN_NAMES
...
class DBCANSUBProcessor:
    ...
    def _derive_output_file(self):
        ...
        return getattr(self.config, 'output_file', None) or os.path.join(self.config.output_dir, "dbCANsub_hmm_results.tsv")
    
    def _derive_mapping_file(self):
        """Derive mapping file path"""
        return getattr(self.config, 'mapping_file', None) or os.path.join(self.config.db_dir, "substrate-mappings.tsv")
