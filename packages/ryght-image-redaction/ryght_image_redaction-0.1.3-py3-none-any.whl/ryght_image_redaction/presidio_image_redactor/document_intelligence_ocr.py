...
class DocumentIntelligenceOCR(OCR):
    ...
    @staticmethod
    def _polygon_to_bbox(polygon: list[float]) -> tuple:
        ...
        left = int(min(polygon[0], polygon[2], polygon[4], polygon[6]))
        right = int(max(polygon[0], polygon[2], polygon[4], polygon[6]))
        top = int(min(polygon[1], polygon[3], polygon[5], polygon[7]))
        bottom = int(max(polygon[1], polygon[3], polygon[5], polygon[7]))
        width = right - left
        height = bottom - top
    ...
    def perform_ocr(self, preprocessed_ocr_result) -> dict:
        ...
        """
        if not preprocessed_ocr_result:
            raise NotImplementedError
        else:
            result = preprocessed_ocr_result
