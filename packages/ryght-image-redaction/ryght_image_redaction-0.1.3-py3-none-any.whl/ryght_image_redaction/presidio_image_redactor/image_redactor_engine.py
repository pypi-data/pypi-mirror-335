...
import ryght_image_redaction.presidio_analyzer  # required for isinstance check which throws an error when trying to specify PatternRecognizer  # noqa: E501
from PIL import Image, ImageChops, ImageDraw
from ryght_image_redaction.presidio_analyzer import PatternRecognizer

from ryght_image_redaction.presidio_image_redactor import BboxProcessor, ImageAnalyzerEngine
        ...
        if isinstance(ad_hoc_recognizers, (list, type(None))):
            if isinstance(ad_hoc_recognizers, list):
                if len(ad_hoc_recognizers) >= 1:
                    are_recognizers = all(
                        isinstance(
                            x, ryght_image_redaction.presidio_analyzer.pattern_recognizer.PatternRecognizer
                        )
