...
def on_post_page(
    ...
    """
    # Find all the headings (h1, h2, ...) whose id starts with "pret"
    soup = BeautifulSoup(output, "html.parser")
    for heading in soup.find_all(["h1", "h2", "h3", "h4", "h5", "h6"]):
        ref = heading.get("id", "")
        if ref.startswith("pret.") and "--" not in ref:
            code = "import pret; " + ref
            ...
            url = f"https://github.com/percevalw/pret/blob/{GIT_COMMIT}/{file}#L{line}"
            heading.append(
                BeautifulSoup(
