...
function convertTypeAnnotation(rootType: ts.Type, checker: ts.TypeChecker) {

    const getUnionType = (type): string[] => {
        // @ts-ignore
        switch (type.flags) {
...
function generateComponentStubFunction(componentName: string, jsModuleName: string, props: string[][], jsDoc: string) {
    ...
    const typedPropsString = Array
        ...
        .join(', ');
    return (
        `@stub_component(js.${jsModuleName}.${componentName}, props_mapping)
def ${componentName}(*children, ${typedPropsString}):
    """${jsDoc || ''}"""`
    );
}

function generateStubFunction(functionName: string, jsModuleName: string, jsFunctionName: string, props: string[][], jsDoc: string) {
...
function getDependencyVersion(dep: string, currentDir?: string) {
...
const run = (packageName: string, jsModuleName: string, outputPath?: string): string | void => {
    const packageVersion = getDependencyVersion(packageName);

    const snakeCaseMapping: { [key: string]: string } = {};
    let sourceString = "";
    ...
    const stubBody = (
        imported
            ...
            .map((componentName) => {
                ...
                return generateComponentStubFunction(
                    componentName,
                    jsModuleName,
                    props,
                    doc,
...
function getImported(languageService: ts.LanguageService, sourceString: string) {
    const prompt = `<MODULE.`;
...
function isRestParameter(node: ts.ParameterDeclaration | ts.JSDocParameterTag): boolean {
...
function getFunctionProps(name: string, languageService: ts.LanguageService, sourceString: string, snakeCaseMapping: { [key: string]: string }) {
    ...
    return {
        "props": type.getCallSignatures()[0].getParameters().map((param) => {
            ...
            if (isRestParameter((param as any).valueDeclaration)) {
                const subType = checker.getTypeArguments(type as any)[0];
...
function getComponentProps(name: string, languageService: ts.LanguageService, sourceString: string, snakeCaseMapping: { [key: string]: string }) {
    const prompt = `<MODULE.${name} `;
    const autocompletePosition = sourceString.indexOf(prompt) + prompt.length;
    const completions = languageService.getCompletionsAtPosition(
        '@fake.tsx',
