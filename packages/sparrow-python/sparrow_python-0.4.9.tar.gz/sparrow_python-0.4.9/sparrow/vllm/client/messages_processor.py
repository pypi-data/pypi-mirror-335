...
from .image_processor import encode_image_to_base64, ImageCacheConfig, DEFAULT_CACHE_DIR
from loguru import logger


async def process_content_recursive(content, session, cache_config=None, **kwargs):
...
async def messages_preprocess(
    ...
    **kwargs
    ...
    Args:
        ...
        cache_config: Image cache configuration object (优先使用此参数)
        use_cache: Whether to use cache for URL images (兼容旧接口)
        cache_dir: Cache directory path (兼容旧接口)
        force_refresh: Whether to force refresh the cache even if cached image exists (兼容旧接口)
        retry_failed: Whether to retry previously failed image URLs (兼容旧接口)
        **kwargs: Additional arguments to pass to image processing functions
    """
    if not inplace:
    ...
    # 创建缓存配置
    if cache_config is None and use_cache:
