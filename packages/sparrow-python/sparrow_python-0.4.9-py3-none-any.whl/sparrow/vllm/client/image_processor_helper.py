...
class ImageProcessor:
    ...
    async def encode_to_base64(
        ...
        """
        session = await self._ensure_session()
        # 注意：这里需要检查encode_to_base64函数是否已更新以支持ImageCacheConfig
        # 如果没有更新，需要在此处添加兼容逻辑
        if hasattr(encode_to_base64, 'cache_config'):
            ...
            if cache_config and cache_config.enabled:
                ...
                force_refresh = cache_config.force_refresh
    ...
    async def preprocess_messages(
        ...
        **kwargs
        ...
        # 转换为旧参数格式，因为messages_preprocess仍然使用旧参数
        use_cache = False
        cache_dir = DEFAULT_CACHE_DIR
        force_refresh = False
        retry_failed = False
        ...
        if cache_config and cache_config.enabled:
            ...
            cache_dir = cache_config.cache_dir
            force_refresh = cache_config.force_refresh
            retry_failed = cache_config.retry_failed
