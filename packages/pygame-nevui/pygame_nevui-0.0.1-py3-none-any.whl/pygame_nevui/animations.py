...
class Animation(ABC):
    def __init__(self, time, start, end, type:AnimationType):
        """
...
class AnimationManager:
    def __init__(self):
        ...
        self.anim_position = None
        self.anim_rotation = None
        self.anim_opacity = None

    def update(self):
        if self.start:
            for anim_type, animation in self.start_animations.items():
                if animation:
                    animation.update()
                    if animation.type == AnimationType.COLOR:
                        self.anim_color = animation.current_value
                    elif animation.type == AnimationType.SIZE:
                        self.anim_size = animation.current_value
                    elif animation.type == AnimationType.POSITION:
                        self.anim_position = animation.current_value
                    elif animation.type == AnimationType.ROTATION:
                        self.anim_rotation = animation.current_value
                    elif animation.type == AnimationType.OPACITY:
                        self.anim_opacity = animation.current_value

                    if animation.ended:
                        self.start_animations[anim_type] = None

            if all(animation is None for animation in self.start_animations.values()):
                self.start = False
                self.transitioning = True  # Start transition
                self._start_transition_animations()

        elif self.transitioning:
            all_transitions_finished = True  
            for anim_type, animation in self.transition_animations.items():
                if animation:
                    animation.update()
                    if animation.type == AnimationType.COLOR:
                        self.anim_color = animation.current_value
            ...
            for anim_type, animation in self.continuous_animations.items():
                if animation:
                    animation.update()
                    if animation.type == AnimationType.COLOR:
                        self.anim_color = animation.current_value
    ...
    def add_start_animation(self, animation: Animation):
        if self.start_animations[animation.type] is None:
            self.start_animations[animation.type] = copy.copy(animation)
            self.start_animations[animation.type].time = 0
            self.start_animations[animation.type].ended = False
        else:
    ...
    def add_continuous_animation(self, animation: Animation):
        if self.continuous_animations[animation.type] is None:
            self.continuous_animations[animation.type] = copy.copy(animation)
            self.continuous_animations[animation.type].time = 0
            self.continuous_animations[animation.type].ended = False
        else:
    ...
    def _get_current_value(self, anim_type):
        if anim_type == AnimationType.COLOR:
            return self.anim_color
        elif anim_type == AnimationType.SIZE:
            return self.anim_size
        elif anim_type == AnimationType.POSITION:
            return self.anim_position
        elif anim_type == AnimationType.ROTATION:
            return self.anim_rotation
        elif anim_type == AnimationType.OPACITY:
            return self.anim_opacity
        return None

    def _start_transition_animations(self):
        for anim_type, cont_anim in self.continuous_animations.items():
            if cont_anim:
                start_value = self._get_current_value(anim_type)
                if start_value is not None:
                    ...
                    transition_anim = AnimationLinear(cont_anim.time_maximum/2, start_value, end_value, anim_type) 
                    self.transition_animations[anim_type] = transition_anim
                    transition_anim.time = 0
                    transition_anim.ended = False
