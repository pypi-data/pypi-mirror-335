...
class Module(ABC):
    ...
    def get_sample_interval(self) -> float:
        if not self.cnv:
            raise ValueError(
                "No CnvFile instance to take the sample_interval from."
            )
        return float(self.cnv.data_table_misc["interval"].split(":")[1])
...
class ArrayModule(Module):
    def __call__(
        ...
        self.sample_interval = self.get_sample_interval()
