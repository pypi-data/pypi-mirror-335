        ...
        while self.runnig:
            try:                
                if self.sensor_name is None:
                    return
                with open("/mnt/1wire/%s/temperature" % self.sensor_name, 'r') as content_file:
                    content = content_file.read()    
                    temp = float(content)    # temp in Celcius
                    self.value = temp
...
@parameters([Property.Select(label="Sensor", options=getSensors()), 
             ...
             Property.Number(label="TempRange", configurable=True, unit="degree", description="Temp range in degree between reading and target temp of fermenter/kettle. Larger difference shows different color in dashboard (default:0 | deactivated: 0)")
            ])
...
def setup(cbpi):
    loop_task = asyncio.create_task(screenDisplay.display())
    ...
    print ("Initializing... DS2482S-800-1WireSensor Sensor Module.")
    print ("Initializing... SSD-1306 Screen.")
    print("(c) 1992 - 2024 Lawrence Wagy")
    print("_______________________________________________________")
    
    cbpi.plugin.register("DS2482S-800-1WireSensor", CustomSensor)
    pass
