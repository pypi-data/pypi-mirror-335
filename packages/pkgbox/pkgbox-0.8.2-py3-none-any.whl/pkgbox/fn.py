...
def convert_value(value, arrow_type):
    ...
    # For date columns (pa.date32 expects an int: days since epoch)
        ...
        if isinstance(value, str):
            ...
            try:
                ...
                # Try alternative format (adjust as needed)
                try:
    ...
    # For other types (like strings), return as-is.
    return value
...
def execute_query_with_retry(cursor, query, retries, wait_sec):
    ...
    while attempt <= retries:
        try:
            print(f"read attempt {attempt+1}/{retries+1}..")
...
def plsqlx(query, params={}, con=None, retries=0, wait_sec=5, batch_size=10000, debug=False):
    """
    ...
    driver_type_codes = [str(desc[1]) for desc in description]  # convert type codes to strings
    ...
    while True:
        ...
        for col, vals in zip(column_names, columns):
            ...
            # For numeric, date, or decimal types, convert each value.
