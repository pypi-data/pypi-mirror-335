...
def plsqlx(query, params={}, con=None, retries=0, wait_sec=5):
    def replace_nan_in_arrow_table(arrow_table):
        new_columns = []
        for col_name in arrow_table.column_names:
            col = arrow_table[col_name]
            if pa.types.is_string(col.type):
                # Replace "NaN" values with nulls in a vectorized manner.
                # pa.nulls(len(col), type=col.type) creates a null array matching the column's type.
                replaced = pac.if_else(
                    pac.equal(col, "NaN"), 
                    pa.nulls(len(col), type=col.type), 
                    col
                )
                new_columns.append((col_name, replaced))
            else:
                new_columns.append((col_name, col))
        return pa.Table.from_arrays(
            [col for _, col in new_columns],
    ...
    for attempt in range(retries + 1):
        print(f"Read attempt: {attempt}... Run started at {dt.now()}")
        try:
            start = time.time()
            ...
            # Build the dictionary without doing Python-level "NaN" checks.
            # data_dict = {
            #     col: [row[i] for row in rows] for i, col in enumerate(column_names)
            # }
            # Create an Arrow Table from the dictionary.
            arrow_table = pa.Table.from_pydict(data_dict)
            # Replace "NaN" strings with null values vectorized.
            arrow_table = replace_nan_in_arrow_table(arrow_table)
            ...
            elapsed = end - start
            if elapsed > 60:
                print("Time:", f"{elapsed / 60:.2f} min")
            else:
