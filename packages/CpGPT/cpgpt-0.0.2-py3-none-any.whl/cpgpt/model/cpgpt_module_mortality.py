...
class CpGPTMortalityLitModule(CpGPTLitModule):
    ...
    def __init__(
        ...
        self.val_zscore_best = MaxMetric()
    ...
    def on_train_epoch_end(self) -> None:
        ...
        if len(self.train_predictions) > 0:
            ...
            try:
                ...
                cph = CoxPHFitter()
                cph.fit(df, 'duration', 'event', ['predictions'], robust=True)
                ...
                self.log("train/zscore", z_score, sync_dist=True, prog_bar=True)
            except Exception as e:
                self.log("train/c_index_error", 1.0, sync_dist=True)
                print(f"Error calculating train c-index: {e}")
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        if len(self.val_predictions) > 0:
            ...
            try:
                ...
                self.val_zscore_best.update(torch.tensor(z_score))
                ...
                self.log("val/zscore", z_score, sync_dist=True, prog_bar=True)
                self.log("val/zscore_best", self.val_zscore_best.compute(), sync_dist=True, prog_bar=True)
            except Exception as e:
                self.log("val/c_index_error", 1.0, sync_dist=True)
                print(f"Error calculating validation c-index: {e}")
