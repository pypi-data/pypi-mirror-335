...
from repro2.models.colombo2021 import DEFAULT_IMAGE, MODEL_NAME
...
class _Colombo2021Model(Model):
    ...
    def predict(self, candidate: TextType, references: List[TextType], *args, **kwargs) -> MetricsType:
        return self.predict_batch([{"candidate": candidate, "references": references}])[0]

    @overrides
    def predict_batch(self, inputs: List[Dict[str, Any]], *args, **kwargs) -> Tuple[MetricsType, List[MetricsType]]:
