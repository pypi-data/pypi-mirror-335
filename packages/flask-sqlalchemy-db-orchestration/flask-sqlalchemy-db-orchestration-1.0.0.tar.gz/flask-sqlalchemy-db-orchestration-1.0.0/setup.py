...
class CustomInstallCommand(install):
    def run(self):
        # Standart kurulumu çalıştır
        install.run(self)
        
        # Kurulum sonrası dosyaları mevcut dizine kopyala
        target_dir = os.getcwd()
        package_dir = os.path.join(self.install_lib, "flask_sqlalchemy_db_orchestration")
...
setup(
    name="flask-sqlalchemy-db-orchestration",
    version="1.0.0",
    ...
    packages=["flask_sqlalchemy_db_orchestration"],
    include_package_data=True,
    package_data={
        'flask_sqlalchemy_db_orchestration': ['*.*', '**/*.*', '**/**/*.*'],
    },
    install_requires=[
        'alembic==1.15.1',
        'blinker==1.9.0',
