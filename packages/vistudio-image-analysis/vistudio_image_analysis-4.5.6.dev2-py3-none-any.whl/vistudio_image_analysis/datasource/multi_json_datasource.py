...
@PublicAPI
class MultiJSONDatasource(FileBasedDatasource):
    ...
    def _read_with_python_json(self, buffer: "pyarrow.lib.Buffer"):
        ...
        if 'annotations' in parsed_json:
            for annotation in parsed_json['annotations']:
                if 'segmentation' in annotation \
                        and isinstance(annotation['segmentation'], dict) and 'counts' in annotation['segmentation']:
                    counts = annotation['segmentation']['counts']
                    annotation['rle'] = annotation['segmentation']
                    annotation['segmentation'] = []


        try:
            import pandas as pd
            yield pa.Table.from_pandas(pd.DataFrame([parsed_json]))
