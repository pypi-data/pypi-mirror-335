...
from vistudio_image_analysis.table.image import ImageData, DATA_TYPE_IMAGE, INFER_STATE_UNINFER, INFER_STATE_INFERED


class InferStateUpdater(BaseMongoUpdater):
    """
    AnnotationStateUpdater
    """
    def update_infer_state(self, source: DataFrame) -> DataFrame:
        """
        ...
        for source_index, source_row in source.iterrows():
            ...
            if annotation_count == 0:
                ImageData.objects(
                    ...
                    __raw__={"$set": {"infer_state": INFER_STATE_UNINFER, "updated_at": time.time_ns()}}
                )
            else:
                ImageData.objects(
                    image_id=source_row['image_id'],
                    ...
                    __raw__={"$set": {"infer_state": INFER_STATE_INFERED, "updated_at": time.time_ns()}}
                )
