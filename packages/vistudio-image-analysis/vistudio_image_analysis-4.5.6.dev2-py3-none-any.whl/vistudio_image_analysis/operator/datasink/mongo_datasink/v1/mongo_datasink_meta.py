...
#@OPERATOR_META.register_module(name="MongoDatasink", version="1")
class MongoDatasinkMetaV1(Variable):
    """
    MongoDatasinkMetaV1
    """
    name: str = None
    local_name: str = "MongoDatasink"
    kind: str = "MongoDatasink"
    kind_display_name: str = Field("MongoDB数据写入", alias="kindDisplayName")
    parent_kind: str = Field("Datasink", alias="parentKind")
    description: str = "MongoDB数据写入"
    runtime: str = "ImageAnalysis"
    version: str = "1"
    category: List[str] = ["Datasink:数据写入节点"]
