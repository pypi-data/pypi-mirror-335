...
from vistudio_image_analysis.processor.updater.infer_state_preprocessor import InferStateUpdaterPreprocessor


class UpdateInferStatePipeline(BasePipeline):
    """
    ...
    def run(self):
        ...
        if ds.count() <= 0:
            bcelogger.info("UpdateInferState Count: 0")
            return

        # 第2步 查找task_kind为Manual的数据，并更新annotation_state
        update_infer_status_processor = InferStateUpdaterPreprocessor(config=self.config)
        u_ds = update_infer_status_processor.transform(ds)

        bcelogger.info(f"UpdateInferState Count: {u_ds.count()}")
...
def test_ppl():
    ...
    pipeline = UpdateInferStatePipeline(config=config, parallelism=10, args=None)
    pipeline.run()
