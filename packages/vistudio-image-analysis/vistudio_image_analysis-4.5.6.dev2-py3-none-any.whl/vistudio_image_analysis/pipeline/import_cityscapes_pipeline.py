...
"""
@File:  import_cityscapes_pipeline.py
"""
import pandas as pd
import ray.data
import sys
...
from vistudio_image_analysis.processor.importer.annotation.cityscapes_preprocessor import CityscapesFormatPreprocessor
...
class ImportCityscapesPipeline(BaseImportPipline):
    ...
    def _import_annotation(self):
        ...
        file_uris = cityscapes_reader.get_file_uris(
            data_uri=self.data_uri,
            data_types=self.data_types,
            annotation_format=self.annotation_format
        )
        bcelogger.info("file_uris======{}".format(file_uris))
        if len(file_uris) == 0:
            self.update_annotation_job("未找到标注文件")
            raise Exception("未获取到相关文件，请检查文件")

        ds = ray.data.from_items(file_uris)
        bcelogger.info("import cityscapes annotation count = {}".format(ds.count()))
        ...
        bcelogger.info(f"import cityscapes need add labels: {need_add_labels}")
        ...
        import_labels_dict = convert_annotation_labels_id(labels=import_labels_list, ignore_parent_id=True)

        cityscapes_preprocessor = CityscapesFormatPreprocessor(
            config=self.config,
        ...
        final_ds_dict = cityscapes_preprocessor.fit(ds).stats_
