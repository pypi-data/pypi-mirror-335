...
def analyze_csv(file_path: str, output_format: str = "markdown") -> Dict[str, Any] | str:
    """
    Load and analyze a CSV file, handling different encodings and large files efficiently.

    Args:
        file_path: Path to the CSV file
        output_format: Either 'json' or 'markdown'. Defaults to 'markdown'.

    Returns:
        Dict[str, Any] | str: Analysis results.  Dictionary if json, string if markdown.
    """
    ...
    if output_format == "markdown":
        return create_markdown_output(analysis)  # type: ignore
    else:
        return analysis
