...
class AssistantModel:
    ...
    def chat(
        ...
        :param top_k: Optional integer to specify the number of context snippets to use.
        ...
        if stream:
            return self._chat_streaming(messages=messages, model=model, filter=filter, include_highlights=include_highlights, top_k=top_k, snippet_size=snippet_size)
        else:
            return self._chat_single(messages=messages, model=model, filter=filter, json_response=json_response, include_highlights=include_highlights, top_k=top_k, snippet_size=snippet_size)
    ...
    def _chat_streaming(
        ...
        if top_k is not None:
            content["top_k"] = top_k
        if snippet_size is not None:
            content["snippet_size"] = snippet_size
