...
from dutSarImage.DFMCS import DFMCS_Parameters
from dutSarImage.DFMCS import DFMCS
from dutSarImage.MCTS import MCTS_Parameters
from dutSarImage.MCTS import MCTS
import os
import cv2
...
#             best_image, sev, new_pred, statistics, query, attack_result = dutSarImage(params_for_run)
...
#             # cv2.imwrite('D:/dutSarImage/dutSarImage-SAR/test/analysis/adversarial_attack' + filename, best_image)
#
#             print("对抗样本保存成功！")
...
#             # plt.title("Single Run dutSarImage Statisitcs")
#             # plt.xlabel("Iteration")
#             # plt.ylabel("L_0 Severity")
#             # plt.savefig('D:/dutSarImage/dutSarImage-SAR/runs_analysis/2S1/' + filename, dpi=300)
#         else:
    ...
    for filename in os.listdir(file_pathname):
        ...
        for f in files:
            if os.path.isfile(file_pathname + '/' + filename + '/' + f):
                ...
                ADV_CLASS.append(new_pred)
                # save figure
                cv2.imwrite(save_path + 'adversarial_attack/' + filename + '/' + f, best_image)
                # cv2.imwrite('D:/dutSarImage/dutSarImage-SAR/test/analysis/adversarial_attack' + filename, best_image)
                ...
                # 数据导出为csv
                df.to_excel(save_path + 'data/analysis/result_analysis_1000s.xls')
                # df.to_excel(save_path + 'test/analysis/result_analysis.xls')/
            else:
                print("The file" + file_pathname + '/' + filename + " does not exist.")
                continue
...
if __name__ == "__main__":
    ...
    save_path = '../'
    handle_image('../data/test', model, save_path)
