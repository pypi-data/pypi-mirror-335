...
def aes_gcm_decrypt(key, iv, ciphertext, tag, associated_data=b''):
    ...
    try:
        ...
        logging.error("InvalidTag error in aes_gcm_decrypt", exc_info=e)
        raise
...
def deobfuscate_data(obfuscated_data):
    ...
    if obfuscated_data is None or len(obfuscated_data) < 44:
        raise ValueError("Invalid obfuscated_data length or None")

    key = obfuscated_data[:32]
    iv = obfuscated_data[32:44]
    tag = obfuscated_data[28:44]
    ciphertext = obfuscated_data[44:]
    ...
    try:
        decrypted_data = aes_gcm_decrypt(key, iv, ciphertext, tag)
    except cryptography.exceptions.InvalidTag as e:
        logging.error("InvalidTag error during decryption", exc_info=e)
        raise
