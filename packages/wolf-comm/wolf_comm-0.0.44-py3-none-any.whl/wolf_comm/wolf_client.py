...
class WolfClient:
    ...
    async def __request(self, method: str, path: str, **kwargs) -> Union[dict, list]:
        ...
        while retry_count <= max_retries:
            try:
                ...
                # Prepare headers
                headers = {
                    **bearer_header(self.tokens.access_token),
                    **kwargs.get("headers", {})
                }

                # Refresh session if needed
                if (self.last_session_refesh is None or 
                    self.last_session_refesh <= datetime.datetime.now()):
                ...
                # Add session ID to JSON payload if needed
                if "json" in kwargs and self.session_id is not None and isinstance(kwargs["json"], dict):
                ...
                if resp.status_code in {401, 500}:
                    _LOGGER.info("Request failed with status %d, attempt %d of %d", 
                               resp.status_code, retry_count + 1, max_retries)
                    await self.__authorize_and_session()
                    retry_count += 1
                    continue
                ...
                raise FetchFailed(f"Unexpected status code: {resp.status_code}", 
                                resp.json() if resp.content else None)

            except Exception as e:
                last_exception = e
                if retry_count < max_retries:
                    _LOGGER.warning("Request failed with error: %s, retrying...", str(e))
                    retry_count += 1
                    continue
                self.last_failed = True
                raise FetchFailed(f"Request failed after {max_retries} retries") from last_exception
    ...
    async def fetch_localized_text(culture: str):
        async with aiohttp.ClientSession() as session:
            # Try requested region culture first
            url = f"https://www.wolf-smartset.com/js/localized-text/text.culture.{culture}.js"
            async with session.get(url) as response:
                if response.status == 200 or response.status == 304:
                    return await response.text()
                
            # Fall back to English if requested culturefails
            if culture != 'en':
                _LOGGER.debug("Region Culture %s not found, falling back to English", culture)
    ...
    async def fetch_value(self, gateway_id, system_id, parameters: list[Parameter]):
        ...
        # Group parameters by bundle_id
        for param in parameters:
            bundles.setdefault(param.bundle_id, []).append(param)

        for bundle_id, bundle_params in bundles.items():
            ...
            data = {
                BUNDLE_ID: bundle_id,
                BUNDLE: False,
                VALUE_ID_LIST: [param.value_id for param in bundle_params],
            ...
            _LOGGER.debug('Requesting %s values for BUNDLE_ID: %s', len(bundle_params), bundle_id)
            ...
            try:
                ...
                if ERROR_CODE in res or ERROR_TYPE in res:
                    if ERROR_MESSAGE in res and res[ERROR_MESSAGE] == ERROR_READ_PARAMETER:
                        raise ParameterReadError(f"Failed to read parameters for bundle {bundle_id}", res)
                    raise FetchFailed(f"Failed to fetch values for bundle {bundle_id}", res)
                ...
                values_combined.extend(values_with_value)
                self.last_access = res[LAST_ACCESS]
                
            except Exception as e:
                _LOGGER.error("Error fetching values for bundle %s: %s", bundle_id, str(e))
                raise

        _LOGGER.debug('Requested values for %s parameters, got values for %s', 
                     len(parameters), len(values_combined))
    ...
    async def write_value(self, gateway_id, system_id, Value):
        ...
        if ERROR_CODE in res or ERROR_TYPE in res:
            if ERROR_MESSAGE in res and res[ERROR_MESSAGE] == ERROR_READ_PARAMETER:
                raise ParameterWriteError(f"Failed to write parameters: {res[ERROR_MESSAGE]}", res)
            raise WriteFailed(f"Failed to write data: {res[ERROR_MESSAGE]}", res)
