...
def lambda_invoke(func, event, context, unpack_lists: bool = True):
    ...
    if isinstance(event, dict):
        try:
            ...
            raise TypeError(f"Dictionary (json) input was unable to be bound to function signature: {e}")

    elif not any(param.kind in {param.POSITIONAL_OR_KEYWORD, param.POSITIONAL_ONLY}
                 for param in unbound_sig.parameters.values()):
        raise TypeError("This function has no positional args please reformat the event as a dictionary.")
        ...
        try:
            ...
            raise TypeError(f"List input was unable to be bound to function signature: {e}")
    elif isinstance(event, str) or isinstance(event, int) or isinstance(event, float) or event is None:
        try:
            bound_sig = unbound_sig.bind(event)
        except TypeError as e:
            raise TypeError(f"{type(event)} input was unable to be bound to function signature: {e}")
    else:
        raise TypeError(f"EasyLambda doesn't know how to handle lambda invocations with type: {type(event)}")

    # TODO: option to return error messages rather than raise the error
