...
def find_target_position(
    ...
    **kwargs,
    ...
    target_path = os.path.join(project_path, ".dev", "snapshot", target_id + ".png")
    selectors_path = os.path.join(project_path, "image.json")
    with open(selectors_path, "r", encoding="utf8") as fp:
        selectors = json.load(fp)
        threshold = selectors[target_id]["threshold"]
        gray_style = (
            selectors[target_id]["gray_style"]
            if "gray_style" in selectors[target_id]
            else True
        )
        match_type = selectors[target_id]["match_type"]
        try:
            if match_type == "template":
                return find_target_position_by_template(
                    target_path, gray_style, threshold
                )
            else:
...
def click_target(
    ...
    **kwargs,
    ...
    if target_position is None:
        raise TemporaryException("目标图片未找到")
...
@log_decorator
def wait_target(target_id: str, wait_result="display", timeout=1000, **kwargs):
    """
    等待目标图片出现
    :param target_id:图片id
