...
from jimuflow.common.fs import move_folder_overwrite
...
class MoveFolderComponent(PrimitiveComponent):

    @classmethod
    def display_description(cls, flow_node: FlowNode):
        return gettext(
            'Move ##{folderPath}## to ##{targetFolder}##, {actionWhenExists}, and save the new folder path to ##{newFolderPath}##').format(
    ...
    async def execute(self) -> ControlFlow:
        ...
        if os.path.exists(new_folder_path):
            if action_when_exists == 'rename':
                ...
                move_folder_overwrite(folder_path, new_folder_path)
        else:
