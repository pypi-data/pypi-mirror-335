...
from maspim.res.constants import elements, DEFAULT_MASS_TOLERANCE
from maspim.res.directory_paths import file_dataBase
# python
...
class AllAnotations(Annotations):
    ...
    def initiate_mzs(self):
        ...
        for window in ('FA', 'Alkenones', 'GDGT'):
            ...
            mzs += list(m._get_data_columns().copy())
            del m
        self.mzs = np.array(mzs).astype(float)
...
class Homologous:
    ...
    def find_series(self, include_missings: bool = True):
        ...
        for repeating_unit in self.repeating_units:
            ...
            for mz_seed in self.mzs:
                ...
                hits = np.abs(mz_closests - mz_targets) < 2 * DEFAULT_MASS_TOLERANCE
    ...
    def find_formula(self):
        ...
        db = Annotations(self.mzs.copy(), DEFAULT_MASS_TOLERANCE)
...
if __name__ == '__main__':
    # a = AllAnotations(DEFAULT_MASS_TOLERANCE)
    pass
