...
class AbletonClient:
    ...
    async def send_osc(self, address: str, args: list) -> dict:
        """
        Send an OSC message to Ableton Live and return the response.
        
        Parameters:
        - address: The OSC address to send to (e.g. '/live/song/get/tempo')
        - args: A list of arguments to include with the message
        
        Returns:
        - A dictionary containing the response status and any returned data
        """
        response = await self.send_rpc_request("send_message", {
            "address": address,
            "args": args
        })
...
async def get_track_names(index_min: Optional[int] = None, index_max: Optional[int] = None) -> str:
    ...
    if index_min is not None and index_max is not None:
        response = await ableton_client.send_osc("/live/song/get/track_names", [index_min, index_max])
    else:
        response = await ableton_client.send_osc("/live/song/get/track_names", [])
...
async def get_track_devices(track_index: int) -> str:
    ...
    """
    response = await ableton_client.send_rpc_request("send_message", {
        "address": "/live/track/get/devices/name",
        "args": [track_index]
    })
    
    # Also get the types and class names for better information
    types_response = await ableton_client.send_rpc_request("send_message", {
        "address": "/live/track/get/devices/type",
        "args": [track_index]
    })
    
    class_names_response = await ableton_client.send_rpc_request("send_message", {
    ...
    combined_response = {
        ...
        "names": response.get("result", {}).get("data", []),
        "types": types_response.get("result", {}).get("data", []),
        "class_names": class_names_response.get("result", {}).get("data", [])
    }
...
async def get_device_parameters(track_index: int, device_index: int) -> str:
    """
    Get all parameters of a device at the specified indices.
    ...
    """
    # Get parameter names
    names_response = await ableton_client.send_rpc_request("send_message", {
        "address": "/live/device/get/parameters/name",
        "args": [track_index, device_index]
    })
    
    # Get parameter values
    values_response = await ableton_client.send_rpc_request("send_message", {
        "address": "/live/device/get/parameters/value",
        "args": [track_index, device_index]
    })
    
    # Get min/max values
    min_response = await ableton_client.send_rpc_request("send_message", {
        "address": "/live/device/get/parameters/min",
        "args": [track_index, device_index]
    })
    
    max_response = await ableton_client.send_rpc_request("send_message", {
        "address": "/live/device/get/parameters/max",
        "args": [track_index, device_index]
    })
    
    combined_response = {
        "status": "ok" if names_response.get("status") == "ok" else "error",
        "names": names_response.get("result", {}).get("data", []),
        "values": values_response.get("result", {}).get("data", []),
        "min_values": min_response.get("result", {}).get("data", []),
        "max_values": max_response.get("result", {}).get("data", [])
    }
...
async def set_device_parameter(track_index: int, device_index: int, parameter_index: int, value: float) -> str:
    ...
    - track_index: The index of the track containing the device
    - device_index: The index of the device on the track
    - parameter_index: The index of the parameter to adjust
    - value: The new value for the parameter (typically 0.0 to 1.0)
    """
    response = await ableton_client.send_osc('/live/device/set/parameter/value', 
...
async def get_device_info(track_index: int, device_index: int) -> str:
    """
    Get detailed information about a specific device (instrument or effect) in Ableton.
    ...
    Returns:
    - Full information about the device including name, class, type, and parameters
    """
    # Get basic device info
    name_response = await ableton_client.send_rpc_request("send_message", {
        "address": "/live/device/get/name",
        "args": [track_index, device_index]
    })
    
    class_response = await ableton_client.send_rpc_request("send_message", {
        "address": "/live/device/get/class_name",
        "args": [track_index, device_index]
    })
    
    type_response = await ableton_client.send_rpc_request("send_message", {
        "address": "/live/device/get/type",
        "args": [track_index, device_index]
    })
    
    # Get parameters info
    parameters_response = await get_device_parameters(track_index, device_index)
    parameters = json.loads(parameters_response)
    
    device_info = {
        "status": "ok" if name_response.get("status") == "ok" else "error",
        "name": name_response.get("result", {}).get("data", ["Unknown"])[2] if name_response.get("result", {}).get("data") else "Unknown",
        "class_name": class_response.get("result", {}).get("data", ["Unknown"])[2] if class_response.get("result", {}).get("data") else "Unknown",
        "type": type_response.get("result", {}).get("data", [0])[2] if type_response.get("result", {}).get("data") else 0,
...
@mcp.tool()
async def browse_vst_plugins() -> str:
    """
    Get a list of available VST plugins from Ableton's browser.
    This uses the AbletonOSC browser API to query plugins.
    
    Returns:
    - A list of available VST plugins and their paths
    """
    # Request VST plugins directory structure from Ableton browser
    response = await ableton_client.send_rpc_request("send_message", {
        "address": "/live/browser/get_tree",
        "args": ["audio_effects"]  # This should include VST effects
    })
    
    vst_info = {
        "status": response.get("status", "error"),
        "browser_tree": response.get("result", {}).get("data", [])
    }
    
    # Also try to get instruments
    instruments_response = await ableton_client.send_rpc_request("send_message", {
        "address": "/live/browser/get_tree",
        "args": ["instruments"]  # This should include VST instruments
    })
    
    if instruments_response.get("status") == "ok":
        vst_info["instruments_tree"] = instruments_response.get("result", {}).get("data", [])
    
    return json.dumps(vst_info)

@mcp.tool()
async def browse_presets_for_device(device_type: str, device_name: str) -> str:
    """
    Browse available presets for a specific device type and name.
    
    Parameters:
    - device_type: Type of device ('instruments', 'audio_effects', 'midi_effects')
    - device_name: Name of the device to browse presets for (e.g., 'Serum', 'Operator')
    
    Returns:
    - A list of available presets for the specified device
    """
    # First get the device category from the browser
    response = await ableton_client.send_osc("/live/browser/get_tree", [device_type])
    
    if response.get("status") != "ok":
        return json.dumps({
            "status": "error",
            "message": f"Could not browse {device_type}"
        })
    
    # The response data might contain the device categories
    browser_data = response.get("result", {}).get("data", [])
    
    # Next, look for the specific device and try to get its presets
    found = False
    path_to_try = ""
    
    # Simple search algorithm for the device in the browser tree
    # This is a basic implementation - a real one would be more robust
    for item in browser_data:
        if isinstance(item, str) and device_name.lower() in item.lower():
            found = True
            path_to_try = f"{device_type}/{item}"
            break
    
    if not found:
        return json.dumps({
            "status": "error",
            "message": f"Could not find device {device_name} in {device_type}"
        })
    
    # Now get the presets for this device
    presets_response = await ableton_client.send_osc("/live/browser/get_items_at_path", [path_to_try])
    
    return json.dumps({
        "status": presets_response.get("status", "error"),
        "device": device_name,
        "presets": presets_response.get("result", {}).get("data", [])
    })
