...
class AbletonOSCDaemon:
    ...
    async def start(self):
        ...
        try:
            ...
            print(f"OSC server listening for Ableton Live messages on {self.socket_host}:{self.receive_port}")
            ...
            print(f"Ableton OSC Daemon socket server running on {self.socket_host}:{self.socket_port}")
            print(f"Sending OSC messages to Ableton Live on {self.ableton_host}:{self.ableton_port}")
            print("Waiting for MCP server connection...")
            print("Press Ctrl+C to exit")
            
            async with server:
                await server.serve_forever()
        except Exception as e:
            print(f"ERROR starting the OSC daemon: {e}")
            print("Check if the ports are already in use or if Ableton Live is running.")
            raise
            ...
            while True:
                ...
                try:
                    ...
                    if command == 'send_message':
                        ...
                        if address.startswith(('/live/device/get', '/live/scene/get', '/live/view/get', '/live/clip/get', '/live/clip_slot/get', '/live/track/get', '/live/song/get', '/live/api/get', '/live/application/get', '/live/test', '/live/error', '/live/browser/get')):
...
def main():
    """Entry point for the OSC daemon when run as a command-line application."""
    parser = argparse.ArgumentParser(description="Ableton OSC Daemon")
    parser.add_argument("--socket-host", default="127.0.0.1", help="Host for the socket server")
    parser.add_argument("--socket-port", type=int, default=65432, help="Port for the socket server")
    ...
    print(f"NOTE: You also need to start the AbletonOSC control surface in Ableton Live.")
    print(f"Make sure AbletonOSC is installed and enabled in Ableton Live's preferences.")
    
    try:
        asyncio.run(daemon.start())
    except KeyboardInterrupt:
        print("\nShutting down OSC daemon...")
    except Exception as e:
        print(f"Error: {e}")
        sys.exit(1)
