...
class TomlDB:
    ...
    def _load_database(self):
        ...
        if os.path.exists(self.filename):
            ...
            # 检查数据库中存储的序列化模块是否与当前一致
            stored_serializer = self.data.get('__serializer__')
            if stored_serializer and stored_serializer != self.serializer_name:
                raise ValueError(f"Serializer mismatch: can not change seriallizer for a created db. expected {stored_serializer}, got {self.serializer_name}")
        else:
            self.data = {}
