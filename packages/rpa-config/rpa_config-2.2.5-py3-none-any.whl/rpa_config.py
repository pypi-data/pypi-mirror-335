...
def download_jira_attachment(jira, issue_key, temp_folder, extensions=None):
    ...
    :return: 다운로드한 DataFrame 또는 None
    """
    
    global downloaded_filename, selected_sheet_name  # 글로벌 변수 사용 선언
    ...
    downloaded_filename = filename  # ⬅️ 글로벌 변수로 파일명 저장
    ...
    if filename.endswith('.xlsx'):
        ...
        selected_sheet_name = xls.sheet_names[sheet_selection]  # ⬅️ 글로벌 변수로 시트명 저장
        load_df = pd.read_excel(xls, sheet_name=selected_sheet_name)
        return load_df
