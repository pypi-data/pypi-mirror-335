            ...
            if isinstance(cmd, str):  # 兼容旧版本
                ...
                for c in cmd:
                    if isinstance(c, (int, float)):
                        ...
                        _cmd.append(c)
                result = subprocess.check_output(_cmd, text=True, stderr=subprocess.STDOUT, timeout=timeout, *args, **kwargs)
        ...
        def _recursive_diff(left, right, details, path="/"):
            ...
            if isinstance(left, dict):
                ...
                if left_lenth > right_lenth:
                    ...
                    for i, item in enumerate(r[left_lenth:], start=left_lenth):
...
class DecimalUtils:
    ...
    scaledTriggerPriceStr = DecimalUtils(triggerPrice).round("0.000").scale(3).toString()
    """
    ...
    def round(self, exp: str):
        """将数值四舍五入到指定精度.
        Usage:
        >>> roundPrice1 = DecimalUtils('3.14145').round('0.0000').toString()
    ...
    def truncate(self, exp: str):
        """将数值截取到指定精度 (不四舍五入)
        Usage:
        >>> DecimalUtils('3.1415').truncate('0.000').toString()
        3.141
        """
        sourcePrecision = DecimalUtils(self._cache).getPrecision()
        targetPrecision = DecimalUtils(exp).getPrecision()
        if sourcePrecision > targetPrecision:
            t = str(self._cache).split(".")
            self._cache = decimal.Decimal(t[0] + "." + t[1][:targetPrecision])  # truncate
    ...
    def scale(self, e: int):
        ...
        Usage:
        >>> DecimalUtils('0.348526').scale(3).toString()
        348.526
        >>> DecimalUtils('348.526').scale(-3).toString()
        0.348526
        """
    ...
    def biz_round(self, size: _NumericLike, mode: Literal["floor", "ceil"] = "floor"):
        ...
        Uasge:
        >>> DecimalUtils(1.66 * 1.05).bizRound("0.001", "floor").toString()
        1.742
        >>> DecimalUtils(1.66 * 1.05).bizRound("0.001", "ceil").toString()
        1.743
        """
        if mode == "floor":
        ...
        self._cache = temp.truncate(size).toDecimal()
        return self
    ...
    def cutdown(self, size: _NumericLike):
        ...
        Uasge:
        >>> DecimalUtils("16000.6").cutdown('0.5').toString()
        16000.5
        >>> DecimalUtils("16000.6").cutdown('0.50').toString()
        16000.50
        """
        isNumeric(size, True)
        x: int = self.floor(size)
    ...
    def is_multiple(self, exp: _NumericLike) -> bool:
        ...
        """
        isNumeric(exp, True)
        precision = DecimalUtils(exp).getPrecision()
        exp_scale = DecimalUtils(exp).scale(precision).toInt()
        cache_scale = DecimalUtils(self._cache).scale(precision).toInt()
