...
class PvFileDialog(PvButton):
    ...
    def open_dialog(self, btn):
        ...
        if self.dialog_mode == "open":
            result, _ = QFileDialog.getOpenFileName(self, "Select File", "", self.file_filter)
        elif self.dialog_mode == "save":
            result, _ = QFileDialog.getSaveFileName(self, "Save File As", "", self.file_filter)
