...
def white_manipulation(val, dim, y):
    return [255, 255, 255]
...
class MCTS_Parameters(object):
    def __init__(self, image, true_class, model, length, predshape=(1, 416, 416, 3)):
        ...
        # 当前图像的检测框个数
        self.LENGTH = length
        self.MANIP = is_dark_pixel
        # 总访问次数
        self.VISIT_CONSTANT = 50
        # sigma常数
        self.SIGMA_CONSTANT = 15
        # self.SIGMA_CONSTANT = 30
        # xy是图片长度
        ...
        def predi(im):
            ...
            # prob = self.model.predict(im_pred)
...
def MCTS(params):
    ATTACK_SUCCESS = True
    # 特征提取
    # image = params.ORIGINAL_IMAGE
    # print(image.shape)
    # SIFT传入的图片是三维的 而输入到神经网络中去预测的图像是经过处理的四维图像
    # image = cv2.normalize(params.ORIGINAL_IMAGE, None, 0, 255, cv2.NORM_MINMAX).astype('uint8')
    if SIFT_Filtered(params.ORIGINAL_IMAGE, params) is not None:
        params.kp, params.des, params.r = SIFT_Filtered(params.ORIGINAL_IMAGE, params)
    else:
        pred, prob = params.predict(params.ORIGINAL_IMAGE)
        return params.ORIGINAL_IMAGE, 0, pred, [0, 0, 0], params.QUERY, ATTACK_SUCCESS
    ...
    while True:
        ...
        # 如果树中还没有该层 则创建新一层
            ...
            if len(pred) == 0 or len(pred) < params.LENGTH:
                print("预处理阶段分类错误，跳出循环！", len(pred))
                best_image = copy.deepcopy(IMAGE)
                min_severity = len(manipulation)
                MISCLASSIFIED = True
                break
            elif not (operator.eq(pred, params.TRUE_CLASS)):
                print("预处理阶段分类错误，跳出循环！", pred, params.TRUE_CLASS)
                best_image = copy.deepcopy(IMAGE)
                min_severity = len(manipulation)
                MISCLASSIFIED = True
                break
            ...
            for i in range(len(visited)):
                if i == (len(visited) - 1):
                    ...
                    reward = list((params.PROBABILITY[i] - NEW_PROBABILITY[i] for i in range(len(params.PROBABILITY))))
                    # print(reward)
                    sum_reward = sum(reward) / len(reward)
                    visited[i].backprop(visited[i + 1].id, sum_reward, current_level + 1)
            ...
            if (not (levels[current_level + 1][nxt].id == -1)):
                ...
                for i in range(len(visited)):
                    if (i == (len(visited) - 1)):
                        ...
                        reward = list(
                            (params.PROBABILITY[i] - NEW_PROBABILITY[i] for i in range(len(params.PROBABILITY))))
                        sum_reward = sum(reward) / len(reward)
                        visited[i].backprop(visited[i + 1].id, sum_reward, current_level + 1)
