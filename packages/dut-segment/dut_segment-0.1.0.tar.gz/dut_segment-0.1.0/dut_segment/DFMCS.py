...
class DFMCS_Parameters:
    # def __init__(self, image, true_class, model, length, predshape=(1, 416, 416, 3)):
    #     self.model = model
    #     self.ORIGINAL_IMAGE = copy.deepcopy(image)
    #     self.TRUE_CLASS = true_class
    #     self.LENGTH = length
    #     self.SIGMA_CONSTANT = 15
    #     # self.SIGMA_CONSTANT = 20
    #     self.VISIT_CONSTANT = 5
    #     self.manip_method = white_manip
    #     self.X_SHAPE = predshape[2]
    #     self.Y_SHAPE = predshape[1]
    ...
    def __init__(self, MCTS_params, image):
        ...
        self.LENGTH = MCTS_params.LENGTH
        ...
        def predi(im):
            ...
            pred, prob, draw_image = seg_predict(self.model, im)
            return pred, prob
...
def DFMCS(params, cutoff=-1):
    ...
    def calculate_reward(orig, new, true_class, target_class=None):
        if target_class is None:
            reward = list((orig[s] - new[s] for s in range(len(orig))))
            sum_reward = sum(reward) / len(reward)
            return sum_reward
        else:
            reward = list((new[s] - orig[s] for s in range(len(orig))))
            sum_reward = sum(reward) / len(reward)
            return sum_reward
        ...
        while expiter != params.VISIT_CONSTANT:
            ...
            try:
                ...
                if (((x, y) in params.MANIPULATIONS) or
                        list(image[y][x]) ==
                        list(params.manip_method(image, x, y))):
                    continue
            except:
                if (((x, y) in params.MANIPULATIONS) or
                        (image[y][x]) ==
                        (params.manip_method(image, x, y))):
                    continue
            # 对像素进行修改
            # image[y][x] = params.manip_method(image[y][x], params.EPSILON)
            image[y][x] = params.manip_method(image, x, y)
            params.MANIPULATIONS.append((x, y))
            expiter += 1

        # 观察预测分类和值
        pred, prob = params.predict(image)
        print("观察预测分类和值: ", pred, prob)
        # 如果是反例 返回
        if len(pred) == 0:
            if params.verbose:
                print("\n")
                ...
                f = """Manipulations: %s
                        未检测到分类！
                    """ % (len(params.MANIPULATIONS))
                sys.stdout.write("\r" + str(f))
                sys.stdout.flush()
            MISCLASSIFIED = True
            return keypoint_distribution, image, TOTAL_PLAYS, MISCLASSIFIED
        elif len(pred) < params.LENGTH:
            print(len(pred), params.LENGTH)
            if params.verbose:
                print("\n")
                print("Adversarial Example Found")
                f = """Manipulations: %s
            ...
            MISCLASSIFIED = True
            return keypoint_distribution, image, TOTAL_PLAYS, MISCLASSIFIED
        elif ((not (operator.eq(pred, true_class)) and target_class is None) or
              ((not (operator.eq(pred, true_class))) and target_class is not None)):
            if params.verbose:
                print("\n")
                print("Adversarial Example Found")
                f = """Current Probability: %s
                       Current Class: %s
                       Manipulations: %s
                    """ % (prob, pred, len(params.MANIPULATIONS))
                sys.stdout.write("\r" + str(f))
                sys.stdout.flush()
            MISCLASSIFIED = True
            return keypoint_distribution, image, TOTAL_PLAYS, MISCLASSIFIED
    ...
    # NEW_PROBABILITY = prob[0]
    params.SOFT_MAX_ARRAY = prob

    if len(pred) != 0 and len(prob) < params.LENGTH:
        if not (operator.eq(pred, params.TRUE_CLASS)):
    ...
    if cutoff == -1:
        ...
        while len(params.MANIPULATIONS) <= 10000 and not params.MISCLASSIFIED:
            # print("一次循环：")
    ...
    def backtracking(adversarial_image, original_image,
        ...
        for x, y in manipulations:
            if pixels_at_a_time != 0:
                ...
                adversarial_image[y][x] = original_image[y][x]
                pixels_at_a_time = cluster
            pred, prob = params.predict(adversarial_image)
            if len(pred) == 0:
                best_bad_example = copy.deepcopy(adversarial_image)
                # l_zero -= (cluster + 1)
                l_zero -= cluster
                if params.verbose:
                    f = """Backtracking Step L_0: %s
                           No targets detected, 
                           progress: %s/%s
                        """ % (l_zero, progress * cluster, len(manipulations))
                    sys.stdout.write("\r" + str(f))
                    sys.stdout.flush()
            elif len(prob) < params.LENGTH:
                if not (operator.eq(pred, params.TRUE_CLASS)):
                    ...
                    if params.verbose:
                        ...
                        sys.stdout.write("\r" + str(f))
                        sys.stdout.flush()
            elif not (operator.eq(pred, params.TRUE_CLASS)):
                best_bad_example = copy.deepcopy(adversarial_image)
                # l_zero -= (cluster + 1)
                l_zero -= cluster
                if params.verbose:
