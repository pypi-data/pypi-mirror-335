...
async def execute_sql_with_column_info_async(
    sql_query,
    database,
    user="select_user",
    password="select_user",
    ...
    async def sql_unprotected():
        # Execute the SQL query
        stmt = await conn.prepare(sql_query)
        results = await stmt.fetch()

        column_names = [desc.name for desc in stmt.get_attributes()]
        column_type_oids = [desc.data_type_oid for desc in stmt.get_attributes()]

        type_map = {}
        type_query = "SELECT oid, typname FROM pg_type WHERE oid = ANY($1);"
        type_records = await conn.fetch(type_query, column_type_oids)
        for record in type_records:
            oid, typname = record['oid'], record['typname']
            if typname.startswith("_"):
                type_map[oid] = typname[1:] + "[]"
            else:
                type_map[oid] = typname
    ...
    try:
        results, column_info = await sql_unprotected()
