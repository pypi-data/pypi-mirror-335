...
// use crate::generated::crc_bindings::calculateCRC16;
use crate::utils::crc::*;
...
#[cfg(feature = "edu")]
use super::edu::msg_builder::EduMessage;
...
mod tests {
  ...
  #[cfg(feature = "edu")]
  use crate::{generated::edu_proto, proto::edu::msg_builder::edu_msg_builder};
  ...
  fn edu_msg_encode() {
    ...
    init_logging(log::Level::Debug);
    let msg = edu_proto::AppDongle {
      msg_id: 666,
      ..Default::default()
    };
    // print msg to uint8 array
    let encoded = msg.encode_to_vec();
    info!("Encoded: {:?}", encoded);

    // info!("Wrap 1: {:?}", edu_msg_builder::sync_time());
    info!("Wrap 2: {:?}", edu_msg_builder::encode_app_to_dongle(msg));
  }

  #[cfg(feature = "edu")]
  #[tokio::test]
  async fn edu_msg_decode() {
    init_logging(log::Level::Debug);
    let mut parser = Parser::new("test-device".into(), MsgType::Edu);

    let mut stream = parser.message_stream();

    parser.receive_data(&[66, 82, 78, 67, 2, 12, 1, 2, 0, 1, 2, 0, 8, 1, 112, 130]);
    parser.receive_data(&[66, 82, 78, 67, 2, 12, 1, 3, 0, 1, 2, 0, 8, 154, 5, 233, 27]);
    ...
    tokio::spawn(async move {
      while let Some(result) = stream.next().await {
        match result {
          Ok(message) => {
            info!(
              "Received edu message: {:?}",
              serde_json::to_string(&message)
            );
          }
          Err(e) => {
            error!("Error receiving edu message: {:?}", e);
          }
        }
      }
      info!("edu message stream finished");
    });
    // 等待一会儿，以便观察输出
