...
pub fn derive_nonce(user_id: &str, sn_code: &str) -> Result<[u8; NONCE_LEN], Error> {
  ...
  // 使用HKDF派生Nonce
  let salt = None::<&[u8]>; // 可选salt，这里未使用
  let hkdf_key = hkdf::Salt::new(hkdf::HKDF_SHA256, salt.unwrap_or(&[]));
  ...
  let nonce_okm = prk
    ...
    .map_err(|_| Error::new(ErrorKind::Other, "HKDF nonce derivation failed"))?;
  nonce_okm
    .fill(&mut nonce_bytes)
    .map_err(|_| Error::new(ErrorKind::Other, "HKDF nonce fill failed"))?;
...
pub fn decrypt_with_key_nonce(
  ...
  less_safe_key
    ...
    .map_err(|_| {
      Error::new(
        ErrorKind::Other,
        "AES-GCM decryption or verification failed",
      )
    })?;
