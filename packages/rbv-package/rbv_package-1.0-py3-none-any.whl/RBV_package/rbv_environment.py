...
class Group_Orgunits:
    ...
    def set_proportions(self, p_low, p_mod, p_high, proportions):
...
class Orgunit:
    ...
    ## CUSTOM - Modify categorization of risk freely :)
    ...
    def set_quantitative_risk_categorization(self, method, args):
        self.categorize_quant_risk = method
        self.quant_risk_args = args

    def get_quantitative_risk(self):
        if self.eligible_for_VBR():
            self.get_ecart_median()
            self.get_ecart_median_per_service()
            self.get_gain_median_par_periode()
            self.categorize_quant_risk(*self.quant_risk_args)
        else:
    ...
    ## CUSTOM - Modify categorization of risk freely, just add a new function :)
    def categorize_risk_PMNS(
    ...
    def categorize_risk_quantity_BUU(self, seuil_max_bas_risk, seuil_max_moyen_risk):
        if self.eligible_for_VBR():
            self.get_ecart_median()
            self.get_ecart_median_per_service()
            self.get_gain_median_par_periode()
            nb_risk_moindre = len(
                [
                    ecart
                    for ecart in self.ecart_median_per_service.ecart_median.values
                    if ecart >= seuil_max_bas_risk
                ]
            )
            if self.ecart_median_per_service.ecart_median.max() <= seuil_max_bas_risk:
                self.risk_quantite = "low"
            elif (
                self.ecart_median_per_service.ecart_median.max() >= seuil_max_moyen_risk
            ):
                self.risk_quantite = "high"
            elif nb_risk_moindre == 1:
                self.risk_quantite = "moderate_1"
            elif nb_risk_moindre < 4:
                self.risk_quantite = "moderate_2"
            else:
                self.risk_quantite = "moderate_3"
        else:
            self.risk_quantite = "uneligible"
            self.ecart_median = pd.NA
            self.ecart_median_per_service = pd.NA
            self.gain_median = pd.NA
            self.risk = "uneligible"
        self.risk_gain_median = "Not_considered"

    def set_mix_risks(self, method, args):
        self.mix_risk_method = method
        self.mix_risks_args = args
    ...
    def mix_risks(self, use_quality_for_risk):
        ...
        self.risk = self.mix_risk_method(risks, *self.mix_risks_args)
