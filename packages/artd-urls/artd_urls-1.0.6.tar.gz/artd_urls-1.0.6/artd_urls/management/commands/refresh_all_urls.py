...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            "-p",
            dest="partner",
            help="Stablish the partner to filter the urls",
        )
    ...
    def handle(self, *args, **options):
        ...
        if partner is None:
            raise CommandError("You must provide a partner")
        else:
            partner = Partner.objects.get(partner_slug=partner)
            url_configuration = UrlConfiguration.objects.filter(
                partner=partner,
            ).first()
            if url_configuration is None:
                raise CommandError("You must provide a url configuration")
            else:
                include_admin_urls = url_configuration.include_admin_urls
                include_dinamic_urls = url_configuration.include_dinamic_urls
        ...
        for func, regex, url_name in view_functions:
            ...
            if include_admin_urls and "admin" in url:
                if include_dinamic_urls and "<" in url:
                    Url.objects.create(
                        partner=partner,
                        module=module,
                        name=url_name,
                ...
                elif not include_dinamic_urls and "<" not in url:
                    Url.objects.create(
                        partner=partner,
                        module=module,
                        name=url_name,
            ...
            elif not include_admin_urls and "admin" not in url:
                Url.objects.create(
                    partner=partner,
                    module=module,
                    name=url_name,
