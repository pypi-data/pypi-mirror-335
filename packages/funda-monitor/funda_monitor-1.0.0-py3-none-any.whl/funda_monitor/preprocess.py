...
def clean_price(x: str) -> int:
    ...
    try:
        # Extract just the number part, removing € symbol and 'kosten koper'
        price_str = str(x).split("kosten koper")[0].strip()  # Remove 'kosten koper'
        price_str = price_str.replace("€", "").strip()  # Remove € symbol
        price_str = price_str.replace(".", "")  # Remove thousand separators
        price_str = price_str.replace(",", "")  # Remove decimal separator if present
...
def preprocess_data(
    ...
    # Keep all rows initially
    if not is_past:
        keep_cols = [col for col in config.keep_cols.selling_data if col != 'house_id']
    else:
        keep_cols = [col for col in config.keep_cols.selling_data if col != 'house_id'] + config.keep_cols.sold_data
    ...
    # Only filter out rows where both price and living area are 0
    df = df[~((df["price"] == 0) & (df["living_area"] == 0))]
    ...
    # Extract house_id from URL (8-digit number before ?old_ldp=true)
    df["house_id"] = df["url"].apply(lambda x: x.split("/")[-2].split("-")[-1])
    
    # Ensure house_id is at the beginning of keep_cols
    if "house_id" not in keep_cols:
        keep_cols.insert(0, "house_id")
