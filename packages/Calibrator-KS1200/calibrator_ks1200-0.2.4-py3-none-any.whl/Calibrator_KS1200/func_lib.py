...
def ReadUShortMBus(comportname,boud,modbusnum,address,numreg,MainWindow):
	...
	global ia
	bytearr=bytearray(numreg*2)
	if(SendMBus(comportname,boud,modbusnum,3,address,0,numreg,MainWindow)):	
		for i in range(numreg):
...
def ReadStMBus(comportname,boud,modbusnum,MainWindow):
	...
	if SendMBus(comportname,boud,modbusnum,0x07,0,0,1,MainWindow):
...
def ReadIntMBus(comportname,boud,modbusnum,address,numint,MainWindow):
	...
	if(SendMBus(comportname,boud,modbusnum,3,address,0,numint*2,MainWindow)):
		bytearr=tuple(inbuff)
...
def ReadFloatMBus(comportname,boud,modbusnum,address,numfloat,MainWindow):
	...
	global fa
	byte_arr=bytearray(255)
	if(SendMBus(comportname,boud,modbusnum,3,address,0,numfloat*2,MainWindow)):	
		bytearr=tuple(inbuff)
		for i in range(numfloat):
			FloatBytes=bytearray(4)
			for j in range(4):
...
def SendMBus(comportname,boud,modbusnum,modbusfun,address,data,lengthreg,MainWindow):
	...
	try:
		MainWindow.serial_port=serial.Serial(comportname, boud, timeout=0.5)
	...
	countdown = 5
	while True:
		if MainWindow.itm_exchange_stop:
			MainWindow.itm_exchange_stop = False
			MainWindow.serial_port.close()
			return
		if inbuff == b'': MainWindow.serial_port.write(outbuff)
		inbuff=MainWindow.serial_port.read(sizein)
		print('read:',inbuff)
		if inbuff == b'': countdown-=1
		if inbuff != b'M\x83\x06\x00\xe5' or countdown == 0: break
	MainWindow.serial_port.close()
