...
try:
    ...
    print("Delta Lake library not found. Please install the library by running 'pip install delta-spark'.")
from pyspark.sql.functions import lit, col, format_number
from pyspark.sql import functions as F
try:
    from adal import AuthenticationContext
except ImportError:
    print("ADAL library not found. Please install the library by running 'pip install adal'.")
...
class Landing:
    ...
    def list_all_sharepoint_files(self, MatchingRegexPattern = '^.*\.(xlsx)$'):
        ...
        if response:
            try:
                existing_files = []
                if self._replacefile == False and self._spark.catalog.tableExists(self._database, self._bronze_table):
    ...
    def convert_to_bronze(self, file):
        ...
        try:
            self._file_path = self.put_bronze()
            return True
        except Exception as e:
    ...
    def put_bronze(self):
        ...
        file_readers = {
            ...
            '.xlsx': lambda path, sheet_name=None: (
            self._spark.read.format('com.crealytics.spark.excel')
            .option("header", "true")
            .option("useHeader", "true")
            .option("inferSchema", "true")
            .option("dataAddress", f"'{sheet_name}'!A1") if sheet_name else self._spark.read.format('com.crealytics.spark.excel')
            .option("header", "true")
            .option("useHeader", "true")
            .option("inferSchema", "true")
            ).load(path),
            '.xls': lambda path, sheet_name=None: (
            self._spark.read.format('com.crealytics.spark.excel')
            .option("useHeader", "true")
            .option("inferSchema", "true")
            .option("dataAddress", f"'{sheet_name}'!A1") if sheet_name else self._spark.read.format('com.crealytics.spark.excel')
        ...
        try:
            ...
            # Check if the file is an Excel file and if tab names are provided
            if file_extension in ['.xlsx', '.xls'] and isinstance(self._excel_tabnames, list) and self._excel_tabnames:
                # Read each specified tab and store them in a DataFrame
                stages = []
                for tab_name in self._excel_tabnames:
                    print(f'Reading Excel tab: {tab_name}')
                    try:
                        stage = reader(self._file_path, tab_name)  # Pass the sheet name
                        stage = stage.withColumn('SourceTabName', lit(tab_name))
                        stages.append(stage)
                    except Exception as e:
                        raise Exception(f"Error reading Excel tab: {tab_name} | {str(e)}")
                # Optionally, you can union the DataFrames if needed
                if stages:
                    stage = reduce(lambda df1, df2: df1.union(df2), stages)
            else:
                # If no specific tabs are provided, read the entire file
            ...
            try:
                for static_col in self._static_cols:
                    for col, value in static_col.items():
                        print(f'Adding col {col} with value {value} to the stage dataframe')
        ...
        try:
            ...
            for col in stage.columns:
                try:
                    ...
                    raise Exception(f'Error cleaning column name "{col}": {e}')

            # Create a mapping of old column names to cleaned column names
            column_mapping = {}
            for old_col, new_col in zip(stage.columns, cleaned_columns):
                if old_col != new_col:  # Only map if the name has changed
            ...
            if column_mapping:
                ...
                for old_col, new_col in column_mapping.items():
                    try:
                        ...
                        raise Exception(f'Error renaming column from "{old_col}" to "{new_col}": {e}')
        ...
        if not table_exists:
            ...
            try:
                ...
                if self._fix_schema:
                    print("Schemas are misaligned; attempting to streamline them.")

                    # Handle missing fields
                    try:
                        for field in missing_fields:
                            print(f"Field '{field}' is missing in stage DataFrame. Adding NULL column to stage data.")
                            try:
                                stage = stage.withColumn(field, F.lit(None).cast(delta_fields[field]))
                            except Exception as e:
                                raise Exception(f"Failed to add missing column '{field}' to stage data. Error: {e}")
                    except:
                        raise Exception(f"Failed to add missing columns to stage data. Error: {e}")
                    # Handle extra fields
                    try:
                        sql = ''
                        for field in extra_fields:
                            sql = f"ALTER TABLE {self._bronze_table_path} ADD COLUMN {field} string"
                            try:
                                print(f"Field '{field}' is extra in bronze table. Adding new column to table using query: {sql}.")
                                self._spark.sql(sql)
                            except Exception as e:
                                raise Exception(f"Failed to add missing column '{field}' to target table. SQL tried: {sql}. Error received: {e}")
                    except Exception as e:
                        raise Exception(f"Failed to add extra columns to target table. Error: {e}, sql tried: {sql}")

                print(f"Schemas are aligned. Attempting to load data to {self._bronze_table_path}")

            except ValueError as ve:
                print(f"ValueError encountered: {ve}")
                raise  # Re-raise ValueError with a meaningful message
            except Exception as e:
                print(f"An unexpected error occurred: {e}. Check the input schemas and the operations performed.")
                raise Exception(f'Schemas are misaligned but failed to give a meaningful error: {e}. Check the input schemas and the operations performed.')
        ...
        if self._loadtype =="overwrite":
            ...
            try:
                schema = self._spark.table(self._bronze_table_path).schema
                stage = self._spark.createDataFrame(stage.toPandas(), schema)
                stage.write.format("delta").mode("append").saveAsTable(self._bronze_table_path)
                self._log_records_influenced = stage.count()
                print(f'The filename {self._file_name} has been appended into {self._bronze_table_path}')
    ...
    def _clean_column_name(self, column_name):
        ...
        protected_columns = ['dbxcreated', 'dbxupdated', 'isdeleted', 'sourcefile']

        # Prepend 'source_' if the cleaned name is in the protected list
        if cleaned_name in protected_columns:
    ...
    def _infer_table(self, df):
      ...
      try:
          ...
          #for column in modified_df.columns:
              #if column not in ['DbxCreated', 'DbxUpdated', 'IsDeleted']:
                  # Use when to format numbers and leave text unchanged
                  #modified_df = modified_df.withColumn(
                  #    column,
                  #    F.when(F.col(column).rlike("^[0-9]*\\.?[0-9]+$"), F.format_number(F.col(column).cast("double"), 0))
                  #    .otherwise(F.col(column))  # Keep original value if not a number
          ...
          inferred_schema = self._spark.sql("DESCRIBE temp_view").toPandas()
