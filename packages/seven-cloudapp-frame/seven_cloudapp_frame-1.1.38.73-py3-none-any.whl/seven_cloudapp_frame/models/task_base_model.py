...
class TaskBaseModel(FrameBaseModel):
    ...
    def get_task_info_dict_list(self, app_id, act_id, module_id, is_release, is_cache=True, source_object_id='', is_del=-1, order_by="sort_index desc,id asc"):
        ...
        if is_cache:
            dict_list = TaskInfoModel(context=self.context, is_auto=True).get_cache_dict_list(condition_where.to_string(), group_by="", order_by=order_by, params=params,dependency_key=DependencyKey.task_info_list(act_id))
        else:
