...
@LastEditTime: 2025-03-19 17:48:58
...
class ActBaseModel():
    ...
    def get_act_info_list(self, app_id, act_name, is_del, page_size, page_index, is_cache=True, act_type=-1, is_release=-1, order_by="id asc", business_type=-1):
        ...
        if is_cache:
            page_list, total = ActInfoModel(context=self.context, is_auto=True).get_cache_dict_page_list(field="*", page_index=page_index, page_size=page_size, where=condition_where.to_string(), group_by="", order_by=order_by, params=params, dependency_key=DependencyKey.act_info_list(app_id))
        else:
