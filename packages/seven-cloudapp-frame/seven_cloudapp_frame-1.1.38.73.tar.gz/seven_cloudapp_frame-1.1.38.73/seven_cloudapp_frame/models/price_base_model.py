...
class PriceBaseModel():
    ...
    def get_price_gear_list(self, app_id, act_id, page_size, page_index, order_by="id desc", is_del=0, is_cache=True, page_count_mode="total", business_type=-1):
        ...
        if is_cache == True:
            page_list = PriceGearModel(context=self.context, is_auto=True).get_cache_dict_page_list(field="*", page_index=page_index, page_size=page_size, where=condition, group_by="", order_by=order_by,params=params,dependency_key=DependencyKey.price_gear_list(act_id),page_count_mode=page_count_mode)
        else:
