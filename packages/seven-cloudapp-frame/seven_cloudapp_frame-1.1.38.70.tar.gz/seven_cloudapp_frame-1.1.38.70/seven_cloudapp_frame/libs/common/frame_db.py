...
class FrameDbModel(BaseModel):
    ...
    def write_or_read_db_config(self, db_config, is_write=False):
        """
        :Description: 合成最终的连接串配置
        :param db_config: 配置文件的配置
        :param is_write: 是否请求写库，True-是，False-否，有主从配置，根据语句自动分配读写库
        :return: 存储过程的结果
        """
        if not db_config:
            return db_config
        covert_config = {}
        if is_write:
            covert_config["host"] = db_config.get("write_host", db_config.get("host", ""))
            covert_config["port"] = db_config.get("write_port", db_config.get("port", ""))
            covert_config["user"] = db_config.get("write_user", db_config.get("user", ""))
            covert_config["passwd"] = db_config.get("write_password", db_config.get("passwd", ""))
        else:
