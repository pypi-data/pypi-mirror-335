...
class ComponentsYaml(YAML):
    ...
    def get_component_info(self, component_type: str, version: str):  # type: ignore
        ...
        if "UnionTech" in os_name:
            os_name = "Ubuntu"
            cpu_model = "Intel"
        if "Ubuntu" in os_name and "Hygon" in cpu_model:
            cpu_model = "Intel"
        file_path = self.components_dict[component_type]
        if component_type == "driver":
            version = DRIVER_CLINFO_MAP.get(version, version)
        data = self.get_yaml_data(file_path)

        if version is None:
            for item in data:
                if item.get("url", 0) == 0 and (
                    item.get("os", "Ubuntu") in os_name
                    and item.get("cpu", "Intel") in cpu_model
                ):
                    return item
        ...
        for d_dict in data:
            if str(d_dict["version"]) == version and (
                d_dict.get("url", 0) == 0
                and (
                    d_dict.get("os", "Ubuntu") in os_name
                    and d_dict.get("cpu", "Intel") in cpu_model
                )
            ):
                return d_dict
        ...
        elif not self._in_host and component_type == "sdk":
            print(
                f"""{FontRed('ERROR:')} OS: {os_name}, CPU: {cpu_model}, version of the {component_type} you want to downloand is incorrect.\
 The currently supported versions are {[DRIVER_VERSION_MAP.get(str(d['version']), d['version']) for d in data if (d['os'] in os_name and d['cpu'] in cpu_model)]}"""
            )
        else:
            print(
