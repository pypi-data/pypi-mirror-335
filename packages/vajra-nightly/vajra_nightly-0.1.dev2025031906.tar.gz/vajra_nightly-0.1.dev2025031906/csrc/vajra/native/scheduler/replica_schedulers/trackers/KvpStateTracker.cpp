...
std::vector<std::size_t> KvpStateTracker::GetBatchTrackerQTokens(
    const std::shared_ptr<Sequence> seq) const {
...
void KvpStateTracker::AddSequenceToBatch(
    const std::shared_ptr<Sequence> seq, std::size_t num_q_tokens,
...
void KvpStateTracker::FreeSeq(const std::shared_ptr<Sequence> seq) {
...
bool KvpStateTracker::CanAppendSlot(const std::shared_ptr<Sequence> seq) const {
  std::size_t last_kvp_group_id = GetLastKvGroupId(seq);
  return block_managers_map_.at(last_kvp_group_id)->CanAppendSlot();
}

bool KvpStateTracker::AppendSlot(const std::shared_ptr<Sequence> seq,
...
void KvpStateTracker::UpdatePrefillWork(const std::shared_ptr<Sequence> seq,
