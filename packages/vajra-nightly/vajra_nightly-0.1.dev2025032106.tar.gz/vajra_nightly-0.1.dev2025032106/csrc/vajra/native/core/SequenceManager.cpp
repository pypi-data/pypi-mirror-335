...
//==============================================================================
#include "native/core/SequenceManager.h"

namespace vajra {

std::pair<Sequences, Sequences> SequenceManager::OnSchedule(
...
void SequenceManager::OnStepCompleted(
...
void SequenceManager::OnStageCompleted(
    const SchedulerOutputPtr scheduler_output) {
  std::lock_guard<std::recursive_mutex> lk(mutex_);
...
std::vector<RequestOutput> SequenceManager::GenerateRequestOutputs(
...
void SequenceManager::PreemptSeq(const std::string& seq_id) {
  ASSERT_VALID_RUNTIME(seq_map_.find(seq_id) != seq_map_.end(),
                       "sequence {} not found", seq_id);
  auto seq = seq_map_[seq_id];
...
void SequenceManager::PauseSeq(const std::string& seq_id) {
  ASSERT_VALID_RUNTIME(seq_map_.find(seq_id) != seq_map_.end(),
                       "sequence {} not found", seq_id);
  auto seq = seq_map_[seq_id];
...
void SequenceManager::ResumeSeq(const std::string& seq_id) {
  ASSERT_VALID_RUNTIME(seq_map_.find(seq_id) != seq_map_.end(),
                       "sequence {} not found", seq_id);
  auto seq = seq_map_[seq_id];
  ASSERT_VALID_RUNTIME(
...
void SequenceManager::ProcessSeqOutput(MutableSequencePtr seq,
