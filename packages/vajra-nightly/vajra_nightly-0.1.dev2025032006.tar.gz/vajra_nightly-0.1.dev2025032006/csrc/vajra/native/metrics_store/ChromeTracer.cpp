...
//==============================================================================
ChromeTracer::ChromeTracer(std::size_t replica_id, std::string output_dir)
    : replica_id_(replica_id), output_dir_(std::move(output_dir)) {
...
//==============================================================================
void ChromeTracer::Put(
    const std::vector<std::shared_ptr<SequenceMetadata>>& seq_metadata_list,
    std::size_t tensor_parallel_rank, std::size_t pipeline_parallel_rank,
    std::size_t kv_parallel_rank, double start_time, double end_time) {
  ...
  event["pid"] = fmt::format("{}-kvp{}", replica_id_, kv_parallel_rank);
...
void ChromeTracer::PutSchedulerEvent(
  ...
  event["ts"] = start_time * 1e6;
  event["dur"] = (end_time - start_time) * 1e6;
  event["pid"] = std::to_string(replica_id_);
  event["tid"] = "Scheduler";
...
//==============================================================================
std::size_t ChromeTracer::GetReplicaId() const { return replica_id_; }
//==============================================================================
