...
from vajra._native.metrics_store import (
    ...
    MetricType,
    comparison_group_type_to_string,
    entity_association_type_to_string,
...
class Plotter:
    @staticmethod
    def print_stats(data_series: pd.Series, metric_name: str) -> None:
        ...
        logger.info(f"{metric_name} stats: {summary_str}")
        if wandb.run:
    ...
    def store_associated_metrics(
        datastores: Dict[MetricType, BaseDataStore], output_dir: str
    ) -> None:
        grouped_metrics: Dict[EntityAssociationType, List[BaseDataStore]] = {}
        ...
        for association_type, grouped_datastores in grouped_metrics.items():
            entity_association_type_string = entity_association_type_to_string(
                association_type
            )
            assert all(
                [
                    d.metric.label_type
                    and label_type_to_string(d.metric.label_type)
                    == entity_association_type_string
    ...
    def store_comparison_metrics(
        datastores: Dict[MetricType, BaseDataStore], plot_dir: str, store_png: bool
    ) -> None:
        grouped_metrics: Dict[ComparisonGroupType, List[BaseDataStore]] = {}
        ...
        for comparison_type, grouped_datastores in grouped_metrics.items():
            ...
            Plotter.plot_comparison_bar_chart(
                summation_data,
                comparison_group_type_to_string(comparison_type),
                unit_type_to_string(unit),
                plot_dir,
                store_png,
            )
