...
class AsyncClient(object):
    ...
    def session(self):
        if not self._session:
            self._session = aiohttp.ClientSession(
                loop=self._loop, timeout=aiohttp.ClientTimeout(total=self._timeout)
            )
    ...
    async def read(self, path, wrap_ttl=None):
        ...
        try:
            return await (
                await self._get("/v1/{0}".format(path), wrap_ttl=wrap_ttl)
            ).json()
    ...
    async def unwrap(self, token=None):
        ...
        if token:
            payload = {"token": token}
            return await (
                await self._post("/v1/sys/wrapping/unwrap", json=payload)
            ).json()
        else:
    ...
    async def start_generate_root(self, key, otp=False):
        ...
        return await (
            await self._put("/v1/sys/generate-root/attempt", json=params)
        ).json()
    ...
    async def generate_root(self, key, nonce):
        ...
        return await (
            await self._put("/v1/sys/generate-root/update", json=params)
        ).json()
    ...
    def tune_secret_backend(
        self, backend_type, mount_point=None, default_lease_ttl=None, max_lease_ttl=None
    ):
        """
    ...
    async def get_secret_backend_tuning(self, backend_type, mount_point=None):
        ...
        return await (
            await self._get("/v1/sys/mounts/{0}/tune".format(mount_point))
        ).json()
    ...
    async def get_policy(self, name, parse=False):
        ...
        try:
            policy = (
                await (await self._get("/v1/sys/policy/{0}".format(name))).json()
            )["rules"]
            if parse:
    ...
    async def audit_hash(self, name, input):
        ...
        return await (
            await self._post("/v1/sys/audit-hash/{0}".format(name), json=params)
        ).json()
    ...
    async def lookup_token(self, token=None, accessor=False, wrap_ttl=None):
        ...
        if token:
            if accessor:
                ...
                return await (
                    await self._post(path, json=accessor_param, wrap_ttl=wrap_ttl)
                ).json()
            else:
    ...
    async def read_userpass(self, username, mount_point="userpass"):
        ...
        """
        return await (
            await self._get("/v1/auth/{}/users/{}".format(mount_point, username))
        ).json()
    ...
    async def get_vault_ec2_client_configuration(self, mount_point="aws-ec2"):
        ...
        """
        return await (
            await self._get("/v1/auth/{0}/config/client".format(mount_point))
        ).json()
    ...
    async def get_vault_ec2_certificate_configuration(
        self, cert_name, mount_point="aws-ec2"
    ):
        """
    ...
    async def get_ec2_role(self, role, mount_point="aws-ec2"):
        ...
        """
        return await (
            await self._get("/v1/auth/{0}/role/{1}".format(mount_point, role))
        ).json()
    ...
    def auth_ldap(
        self, username, password, mount_point="ldap", use_token=True, **kwargs
    ):
        """
        POST /auth/<mount point>/login/<username>
        """
    ...
    async def get_auth_backend_tuning(self, backend_type, mount_point=None):
        ...
        return await (
            await self._get("/v1/sys/auth/{0}/tune".format(mount_point))
        ).json()
    ...
    async def list_roles(self, mount_point="approle"):
        ...
        return await (
            await self._get("/v1/auth/{0}/role?list=true".format(mount_point))
        ).json()
    ...
    async def get_role(self, role_name, mount_point="approle"):
        ...
        """
        return await (
            await self._get("/v1/auth/{0}/role/{1}".format(mount_point, role_name))
        ).json()

    async def create_role_secret_id(
        self, role_name, meta=None, cidr_list=None, wrap_ttl=None, mount_point="approle"
    ):
        """
    ...
    async def get_role_secret_id_accessor(
    ...
    def delete_role_secret_id_accessor(
        ...
        """
        url = "/v1/auth/{0}/role/{1}/secret-id-accessor/{2}".format(
            mount_point, role_name, secret_id_accessor
        )
        return self._delete(url)

    async def create_role_custom_secret_id(
    ...
    async def transit_export_key(
        self, name, key_type, version=None, mount_point="transit"
    ):
        """
    ...
    async def transit_encrypt_data(
        self,
        name,
        plaintext,
        context=None,
    ...
    async def transit_decrypt_data(
        self,
        name,
        ciphertext,
        context=None,
    ...
    async def transit_generate_data_key(
    ...
    async def transit_generate_rand_bytes(
    ...
    async def transit_generate_hmac(
    ...
    async def transit_verify_signed_data(
        self,
        name,
        input_data,
        algorithm=None,
