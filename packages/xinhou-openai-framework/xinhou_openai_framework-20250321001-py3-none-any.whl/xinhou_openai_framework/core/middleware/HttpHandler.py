...
class HttpHandler:
    ...
    def init_http_filter(app, context):
        ...
        async def add_process_filter(request: Request, call_next):
            ...
            current_route = None
            # 添加调试日志
            logger.debug(f"Current request path: {request.url.path}")
            logger.debug(f"Available routes: {[(route.path, route.methods) for route in request.app.routes if isinstance(route, APIRoute)]}")
                ...
                if isinstance(route, APIRoute):
                    ...
                    if len(route_path_parts) == len(request_path_parts):
                        ...
                        if match and request.method in route.methods:
                            current_route = route
                            logger.debug(f"Matched route: {route.path}, methods: {route.methods}")
                            break
            ...
            if current_route:
                ...
                def check_no_logger(func):
                    logger.debug(f"Checking function: {func.__name__}")
                    if hasattr(func, 'no_logger'):
                        logger.debug(f"Found no_logger attribute on {func.__name__}")
                        return True
                    if hasattr(func, "__wrapped__"):
                        logger.debug(f"Function {func.__name__} has __wrapped__ attribute, checking wrapped function")
                        return check_no_logger(func.__wrapped__)
                    logger.debug(f"No no_logger attribute found on {func.__name__}")
                    return False
                
                logger.debug(f"Endpoint: {current_route.endpoint.__name__}")
                skip_logging = check_no_logger(current_route.endpoint)
                logger.debug(f"Skip logging: {skip_logging}")
            else:
