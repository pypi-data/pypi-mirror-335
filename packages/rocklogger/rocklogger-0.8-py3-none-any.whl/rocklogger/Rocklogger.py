...
class Rocklogger():
    ...
    @classmethod
    def get_instance(cls, level='info', use_date_in_filename=True, base_dir=None, excluded_modules=None):
        """
        ...
        Args:
            ...
            excluded_modules (list, optional): List of module names to exclude from logging.
                                              Default modules like matplotlib, PIL, etc. are
                                              already excluded.
        ...
        with cls._lock:
            if cls._instance is None:
                ...
                # Create the instance
                cls._instance = cls(level, use_date_in_filename, base_dir, excluded_modules)
            else:
                print(f"Using existing Rocklogger instance (created from {cls._caller_info[1]})")
        return cls._instance
    
    def __init__(self, level='info', use_date_in_filename=True, base_dir=None, excluded_modules=None):
        ...
        # Set up module filtering first, before any loggers are configured
        self.__setup_module_filter()
    ...
    def __setup_module_filter(self):
        """Set up a filter to exclude logs from specified modules"""
        # Default excluded modules
        default_excluded = ['matplotlib', 'PIL', 'fontTools', 'tensorflow', 'torch', 'numpy']
        excluded = default_excluded + self.excluded_modules
        
        class ModuleFilter(logging.Filter):
            def __init__(self, excluded_modules):
                super().__init__()
                self.excluded_patterns = [re.compile(f"^{re.escape(mod)}") for mod in excluded_modules]
                
            def filter(self, record):
                # Check if the record's logger name matches any excluded pattern
                for pattern in self.excluded_patterns:
                    if pattern.match(record.name):
                        return False
                return True
        
        # Apply the filter to the root logger to prevent library logs
        root_logger = logging.getLogger()
        for handler in root_logger.handlers[:]:
            root_logger.removeHandler(handler)
        
        # Add the filter to the root logger
        module_filter = ModuleFilter(excluded)
    ...
    def __setup_logger(self, log_lvl):
        ...
        # Get a custom logger with the caller's filename
        logger_name = f'rocklogger.{caller_filename}'
        ...
        # Add a custom console handler to the root logger
        # This will be filtered by our module filter
        root_console = logging.StreamHandler(sys.stdout)
        root_console.setLevel(log_lvl)
    ...
    def add_excluded_module(self, module_name):
        """
        Add a module to the excluded modules list.
        
        Args:
            module_name (str): Name of the module to exclude
        """
...
if __name__ == "__main__":
    ...
    # Example of excluding additional modules
    rocklogger_instance = Rocklogger.get_instance()
    rocklogger_instance.add_excluded_module('requests')
    ...
    try:
        ...
        print("Exception caught in example")
