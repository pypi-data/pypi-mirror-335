...
class DragonTargetCodeError(DragonProcessGroupError):
    pass
...
class MakeJoiner(BaseState):
    ...
    def run(
        ...
        th = threading.Thread(
            target=self._join_runner,
            args=(
                pstate,
                cur_procs,
                props,
                pstate.walltime_event,
            ),
            daemon=False,
        )
        th.start()
    ...
    def _join_runner(pstate: PGState, cur_procs: PGProcessHistory, props: PGProperties, event: threading.Event):
        ...
        try:
            ...
            if not props.ignore_error_on_exit and len(failed_exits) > 0:
                ...
                pstate.exq.put(
                    DragonTargetCodeError(
                        DragonError.TARGET_CODE_ERROR, f"Error(s) in user target code resulted in exit codes: {exit_codes}"
                    )
                )
        ...
        while not self._exq_shutdown.is_set():
            ...
            if self._exq.poll(timeout=self._EXCEPTION_Q_PATIENCE):
                ...
                if isinstance(ex, Exception):
                    ...
                    # there was a failure in user target code.
                    return
