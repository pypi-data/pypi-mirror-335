...
def test_list_datasets(dataset_client):
    ...
    # Now test the list_datasets method
    datasets = dataset_client.list_datasets()
    datasets = datasets.to_dict()
    print("\nList Datasets Response: ", datasets)
    assert datasets is not None
    assert isinstance(datasets, dict)
    assert len(datasets) > 0
    global dataset_name
    if dataset_name is None:
        print("\ndataset_name is None, picking one from response")
        dataset_name = datasets["datasets"][0]
...
def test_get_dataset_task(dataset_client):
    ...
    global dataset_name
    if dataset_name is None:
        print("\ndataset_name is None, picking one from list_datasets")
        response = dataset_client.list_datasets()
        dataset_name = response.datasets[0]
        print("\nPicked dataset_name: ", dataset_name)
        assert dataset_name is not None

    # Now test the get_dataset_task method
    dataset_task = dataset_client.get_dataset_task(dataset_name=dataset_name)
    print("\nDataset Task: ", dataset_task)
    assert dataset_task is not None
    assert hasattr(dataset_task, "dataset_name")
    assert dataset_task.dataset_name == dataset_name
...
def test_get_dataset_task_status(dataset_client):
    ...
    assert dataset_task_status.dataset_name == dataset_name
...
def test_get_datacard(dataset_client):
    ...
    datacard = dataset_client.get_datacard(dataset_name=dataset_name)
    print("\nDatacard: ", datacard)
    assert datacard is not None
    # # Dataset might not be generated yet
    # # TODO: How to handle this?
    # assert hasattr(datacard, "dataset_name")
    # assert datacard.dataset_name == dataset_name
...
def test_get_summary(dataset_client):
    ...
    summary = dataset_client.get_summary(dataset_name=dataset_name)
    print("\nsummary: ", summary)
