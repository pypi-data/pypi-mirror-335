...
class PermissionBasedModelSerializerMixin(_SerializerFFPsContextMetaMixin):
    ...
    def _raise_for_no_fields(self) -> None:
        exc = self.get_no_fields_exception()
        # set "_no_fields_exception" attr to be used in renderer
        # for removing a http method from browsable api
        # instead of raising for a whole page
        setattr(exc, '_no_fields_exception', True)
        raise exc
