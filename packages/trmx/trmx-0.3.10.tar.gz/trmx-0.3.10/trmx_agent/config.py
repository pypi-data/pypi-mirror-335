...
class Config:
    ...
    def list_models(self, provider: Optional[str] = None) -> None:
        ...
        try:
            # Try using ListModelsSkillFactory if available
            if HAS_LIST_MODELS_FACTORY:
                console.print(f"[bold]Fetching models for {provider_to_use}...[/bold]")
                models_list = self._get_models_with_factory(provider_to_use)
                ...
                for model in models_list:
                    ...
                    is_active = model_id == self.active_model and provider_to_use == self.active_provider
                    model_id_display = f"[bold]{model_id}[/bold]" if is_active else model_id
                    status = "[bold green]● Active[/bold green]" if is_active else ""
                    
                    table.add_row(model_id_display, display_name, status)
                
                console.print(table)
                return
        except Exception as e:
            console.print(f"[yellow]Warning: Could not fetch models using ListModelsSkillFactory: {e}[/yellow]")
            console.print("[yellow]Falling back to default model list.[/yellow]")
            
        # Fall back to the default approach if the factory doesn't work
    ...
    def _get_models_with_factory(self, provider: str) -> List[Dict[str, Any]]:
        """Get models using ListModelsSkillFactory."""
        if not HAS_LIST_MODELS_FACTORY:
            raise ImportError("ListModelsSkillFactory not available")
        
        try:
            # Get API key if available
            api_key = self.get_provider_api_key(provider)
            
            # Import and create appropriate credentials if we have an API key
            credentials = None
            if api_key:
                credential_module = f"airtrain.integrations.{provider}.credentials"
                credential_class = f"{provider.capitalize()}Credentials"
                
                # Special case for TogetherAI
                if provider == "together":
                    credential_class = "TogetherAICredentials"
                
                try:
                    module = __import__(credential_module, fromlist=[credential_class])
                    credentials_class = getattr(module, credential_class)
                    credentials = credentials_class(api_key=api_key)
                except (ImportError, AttributeError) as e:
                    console.print(f"[yellow]Warning: Could not create credentials for {provider}: {e}[/yellow]")
            ...
            result = skill.process(GenericListModelsInput(api_models_only=False))
            
            # Return models from the result
            if hasattr(result, "models"):
                return result.models
            else:
                # Handle special cases like Together which has a different output format
                if hasattr(result, "data") and provider == "together":
                    # Convert Together models to standard format
                    models = []
                    for model in result.data:
                        if hasattr(model, "dict"):
                            ...
                            models.append({"id": str(model), "display_name": str(model)})
                    return models
                
                # Fallback: try to convert whatever we got to a list
                return list(result)
                
        except Exception as e:
            raise Exception(f"Failed to get models with factory: {str(e)}")
