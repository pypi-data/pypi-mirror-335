...
velicina = lambda p: int(os.path.getsize(p) / 1024)
LOG = lambda t: print(f"{t}")


class StrategyCopyHuggingfaceModels(IStrategy):
    def velicina_datoteke(path):
    ...
    def execute(
        ...
        if not os.path.exists(source_dir):
            LOG(f"Izvorni direktorij '{source_dir}' ne postoji.")
            return

        if not os.path.exists(destination_dir):
            LOG("Budem kreirao direktorij {}".format(destination_dir))
        ...
        for root, dirs, files in os.walk(source_dir):
            ...
            for file in files:
                ...
                # prikazi velicinu datoteke
                LOG(
                    'kopiram "{}" [{}]'.format(basename, velicina_datoteke(source_path))
                )
                if os.path.exists(target_path):
                    if os.path.samefile(source_path, target_path):
                        break

                # ako vec postoji ne kopiraj
                shutil.copy2(source_path, target_path)
                LOG(f"Kopirano: {basename} -> {dirname}")

        LOG("Kopiranje završeno.")
