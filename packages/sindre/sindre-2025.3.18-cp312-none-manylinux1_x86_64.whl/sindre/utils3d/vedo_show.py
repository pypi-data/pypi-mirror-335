        ...
        **kwargs,
        ...
        def add_text():
            if self.current_i is not  None and self.current_j is not  None and self.current_k is not  None:
                text =vedo.Text2D(f"i={self.current_i} j={self.current_j} k={self.current_k} == {self.o_data[self.current_i,self.current_j,self.current_k]}")
        ...
        def slider_function_x(widget, event):
            ...
            add_text()
            m= vedo.pyplot.matrix(self.o_data[self.current_i,:,:],cmap=self.cmap_slicer,xtitle="X").clone2d(pos=[-0.325, -0.88], size=0.3)
            m.name ="mx"
            self.remove("mx")
        ...
        def slider_function_y(widget, event):
            ...
            m= vedo.pyplot.matrix(self.o_data[:,self.current_j,:],cmap=self.cmap_slicer,xtitle="Y").clone2d(pos=[0, -0.88], size=0.3)
            m.name ="my"
            self.remove("my")
            add_text()
            self.remove("YSlice")
        ...
        def slider_function_z(widget, event):
            ...
            self.zslice.name = "ZSlice"
            add_text()
            m= vedo.pyplot.matrix(self.o_data[:,:,self.current_k],cmap=self.cmap_slicer,xtitle="Z").clone2d(pos=[0.325, -0.88], size=0.3)
            m.name ="mz"
            self.remove("mz")
            self.remove("ZSlice")
            if 0 < k < dims[2]:
