...
class CommandDesign(Command):
    ...
    def __init__(self, config:dict, command_name:str):
        ...
        self.cached_deps_files_only_lines = dict() # keey root DEPS keys w/ lineno info for debug.
    ...
    def resolve_target_core(self, target, no_recursion, caller_info={}):
        ...
        deps_file = util.get_deps_markup_file(target_path)
        data = None
        if self.config['deps_markup_supported'] and not found_deps_file:

            # Have we already unmarshalled this file before?
            data = self.cached_deps_files.get(target_path, None)
            data_only_lines = self.cached_deps_files_only_lines.get(target_path, {})

            # If we haven't, try reading it
            # We also unmarshall the file root keys and line numbers, for better error messaging.
            if data is None and deps_file:
                data = util.deps_markup_safe_load(deps_file)
                data_only_lines = util.deps_markup_safe_load(deps_file, only_root_line_numbers=True)
                self.cached_deps_files[target_path] = data
                self.cached_deps_files_only_lines[target_path] = data_only_lines
        ...
        if data is not None:
            ...
            if target_node not in data:
                ...
                t_path = os.path.relpath(target_path)
                t_node = target_node
                t_full = os.path.join(t_path, t_node)
                deps_path, deps_leaf = os.path.split(deps_file)
                rel_deps_file = os.path.join(os.path.relpath(deps_path), deps_leaf)
                if len(caller_info.keys()) == 0:
                    ...
                    if '.' in target_node:
                        ...
                        self.error(f'Trying to resolve command-line target={t_full}:',
                                   f'was not found in deps_file={rel_deps_file},',
                                   f'possible targets in deps file = {list(data.keys())}')
                else:
                    # If we have caller_info, then this was a recursive call from another
                    # DEPS file. Get some userful info from it for error messaging:
                    caller_path, caller_file = os.path.split(caller_info.get("deps_file", "?"))
                    caller_file = os.path.join(os.path.relpath(caller_path), caller_file)
                    _, caller_target = os.path.split(caller_info.get("target", "?"))
                    line = data_only_lines.get(caller_target, '??')

                    if '.' in target_node:
                        # Likely a filename:
                        self.error(f'Trying to resolve target={t_full} (file?):',
                                   f'called from {caller_file}::{caller_target}::{line=},',
                                   f'File={t_node} not found in directory={t_path}')
                    else:
                        self.error(f'Trying to resolve target={t_full}:',
                                   f'called from {caller_file}::{caller_target}::{line=},',
                                   f'Target not found in deps_file={rel_deps_file}')
            else:
