...
class MultKAN(nn.Module):
    ...
    def fit(self, train_loader, test_loader, opt="LBFGS", epochs=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2.,
            lamb_coef=0.,
            lamb_coefdiff=0., update_grid=True, grid_update_num=10, loss_fn=None, lr=1., start_grid_update_step=-1,
            stop_grid_update_step=50, metrics=None, save_fig=False, in_vars=None, out_vars=None, beta=3,
        ...
        '''
        Modified training function using DataLoader for full dataset iteration each epoch
        ...
        # 定义闭包函数（用于LBFGS）
        def closure(batch_data):
            optimizer.zero_grad()
            inputs, labels = batch_data
            pred = self.forward(inputs, singularity_avoiding=singularity_avoiding, y_th=y_th)
        ...
        for epoch in pbar:
            ...
            for batch_idx, (inputs, labels) in enumerate(train_loader):
                # Grid更新逻辑
                if (epoch * len(train_loader) + batch_idx) % grid_update_freq == 0 and \
                        (epoch * len(train_loader) + batch_idx) < stop_grid_update_step and \
                        update_grid and (epoch * len(train_loader) + batch_idx) >= start_grid_update_step:
                    self.update_grid(inputs)

                if opt == "LBFGS":
                    # 为每个batch重新定义闭包
                    def lbfgs_closure():
                        return closure((inputs, labels))

                    optimizer.step(lbfgs_closure)

                elif opt == "Adam":
                    # 标准Adam优化步骤
                    def adam_step(inputs, labels):
                        optimizer.zero_grad()
                        pred = self.forward(inputs, singularity_avoiding=singularity_avoiding, y_th=y_th)
                        train_loss = loss_fn(pred, labels)
                        if self.save_act:
                    ...
                    loss, reg_ = adam_step(inputs, labels)
                    optimizer.step()
            ...
            with torch.no_grad():
                for inputs, labels in test_loader:
                    ...
                    test_loss = loss_fn_eval(pred, labels)
                    epoch_test_loss += test_loss.item()

            # 计算epoch平均损失
            epoch_train_loss /= len(train_loader)
            epoch_test_loss = torch.sqrt(torch.tensor(epoch_test_loss / len(test_loader)))
            epoch_reg /= len(train_loader)

            # 记录结果
            results['train_loss'].append(epoch_train_loss)
            results['test_loss'].append(epoch_test_loss.item())
            results['reg'].append(epoch_reg)

            # 日志记录
            if epoch % log == 0:
                if display_metrics is None:
                    pbar.set_description(f"Epoch {epoch + 1}/{epochs} | Train Loss: {epoch_train_loss:.2e} | "
                                         f"Test Loss: {epoch_test_loss:.2e} | Reg: {epoch_reg:.2e}")
            else:
                metric_str = " | ".join([f"{m.__name__}: {results[m.__name__][-1]:.2e}" for m in display_metrics])
                pbar.set_description(f"Epoch {epoch + 1}/{epochs} | Train Loss: {epoch_train_loss:.2e} | "
                                     f"Test Loss: {epoch_test_loss:.2e} | {metric_str}")

            # 可视化保存
            if save_fig and epoch % save_fig_freq == 0:
                self.plot(folder=img_folder, in_vars=in_vars, out_vars=out_vars,
                          title=f"Epoch {epoch}", beta=beta)
            plt.savefig(os.path.join(img_folder, f'{epoch}.jpg'),
