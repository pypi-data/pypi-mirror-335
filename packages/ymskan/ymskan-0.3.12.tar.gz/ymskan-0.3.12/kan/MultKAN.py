...
class MultKAN(nn.Module):
    ...
    def fixs(self, dataset: dict, batch_size, batch_size_test, opt="LBFGS", epochs=100, log=1, lamb=0., lamb_l1=1.,
        ...
        def closure():
            ...
            train_loss = loss_fn(pred, batch_train_label)
            if self.save_act:
                if reg_metric == 'edge_backward':
                    self.attribute()
                if reg_metric == 'node_backward':
        ...
        for epoch in pbar:
            ...
            train_indices = np.array(dataset['train_input'].shape[0])
            ...
            if epoch % log == 0:
                if display_metrics is None:
                    pbar.set_description("| train_loss: %.6f | test_loss: %.6f | reg: %.6f | " % (
                        torch.sqrt(train_loss).cpu().detach().numpy(), test_loss.item(),
                        reg_.cpu().detach().numpy()))
                else:
                    string = ''
                    data = ()
            ...
            if save_fig and epoch % save_fig_freq == 0:
                self.plot(folder=img_folder, in_vars=in_vars, out_vars=out_vars, title="Step {}".format(epoch),
                          beta=beta)
                plt.savefig(img_folder + '/' + str(epoch) + '.jpg', bbox_inches='tight', dpi=200)
                plt.close()
                self.save_act = save_act
    ...
    def fix(self, train_loader, test_loader, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2.,
            lamb_coef=0.,
            lamb_coefdiff=0., update_grid=True, grid_update_num=10, loss_fn=None, lr=1., start_grid_update_step=-1,
            stop_grid_update_step=50,
            metrics=None, save_fig=False, in_vars=None, out_vars=None, beta=3, save_fig_freq=1, img_folder='./video',
        ...
        # >>> model.fit(train_loader, test_loader, opt='LBFGS', steps=20, lamb=0.001);
        # >>> model.plot()
        # Most examples in toturals involve the fit() method. Please check them for useness.
        """
        ...
        for _ in pbar:
            ...
            if _ % grid_update_freq == 0 and _ < stop_grid_update_step and update_grid and _ >= start_grid_update_step:
                for inputs, _ in train_loader:
                    self.update_grid(inputs.to(self.device))
            global train_loss, reg_
            # 训练阶段
            train_loss_total = 0
            reg_total = 0
            ...
            reg_avg = reg_total / num_train_batches
            ...
            with torch.no_grad():
                for inputs, labels in test_loader:
                    inputs, labels = inputs.to(self.device), labels.to(self.device)
                    test_loss = loss_fn_eval(self.forward(inputs), labels)
                    test_loss_total += test_loss.item()

            test_loss_avg = test_loss_total / num_test_batches

            if metrics is not None:
                for i in range(len(metrics)):
                    results[metrics[i].__name__].append(metrics[i]().item())

            results['train_loss'].append(np.sqrt(train_loss_avg))
            results['test_loss'].append(np.sqrt(test_loss_avg))
            results['reg'].append(reg_avg)
            log = int(log)
            if display_metrics is None:
                pbar.set_description("| train_loss: %.6f | test_loss: %.6f | reg: %.6f | " % (
                    np.sqrt(train_loss_avg), np.sqrt(test_loss_avg), reg_avg))
            else:
                string = ''
                data = ()
                for metric in display_metrics:
