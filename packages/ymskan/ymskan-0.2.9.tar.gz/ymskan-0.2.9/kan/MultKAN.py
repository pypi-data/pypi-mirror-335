    ...
    def fit(self, train_loader, test_loader, opt="LBFGS", epochs=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2.,
        ...
        -----
            ...
            lamb_coef : float
                coefficient magnitude penalty strength
            lamb_coefdiff : float
                difference of nearby coefficients (smoothness) penalty strength
            update_grid : bool
                If True, update grid regularly before stop_grid_update_step
            grid_update_num : int
                the number of grid updates before stop_grid_update_step
        ...
        def closure():
            ...
            for batch_idx, (x, y) in enumerate(train_loader):
                ...
                if self.save_act:
                    ...
                    if reg_metric == 'node_backward':
                        self.node_attribute()
                    batch_reg = self.get_reg(reg_metric, lamb_l1, lamb_entropy, lamb_coef, lamb_coefdiff)
                else:
                    batch_reg = torch.tensor(0.).to(batch_loss.device)

                objective = batch_loss + lamb * batch_reg
                objective.backward()

                total_train_loss += batch_loss.item()
                total_reg += batch_reg.item()
                batch_count += 1

            train_loss = torch.tensor(total_train_loss / batch_count)
            reg_ = torch.tensor(total_reg / batch_count)
            return train_loss + lamb * reg_
        ...
        for epoch in pbar:
            ...
            if opt == "LBFGS":
                ...
                for batch_idx, (x, y) in enumerate(train_loader):
                    ...
                    loss = batch_loss + lamb * batch_reg
                    optimizer.zero_grad()
                    loss.backward()
                    optimizer.step()
            ...
            # Update results
            results['train_loss'].append(torch.sqrt(train_loss).cpu().detach().numpy())
            results['test_loss'].append(torch.sqrt(torch.tensor(test_loss)).cpu().detach().numpy())
            results['reg'].append(reg_.cpu().detach().numpy())
            ...
            if epoch % log == 0:
                if display_metrics is None:
                    pbar.set_description(
                        f"Epoch {epoch + 1}/{epochs} | Train Loss: {torch.sqrt(train_loss):.2e} | Test Loss: {torch.sqrt(torch.tensor(test_loss)):.2e} | Reg: {reg_:.2e}")
                else:
                    string = ' | '.join([f"{metric}: {results[metric][-1]:.2e}" for metric in display_metrics])
                    pbar.set_description(f"Epoch {epoch + 1}/{epochs} | {string}")

            if save_fig and epoch % save_fig_freq == 0:
                self.plot(folder=img_folder, in_vars=in_vars, out_vars=out_vars, title=f"Epoch {epoch}", beta=beta)
                plt.savefig(os.path.join(img_folder, f'epoch_{epoch}.jpg'), bbox_inches='tight', dpi=200)
                plt.close()
                self.save_act = save_act
