...
class GasInjectionValveResponse(IdsBaseClass):
    ...
    voltage: Optional[np.ndarray] = field(
        default_factory=lambda: np.zeros(shape=(0,) * 1, dtype=float),
        metadata={
            "imas_type": "FLT_1D",
            "ndims": 1,
    ...
    flow_rate: Optional[np.ndarray] = field(
        ...
        metadata={
            "imas_type": "FLT_1D",
            "ndims": 1,
            "coordinates": {"coordinate1": "../voltage"},
            "field_type": np.ndarray,
        },
    )
...
class GasInjectionPipeValve(IdsBaseClass):
    ...
    :ivar pipe_indices : Indices (from the ../../pipe array of structure) of the pipe(s) that are fed by this valve
    :ivar voltage : Voltage applied to open the valve (raw data used to compute the gas flow rate)
    :ivar response_curve : Response curve of the valve, i.e. gas flow rate obtained as a function of the applied voltage.
    """
    ...
    flow_rate_min: Optional[float] = field(
        default=9e40, metadata={"imas_type": "FLT_0D", "field_type": float}
    )
    flow_rate_max: Optional[float] = field(
        default=9e40, metadata={"imas_type": "FLT_0D", "field_type": float}
    )
    ...
    pipe_indices: Optional[np.ndarray] = field(
        default_factory=lambda: np.zeros(shape=(0,) * 1, dtype=int),
        metadata={
            "imas_type": "INT_1D",
            "ndims": 1,
    ...
    response_curve: Optional[GasInjectionValveResponse] = field(
        default=None,
        metadata={
            "imas_type": "gas_injection_valve_response",
            "field_type": GasInjectionValveResponse,
        },
    )
...
class GasInjectionPipe(IdsBaseClass):
    ...
    :ivar name : Short string identifier (unique for a given device)
    :ivar description : Description, e.g. “channel viewing the upper divertor”
    :ivar species : Species injected by the pipe (may be more than one in case the valve injects a gas mixture)
    :ivar length : Pipe length
    :ivar exit_position : Exit position of the pipe in the vaccum vessel
    :ivar second_point : Second point indicating (combined with the exit_position) the direction of the gas injection towards the plasma
    :ivar flow_rate : Flow rate at the exit of the pipe
    :ivar valve_indices : Indices (from the ../../valve array of structure) of the valve(s) that are feeding this pipe
    """
    ...
    exit_position: Optional[Rphiz0DStatic] = field(
        default=None,
    ...
    valve_indices: Optional[np.ndarray] = field(
        default_factory=lambda: np.zeros(shape=(0,) * 1, dtype=int),
        metadata={
            "imas_type": "INT_1D",
            "ndims": 1,
...
class GasInjection(IdsBaseClass):
    ...
    pipe: Optional[GasInjectionPipe] = field(
        default_factory=lambda: StructArray(type_input=GasInjectionPipe),
        metadata={
            "imas_type": "gas_injection_pipe",
            "ndims": 1,
            "coordinates": {"coordinate1": "1...N"},
            "field_type": GasInjectionPipe,
        },
    )
    valve: Optional[GasInjectionPipeValve] = field(
        default_factory=lambda: StructArray(type_input=GasInjectionPipeValve),
        metadata={
            "imas_type": "gas_injection_pipe_valve",
            "ndims": 1,
            "coordinates": {"coordinate1": "1...N"},
            "field_type": GasInjectionPipeValve,
        },
    )
