...
class PolarimeterChannelWavelengthInterf(IdsBaseClass):
    ...
    :ivar fring_jump_correction : Signed number of 2pi phase corrections applied to remove a fringe jump, for each time slice on which a correction has been made
    :ivar fring_jump_correction_times : List of time slices of the pulse on which a fringe jump correction has been made
    ...
    fring_jump_correction: Optional[np.ndarray] = field(
        default_factory=lambda: np.zeros(shape=(0,) * 1, dtype=int),
        metadata={
            "imas_type": "INT_1D",
            "ndims": 1,
            "coordinates": {"coordinate1": "../fring_jump_correction_times"},
            "field_type": np.ndarray,
        },
    )
    fring_jump_correction_times: Optional[np.ndarray] = field(
        default_factory=lambda: np.zeros(shape=(0,) * 1, dtype=float),
        metadata={
            "imas_type": "FLT_1D",
            "ndims": 1,
...
class PolarimeterChannel(IdsBaseClass):
    ...
    :ivar polarization_initial : Initial polarization vector before entering the plasma
    :ivar ellipticity_initial : Initial ellipticity before entering the plasma
    :ivar faraday_angle : Faraday angle (variation of the Faraday angle induced by crossing the plasma)
    ...
    polarization_initial: Optional[float] = field(
        default=9e40, metadata={"imas_type": "FLT_0D", "field_type": float}
    )
    ellipticity_initial: Optional[float] = field(
        default=9e40, metadata={"imas_type": "FLT_0D", "field_type": float}
    )
...
class Polarimeter(IdsBaseClass):
    ...
    channel: Optional[PolarimeterChannel] = field(
        default_factory=lambda: StructArray(type_input=PolarimeterChannel),
        metadata={
            "imas_type": "polarimeter_channel",
            "ndims": 1,
            "coordinates": {"coordinate1": "1...N"},
            "field_type": PolarimeterChannel,
        },
    )
