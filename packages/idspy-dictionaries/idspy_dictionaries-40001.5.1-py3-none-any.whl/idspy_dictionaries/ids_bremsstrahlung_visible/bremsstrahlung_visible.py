...
class BremsstrahlungChannel(IdsBaseClass):
    ...
    :ivar intensity : Intensity, i.e. number of photoelectrons detected by unit time, taking into account electronic gain compensation and channels relative calibration
    :ivar radiance_spectral : Calibrated spectral radiance (radiance per unit wavelength)
    :ivar zeff_line_average : Average effective charge along the line of sight
    """
    ...
    filter: Optional[FilterWavelength] = field(
        default=None,
        metadata={
            "imas_type": "filter_wavelength",
            "field_type": FilterWavelength,
        },
    )
...
class BremsstrahlungVisible(IdsBaseClass):
    ...
    channel: Optional[BremsstrahlungChannel] = field(
        default_factory=lambda: StructArray(type_input=BremsstrahlungChannel),
        metadata={
            "imas_type": "bremsstrahlung_channel",
            "ndims": 1,
            "coordinates": {"coordinate1": "1...N"},
            "field_type": BremsstrahlungChannel,
        },
    )
