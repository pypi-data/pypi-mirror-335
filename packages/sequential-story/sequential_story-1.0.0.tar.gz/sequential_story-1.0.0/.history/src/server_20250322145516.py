...
class SequentialToolsServer:
    ...
    def __init__(self, metadata: dict[str, Any] | None = None, config: dict[str, Any] | None = None) -> None:
        ...
        Args:
            ...
            config: Optional configuration dictionary to control which tools are enabled.
                    ...
                    - enabled_tools: List of tool types to enable. Options: [ToolType.STORY]
                      or [ToolType.THINKING]. By default, all tools are enabled.

        """
        # Initialize settings and configuration
        self.settings = get_settings()
        self.meta = metadata or self.settings.server_metadata
        self.config = config or self.settings.server_config

        # Initialize tool processors and determine enabled tools
        ...
        self.mcp = FastMCP(
            name=self.meta.get("name", "sequential-tools-server"),
            version=self.meta.get("version", "0.1.0"),
            description=self._get_description(),
        )

        # Initialize processors and register tools
        self._initialize_and_register_tools()

    def _resolve_enabled_tools(self) -> set[str]:
        """Determine which tools should be enabled based on configuration."""
        enabled_config = self.config.get("enabled_tools", self.settings.get_enabled_tools())
        return set(enabled_config)
    ...
    def _initialize_and_register_tools(self) -> None:
