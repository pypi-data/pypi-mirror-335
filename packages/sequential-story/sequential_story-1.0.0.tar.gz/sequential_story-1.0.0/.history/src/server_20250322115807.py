...
class SequentialStoryInput(BaseModel):
    ...
    element_number: int = Field(..., alias="elementNumber", description="Current element number in sequence")
    total_elements: int = Field(..., alias="totalElements", description="Current estimate of elements needed")
    next_element_needed: bool = Field(..., alias="nextElementNeeded", description="True if the story should continue")
    is_revision: bool | None = Field(
        None, alias="isRevision", description="A boolean indicating if this revises a previous element"
    )
    revises_element: int | None = Field(
        None, alias="revisesElement", description="If is_revision is true, which element is being reconsidered"
    )
    branch_from_element: int | None = Field(
        None, alias="branchFromElement", description="If branching, which element number is the branching point"
    )
    branch_id: str | None = Field(None, alias="branchId", description="Identifier for the current branch")
    needs_more_elements: bool | None = Field(
        None, alias="needsMoreElements", description="If reaching end but realizing more elements needed"
    )
    ...
    plot_point: str | None = Field(None, alias="plotPoint", description="Optional plot development in this element")
...
class SequentialStoryServer:
    ...
    def create_sequentialstory_tool(self) -> Any:
...
7. Only set nextElementNeeded to false when the story is complete"""
        ...
        def sequentialstory(data: SequentialStoryInput) -> ProcessResult:
            ...
            """
            # We need to convert back to camelCase for the processor
            result = self.story_processor.process_element(data.model_dump(by_alias=True))
            return result
