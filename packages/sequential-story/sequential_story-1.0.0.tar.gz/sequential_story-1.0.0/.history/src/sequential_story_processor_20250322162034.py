...
class StoryElementData(BaseModel):
    ...
    def validate_element_not_empty(cls, v: str) -> str:
        ...
        """
        if not v.strip():
            raise ValueError("Story element cannot be empty")
        return v
    ...
    def validate_positive_numbers(cls, v: int) -> int:
        ...
        """
        if v <= 0:
            raise ValueError("Number values must be positive")
        return v
    ...
    def validate_branch_id(self) -> Self:
        ...
        if self.branch_from_element is not None and self.branch_id is None:
            raise ValueError("branch_id must be set when branch_from_element is set")
        return self
    ...
    def validate_revision_element(self) -> Self:
        ...
        if self.is_revision and self.revises_element is None:
            raise ValueError("revises_element must be set when is_revision is True")
        return self
...
class SequentialStoryProcessor:
    ...
    def process_element(self, element: StoryElementData) -> ProcessResult:
        ...
        try:
            ...
            if element.revises_element is not None and (
                element.revises_element <= 0 or element.revises_element > len(self.element_history)
            ):
                raise ValueError(f"revises_element {element.revises_element} does not refer to an existing element")

            if element.branch_from_element is not None and (
                element.branch_from_element <= 0 or element.branch_from_element > len(self.element_history)
            ):
                raise ValueError(
                    f"branch_from_element {element.branch_from_element} does not refer to an existing element"
                )
