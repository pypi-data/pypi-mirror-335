...
class SequentialToolsServer:
    ...
    def __init__(self, metadata: dict[str, Any] | None = None, config: dict[str, Any] | None = None) -> None:
        ...
        Args:
            ...
            config: Optional configuration dictionary to control which tools are enabled.
                    Available options:
                    - enabled_tools: List of tool types to enable. Options: [ToolType.STORY, ToolType.THINKING]
                      or [ToolType.ALL]. Defaults to [ToolType.ALL].
    ...
    def _resolve_enabled_tools(self) -> set[str]:
        ...
        all_tools = {ToolType.STORY.value, ToolType.THINKING.value}

        # Get enabled tools from config
        enabled_config = self.config.get("enabled_tools", [ToolType.ALL.value])

        # If "all" is in the config, enable all tools
        for tool in enabled_config:
            tool_value = tool.value if isinstance(tool, ToolType) else tool
            if tool_value == ToolType.ALL.value:
                return all_tools

        # Otherwise, add specific enabled tools
        enabled_tools = set()
        for tool in enabled_config:
            tool_value = tool.value if isinstance(tool, ToolType) else tool
            enabled_tools.add(tool_value)

        # Default to all tools if nothing specified
        return enabled_tools if enabled_tools else all_tools
    ...
    def run(self) -> None:
        ...
        print(f"Sequential Tools MCP Server running with enabled tools: {enabled_tools_str}", file=sys.stderr)
