...
# StrEnum values are strings, so this is a list of strings
AVAILABLE_TOOLS: list[str] = [str(tool) for tool in ToolType]
    ...
    def validate_enabled_tools(cls, tools: list[str] | None) -> list[str]:
        ...
        for tool in tools:
            if tool in AVAILABLE_TOOLS:
                ...
                msg = f"Invalid tool: {tool}. Available tools are: {', '.join(AVAILABLE_TOOLS)}"
