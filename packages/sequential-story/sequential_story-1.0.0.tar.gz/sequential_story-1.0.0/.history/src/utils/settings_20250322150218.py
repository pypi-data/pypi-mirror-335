...
class Settings(BaseSettings):
    ...
    def validate_enabled_tools(cls, tools: list[str] | None) -> list[str]:
        ...
        if tools is None:
            return [tool.value for tool in ToolType]

        valid_tools = []
        for tool in tools:
            if tool in [tool.value for tool in ToolType]:
                valid_tools.append(tool)
            else:
                msg = f"Invalid tool: {tool}. Available tools are: {', '.join([tool.value for tool in ToolType])}"
