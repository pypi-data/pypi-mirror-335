...
AVAILABLE_TOOLS: list[str] = [tool.value for tool in ToolType]
...
class Settings(BaseSettings):
    ...
    @classmethod
    def validate_enabled_tools(cls, tools: list[str | ToolType] | None) -> list[ToolType]:
        """Validate that the enabled tools are in the list of available tools."""
        if tools is None:
            return [ToolType(tool) for tool in AVAILABLE_TOOLS]

        valid_tools = []
        for tool in tools:
            tool_str = str(tool)  # Works for both string and ToolType
            if tool_str in AVAILABLE_TOOLS:
                valid_tools.append(ToolType(tool_str))
            else:
                msg = f"Invalid tool: {tool_str}. Available tools are: {', '.join(AVAILABLE_TOOLS)}"
    ...
    def get_enabled_tools(self) -> list[str]:
        ...
        # ToolType is a StrEnum so we can use str() to get the string value
        return [str(tool) for tool in self.tools]
