...
class SequentialToolsServer:
    ...
    def __init__(self, metadata: dict[str, Any] | None = None, config: dict[str, Any] | None = None) -> None:
        ...
        self.mcp = FastMCP(
            ...
            description=self._get_description(meta.get("description", "")),
        )
    ...
    def _get_description(self, base_description: str = "") -> str:
        """Generate appropriate description based on enabled tools.

        Args:
            base_description: Base description from metadata to enhance with tool info.

        Returns:
            Enhanced description including tool information

        """
        thinking_enabled = ToolType.THINKING.value in self.enabled_tools
        story_enabled = ToolType.STORY.value in self.enabled_tools

        # If a base description is provided, use it
        if base_description:
            # If no tools are enabled, just return the base description
            if not thinking_enabled and not story_enabled:
                return base_description

            # Otherwise, enhance it with tool information
            tools_desc = ""
            if thinking_enabled and story_enabled:
                tools_desc = "Sequential Thinking and Sequential Story tools for MCP"
            elif thinking_enabled:
                tools_desc = "Sequential Thinking tool for MCP"
            elif story_enabled:
                tools_desc = "Sequential Story tool for MCP"

            # Return combined description
            return f"{base_description} - {tools_desc}"

        # If no base description, fall back to the original behavior
        if thinking_enabled and story_enabled:
