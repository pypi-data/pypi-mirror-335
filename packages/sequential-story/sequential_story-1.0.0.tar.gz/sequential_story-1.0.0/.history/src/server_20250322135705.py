...
class SequentialToolsServer:
    ...
    def run(self, transport_type: Literal["stdio", "tcp"] = "stdio", port: int | None = None) -> None:
        """Run the server with the specified transport.

        Args:
            transport_type: The transport type to use ("stdio" or "tcp")
            port: The port to use for TCP transport (required if transport_type is "tcp")
        ...
        if transport_type == "stdio":
            ...
            self.mcp.run_with_transport(transport_type="tcp", port=port)
            print(
                f"Sequential Tools MCP Server running on TCP port {port} with Story and Thinking tools",
