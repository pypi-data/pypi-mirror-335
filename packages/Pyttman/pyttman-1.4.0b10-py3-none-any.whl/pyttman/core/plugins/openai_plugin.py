...
class OpenAIPlugin(PyttmanPlugin):
    ...
    detect_memory_prompt = ("Determine if the last message with the user contains "
    ...
    def time_awareness_prompt(self):
        pre_prompt = "You memorized this the following date and time: {}"
        if self.zone_info:
    ...
    def create_memory_if_applicable(self, message) -> str or None:
        ...
        """
        if self.enable_conversations:
            conversation_context = self._prepare_rag_prompt(message)
            detect_memory_prompt = conversation_context + self.detect_memory_prompt
        else:
        ...
        try:
            ...
            if str(memory) == "0":
                return None
            if self.time_aware:
                memory = f"{memory} - {self.time_awareness_prompt}"
            return memory
