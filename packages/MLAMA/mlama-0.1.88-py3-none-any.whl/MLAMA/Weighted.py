...
#     Returns:
#         dict: A dictionary containing the optimal weights for each shift and length combination.
#     """
#     optimal_weights = {}
...
#             #print(data[(data['shift'] == shift) & (data['Length'] == length)])
...
#             # Storing the results
#             optimal_weights[f"{shift} {length}"] = opt_res.x
...
#             combo_data = data[(data['shift'] == shift) & (data['Length'] == length)]
#             print(shift, length)
#             #print('combo_data before ',combo_data.dtypes)
#             # print(combo_data.columns)
#             combo_data['Observed'] = pd.to_numeric(combo_data['Observed'], errors='coerce')

#             for model_name in models:
#               combo_data['diff_'+model_name] = pd.to_numeric(combo_data['diff_'+model_name], errors='coerce')


#             #print('combo_data after', combo_data.dtypes)
#             # Identify numeric columns but ensure Wave is included
#             numeric_columns = combo_data.select_dtypes(include=['number']).columns.tolist()
#             if 'Wave' not in numeric_columns:  # Ensure Wave is retained even if non-numeric
#                 numeric_columns.append('Wave')
#             print('numeric columns', numeric_columns)

#             #print(combo_data[0:1])

#             # Group by 'Wave' and calculate mean for numeric columns
#             combo_data = combo_data[numeric_columns].groupby('Wave', as_index=False).mean()
...
#     Returns:
#         theDD (pd.DataFrame): Summary of average MAPE by algorithm and adaptability (shift).
...
#     # Grouping by shift, Wave, and Length, and summarizing MAPE
#     mape_columns = {f"{model}_MAPE": (f"{model}_MAPE", 'mean') for model in models.keys()}
#     mape_columns['Weighted_MAPE'] = ('Weighted_MAPE', 'mean')

#     theD = joined_df.groupby(['shift', 'Wave', 'Length']).agg(**mape_columns).reset_index()
