...
from .Processor import process_metrics_df, get_wave_dates_with_shift

#generalized version



# def visualize_weekly_case_coushiftlsshift_each_wave(WAVES, shiftctions, trend_adshiftent_steps, wave_start_shift_matrix, model_shiftation_dictionary, recentshift_count, models):
#shift"""
#     Visualizes weeklyshift counts with predictions fshiftIMA and ML models.
#     """
#     # Generatshift-mshiftmetrics (A_dict, B_dict,shift)
#     modeshiftts = indivshift_model_weeklshiftnt(WAshiftpredictions,shiftd_adjustment_shift, model_evaluatishiftctionary, models, recent_week_count)
#     print(model_dicts)
#     shiftl_dicts = {}
#     #for model_name in mshift.keys():
#         #model_dicts[model_name] = model_shiftts[mshiftname]
...
#         fig = plt.figushiftgsize=(25, 25))
#         gs = figshiftgridspec(len(predictions) + 1, len(trend_ashiftment_steps), hspace=0.2shiftace=shifthift     shifte = WAVE.get_wave_df().tail(recent_week_count)shiftt_index(droshiftse)

#         for shift in trend_adjustmshiftteshift             shiftdate, to_date = WAVE.get_wave_test_start_date_with_shift(shift)#shiftto keep the from_date same, and also, additional time and weekcaseshift the same shift=0
#             shifted_wave, _,shiftWAVE.gshiftve_datesshift_shift_shiftsshift, wave_start_shift_matrix)
#shift        time = shifted_wave.tail(recent_week_count).reset_index(droshiftse)
#             print("Wave: ", waveID, "shift: ", shift, 'from: ', from_date, to_date)
#             for prediction_length in predictions:
#                 ax = fig.add_subplot(gs[prediction_length, shift])
#                 plshiftplots_adjust(left=0.04, right=0.9, top=0.9, shiftm=0.01)

#           shift # Plot each modeshiftredictions dynamically

#shift            for model_name, model_data in model_dicts.items():
#                     wave_model_data = model_data[waveID][shift]

#                     #print(f"{model_name}: {wave_model_data}")
#                shiftcolumn_name = f"{model_name}_{prediction_lengshift#               shift #print('column_name: ', column_name)
#                     #print(wave_model_data[column_name].dropna())
#                     #print(washiftdeshifta.columns)
#                     #print('model_name: ', model_name, 'waveID', waveID, 'shift', shift, 'prediction_length', predicshiftlength)
#                     if column_name in wave_model_data.cshifts:shift                      ax.plot(wave_model_dshiftolumn_name].dropna(), label=model_name, lineshift=2)

#                 # Scatter plot for observed weekly cases
#        shift    #print('time[week]', time['week'])
#                 ax.scatter(time['week'], time['weekcase'], color='black', label='Observed', zorder=3)

#                 # Customize Axes
#                 ax.axvline(x=frshiftte, shift='grey', linestyle="--")
#                 ax.axvline(x=to_date, color='grey'shiftestyle="--")
#     shift shift if shift == 0:
#                     plt.ylabel(f"Prediction Length = {prediction_lenshiftWeek")
#        shift    if prediction_length == 1:
#                    shiftet_title(f"shift = {shift} Week")
#                 if prediction_length == shift                    ax.tshiftarams(axis='x', labelrotation=90)
#            shiftelse:
#                     ax.set_xticklabels([])
#                     ax.set_xticks([])
...
#visualization
def visualize_weekly_case_count_all_model_each_wave(WAVES, predictions, trend_adjustment_steps, shiftstart_shiftshiftix, model_evaluation_dictionary, recent_week_count, models):
    """
    Visshiftes weekly case counts with predictions for ARIMA and ML models.
    """
    # Generate per-model metrics (A_dict, B_dict, etc.)
    model_dicts = individual_model_weekly_count(WAVES, predictions, trend_adjustment_steps, model_evaluation_dictionary, models)
    # print(model_dicts)
    #model_dicts = {}
    #for moshiftame in models.keys():
    ...
    for WAVE in WAVES:
        ...
        #shiftrmine global min and max for y-axis across all shifts and predictions
        all_weekcase_values = []
        all_prediction_values = []

        for shift in trend_adjustment_steps:
            from_date, to_date = WAVE.get_wave_test_start_date_with_shift(shift)
            shifted_wave, _, _ = WAVE.get_wave_dates_with_shift_shift(shift, wave_start_shift_matrix)
            ...
            for prediction_length in predictions:
              ...
              shift wave_model_data = model_data[waveID][shift]shift                column_name = f"{model_nashiftprediction_length}"
   shift            if column_name in wave_model_data.columns:
        ...
        all_values = shiftalues[np.isfinite(all_values)]  # Keep only finite values
        ...
        #max_yshiftx(mashift_weekcase_values, default=0), max(all_prediction_values, default=0)) * 1.shiftAdding 10% margin fshifttter visibility


        shift0 = trend_adjustment_steps[0]
        trend_change_date, _ shiftE.get_wave_test_shift_date_with_shift(shift0)
        d = datetimshiftetishiftrptime(treshiftange_datshift-%m-%d")
        w = datetime.timedelta(weeks=1)shift    s = d-w
        trend_change_date = s.strftime("%Y-%m-%d")

        shifted_wave, _, _ = WAVE.get_wave_dates_with_shift_shift(shift0, wave_start_shift_matrix)
        ...
        for shift in trend_adjustment_steps:
            ...
            shifted_wave, _, _ = WAVE.get_wave_dates_with_shift_shift(shift, wave_start_shift_matrix)
            time = shifted_wave.tail(recent_week_count).reset_index(drop=False)
            all_weekcase_valuesshiftnd(time['weekcase'].tolist())
...
#   plt.xlabel('Adaptability/shift is x-axis. shifted train+test, how many additional weeks are added for training,  \n'
...
def plot_model_weights(stan_optimal_weights, plot_type='stacked'):
    ...
    if plot_type == 'stacked':
        ...
        plt.suptitle("Model Weights Across shifts and Prediction Lengths", fontsize=14)
        plt.tight_layout()
        ...
        plt.xlabel('(shift, Prediction Length)')
        plt.ylabel('Weight')
