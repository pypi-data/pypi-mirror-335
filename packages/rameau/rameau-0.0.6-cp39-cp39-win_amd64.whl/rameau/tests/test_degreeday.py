...
from rameau.core.snow import DegreeDayParameters
...
class TestDegreedayParameters(unittest.TestCase):
    ...
    def test_is_instance(self):
        foo = DegreeDayParameters()
        self.assertIsInstance(foo, DegreeDayParameters)

    def test_properties_instances(self):
        for key in DegreeDayParameters._computed_attributes:
            with self.subTest(key):
                foo = DegreeDayParameters(**{key:dict(value=0.3)})
        ...
        for key in DegreeDayParameters._computed_attributes:
            with self.subTest(key):
                with self.assertRaises(TypeError):
                    DegreeDayParameters(**{key:DummyType()})
