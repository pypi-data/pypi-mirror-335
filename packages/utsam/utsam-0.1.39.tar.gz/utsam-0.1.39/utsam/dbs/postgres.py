...
class PgTable(PgDatabase):
    ...
    def set_select_statement(self, where_dict: dict = None):
        ...
        if where_dict:
            where_filters = []
            where_filters += [column(key) == value for key, value in where_dict.items() if not isinstance(value, list)]
            where_filters += [column(key).in_(value) for key, value in where_dict.items() if isinstance(value, list)]
