...
def execute_command(cmd_parts, env, timeout=None):
    ...
    """
    try:
        # Check if we're executing a Python command
        is_python_cmd = cmd_parts[0].endswith('python') or cmd_parts[0].endswith('python3')
        
        # For Python commands, add the -W flag to change warning behavior
        if is_python_cmd and len(cmd_parts) > 1:
            # Add warning control flags to prevent warnings from causing issues
            # Insert after the command but before any script or parameters
            python_cmd = cmd_parts[0]
            script_and_params = cmd_parts[1:]
            
            # Insert the warning flags after the python command
            # This changes warning behavior to not treat warnings as errors
            cmd_parts = [python_cmd, "-W", "once"] + script_and_params
        
        # Run the command with the modified command parts
        ...
        # Always consider it a success based on return code, not stderr content
        return result
