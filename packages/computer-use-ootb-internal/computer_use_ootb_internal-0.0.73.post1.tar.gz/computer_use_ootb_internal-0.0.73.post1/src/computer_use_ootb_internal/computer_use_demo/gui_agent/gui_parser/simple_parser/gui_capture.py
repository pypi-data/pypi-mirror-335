...
class GUICapture:
    ...
    def get_gui_meta_data(self):
        ...
        # current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        # log_str = f"[get_gui_meta_data] success get handle_to_window {handle_to_window.keys()}"
        # os.makedirs(os.path.join(self.cache_folder, current_time), exist_ok=True)
        # save_path = os.path.join(self.cache_folder, current_time, f'gui_capture_log_{current_time}.log')
        # with open(save_path, 'a') as f:
        #     f.write(log_str + '\n')
        ...
        # print(f"[get_gui_meta_data] success get meta_data {meta_data}")
        return meta_data
