...
# Use a context settings dict that allows unknown options to be passed through
...
def run_command(account, role, source_profile, saas_account, saas_role, debug, aws_args):
    ...
    Examples:
        ...
        aws-co run -a XXXXXXXXXXXXXX s3 ls --query "length(@)" --output text  # AWS CLI query
        aws-co run -a XXXXXXXXXXXXXX ec2 describe-instances --filters "Name=instance-state-name,Values=running"  # AWS CLI filters
    """
    if debug:
