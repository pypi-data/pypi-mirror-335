...
def run_aws_command(account_id, profile, role_name, aws_args, source_profile=None, saas_account=None, saas_role=None):
    ...
    # Convert aws_args to a list if it's not already
    if isinstance(aws_args, tuple):
        aws_args = list(aws_args)
    elif not isinstance(aws_args, list):
        aws_args = [aws_args]
    
    # Build the AWS command
    cmd = ["aws"]
    
    # Process AWS CLI arguments to handle complex parameters correctly
    i = 0
    while i < len(aws_args):
        arg = aws_args[i]
        
        # Handle parameters with values that might contain spaces or special characters
        if arg.startswith('--') and i + 1 < len(aws_args) and not aws_args[i+1].startswith('--'):
            # Add the parameter and its value as separate list items
            cmd.append(arg)
            cmd.append(aws_args[i+1])
            i += 2
        else:
    ...
    # Use subprocess.run instead of os.execvpe to handle all AWS CLI parameters
    try:
        result = subprocess.run(cmd, env=env, check=True)
        sys.exit(result.returncode)
    except subprocess.CalledProcessError as e:
        logger.error(f"AWS command failed with exit code {e.returncode}")
