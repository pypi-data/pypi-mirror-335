...
class Database:
    ...
    def select(self, table:str, distinct:bool=False, columns:list=['*'], where:str=None, order_by:str=None, return_mode=ReturnMode.DICT, is_single_record=False):
        """
        ...
        # Execute query
        return self.query(query, return_mode, is_single_record)

    def insert(self, table:str, data:dict, set_input_sizes:list=None):
        with self.login() as connection:
            with connection.cursor() as cursor:
                if set_input_sizes:
                    cursor.setinputsizes(set_input_sizes)
        ...
        with self.login() as connection:
            with connection.cursor() as cursor:
                try:
                    ...
                    if len(keys) > 1:
                        raise KeyError('Table contains more than 1 primary key')
    ...
    def get_primary_key_names(self, table:str) -> list:
        with self.login() as connection:
            with connection.cursor() as cursor:
                try:
                    results = cursor.primaryKeys(table)
    ...
    def get_non_key_attributes(self, table:str):
        keys = self.get_primary_key_names(table=table)
...
if __name__ == "__main__":
    ...
    print(db.get_non_key_attributes('receipt'))
