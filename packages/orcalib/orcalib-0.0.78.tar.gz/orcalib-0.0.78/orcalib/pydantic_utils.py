...
def decode_base64_image(base64_image: str) -> pil.Image:
    return pil.open(io.BytesIO(base64.b64decode(base64_image)))
...
# float32 arrays when using gzip compression, probably not worth the complexity for now
...
def decode_base64_numpy_array(base64_array: str) -> NDArray:
    buffer = io.BytesIO(base64.b64decode(base64_array))
...
InputType = str | Image
InputTypeList = list[str] | list[str] | list[Image] | list[InputType]  # this is not equivalent to list[InputType]
