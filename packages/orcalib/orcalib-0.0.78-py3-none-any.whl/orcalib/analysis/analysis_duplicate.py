...
from ..pydantic_utils import UUID7, InputType
...
class MemoryDuplicateMetrics(BaseModel):
    ...
    possible_duplicate_memory_ids: list[UUID7]
...
class MemorysetDuplicateAnalysisConfig(BaseModel):
    possible_duplicate_threshold: float = 0.97
    """Lookup score threshold for a memory to be considered a possible duplicate"""
...
class MemorysetDuplicateAnalysis(
    ...
    """
    Find exact and possible duplicates in a memoryset
    ...
    def is_duplicate_value(v1: InputType, v2: InputType) -> bool:
        if isinstance(v1, pil.Image) and isinstance(v2, pil.Image):
            hash_image = lambda image: hashlib.md5(image.convert("RGB").tobytes()).hexdigest()
    ...
    def on_batch(
        ...
        for i, memory in enumerate(memories_batch):
            ...
            for neighbor in neighbors_batch[i]:
                ...
                elif neighbor.lookup_score > self.config.possible_duplicate_threshold:
                    possible_duplicate_memory_ids.append(neighbor.memory_id)
            ...
            metrics.append(
                (
                    ...
                    MemoryDuplicateMetrics(
                        ...
                        has_possible_duplicates=len(possible_duplicate_memory_ids) > 0,
