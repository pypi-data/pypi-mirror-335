...
def _create_labeled_memory(
    ...
    *,
    ...
    return LabeledMemory(
        value=value,
        label=label,
        embedding=embedding if embedding is not None else EmbeddingModel.CLIP_BASE.embed([value])[0],
    ...
    repository.insert(
        [
            _create_labeled_memory(
                value=image,
                label=i,
                embedding=EmbeddingModel.CLIP_BASE.embed([image])[0],
                memory_id=uuid7(),
                source_id=None,
            )
...
def test_list_with_filters_memory(repository):
    # Given a storage backend with some data
    mems_to_insert = [
        _create_labeled_memory(
            value="red",
...
def scored_memory_repository(repo_params):
    ...
    config = MemorysetConfig(
        ...
        embedding_model_name=EmbeddingModel.DISTILBERT.name,
        embedding_model_max_seq_length_override=None,
    )
