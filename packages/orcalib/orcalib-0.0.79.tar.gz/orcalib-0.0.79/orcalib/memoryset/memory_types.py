...
from ..pydantic_utils import UNSET, UUID7, InputType, InputTypeList, Metadata, Vector
...
class MemoryMetrics(TypedDict, total=False):
    ...
    Note:
        ...
        indicate if the memory has possible duplicates, check if `possible_duplicate_memory_ids` is
        not `None` for that.
    """
    ...
    """Whether the memory has possible duplicates."""

    possible_duplicate_memory_ids: list[UUID7] | None
    """
    IDs of potentially duplicate memories.

    Note:
        This will be `None` if the memory has no possible duplicates. Exact duplicates
        will not be included in this list. Unlike exact duplicates, this will be set on all
        memories that have possible duplicates not just the first memory of a set of duplicates.
    """
...
class MemoryLookup(Memory):
    ...
    """The similarity score assigned by the reranker if one is attached to the memoryset."""

    reranker_embedding: Vector | None = None
    """The reranker embedding for this memory value if one is attached to the memoryset."""
...
class LabeledMemory(Memory):
    ...
    metrics: LabeledMemoryMetrics | None  # type: ignore
    """Analysis metrics about this memory."""
...
FilterableMemoryField = Literal[
    "value", "label", "metadata", "source_id", "created_at", "updated_at", "metrics", "score"
]
...
class LabeledMemoryLookupColumnResult(TypedDict):
    ...
    memories_reranker_scores: NDArray[np.float32] | None  # batch_size x memory_count
    memories_reranker_embeddings: NDArray[np.float32] | None  # batch_size x memory_count x embedding_dim
    # non-tensor fields
    ...
    def __repr__(self) -> str:
        return "".join(
            [
                ...
                f"    memory_version={self.memory_version},\n",
                f"    lookup_score={self.lookup_score},\n",
                f"    reranker_score={self.reranker_score},\n" if self.reranker_score is not None else "",
                (
                    f"    reranker_embedding=<array.{self.reranker_embedding.dtype}{self.reranker_embedding.shape}>,\n"
                    if self.reranker_embedding is not None
                    else ""
                ),
                ")",
            ]
...
class ScoredMemoryLookupColumnResult(TypedDict):
    input_embeddings: NDArray[np.float32]  # batch_size x embedding_dim
    memories_embeddings: NDArray[np.float32]  # batch_size x memory_count x embedding_dim
