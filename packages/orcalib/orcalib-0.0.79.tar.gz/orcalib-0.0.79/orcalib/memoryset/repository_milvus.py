...
from ..pydantic_utils import Vector, base64_encode_image, decode_base64_image
...
def _parse_row(row: dict[str, Any]) -> dict:
    base_dict = dict(
        ...
        value=decode_base64_image(row["image"]) if _empty_to_none(row["image"], str) is not None else row["text"],
