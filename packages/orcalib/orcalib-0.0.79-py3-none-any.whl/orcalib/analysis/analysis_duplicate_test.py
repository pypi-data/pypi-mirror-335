...
def test_possible_duplicates():
    ...
    # When we run a duplicate analysis with a lower threshold for possible duplicates
    result = run_analyses(
        memoryset,
        MemorysetDuplicateAnalysis(possible_duplicate_threshold=0.95),
        lookup_count=3,
        show_progress_bar=False,
    )["duplicate"]
    ...
    assert isinstance(result, MemorysetDuplicateMetrics)
    assert result.num_duplicates == 1
    # And the possible duplicates are found
    possible_duplicates = memoryset.query(filters=[("metrics.has_possible_duplicates", "==", True)])
    assert len(possible_duplicates) == 3
    assert memoryset[0].metrics is not None
    assert set(memoryset[0].metrics.get("possible_duplicate_memory_ids") or []) == set(
        [memoryset[1].memory_id, memoryset[2].memory_id]
    )
    assert memoryset[1].metrics is not None
    assert memoryset[1].metrics.get("possible_duplicate_memory_ids") == [memoryset[0].memory_id]
    assert memoryset[2].metrics is not None
    assert memoryset[2].metrics.get("possible_duplicate_memory_ids") == [memoryset[0].memory_id]
