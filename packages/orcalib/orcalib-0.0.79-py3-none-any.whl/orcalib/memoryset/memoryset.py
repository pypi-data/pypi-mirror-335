...
from ..dataset_utils import parse_dataset, parse_label_names
from ..progress_utils import OnProgressCallback, safely_call_on_progress
from ..pydantic_utils import UNSET, UUID7, Metadata, Vector
from .embedding_models import EmbeddingModel, EmbeddingModelContext
...
class BaseMemoryset[
    ...
    def _init_base(
        ...
        *,
        ...
        if config is None:
            ...
            if embedding_model and embedding_model.name != config.embedding_model_name:
                raise ValueError(
                    f"Given embedding model ({embedding_model.name}) does not match previously used embedding model ({config.embedding_model_name})."
                )
    ...
    @abstractmethod
    def insert(self, dataset: Dataset | list[TMemory] | list[TInsert] | list[dict], **kwargs):
        """Insert new memories."""
        pass
    ...
    @property
    @abstractmethod
    def value_type(self) -> Literal["string", "image"]:
        """Get the type of values stored."""
        pass
...
class LabeledMemoryset(
    ...
    @property
    def value_type(self) -> Literal["string", "image"]:
        return "string" if isinstance(self[0].value, str) else "image"
    ...
    def insert(
        ...
        **kwargs,
        ...
        if compute_embeddings and insert_num_rows > 0:
            ...
            insert_items = sorted(
                insert_items,
                key=lambda x: len(x.value) if isinstance(x.value, str) else sum(x.value.size),
                reverse=True,
            )
    ...
    def lookup(
        self,
        query: InputTypeList | list[Vector],
        *,
        exclude_exact_match: bool = False,
    ...
    def lookup(  # noqa: C901
        self,
        # TODO: remove list[Vector] as query input after removing old_model.py
        query: InputType | InputTypeList | list[Vector],
        *,
        count: int = 1,
        exclude_exact_match: bool = False,
        ...
        assert len(embedded_queries) == num_queries
        ...
        if rerank:
            ...
            reranked_results = [
                ...
                for q, ms in zip(queries_list, memory_lookups_batch)
            ]
            memory_lookups_batch = [
                [
                    LabeledMemoryLookup(
                        ...
                        **memory_lookups_batch[j][idx].model_dump(),
                    )
        ...
        if return_type == "columns":
            return LabeledMemoryLookupColumnResult(
                ...
                memories_reranker_scores=(
                    np.array([[m.reranker_score for m in memories] for memories in memory_lookups_batch])
                    if rerank
                    else None
                ),
                # TODO: add reranker embeddings
                memories_reranker_embeddings=None,
    ...
    def _update_multi(
        ...
        for memory_id, existing_memory in existing_memories.items():
            ...
            if update.metadata is None:
                ...
                # otherwise merge the metadata
                update.metadata = existing_memory.metadata | update.metadata
            ...
            if update.metrics is not UNSET and update.metrics is not None:
...
class ScoredMemoryset(
    ...
    def lookup(
        ...
        *,
        ...
        embedded_queries, num_queries = self._compute_embeddings(query, use_cache)
        assert len(embedded_queries) == num_queries

        rerank = rerank or (rerank is None and self.reranker is not None)
        if rerank:
            if not self.reranker:
                raise ValueError("rerank is set to true but no reranker model has been set on this scored memoryset")
            count = count * self.reranker.compression
        ...
        if rerank:
            memory_lookups_batch = self._rerank_results(memory_lookups_batch, query, count)

        if return_type == "columns":
            return self._format_column_result(memory_lookups_batch, embedded_queries, rerank)
