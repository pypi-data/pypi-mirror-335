...
class BaseData:
    ...
    @classmethod
    def random(cls, field_name: str, max_value: Union[float,int] = None, min_value: Union[float,int] = 0) -> Any:
        for f in fields(cls):  # type: ignore
            if f.name == field_name and (f.type == int or f.type == float):
                min_ = f.metadata.get('min', min_value) if f.metadata is not MISSING else min_value
                max_ = f.metadata.get('max', max_value) if f.metadata is not MISSING else max_value
                return random.randint(min_, max_) if f.type == int else random.uniform(min_, max_)
        raise ValueError(f"Field '{field_name}' not found in {cls.__name__}")