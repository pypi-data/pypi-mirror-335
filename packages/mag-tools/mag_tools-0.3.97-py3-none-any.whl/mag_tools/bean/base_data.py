...
class BaseData:
    ...
    def set_random_array_3d(
        ...
        :param field_name: 数据类中的字段名
        :param dimens: 数组的形状，例如 [2, 3, 4] 表示 2x3x4 的三维数组
        :param max_value: 最大值（可选）
        :param min_value: 最小值（默认为 0）
        """
        for f in fields(self):   # type: ignore
            # 处理 List[List[List[int]]] 和 List[List[List[float]]] 类型
            if f.name == field_name and f.type in (List[List[List[int]]], List[List[List[float]]],list[list[list[int]]], list[list[list[float]]]):
                min_ = f.metadata.get("min", min_value) if f.metadata else min_value
                max_ = f.metadata.get("max", max_value) if f.metadata else max_value
                ...
                # 生成随机嵌套数组
                value = generate_nested_array([dimens.nx, dimens.ny, dimens.nz])
                setattr(self, field_name, value)
                return value
