...
from typing import Callable, Union, List, Type, cast, Generator, NewType, Iterator, Self
...
class Concept(Annotation):
    """
    :class:`Concept` is a class that contains all the information of a concept
    """
    ...
    def __init__(self, begin_offset: int, end_offset: int, uri: str):
        """
        Initialize a :class:`Concept` instance
        ...
        """
        self._attributes = {}
        super(Concept, self).__init__(begin_offset, end_offset)
        self._uri = uri
    ...
    def __getattr__(self, name: str) -> Union[None, str, Concept]:
        """
        ...
        * ``attributes``: attributes (``dict``) collected on the concept
        ...
        if concept:
            return cast(Concept, concept)

    def find_first(self, uri: str) -> Union[None, Concept]:
        """
    ...
    def find(self, uri: str) -> List[Concept]:
        """
        ...
        :rtype: A ``list`` of :class:`Concept`
        """
        return [concept for concept in self._find(uri)]

    def _find(self, uri: str) -> Generator[Concept, None, None]:
        assert self._sentence_annotations is not None
        ...
        while idx < sent_len:
            ...
            if annotation.is_concept and uri == cast(Concept, annotation).uri:
                yield cast(Concept, annotation)
    ...
    def match(self, call_able: Callable[[str], bool]) -> List[Concept]:
        """
        Find all child concepts with the given URI in this concept
    ...
    def _match(self, call_able: Callable[[str], bool]) -> Generator[Concept, None, None]:
        assert self._sentence_annotations is not None
        idx = _align(self._annotation_idx, self._sentence_annotations)
        sent_len = len(self._sentence_annotations)
        while idx < sent_len:
    ...
    def rich(self) -> str:
        """
        :return: A rich string representation of the concept
        :rtype: ``str``
        """
    ...
    @staticmethod
    def _document_iter(doc, filter: FilterType = _filter_pass_thru) -> Iterator[Concept]:
        for sentence in doc:
    ...
    @staticmethod
    def _sentence_iter(sentence, filter: FilterType = _filter_pass_thru) -> Iterator[Concept]:
        for annotation in sentence:
            if annotation.is_concept and filter(annotation):
                yield annotation

    @staticmethod
    def _concept_iter(concept, filter: FilterType = _filter_pass_thru, align: bool = True) -> Iterator[Concept]:
    ...
    @staticmethod
    def iter(object, filter: FilterType = _filter_pass_thru, align: bool = True) -> Iterator[Concept]:
        """
        ...
        :type filter: Callable accepting a :class:`Concept` and returning a ``bool``

        :return: A generator expression yielding concepts
        :rtype: :class:`Concept <wowool.annotation.concept.Concept>`
        """
        ...
        if isinstance(object, Document) and object.analysis:
            yield from Concept._document_iter(object.analysis, filter)
        elif isinstance(object, Analysis):
            yield from Concept._document_iter(object, filter)
        elif isinstance(object, Sentence):
            yield from Concept._sentence_iter(object, filter)
        elif isinstance(object, Concept):
            yield from Concept._concept_iter(object, filter, align)
        else:
            raise TypeError(f"Expected Document, Analysis, Sentence, or Concept but got '{type(object)}'")

    @staticmethod
    def next(sentence: List[Annotation], object) -> Concept:
        """returns the next Concept"""
        index = object.index + 1
        sentence_length = len(sentence)
        while index < sentence_length:
            if sentence[index].is_concept:
                return cast(Concept, sentence[index])
            else:
                index += 1
        raise ErrorAnnotationNotFound

    @staticmethod
    def prev(sentence: List[Annotation], object) -> Concept:
