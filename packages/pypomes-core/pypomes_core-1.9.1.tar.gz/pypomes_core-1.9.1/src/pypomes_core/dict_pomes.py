...
def dict_hexify(source: dict,
    ...
    """
    Convert the *[key, value]* pairs in *source* into their hexadecimal representantions.
    ...
    Possible transformations of keys and values:
      - *integer* and *float* are changed to the hexadecimal form of their string representation
      - *bytes* e *bytearray* are changed using their built-in *hex()* method
      - *str* is changed to its hexadecimal form
      ...
      - *dict* is recursively *hexified* with *dict_hexify()* (values, only)
      - *list* is recursively *hexified* with *list_hexify()* (values, only)
      - all other types are left unchanged
    ...
    *jsonified* accordingly. For convenience, the possibly modified *source* itself is returned.
    ...
    for key, value in source.items():
        ...
        if hexify_values:
            if isinstance(value, dict):
                ...
                source[key] = str_to_hex(source=str(value))
            elif isinstance(value, Path):
                source[key] = str_to_hex(source=value.as_posix())
            elif isinstance(value, bytes | bytearray):
                source[key] = value.hex()
            elif isinstance(value, date):
                source[key] = str_to_hex(source=value.isoformat())
    ...
    for key in keys:
        if isinstance(key, str):
            source[str_to_hex(key)] = source.pop(key)
        elif isinstance(key, int | float):
            source[str_to_hex(source=str(key))] = source.pop(key)
        elif isinstance(key, Path):
            source[str_to_hex(key.as_posix())] = source.pop(key)
        elif isinstance(key, bytes | bytearray):
            source[key.hex()] = source.pop(key)
        elif isinstance(key, date):
            source[str_to_hex(key.isoformat())] = source.pop(key)
