...
class AcelerAIHttpClient():
    ...
    async def insert_data_external(self, conn_key:str, table: str, data:list[dict], batch_idx:int, batch_size:int) -> None:
        """
        insert data into external connection using http2
        params:
            conn_key: str -> connection key
            table_name: str
            data: list[dict]
        ...
        async with SEM:
            ...
            async with httpx.AsyncClient(http2=True, verify=VERIFY_HTTPS, timeout=timeout) as client:
                ...
                logger.info(f"Inserting batch {batch_idx} into external connection {conn_key}...")
                response = await client.post(f"{QUERY_MANAGER}/QueryData/InsertDataExternal",
                    content = json.dumps(my_body, cls=CustomJSONEncoder), 
                    headers = headers
                )
                if response.status_code != 200:
                    msg = response.json()
                    raise Exception(f"Success: {msg['success']}, Status code: {response.status_code}, Message: {msg['message']}")
