        ...
        if len(itens_nota_separada) > 0:
            ...
            for nf in itens_nota_separada:
                ...
                if cadastro_pre_venda["IsOpened"] == True:
                    if not item_arla:
                        preenchimento_header_pre_venda = await cadastro_pre_venda_header("5667 - VENDA DE COMB OU LUBRI - SEM ESTOQ E COM FINANC", cod_cliente_correto, "21 DIAS", "5656 - VENDA DE COMB OU LUB ADQ DE TERCEIRO C/ FIN S/ ESTO")
                    else:
                    ...
                    if preenchimento_header_pre_venda.sucesso:
                        try:
                            ...
                            # if not correct_nop_selected:
                            #     retorno = f"Não foi possivel encontrar a nop para o item \nEtapas Executadas:\n{steps}"
                            #     return RpaRetornoProcessoDTO(
                            #         sucesso=False,
                            #         retorno=retorno,
                            #         status=RpaHistoricoStatusEnum.Falha,
