                ...
                for item in items:
                    ...
                    try:
                        ...
                        if (evs := self.stats.events
                            ...
                            logger.info(
                                f"google.Synchronizer ({self.user}): synchronized {
                                                       evs} events so far."
                            )
        ...
        try:
            if not contacts_only:
                if self.has_scope_calendar():
                    ...
                    logger.info(
                        f"google.Synchronizer({
                                              self.user}): does not have the necessary scopes to sync Google calendar"
                    )
            ...
            if not cal_only:
                if self.has_scope_contacts():
                    ...
                    logger.info(
                        f"google.Synchronizer({
                                              self.user}): does not have the necessary scopes to sync Google contacts"
                    )
...
class SyncableEvents(dd.Table):
    ...
    def get_request_queryset(cls, ar, **filter):
        ...
        for user, *_ in rt.models.google.CalendarSubscription.objects.values_list(
                "user").distinct():
            if qs is None:
                qs = rt.models.cal.Event.get_outward_insert_update_queryset(
                    user)
            else:
                qs = qs.union(
                    rt.models.cal.Event.get_outward_insert_update_queryset(
                        user))
        if qs is None:
