...
class AnalysisTreeNode(AnyNode):
    ...
    def add_subset_node(self, algo_tuple, keep_arrangement=True):
        ...
        new_node = AnalysisTreeNode(
            node_type="subset",
            parent=parent,
            label=f"Subset node from {algorithm_name}",
    ...
    def available_algorithms(self):
        ...
        An algorithm is considered available if:
        1. Its type is not "annotation".
        2. All conditions specified in `algorithm_metadata["conditions"]`
           match the corresponding attributes of the node.

        Returns:
            dict: A dictionary of available algorithms where keys are algorithm names
        ...
        for algo_name, algo_metadata in concordance.available_algorithms.items():
            # Skip algorithms of type "annotation"
            if algo_metadata.get("algorithm_type") == "annotation":
                continue

            # Check conditions
            # TODO: check this; get always returns the default value -> unimplemented?
            conditions = algo_metadata.get("conditions", {})
            matches = all(
                getattr(self, attr, None) == value for attr, value in conditions.items()
            )

            if matches:
                available_algos[algo_name] = algo_metadata

        return available_algos
