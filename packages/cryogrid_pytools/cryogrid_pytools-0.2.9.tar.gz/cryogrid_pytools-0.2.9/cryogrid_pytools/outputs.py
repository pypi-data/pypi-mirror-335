...
def read_OUT_regridded_FCI2_file(fname: str, deepest_point=None) -> xr.Dataset:
    ...
    ds : xarray.Dataset
        ...
        surface).
    """
    from .matlab_helpers import matlab2datetime, read_mat_struct_flat_as_dict
    ...
    if deepest_point is not None:
        ...
        depth = np.arange(shallowest_point, deepest_point, s, dtype="float32").round(
            significant_number
        )
...
def _read_OUT_regridded_FCI2_parallel(
    fname_glob: str, deepest_point: float, **joblib_kwargs
) -> list:
    """
...
def read_OUT_regridded_FCI2_clusters(
    fname_glob: str, deepest_point: float, **joblib_kwargs
) -> xr.Dataset:
    """
    Reads multiple files that are put out by the OUT_regridded_FCI2 class
    ...
    list_of_ds = _read_OUT_regridded_FCI2_parallel(
        fname_glob, deepest_point, **joblib_kwargs
    )
