...
from experimentalTreatingIsiPol.docConfig import _alpha, _beta, _gamma, _delta, _epsilon, _tau, _zeta, _eta, _omicron, _pi,_rho,_sigma,_upsilon
import pickle
...
class SmoothDataType():
    ...
    def __init__(self, methodology = 'mean', window_length = 5, jump_space = 1, num_divisions = 3):
    ...
    def __smothData(self, x, y):
            ...
            if self.smoth_data.methodology == 'polynomial':
                ...
                for each_x, each_y in zip(x_s,y_s):
                    x_smoth, y_smoth = self.__polynomialSmoth(each_x, each_y)
                    new_x += x_smoth.to_list()
                    new_y += y_smoth.to_list()

                x,y =  pd.Series(new_x), pd.Series(new_y)
            return x,y
    def __polynomialSmoth(self, x,y)->tuple[pd.Series,pd.Series]:
        '''
        ...
        for each_file in os.listdir(parent_dir):
            try:
                if re.search(pattern=r"\d*.csv", string=each_file):
                    ...
                    if testType == 'tensile' and (docConfig == '_alpha' or docConfig == '_beta' or docConfig == '_rho' or docConfig=='_upsilon'):
