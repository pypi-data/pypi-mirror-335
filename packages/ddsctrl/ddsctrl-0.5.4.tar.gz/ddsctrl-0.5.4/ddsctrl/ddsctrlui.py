...
from PyQt5.QtWidgets import QWidget, QLineEdit, QPushButton, QLabel, QGridLayout, \
     ...
     QDoubleSpinBox, QCheckBox, QListWidget, QMessageBox, QDialog

import ddsctrl.constants as cst


#==============================================================================
class DdsBoardSelection(QDialog):
    """Widget dialog dedicated to select a DDS board.
    """

    def __init__(self, dds_list):
        super().__init__()
        self.setWindowTitle("Select a DDS board")
        label = QLabel("Select a DDS board")
        self.ok_btn = QPushButton("OK")
        self.cancel_btn = QPushButton("Cancel")
        self.dds_lw = QListWidget()
        layout = QVBoxLayout()
        layout.addWidget(label)
        layout.addWidget(self.dds_lw)
        layout.addWidget(self.ok_btn)
        layout.addWidget(self.cancel_btn)
        self.setLayout(layout)

        self.dds_lw.addItems(dds_list)
        self.ok_btn.released.connect(self.accept)
        self.cancel_btn.released.connect(self.reject)

    def dds_selected(self):
        return self.dds_lw.currentItem().text()
...
class OutControllerUi(QWidget):
    ...
    def __init__(self, model, ofmax=400000000.0, amax=1023):
        """The constructor.
        """
        super().__init__()
...
class DdsCtrlUi(QWidget):
    ...
    def __init__(self, model="ad9912"):
        """The constructor.
        """
        super().__init__()
        self._setup_ui(model)

    def _setup_ui(self, model):
        """Setup UI and defines basic handling between widget.
        """
        self.ddsctrl_tabs = QTabWidget()
        self.ocontroller_ui = OutControllerUi(model)
        self.icontroller_ui = InControllerUi(model)
...
if __name__ == '__main__':
    ...
    APP = QApplication(sys.argv)
    UI = DdsBoardSelection(['a', 'b', 'c', 'd', 'e'])
    retval = UI.exec_()
    if retval == 1:
        print("Accepted", UI.dds_selected())
    elif retval == 0:
        print("Rejected")
    else:
        print("Undefined")
