...
class BXYTable:
    def __init__(self, x_name, y_name, x_seq, y_seq):
        self.x_seq = self._seq2strlist(x_seq)
        self.y_seq = self._seq2strlist(y_seq)
        self.x_name = x_name
        self.y_name = y_name
        self.dict = self._create_dict(self.x_seq, self.y_seq)
        self._widths = {x: 0 for x in self.y_seq}
        self._update_widths(MyDict({y: y for y in self.y_seq}))

    def set(self, x_element, y_element, content):
        x_element, y_element, content = str(x_element), str(y_element), str(content)
        assert x_element in self.dict.keys(), "x_element 与 不是指定好的属性"
        assert y_element in self.dict[x_element].keys(), "y_element 与 不是指定好的属性"
        self.dict[x_element][y_element] = str(content)

    def get(self, x_element, y_element):
        return self.dict[x_element][y_element]
    def items(self):
        '''
        (key1, key2, value)
        '''
        result = [(x, y, self.dict[x][y]) for x in self.x_seq for y in self.y_seq]
        return result
    ...
    def _create_prefix(self):
        ...
        n = max([self._get_width(x) for x in self.x_seq])
    ...
    def __len__(self):
        return len(self.x_seq) * len(self.y_seq)
...
if __name__ == '__main__':
    my_table = BXYTable("xx","yy",[123, 234], [345, 456])
