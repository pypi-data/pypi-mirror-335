...
async def get_btn_pushes(BOT_TID, MSG_BUTTONS, ENT_VID, ENT_TYPE, BASE_P):
    result = MSG_BUTTONS
    try:
        print(f"{MSG_BUTTONS=} before")
        for btn in MSG_BUTTONS:
            print(f"{btn=}")
            sql = f"SELECT COUNT(*) FROM BOT_{BOT_TID}.PUSH WHERE ENT_VID=$1 AND ENT_TYPE=$2 AND BTN_BID=$3"
            data_cnt = await db_select_pg(sql, (ENT_VID, ENT_TYPE, btn['bid'],), BASE_P)
            pushes = str(data_cnt[0][0]) if len(data_cnt) and len(data_cnt[0]) else '0'
            btn['pushes'] = upper_register_sync(pushes)

        print(f"{MSG_BUTTONS=} fter")
    except Exception as e:
        logger.info(log_ % str(e))
        await asyncio.sleep(round(random.uniform(0, 1), 2))
    finally:
