    ...
    try:
        ...
        while max_cnt > 0:
            ...
            for POCAST_TID in POCAST_TIDs:
                ...
                sql = f"SELECT POST_TID, POST_TYPE, POST_TEXT, POST_MEDIA FROM BOT_{ENT_TID}.POST WHERE POST_TID=$1"
...
async def region_web(bot, username, ENT_TID, ENT_LC, POST_ID, POST_TYPE, POST_TEXT, POST_LNK, POST_BUTTONS, ENT_TYPE,
    ...
    try:
        ...
        sql = f"SELECT VIEW_ID FROM BOT_{ENT_TID}.VIEW WHERE ENT_VID=$1 AND ENT_TYPE=$2"
...
async def get_user_vars(chat_id, ENT_TID, BASE_D):
    USER_GAMES = {}
    USER_VARS = {}
    USER_ISPAID = 0
    try:
        sql = f"SELECT USER_GAMES, USER_VARS, USER_ISPAID FROM BOT_{ENT_TID}.USER WHERE USER_TID=$1"
        data_usr = await db_select_pg(sql, (chat_id,), BASE_D)
...
async def save_posts(chat_id, lz, username, bot, data, tz, MEDIA_D, BASE_D, PROJECT_USERNAME, PROJECT_TYPE):
    ...
    try:
        ...
        if PROJECT_USERNAME == 'FereyPostBot':
            ...
            HASH_STR = f"pst-{ENT_TID}-{POST_TID}-{'0'}"
            ...
            web_val = f"pst-{ENT_TID}-{POST_TID}-{''}-{ENT_TID}"
    ...
    try:
        ...
        if not is_private:
            ...
            try:
                ...
                await asyncio.sleep(round(random.uniform(0, 1), 2))
                address = f"https://t.me/{PROJECT_USERNAME}/web?startapp={ENT_TID}_{POST_TID}_0_post"
        ...
        if POST_CHKBOX['POST_ISTAG'] and POST_TEXT and len(POST_TEXT) < 900:
            sql = f"SELECT USER_ID FROM BOT_{ENT_TID}.USER"
...
async def get_ent_rm(chat_id, reply_markup, ENT_TID, POST_USERTUN, POST_TID, POST_BUTTONS, BASE_D, PROJECT_USERNAME):
    try:
        sql = f"SELECT BTN_BID FROM BOT_{ENT_TID}.PUSH WHERE ENT_VID=$1"
        ...
        for button in sorted_buttons:
            try:
                ...
                if button['knd'] == 'button_in':
                    ...
                    web_id = button['lnk']
                    if button['lnk'].strip() == '':
                        sql = f"SELECT POST_TID FROM BOT_{ENT_TID}.POST"
...
async def add_to_push_podcast(bot, app, user_id, ENT_TID, POST_ID, BASE_D):
    ...
    try:
        try:
            ...
            sql = f"INSERT INTO BOT_{ENT_TID}.PUSH (CHAT_TID, CHAT_FULLNAME, CHAT_USERNAME, POST_ID, BTN_BID) " \
