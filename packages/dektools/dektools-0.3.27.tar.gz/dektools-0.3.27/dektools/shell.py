...
def shell_wrapper(command, check=True, chdir=None, timeout=None, env=None):
    dir_last = None
    if chdir:
        dir_last = os.getcwd()
        os.chdir(chdir)
    err = subprocess.call(shell_quote(command), shell=True, timeout=timeout, env=env)  # equals to bash -c '<command>'
    if check and err:
        if dir_last:
            os.chdir(dir_last)
...
def shell_output(command, error=True, timeout=None, env=None, encode=None):
    return shell_result(command, error, timeout, env, encode)[1]


def shell_exitcode(command, error=True, timeout=None, env=None, encode=None):
    return shell_result(command, error, timeout, env, encode)[0]


def shell_result(command, error=True, timeout=None, env=None, encode=None):
    return getstatusoutput(command, error, timeout, env, encode)


def getstatusoutput(command, error=True, timeout=None, env=None, encode=None):
    try:
        data = subprocess.check_output(
            shell_quote(command), shell=True, timeout=timeout, env=env,
...
def shell_command(command, sync=True, headless=False, quiet=False, ignore=False, timeout=None, env=None):
    result = None
    if os.name == 'nt' and headless:
        result = shell_command_nt_headless(command, sync, timeout=timeout)
    else:
        if not sync:
            command = _cmd_to_async(command)
        if quiet:
            shell_output(command, timeout=timeout, env=env)
        else:
            try:
                subprocess.run(shell_quote(command), check=True, shell=True, timeout=timeout, env=env)
...
def shell_command_nt_headless(command, sync=True, timeout=None, env=None):
    executor = 'wscript'
    if not env:
        env = os.environ.copy()
    ...
    if env.get(marker) == 'true':  # As vbs call deep bug
        if sync:
            return shell_command(command, sync, headless=False, timeout=timeout, env=env)
        else:
...
WScript.quit
    ...
    try:
        subprocess.run(f'{executor} {fp}', check=True, shell=True, timeout=timeout, env=env)
    except subprocess.CalledProcessError as e:
        return e
    return None


def shell_command_nt_as_admin(command, timeout=None, env=None):
    executor = 'wscript'
    if not env:
        env = os.environ.copy()
