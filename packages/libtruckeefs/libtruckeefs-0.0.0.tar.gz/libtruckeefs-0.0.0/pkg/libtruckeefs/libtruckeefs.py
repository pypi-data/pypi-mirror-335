...
class RiverDelta(eons.Functor):
	...
	async def SetRedisInodeValue(this, inode, key, value, expectedValue=None):
		...
		# FIXME: expectedValue checks don't work "Attempt to modify a readonly table script"
		expectedValue = None
...
class Daemon(eons.Functor):
	...
	def Function(this):
		...
		this.executor.Async(this.WorkerLoop())
	...
	async def Worker(this):
		...
		for inode in inodes:
			...
			if file_path and os.path.exists(file_path):
				...
				logging.info(f"{this.name}: File '{file_path}' does not exist for inode {inode.id}")

				logging.info(f"{this.name}: Cleared cache file for inode {inode}")
...
class Inode (eons.Functor):
	...
	async def GetFromCache(
		...
		if (ret is not None):
			...
			if (await ret.GetState('delete') == ProcessState.COMPLETE):
			...
			if (
				...
				logging.info(f"Waiting for {ret.StringId()} to be created...")
				await ret.WaitForState('sync', ProcessState.COMPLETE)
				logging.info(f"{ret.StringId()} has been created. Purging cache.")
				await executor.PurgeCachedInode(ret)
				return None
	...
	async def InitiateSync(this, direction="UP"):
		...
		if (not syncHost):
			logging.debug(f"Sync host not set for {this.StringId()}; will set it to me!")
			syncHost = socket.gethostname()
			await this.SetEphemeral('sync_host', syncHost, expectedValue="")
			...
			if (syncPid):
				try:
					if (socket.gethostname() == syncHost):
						...
						logging.info(f"Sync process already running for {this.StringId()} (PID: {syncPid}).")
						await this.SetEphemeral('sync_again', True)
						this.pending.sync = False
						return
					else:
						logging.error(f"Sync process already running for {this.StringId()} (PID: {syncPid} is running on {syncHost}, but I am {os.getpid()} on {socket.gethostname()}).")
						raise Exception(f"Sync process already running for {this.StringId()} (PID: {syncPid} is running on {syncHost}, but I am {os.getpid()} on {socket.gethostname()}).")
				except OSError:
					syncPid = None
					syncHost = None
			else:
		...
		if (not syncPid):
			...
			sync_process.start()
			await this.SetEphemeral('sync_pid', sync_process.pid, expectedValue="")
			logging.info(f"Sync process started for {this.StringId()} (PID: {sync_process.pid}).")
		else:
			logging.error(f"Sync process already running for {this.StringId()} (PID: {syncPid} is running on {syncHost}, but I am {os.getpid()} on {socket.gethostname()}).")
			raise Exception(f"Sync process already running for {this.StringId()} (PID: {syncPid} is running on {syncHost}, but I am {os.getpid()} on {socket.gethostname()}).")
	...
	async def Save(this, mutated=True):
		...
		try:
			...
			logging.info(f"{this.StringId()} updated in the database.")
		...
		try:
			...
			if (Stage.TAHOE in stages and path is not None):
				...
				if (deleteResponse.status != 200):
					logging.error(f"Error deleting {this.StringId()} from Tahoe: {deleteResponse.status}")
	...
	async def CommonMove(this, dest_upath):
		...
		this.WasJustWrittenTo() # Update the last written time, regardless of if we saved *this or not.
	...
	async def Mknod(this, mode, dev=0):
		logging.debug(f"{this.StringId()} Mknod({mode}, dev={dev}) called.")
	...
	async def Link(this, target):
		logging.debug(f"{this.StringId()} Link(target={target}) called.")
	...
	async def Symlink(this, target):
		logging.debug(f"{this.StringId()} Symlink(target={target}) called.")
...
class File(Inode):
		...
		if (this.data is not None):
			if (not this.data.IsOpen()):
				try:
					...
					logging.error(f"Error opening {this.StringId()}: {e}. Attempting to rectify.")
					this.pending.sync = True
					this.pending.download = True
					await this.SetEphemeral("pending_download", this.pending.download)
	...
	async def Move(this, dest_upath):
		...
		newFilePath = this.executor.cache_dir.joinpath(dest_upath).resolve()
		newFilePath.parent.mkdir(parents=True, exist_ok=True)
		os.rename(this.data.path, newFilePath)
		...
		this.data = FileOnDisk(newFilePath, mode='r+b')
		await this.data.Open()
	...
	async def Move(this, dest_upath):
		...
		for (childId) in this.children:
			try:
				...
				logging.error(f"Error moving child {child.StringId()} to {dest_upath}: {e}")
				continue
...
class RiverFS(eons.Executor):
	...
	def Function(this):
		...
		if (not this.is_daemon):
			this.process.garbage = multiprocessing.Process(
				target=GarbageDaemon.Run,
				args=(this.GetWorkerArgs(),),
				daemon=True,
				name="GarbageDaemon"
			)
			this.process.garbage.start()
			logging.debug(f"GarbageDaemon is {this.process.garbage.is_alive()}.")

		if (this.register_classes and this.test_functionality):
			this.TestFunctionality()

		if (not this.is_daemon and this.cache_maintenance):
			# Create and start the background process.
			this.process.prune = multiprocessing.Process(
				target=CachePruneDaemon.Run,
				args=(this.GetWorkerArgs(),),
				daemon=True,
				name="CachePruneDaemon"
			)
			this.process.prune.start()
			logging.info(f"CachePruneDaemon is {this.process.prune.is_alive()}.")
	...
	def Stop(this):
		...
		for name, process in this.process.items():
			if (process is not None):
				logging.info(f"Stopping {name} daemon...")
				process.terminate()
				process.join()
...
class TahoeSyncWorker(eons.Functor):
	...
	def UpstreamSyncWorker(kwargs):
		...
		# 5 minute timeout.
		for i in range(300):
			syncPid = executor.Async(inode.GetEphemeral('sync_pid'))
			syncHost = executor.Async(inode.GetEphemeral('sync_host'))
			if ((syncPid is None or not len(syncPid) or (syncHost is None or not len(syncHost)))):
				if (i == 299):
					logging.error(f"Sync process for {inodeId} ({inode.name}) timed out.")
					executor.Async(TahoeSyncWorker.CompleteSync(executor, inode, False))
					raise Exception(f"Sync process for {inodeId} ({inode.name}) timed out.")
				else:
					time.sleep(1)
					continue
		...
		while (True):
			...
			try:
				...
				if (frozen_data is None):
					...
					if (not syncAgain):
						logging.info(f"Inode {inode.name} (id: {inodeId}) has synchronized. Exiting.")
						break
	...
	async def PullDownstreamFromSource(executor, inode):
		try:
			...
			logging.info(f"Inode {inode.name} (id: {inode.id}) pulled downstream and saved.")
		except Exception as e:
			logging.error(f"Error syncing {inode.id}: {e}")
			eons.util.LogStack()
			raise e
