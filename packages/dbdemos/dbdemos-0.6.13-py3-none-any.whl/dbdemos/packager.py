...
class Packager:
    ...
    def package_demo(self, demo_conf: DemoConf):
        ...
        # Filter out the 04.4-GenAI-for-churn notebook that has been removed
        filtered_notebooks = [notebook for notebook in demo_conf.notebooks if not notebook.path.endswith("04.4-GenAI-for-churn")]
        
        # Process notebooks in parallel with max 5 workers
        with ThreadPoolExecutor(max_workers=10) as executor:
            # Submit all notebooks for processing and collect futures
            futures = [executor.submit(download_notebook_html, notebook) for notebook in filtered_notebooks]
    ...
    def build_minisite(self, demo_conf: DemoConf, iframe_root_src = "./"):
        # Filter out the 04.4-GenAI-for-churn notebook that has been removed
        notebooks_to_publish = [notebook for notebook in demo_conf.get_notebooks_to_publish() 
                               if not notebook.path.endswith("04.4-GenAI-for-churn")]
