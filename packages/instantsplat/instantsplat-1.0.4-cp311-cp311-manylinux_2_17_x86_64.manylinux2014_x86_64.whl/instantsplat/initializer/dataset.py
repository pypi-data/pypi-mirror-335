...
class InitializedCameraDataset(CameraDataset):
    ...
    def save_colmap_cameras(self, path):
        for f in glob.glob(os.path.join(path, "cameras.*")):
            os.remove(f)  # Remove other cameras
        for f in glob.glob(os.path.join(path, "images.*")):
            os.remove(f)  # Remove other images
