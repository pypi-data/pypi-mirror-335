...
class Wav:
    def __init__(self, data: Union[bytes, str]):
        if isinstance(data, str):
            self.buffer = self._read_file(data)

        elif isinstance(data, bytes):
            self.buffer = data

        self._header_chunk, self._pps_chunk, self._data_chunk = self.read()

    @property
    def header(self) -> HeaderChunk:
        if self._header_chunk is None:
            raise ValueError("This WAV file does not contain any header")
        return self._header_chunk

    @property
    def pps(self) -> PPSChunk:
        if self._pps_chunk is None:
            raise ValueError("This WAV file does not contain any PPS")
        return self._pps_chunk

    @property
    def data(self) -> DataChunk:
        if self._data_chunk is None:
            raise ValueError("This WAV file does not contain any header")
    ...
    def __str__(self):
        return str(self.header) + str(self.pps) + str(self.data)
    ...
    def read(self):
        ...
        while chunkOffset < size:
            ...
            if subchunkid == b"BRA1":
                ...
                brams_header = HeaderChunk(
                    stream.read(struct.calcsize(HeaderChunk.fmt_main))
                )

            elif subchunkid == b"data":
                brams_data = DataChunk(
                    stream.read(subchunksize),
                    np.int16 if self.aformat == 1 else np.float64,
                )

            elif subchunkid == b"BRA2":
                brams_pps = PPSChunk(stream.read(subchunksize))
            else:
                pass
            chunkOffset += subchunksize + 8
