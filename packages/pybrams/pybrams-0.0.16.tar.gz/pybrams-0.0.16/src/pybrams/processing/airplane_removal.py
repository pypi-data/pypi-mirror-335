...
class AirplaneRemoval(InterferenceRemoval):
    ...
    def remove_interference(self):
        ...
        _, axes = plt.subplots(8, 1, figsize=(6, 40))
    ...
    def fit_airplanes(self):
        # Extract airplane parameters
        airplane_frequency = np.array([arr[0] for arr in self.airplanes_dict.values()])
        airplane_amplitude = np.array([arr[1] for arr in self.airplanes_dict.values()])
        airplane_phase = np.array([arr[2] for arr in self.airplanes_dict.values()])
        airplane_fit_residual = np.array(
            [arr[3] for arr in self.airplanes_dict.values()]
        )

        airplane_intervals = np.array(
            [key[0] for key in self.airplanes_dict.keys()], dtype=int
        )
        interpolated_airplane_intervals = np.arange(
            min(airplane_intervals), max(airplane_intervals) + 1
        )

        airplane_average_time = self.start_fit_time[airplane_intervals]
        interpolated_airplane_average_time = self.start_fit_time[
            interpolated_airplane_intervals
        ]

        start_airplane_indices = self.start_fit_indices[airplane_intervals]
        end_airplane_indices = self.end_fit_indices[airplane_intervals]
        start_airplane_fit_time = self.start_fit_time[airplane_intervals]

        median_airplane_residual = np.median(airplane_fit_residual)
        mad_airplane_residual = np.median(
            np.abs(airplane_fit_residual - median_airplane_residual)
        )
        mad_airplane_residual_limit = (
            median_airplane_residual + MAD_FACTOR * MAD_SCALE * mad_airplane_residual
        )

        airplane_residual_indices = np.arange(len(airplane_fit_residual))
        bad_airplane_residual_indices = np.where(
            airplane_fit_residual > mad_airplane_residual_limit
        )[0]
        good_airplane_residual_indices = np.setdiff1d(
            airplane_residual_indices, bad_airplane_residual_indices
        )

        good_airplane_intervals = airplane_intervals[good_airplane_residual_indices]
        good_airplane_average_time = airplane_average_time[
            good_airplane_residual_indices
        ]
        good_airplane_frequencies = airplane_frequency[good_airplane_residual_indices]

        airplane_doppler_effect = airplane_frequency - self.signal.beacon_frequency

        plt.figure()
        plt.plot(
            airplane_average_time[bad_airplane_residual_indices],
            airplane_frequency[bad_airplane_residual_indices],
            "*r",
        )
        plt.plot(good_airplane_average_time, good_airplane_frequencies, "*b")
        plt.title("Airplane frequency as a function of time")
        plt.show()

        plt.figure()
        # plt.plot(airplane_average_time[bad_airplane_residual_indices], airplane_amplitude[bad_airplane_residual_indices], '*r')
        plt.plot(
            airplane_average_time[good_airplane_residual_indices],
            airplane_amplitude[good_airplane_residual_indices],
            "*b",
        )
        plt.title("Airplane amplitude as a function of time")
        plt.show()

        plt.figure()
        plt.plot(airplane_average_time, airplane_fit_residual, "*")
        plt.title("Airplane residual as a function of time")
        plt.show()

        airplane_interference = AirplaneInterference(
            airplane_average_time[good_airplane_residual_indices],
            airplane_doppler_effect[good_airplane_residual_indices],
            airplane_amplitude[good_airplane_residual_indices],
            airplane_fit_residual[good_airplane_residual_indices],
            self.signal.location,
        )
        airplane_frequencies = airplane_interference.fit_doppler_effect()

        linearly_interpolated_airplane_frequencies = np.interp(
            interpolated_airplane_average_time,
            airplane_average_time[good_airplane_residual_indices],
            airplane_frequencies,
        )

        condition_crossing_1 = linearly_interpolated_airplane_frequencies < Config.get(
            "pybrams.processing.beacon_removal", "find_half_range_frequency"
        ) + Config.get(
            "pybrams.processing.beacon_removal", "fit_half_range_frequency"
        ) + Config.get(
        ...
        condition_crossing_2 = linearly_interpolated_airplane_frequencies > -Config.get(
            "pybrams.processing.beacon_removal", "beacon_find_half_range_frequency"
        ) - Config.get(
            "pybrams.processing.beacon_removal", "fit_half_range_frequency"
        ) - Config.get(
            __name__, "fit_half_range_frequency"
        )
        crossing_beacon = np.where(condition_crossing_1 & condition_crossing_2)[0]

        interpolated_intervals_crossing_beacon = interpolated_airplane_intervals[
            crossing_beacon
        ]
        interpolated_indices_crossing_beacon = np.array(
            [
                i
                for i, val in enumerate(airplane_intervals)
                if val in interpolated_intervals_crossing_beacon
            ]
        )

        sorted_final_airplane_average_time = airplane_average_time[
            np.setdiff1d(
                good_airplane_residual_indices, interpolated_indices_crossing_beacon
            )
        ]
        sorted_final_airplane_amplitude = airplane_amplitude[
            np.setdiff1d(
                good_airplane_residual_indices, interpolated_indices_crossing_beacon
            )
        ]

        # airplane_interference_2 = AirplaneInterference(sorted_final_airplane_average_time, airplane_doppler_effect[good_airplane_residual_indices], sorted_final_airplane_amplitude, airplane_fit_residual[good_airplane_residual_indices], self.signal.location)
        # fitted_airplane_amplitude = airplane_interference_2.fit_amplitude_sinc()

        corrected_airplane = np.zeros(len(self.padded_extended_data))
        for j in range(len(airplane_frequency)):
            corrected_airplane[
                start_airplane_indices[j] : end_airplane_indices[j] + 1
            ] = airplane_amplitude[j] * np.cos(
                2
                * np.pi
                * airplane_frequency[j]
                * (
                    self.time_vector[
                        start_airplane_indices[j] : end_airplane_indices[j] + 1
                    ]
        ...
        self.data_airplane = corrected_airplane[
            self.samples_per_interval
            // 2 : (-self.pad_width - self.samples_per_interval // 2)
        ]
