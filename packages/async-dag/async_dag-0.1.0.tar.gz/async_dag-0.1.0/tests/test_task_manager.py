...
async def test_calling_order_of_dag() -> None:
    def define_step(expected_state: int) -> Callable[[int], Awaitable[int]]:
        async def inner(current_state: int) -> int:
            assert expected_state == current_state
            return current_state + 1

        return inner

    async def assert_equal(a: int, b: int) -> None:
        assert a == b

    with build_dag(int) as tm:
        starting_node = tm.add_node(define_step(0))

        node_1_1 = tm.add_node(define_step(1), starting_node)
        node_1_2 = tm.add_node(define_step(1), starting_node)

        node_2_1 = tm.add_node(define_step(2), node_1_1)
        node_2_2 = tm.add_node(define_step(2), node_1_2)
        tm.add_node(assert_equal, node_2_1, node_2_2)
