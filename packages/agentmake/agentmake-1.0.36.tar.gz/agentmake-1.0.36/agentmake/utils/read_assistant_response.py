...
def getChatCompletionText(
    ...
    if stream and not stream_openai_reasoning_model: # openai reasoning models do not support streaming
        ...
        elif backend in ("azure_any", "github_deepseek"):
...
def readStreamingChunks(
    ...
    if isinstance(completion, str):
        # in case of mistral
        return completion
    openai_style = True if backend in ("azure", "azure_any", "custom", "deepseek", "github", "github_deepseek", "googleai", "groq", "llamacpp", "mistral", "openai", "xai") else False
    try:
