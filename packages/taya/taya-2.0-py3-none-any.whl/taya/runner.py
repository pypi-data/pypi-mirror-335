...
def main():
    current_dir = os.path.dirname(os.path.abspath(__file__))  # الحصول على مسار الملف الحالي
    wz_file_path = os.path.join(current_dir, "wz.py")  # إنشاء المسار الكامل للملف wz.py

    while True:
        choice = main_menu()
        if choice == "1":
            if os.path.exists(wz_file_path):
                os.system(f"python {wz_file_path}")  # تشغيل wz.py باستخدام المسار الكامل
            else:
                print(f"{RED}Error: File wz.py not found in the same directory as this script.{RESET}")
        elif choice == "2":
            print(f"{RED}Ngrok starting on port 2222...{RESET}")
