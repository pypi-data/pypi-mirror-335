...
extern "C"
int ggwave_protocolCount() {
    return GGWAVE_PROTOCOL_COUNT;
}

extern "C"
bool ggwave_isDSSEnabled(ggwave_Instance id) {
    GGWave * ggWave = (GGWave *) g_instances[id];
    return ggWave->isDSSEnabled();
}

extern "C"
float ggwave_hzPerSample(ggwave_Instance id) {
    GGWave * ggWave = (GGWave *) g_instances[id];
    return ggWave->hzPerSample();
}

extern "C"
int ggwave_heapSize(ggwave_Instance id) {
    GGWave * ggWave = (GGWave *) g_instances[id];
    return ggWave->heapSize();
}

extern "C"
int ggwave_txTonesSize(ggwave_Instance id) {
    GGWave * ggWave = (GGWave *) g_instances[id];
    return ggWave->txTones().size();
}

extern "C"
char ggwave_txGetTone(ggwave_Instance id, int index) {
    GGWave * ggWave = (GGWave *) g_instances[id];
    return ggWave->txTones()[index];
}

extern "C"
bool ggwave_txHasData(ggwave_Instance id) {
    GGWave * ggWave = (GGWave *) g_instances[id];
    return ggWave->txHasData();
}
...
//
// C interface
// with access to C++ namespace
//

extern "C"
uint8_t ggwave_getDSSMagic(int i) {
    return getDSSMagic(i);
}

extern "C"
void ggwave_rdft(float * f, int N, int * wi, float * wf) {
    FFT(f, N, wi, wf);
}

extern "C"
void ggwave_FFT(const float * src, float * dst, int N, int * wi, float * wf) {
    FFT(src, dst, N, wi, wf);
}

extern "C"
int ggwave_getECCBytesForLength(int len) {
    return getECCBytesForLength(len);
}
