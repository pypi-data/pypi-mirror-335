...
class PDFDeepSeekRAGAgent(BasePDFRAGAgent):
    def initialize_history_aware_retriever(self, retriever, temperature=0.2, max_tokens=256):
        if importlib.util.find_spec("langchain_deepseek") is None:
            raise ImportError(
                "DeepSeek model requires `dspackage[langchain_deepseek]`. Install it with `pip install dspackage["
                "langchain_deepseek]`."
            )
...
class PDFOpenAIRAGAgent(BasePDFRAGAgent):
    def initialize_history_aware_retriever(self, retriever, temperature=0.2, max_tokens=256):
        if importlib.util.find_spec("langchain_openai") is None:
            raise ImportError(
                "OpenAI model requires `dspackage[langchain_openai]`. Install it with `pip install dspackage["
                "langchain_openai]`."
            )
