    ...
    def run(self):
        for i in range(1, self.frame.shape[0]):
            self.vars.position.target_volume[i] = update_target_volume(
                ...
                target_volume_limit=self.vars.input.target_volume_limit[i - 1]
                if self.trade.target_volume_limit_args is not None
                else None,
                redistribute_max_iter=self.trade.target_volume_limit_args[
