...
class DynamoDBAPI:
    ...
    def put_item(
        ...
        """
        is_model = DynamoModel.is_dynamo_model(item.__class__)

        item_dict = item.to_dict() if is_model else item
        item = self.process_item_with_partition(item_dict, partition)
        ...
        self.dynamo_db.put_item(item=item, can_overwrite=can_overwrite)

        # Restore _id after storing the item
        if _id:
            item['_id'] = _id

        result = encode_dict(pop_system_keys(item))
        if is_model:
            return type(item).from_dict(result)  # type: ignore
    ...
    def put_items(
        ...
        is_model = DynamoModel.is_dynamo_model(items[0].__class__)
        processed_items = []
