...
from chainsaws.aws.kms.kms_models import (
    KMSAPIConfig, KMSKeyMetadata, EncryptResponse, DecryptResponse, ListKeysResponse,
    KeyUsage, CustomerMasterKeySpec, KeyManager, OriginType, EncryptionContext,
    GrantConstraints, Tag, GenerateDataKeyResponse, GenerateRandomResponse
)
...
class KMSAPI:
    ...
    def create_key(
        ...
        Returns:
            KMSKeyMetadata: Metadata of the created key
        """
        ...
        return KMSKeyMetadata.from_response(response)
    ...
    def encrypt(
        ...
        Returns:
            EncryptResponse: Encrypted data and metadata
    ...
    def decrypt(
        ...
        Returns:
            DecryptResponse: Decrypted data and metadata
    ...
    def list_keys(
        ...
        Returns:
            ListKeysResponse: List of keys and next page marker
        """
        response = self.kms.list_keys(limit=limit, marker=marker)
        keys = [KMSKeyMetadata.from_response({'KeyMetadata': key})
                for key in response['Keys']]

        result: ListKeysResponse = {'Keys': keys}
        if 'NextMarker' in response:
            result['NextMarker'] = response['NextMarker']
    ...
    def decrypt_string(
        ...
        try:
            ...
            response = self.decrypt(
                ciphertext_blob, encryption_context, grant_tokens, key_id)
