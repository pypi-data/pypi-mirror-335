...
from .data_converter import MonkConverter, DataType
from .exceptions import MonkProgrammingError
...
class MonkCursor:
    ...
    def __init__(self, connection, converter: MonkConverter, **kwargs):
        self.arraysize = 1
    ...
    def execute(self, sql, parameters=None, bulk_parameters=None):
        ...
        if self.connection._closed:
            raise MonkProgrammingError("Connection has been closed")

        if self._closed:
            raise MonkProgrammingError("Cursor has been closed")
    ...
    def next(self):
        ...
        if self.rows is None:
            raise MonkProgrammingError(
                "No result is available for the given request. "
                + "execute() or executemany() must be called first."
            )
        if not self._closed:
            return next(self.rows)
        else:
            raise MonkProgrammingError("Cursor is closed")
    ...
    def _convert_rows(self):
        ...
        if not ("col_types" in self._result and self._result["col_types"]):
            raise ValueError(
                "Not able to apply type conversion currently "
    ...
    def time_zone(self, tz):
        ...
        self._converter.set(DataType.TIMESTAMP_WITHOUT_TZ,
