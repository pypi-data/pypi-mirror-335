...
def parse_input_params(spider):
    """Parse args input parameters, populating ``spider.start_urls`` and
    ``spider.allowed_domains``.
    """
    if hasattr(spider, "urls_file") and spider.urls_file:
        try:
            if spider.urls_file.startswith(("http://", "https://")):
                urls_content = requests.get(spider.urls_file).text
            else:
                with open(spider.urls_file) as f:
                    urls_content = f.read()
        except (requests.RequestException, OSError) as e:
            raise ValueError(f"Cannot access {spider.urls_file}: {e}")
        
        url_results = load_url_list(urls_content)
        # Initialize metadata dictionary
        spider.start_urls_metadata = {}
        
        # Separate URLs from metadata
        start_urls = []
        for item in url_results:
            if isinstance(item, tuple):
                url, metadata = item
                start_urls.append(url)
                spider.start_urls_metadata[url] = metadata
            else:
                # Just a URL with no metadata
                start_urls.append(item)
                
        spider.start_urls = start_urls
    elif hasattr(spider, "urls") and spider.urls:
        # For directly specified URLs list, check if they include metadata
        url_results = spider.urls
        # Initialize metadata dictionary
        spider.start_urls_metadata = {}
        
        # Process URLs and metadata
        start_urls = []
        for item in url_results:
            if isinstance(item, tuple):
                url, metadata = item
        ...
        spider.start_urls = start_urls
    elif hasattr(spider, "url") and spider.url:
        # For a single URL
        spider.start_urls = [spider.url]
        spider.start_urls_metadata = {}
    else:
        # No URL input set.
        spider.start_urls = []
    ...
    # Set allowed_domains based on start_urls.
    if hasattr(spider, "domain"):
        spider.allowed_domains = [spider.domain]
    elif hasattr(spider, "allowed_domains"):
        if not spider.allowed_domains and spider.start_urls:
            spider.allowed_domains = sorted(
                set(get_domain(url) for url in spider.start_urls)
            )
    else:
