...
class MiniRpaSapAutomation:
    ...
    def clear_existed_files_and_screenshots(self, report_save_path, except_str='Common'):
        """ This function is used to clear existed files and screenshots.

        Args:
            report_save_path(str): This is the folder path of save folder
            except_str(str): This is the string to exclude when file name or folder name contains current string
        """
        except_str_list = [item.upper().strip() for item in except_str.replace('，', ',').split(',')]
    ...
    def login_sap(self, sap_system, sap_user, sap_password, need_clear=False, except_str='Common'):
        ...
        """
        if need_clear:
            self.clear_existed_files_and_screenshots(self.report_save_path, except_str)
