...
def main() -> None:
    ...
    with torch.inference_mode():
        ...
        print(args.use_temp_file)
        print(args.no_lazy)
        print(args.metadata)
        print(args.model_name)
        print(args.split_max_tensors)
        print(args.split_max_size)
        print(args.dry_run)
        print(args.no_tensor_first_split)
...
def generate_model_for_acc(model_path: str, outfile: str, outtype: str = "q8_0"):
    ftype_map: dict[str, gguf.LlamaFileType] = {
        "f32": gguf.LlamaFileType.ALL_F32,
        "f16": gguf.LlamaFileType.MOSTLY_F16,
        "bf16": gguf.LlamaFileType.MOSTLY_BF16,
    ...
    hparams = Model.load_hparams(Path(model_path))
    with torch.inference_mode():
        output_type = ftype_map[outtype]
        model_architecture = hparams["architectures"][0]
        ...
        model_instance = model_class(dir_model=Path(model_path), ftype=output_type, fname_out=Path(outfile),
                                     is_big_endian=False, use_temp_file=False, eager=False,
                                     metadata_override=None, model_name=None,
                                     split_max_tensors=0,
                                     split_max_size=split_str_to_n_bytes(str(0)), dry_run=False,
                                     small_first_shard=False)
        model_instance.write()

if __name__ == '__main__':
    # main()
    generate_model_for_acc("./temp", "./Llama/models/temp0.gguf")
