...
class EVMTransaction:
    ...
    def __init__(self, to: str, value: int = 0, data: str = "", gas: Optional[int] = None):
...
class EVMWalletClient:
    ...
    def sign_message(self, message: str) -> Dict[str, str]:
        ...
        signed = self.web3.eth.account.sign_message(message_bytes, private_key=self.account.key)
    ...
    def send_transaction(self, transaction: EVMTransaction) -> Dict[str, str]:
        ...
        signed_tx = self.web3.eth.account.sign_transaction(tx_params, private_key=self.account.key)
