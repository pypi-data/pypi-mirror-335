...
from tkasegui import __version__
import tkasegui.gui.ui as ui
from tkasegui.gui.defaults import read_defaults
from tkasegui.gui.images import Images
from tkasegui.gui.nanoparticle import SetupNanoparticle
from tkasegui.gui.nanotube import SetupNanotube
from tkasegui.gui.save import save_dialog
from tkasegui.gui.settings import Settings
from tkasegui.gui.status import Status
from tkasegui.gui.surfaceslab import SetupSurfaceSlab
from tkasegui.gui.view import View
...
class GUI(View, Status):
    ...
    def constraints_window(self):
        from tkasegui.gui.constraints import Constraints
    ...
    def bonds(self):
        from tkasegui.gui.bonds import BondsStat
        self.bonds_window = BondsStat(self)

    def rdf(self):
        from tkasegui.gui.rdf import Rdf
        self.rdf_window = Rdf(self)
    ...
    def bulk_modulus(self):
        try:
            ...
            e = [a.get_energy() for a in self.images]
    ...
    def modify_atoms(self, key=None):
        from tkasegui.gui.modify import ModifyAtoms
        ModifyAtoms(self)

    def add_atoms(self, key=None):
        from tkasegui.gui.add import AddAtoms
        return AddAtoms(self)

    def cell_editor(self, key=None):
        from tkasegui.gui.celleditor import CellEditor
        return CellEditor(self)

    def quick_info_window(self, key=None):
        from tkasegui.gui.quickinfo import info
    ...
    def calculator_window(self):
        from tkasegui.gui.calculator import SetCalculator
        return SetCalculator(self)

    def energy_window(self):
        from tkasegui.gui.energyforces import EnergyForces
        return EnergyForces(self)

    def energy_minimize_window(self):
        from tkasegui.gui.minimize import Minimize
        return Minimize(self)

    def dynamics_window(self):
        from tkasegui.gui.dynamics import Dynamics
        return Dynamics(self)
        ...
        return [
            ...
            (_('_Simulation'),
             [M(_('Set _Calculator ...'), self.calculator_window, disabled=False),
              ...
              M(_('Relaxation ...'), self.energy_minimize_window,
                disabled=False),
              M(_('Dynamics ...'), self.dynamics_window,
                disabled=False)]),

            (_('_Analysis'),
             [M(_('Av. bonds ...'), self.bonds),
              M(_('RDF ...'), self.rdf),
              M(_('NE_B plot'), self.neb),
              M(_('B_ulk Modulus'), self.bulk_modulus)]),

            (_('_Help'),
             [M(_('_About'), partial(ui.about, 'TK ASE GUI (based on ASE)',
                                     version=__version__,
                                     webpage='https://wiki.fysik.dtu.dk/'+\
                                     'ase/ase/gui/gui.html')
                                     ),
              M(_('ASE webpage ...'), webpage)])]
