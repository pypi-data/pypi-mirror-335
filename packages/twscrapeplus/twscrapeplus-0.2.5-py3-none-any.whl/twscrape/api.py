...
# Experimental Features -------------------------------------------------------------------------------------------------------------
    ...
    async def dm_alt(self, text: str, receivers: list[int] | int | str, media: str = None, wait_for_account=False):
        ...
        # 不使用QueueClient，而是直接获取账号并使用其客户端
        # 这样确保所有cookie和headers完全匹配
        if wait_for_account:
            account = await self.pool.get_for_queue_or_wait(queue)
        else:
            account = await self.pool.get_for_queue(queue)
        ...
        try:
            ...
            if self.debug:
                print(f"使用账号 {account.username} 发送私信")
            ...
            if not rep or rep.status_code >= 400:
                error_msg = f"请求失败: {rep.status_code if rep else 'No response'}"
                if rep:
                    error_msg += f" - {rep.text}"
                if self.debug:
                    print(error_msg)
                return {"error": error_msg}
            ...
            if "data" in response_data and "create_dm" in response_data["data"]:
                ...
                if create_dm and create_dm.get("__typename") == "CreateDmFailed":
                    ...
                    if failure_type == "SenderDoesNotHavePrivilegeToDmRecipient":
                        if self.debug:
                            print(f"发送者无权向接收者发送私信")
                        return {"error": f"发送者无权向接收者发送私信", "raw_response": response_data}
