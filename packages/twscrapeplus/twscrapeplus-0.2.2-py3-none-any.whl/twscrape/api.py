...
# Experimental Features -------------------------------------------------------------------------------------------------------------
    ...
    async def _upload_media_alt(self, filename: str, is_dm: bool = False, is_profile=False):
        ...
        async with QueueClient(self.pool, queue, self.debug, proxy=self.proxy) as client:
            ...
            if not rep or rep.status_code >= 400:
                return None
            ...
            try:
                ...
                with open(file, 'rb') as fp:
                    ...
                    while chunk := fp.read(UPLOAD_CHUNK_SIZE):
                        ...
                        rep = await client.req('POST', url=url, params=params, files=files)
                        if not rep or rep.status_code >= 400:
                            return None
                        i += 1
    ...
    async def dm_alt(self, text: str, receivers: list[int], media: str = None):
        """使用原生GraphQL调用发送私信"""
        if not receivers:
            return {"error": "接收者列表不能为空"}
        ...
        # 处理媒体
        if media:
            media_id = await self._upload_media_alt(media, is_dm=True)
            if media_id:
                variables['message'] = {'media': {'id': media_id, 'text': text}}
        
        # 使用原生GraphQL调用
        try:
            rep = await self._gql_item(
                OP_SendMessageMutation,
                variables,
                method="POST"
            )
            ...
            if rep.status_code >= 400:
                return {"error": f"发送失败: {rep.status_code} - {rep.text}"}
