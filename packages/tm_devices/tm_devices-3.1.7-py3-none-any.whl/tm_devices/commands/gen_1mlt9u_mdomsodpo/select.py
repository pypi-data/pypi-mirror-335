...
class SelectRfPhase(SCPICmdWrite, SCPICmdRead):
    ...
    Description:
        - This command switches the RF Phase vs. Time trace display on or off in the time domain
          graticule.

    Usage:
        - Using the ``.query()`` method will send the ``SELect:RF_PHASe?`` query.
        - Using the ``.verify(value)`` method will send the ``SELect:RF_PHASe?`` query and raise an
...
class SelectRfFrequency(SCPICmdWrite, SCPICmdRead):
    """The ``SELect:RF_FREQuency`` command.

    Description:
        - This command switches the RF Frequency vs. Time trace display on or off in the time domain
          graticule.

    Usage:
        - Using the ``.query()`` method will send the ``SELect:RF_FREQuency?`` query.
        - Using the ``.verify(value)`` method will send the ``SELect:RF_FREQuency?`` query and raise
          an AssertionError if the returned value does not match ``value``.
...
class SelectRfAmplitude(SCPICmdWrite, SCPICmdRead):
    """The ``SELect:RF_AMPlitude`` command.

    Description:
        - This command switches the RF Amplitude vs. Time trace display on or off in the time domain
          graticule.

    Usage:
        - Using the ``.query()`` method will send the ``SELect:RF_AMPlitude?`` query.
        - Using the ``.verify(value)`` method will send the ``SELect:RF_AMPlitude?`` query and raise
          an AssertionError if the returned value does not match ``value``.
...
class Select(SCPICmdRead):
    ...
    def __init__(self, device: Optional["PIControl"] = None, cmd_syntax: str = "SELect") -> None:
        ...
        self._rf_frequency = SelectRfFrequency(device, f"{self._cmd_syntax}:RF_FREQuency")
    ...
    @property
    def rf_amplitude(self) -> SelectRfAmplitude:
        """Return the ``SELect:RF_AMPlitude`` command.

        Description:
            - This command switches the RF Amplitude vs. Time trace display on or off in the time
              domain graticule.

        Usage:
            - Using the ``.query()`` method will send the ``SELect:RF_AMPlitude?`` query.
            - Using the ``.verify(value)`` method will send the ``SELect:RF_AMPlitude?`` query and
              raise an AssertionError if the returned value does not match ``value``.
            - Using the ``.write(value)`` method will send the ``SELect:RF_AMPlitude value``
              command.
    ...
    @property
    def rf_frequency(self) -> SelectRfFrequency:
        """Return the ``SELect:RF_FREQuency`` command.

        Description:
            - This command switches the RF Frequency vs. Time trace display on or off in the time
              domain graticule.

        Usage:
            - Using the ``.query()`` method will send the ``SELect:RF_FREQuency?`` query.
            - Using the ``.verify(value)`` method will send the ``SELect:RF_FREQuency?`` query and
              raise an AssertionError if the returned value does not match ``value``.
            - Using the ``.write(value)`` method will send the ``SELect:RF_FREQuency value``
              command.
    ...
    @property
    def rf_phase(self) -> SelectRfPhase:
        ...
        Usage:
            - Using the ``.query()`` method will send the ``SELect:RF_PHASe?`` query.
            - Using the ``.verify(value)`` method will send the ``SELect:RF_PHASe?`` query and raise
              an AssertionError if the returned value does not match ``value``.
            - Using the ``.write(value)`` method will send the ``SELect:RF_PHASe value`` command.
