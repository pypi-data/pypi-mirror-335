...
class WfmoutpreRecordlength(SCPICmdRead):
    """The ``WFMOutpre:RECOrdlength`` command.

    Description:
        - Returns the record length for the source waveform as specified by the ``DATA:SOURCE``
          command.

    Usage:
        - Using the ``.query()`` method will send the ``WFMOutpre:RECOrdlength?`` query.
        - Using the ``.verify(value)`` method will send the ``WFMOutpre:RECOrdlength?`` query and
...
class WfmoutpreFractional(SCPICmdRead):
    ...
    Description:
        - The set form of this command is ignored. The query form always returns a 0, if the
          ...
          displayed, the query form generates an error and returns event code 2244. This command is
          for compatibility with other Tektronix oscilloscopes.

    Usage:
        - Using the ``.query()`` method will send the ``WFMOutpre:FRACTional?`` query.
        - Using the ``.verify(value)`` method will send the ``WFMOutpre:FRACTional?`` query and
          raise an AssertionError if the returned value does not match ``value``.
...
class WfmoutpreFilterfreq(SCPICmdRead):
    """The ``WFMOutpre:FILTERFreq`` command.

    Description:
        - Returns the FilterVu low pass filter frequency applied to the LRL waveform record of the
          source waveform specified by ``DATA:SOURCE``. If the ``DATA:RESOLUTION`` is set to FULL,
          then this is the filter frequency applied to the full resolution (LRL) waveform. If the
          ``DATA:RESOLUTION`` is set to REDUced, then this is the filter frequency applied to the
          reduced resolution (thumbnail) waveform.

    Usage:
        - Using the ``.query()`` method will send the ``WFMOutpre:FILTERFreq?`` query.
        - Using the ``.verify(value)`` method will send the ``WFMOutpre:FILTERFreq?`` query and
          raise an AssertionError if the returned value does not match ``value``.
...
class WfmoutpreComposition(SCPICmdWrite, SCPICmdRead):
    ...
    Usage:
        ...
        - Using the ``.verify(value)`` method will send the ``WFMOutpre:COMPosition?`` query and
          raise an AssertionError if the returned value does not match ``value``.
...
class WfmoutpreBnFmt(SCPICmdWrite, SCPICmdRead):
    ...
    Description:
        - Sets or returns the format of binary data for outgoing waveforms specified by the
          ``DATA:SOURCE`` command. Changing the value of ``WFMOUTPRE:BN_FMT`` also changes the value
          of ``DATA:ENCDG``.
...
class Wfmoutpre(SCPICmdRead):
    ...
    def __init__(self, device: Optional["PIControl"] = None, cmd_syntax: str = "WFMOutpre") -> None:
        ...
        self._composition = WfmoutpreComposition(device, f"{self._cmd_syntax}:COMPosition")
        self._encdg = WfmoutpreEncdg(device, f"{self._cmd_syntax}:ENCdg")
        self._filterfreq = WfmoutpreFilterfreq(device, f"{self._cmd_syntax}:FILTERFreq")
        self._fractional = WfmoutpreFractional(device, f"{self._cmd_syntax}:FRACTional")
        ...
        self._recordlength = WfmoutpreRecordlength(device, f"{self._cmd_syntax}:RECOrdlength")
    ...
    def bn_fmt(self) -> WfmoutpreBnFmt:
        ...
        Description:
            - Sets or returns the format of binary data for outgoing waveforms specified by the
              ``DATA:SOURCE`` command. Changing the value of ``WFMOUTPRE:BN_FMT`` also changes the
              value of ``DATA:ENCDG``.
    ...
    @property
    def composition(self) -> WfmoutpreComposition:
        ...
        Usage:
            ...
            - Using the ``.verify(value)`` method will send the ``WFMOutpre:COMPosition?`` query and
              raise an AssertionError if the returned value does not match ``value``.
            - Using the ``.write(value)`` method will send the ``WFMOutpre:COMPosition value``
              command.
    ...
    @property
    def filterfreq(self) -> WfmoutpreFilterfreq:
        """Return the ``WFMOutpre:FILTERFreq`` command.

        Description:
            - Returns the FilterVu low pass filter frequency applied to the LRL waveform record of
              the source waveform specified by ``DATA:SOURCE``. If the ``DATA:RESOLUTION`` is set to
              FULL, then this is the filter frequency applied to the full resolution (LRL) waveform.
              If the ``DATA:RESOLUTION`` is set to REDUced, then this is the filter frequency
              applied to the reduced resolution (thumbnail) waveform.
    ...
    @property
    def fractional(self) -> WfmoutpreFractional:
        """Return the ``WFMOutpre:FRACTional`` command.

        Description:
            - The set form of this command is ignored. The query form always returns a 0, if the
              waveform specified by ``DATA:SOUrce`` is on or displayed. If the waveform is not
              displayed, the query form generates an error and returns event code 2244. This command
              is for compatibility with other Tektronix oscilloscopes.
    ...
    @property
    def recordlength(self) -> WfmoutpreRecordlength:
        ...
        Usage:
            - Using the ``.query()`` method will send the ``WFMOutpre:RECOrdlength?`` query.
            - Using the ``.verify(value)`` method will send the ``WFMOutpre:RECOrdlength?`` query
