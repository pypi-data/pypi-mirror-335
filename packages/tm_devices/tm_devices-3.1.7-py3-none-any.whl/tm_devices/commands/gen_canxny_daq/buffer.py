...
class BufferWrite(BaseTSPCmd):
    ...
    def reading(
        ...
        Args:
            ...
            channel (optional): A string that specifies the channel to which to assign the data.
        ...
        try:
            function_args = ", ".join(
                ...
                for x in (
                    ...
                    fractional_seconds,
                    status,
                    f'"{channel}"' if channel is not None else None,
                )
                if x is not None
            )
            self._device.write(  # type: ignore[union-attr]
...
class Buffer(BaseTSPCmd):
    ...
    def channelmath(
        ...
        try:
            ...
            msg = f"No TSPControl object was provided, unable to run the ``{self._cmd_syntax}.channelmath()`` function."  # noqa: E501
            raise NoDeviceProvidedError(msg) from error
    ...
    def getstats(
        ...
        abs_start_time: Optional[float] = None,
        abs_start_fractional: Optional[str] = None,
        abs_end_time: Optional[float] = None,
        abs_end_fractional: Optional[str] = None,
    ) -> Dict[Any, Any]:
        ...
        Args:
            ...
            abs_start_time (optional): An integer that represents the absolute start time in
                seconds.
            abs_start_fractional (optional): An integer that represents the portion of the absolute
                start time that is in fractional seconds.
            abs_end_time (optional): An integer that represents the absolute end time in seconds.
            abs_end_fractional (optional): An integer that represents the portion of the absolute
                end time that is in fractional seconds.
