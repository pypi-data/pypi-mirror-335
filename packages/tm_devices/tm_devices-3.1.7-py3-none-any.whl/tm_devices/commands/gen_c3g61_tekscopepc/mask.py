...
class MaskMaskItemSegItemPoints(SCPICmdWrite, SCPICmdRead):
    ...
    Usage:
        ...
        - Using the ``.verify(value)`` method will send the ``MASK:MASK<x>:SEG<x>:POINTS?`` query
          and raise an AssertionError if the returned value does not match ``value``.
        - Using the ``.write(value)`` method will send the ``MASK:MASK<x>:SEG<x>:POINTS value``
          command.
    ...
    Info:
        ...
        - ``<NR3>`` specifies the X/Y coordinates of all points in the designated mask segment.
    """


class MaskMaskItemSegItemCountHits(SCPICmdRead):
...
class MaskMaskItemSegItemCount(SCPICmdRead):
    ...
    @property
    def hits(self) -> MaskMaskItemSegItemCountHits:
...
class MaskMaskItemSegItem(ValidatedDynamicNumberCmd, SCPICmdRead):
    ...
    Properties:
        - ``.count``: The ``MASK:MASK<x>:SEG<x>:COUNT`` command tree.
    ...
    @property
    def count(self) -> MaskMaskItemSegItemCount:
