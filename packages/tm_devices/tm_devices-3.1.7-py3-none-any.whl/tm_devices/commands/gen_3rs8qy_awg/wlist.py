...
class WlistWaveformRotate(SCPICmdRead):
    ...
    Properties:
        - ``.points``: The ``WLISt:WAVeform:ROTate:POINts`` command.
...
class WlistWaveformDataI(SCPICmdWrite, SCPICmdReadWithArguments):
    ...
    Description:
        - This command transfers analog waveform data from the external controller into the
          ...
          the command ``WLIST:WAVEFORM:DATA:Q``. This command has a limit of 999,999,999 bytes of
          data. The IEEE 488.2 limits that the largest read or write that may occur in a single
          command is 999,999,999 bytes. Because of the size limitation, it is suggested that the
    ...
    def i(self) -> WlistWaveformDataI:
        ...
        Description:
            - This command transfers analog waveform data from the external controller into the
              ...
              use the command ``WLIST:WAVEFORM:DATA:Q``. This command has a limit of 999,999,999
              bytes of data. The IEEE 488.2 limits that the largest read or write that may occur in
              a single command is 999,999,999 bytes. Because of the size limitation, it is suggested
              that the user make use of the starting index (and size for querying) to append data in
...
class WlistWaveform(SCPICmdRead):
    ...
    Properties:
        ...
        - ``.aoffset``: The ``WLISt:WAVeform:AOFFset`` command.
        - ``.data``: The ``WLISt:WAVeform:DATA`` command tree.
        ...
        - ``.rotate``: The ``WLISt:WAVeform:ROTate`` command tree.
        - ``.scale``: The ``WLISt:WAVeform:SCALe`` command.
        - ``.sdcorrection``: The ``WLISt:WAVeform:SDCorrection`` command.
    ...
    def __init__(self, device: Optional["PIControl"], cmd_syntax: str) -> None:
        ...
        self._aoffset = WlistWaveformAoffset(device, f"{self._cmd_syntax}:AOFFset")
        ...
        self._scale = WlistWaveformScale(device, f"{self._cmd_syntax}:SCALe")
        self._sdcorrection = WlistWaveformSdcorrection(device, f"{self._cmd_syntax}:SDCorrection")
