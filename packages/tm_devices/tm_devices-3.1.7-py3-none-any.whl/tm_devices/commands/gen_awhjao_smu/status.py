...
class StatusQuestionable(BaseTSPCmd):
    ...
    Constants:
        ...
        - ``.CALIBRATION``: B8. An enabled bit in the questionable status calibration summary event
          register is set.
        - ``.HIGHV_NOT_READY``: B10. Either the interlock is not engaged or the interlock was
          engaged recently and the high voltage supply is still stabilizing. If the interlock is
          engaged and this bit is set, attempting to turn on the output on the 200 V range results
          in error code 5052, 'Interlock engaged; system stabilizing.'.
...
class Status(BaseTSPCmd):
    ...
    def questionable(self) -> StatusQuestionable:
        ...
        Constants:
            ...
            - ``.HIGHV_NOT_READY``: B10. Either the interlock is not engaged or the interlock was
              engaged recently and the high voltage supply is still stabilizing. If the interlock is
              engaged and this bit is set, attempting to turn on the output on the 200 V range
              results in error code 5052, 'Interlock engaged; system stabilizing.'.
            - ``.INST``: B13. An enabled bit in the questionable status instrument summary event
              register is set.
