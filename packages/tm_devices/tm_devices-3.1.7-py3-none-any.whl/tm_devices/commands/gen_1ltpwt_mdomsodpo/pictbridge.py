...
Commands and Queries:
    ...
    - PICTBridge:IDPrint?
    - PICTBridge:IMAGESize {DEFLT|IN2P5BY3P25|L|IN4BY6|L2|IN8BY10|L4|E|CARD|HAGAKIPcard|CM6BY8|CM7BY10|CM9BY13|CM10BY15|CM13BY18|CM15BY21|CM18BY24|A4|LETTER}
...
class PictbridgePrintqual(SCPICmdWrite, SCPICmdRead):
    ...
    Usage:
        - Using the ``.query()`` method will send the ``PICTBridge:PRINTQual?`` query.
        - Using the ``.verify(value)`` method will send the ``PICTBridge:PRINTQual?`` query and
...
class PictbridgePapertype(SCPICmdWrite, SCPICmdRead):
    ...
    Usage:
        - Using the ``.query()`` method will send the ``PICTBridge:PAPERType?`` query.
        - Using the ``.verify(value)`` method will send the ``PICTBridge:PAPERType?`` query and
          raise an AssertionError if the returned value does not match ``value``.
...
class PictbridgePapersize(SCPICmdWrite, SCPICmdRead):
    ...
    Usage:
        - Using the ``.query()`` method will send the ``PICTBridge:PAPERSize?`` query.
        - Using the ``.verify(value)`` method will send the ``PICTBridge:PAPERSize?`` query and
          raise an AssertionError if the returned value does not match ``value``.
...
class PictbridgeImagesize(SCPICmdWrite, SCPICmdRead):
    ...
    Usage:
        - Using the ``.query()`` method will send the ``PICTBridge:IMAGESize?`` query.
        - Using the ``.verify(value)`` method will send the ``PICTBridge:IMAGESize?`` query and
          raise an AssertionError if the returned value does not match ``value``.
...
class PictbridgeDateprint(SCPICmdWrite, SCPICmdRead):
    ...
    Usage:
        - Using the ``.query()`` method will send the ``PICTBridge:DATEPrint?`` query.
        - Using the ``.verify(value)`` method will send the ``PICTBridge:DATEPrint?`` query and
          raise an AssertionError if the returned value does not match ``value``.
