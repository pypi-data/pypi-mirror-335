...
Attributes and Functions:
    ...
    - smu.digitize.func
    - smu.digitize.limit[Y].clear()
    - smu.digitize.limit[r].audible
    - smu.digitize.limit[r].autoclear
    - smu.digitize.limit[r].enable
    - smu.digitize.limit[r].fail
    - smu.digitize.limit[r].high.value
    - smu.digitize.limit[r].low.value
...
class SmuSourcePulseVlimit(BaseTSPCmd):
    """The ``smu.source.pulse.vlimit`` command tree.

    Properties and methods:
        - ``.level``: The ``smu.source.pulse.vlimit.level`` attribute.
    """

    @property
    def level(self) -> str:
        """Access the ``smu.source.pulse.vlimit.level`` attribute.

        Description:
            - This attribute sets the source limit for pulsed output for the selected function when
              pulsing. (voltage)

        Usage:
            - Accessing this property will send the ``print(smu.source.pulse.vlimit.level)`` query.
            - Setting this property to a value will send the
              ``smu.source.pulse.vlimit.level = value`` command.

        TSP Syntax:
            ```
            - smu.source.pulse.vlimit.level = value
            - print(smu.source.pulse.vlimit.level)
            ```
...
class SmuSourcePulseIlimit(BaseTSPCmd):
    """The ``smu.source.pulse.ilimit`` command tree.

    Properties and methods:
        - ``.level``: The ``smu.source.pulse.ilimit.level`` attribute.
    """

    @property
    def level(self) -> str:
        """Access the ``smu.source.pulse.ilimit.level`` attribute.
        ...
        Usage:
            - Accessing this property will send the ``print(smu.source.pulse.ilimit.level)`` query.
            - Setting this property to a value will send the
              ``smu.source.pulse.ilimit.level = value`` command.

        TSP Syntax:
            ```
            - smu.source.pulse.ilimit.level = value
...
class SmuSourcePulse(BaseTSPCmd):
    """The ``smu.source.pulse`` command tree.

    Properties and methods:
        - ``.level``: The ``smu.source.pulse.level`` attribute.
        - ``.ilimit``: The ``smu.source.pulse.ilimit`` command tree.
        - ``.vlimit``: The ``smu.source.pulse.vlimit`` command tree.
    """

    def __init__(self, device: Optional["TSPControl"], cmd_syntax: str) -> None:
        super().__init__(device, cmd_syntax)
        self._ilimit = SmuSourcePulseIlimit(device, f"{self._cmd_syntax}.ilimit")
        self._vlimit = SmuSourcePulseVlimit(device, f"{self._cmd_syntax}.vlimit")

    @property
    def level(self) -> str:
        """Access the ``smu.source.pulse.level`` attribute.

        Description:
            - This attribute selects a fixed amplitude for the selected source function when
              pulsing.

        Usage:
            - Accessing this property will send the ``print(smu.source.pulse.level)`` query.
            - Setting this property to a value will send the ``smu.source.pulse.level = value``
              command.

        TSP Syntax:
            ```
            - smu.source.pulse.level = value
    ...
    @property
    def ilimit(self) -> SmuSourcePulseIlimit:
        """Return the ``smu.source.pulse.ilimit`` command tree.
    ...
    @property
    def vlimit(self) -> SmuSourcePulseVlimit:
        """Return the ``smu.source.pulse.vlimit`` command tree.
...
class SmuSource(BaseTSPCmd):
    ...
    Properties and methods:
        ...
        - ``.pulse``: The ``smu.source.pulse`` command tree.
        - ``.pulsesweeplinear()``: The ``smu.source.pulsesweeplinear()`` function.
        - ``.pulsesweeplinearstep()``: The ``smu.source.pulsesweeplinearstep()`` function.
        - ``.pulsesweeplist()``: The ``smu.source.pulsesweeplist()`` function.
        - ``.pulsesweeplog()``: The ``smu.source.pulsesweeplog()`` function.
        - ``.pulsetrain()``: The ``smu.source.pulsetrain()`` function.
    ...
    def __init__(self, device: Optional["TSPControl"], cmd_syntax: str) -> None:
        ...
        self._pulse = SmuSourcePulse(device, f"{self._cmd_syntax}.pulse")
    ...
    @property
    def pulse(self) -> SmuSourcePulse:
        """Return the ``smu.source.pulse`` command tree.
    ...
    def pulsesweeplinear(
        ...
        Args:
            ...
            bias_level: Output level the instrument sources before the first pulse and returns to
                between pulses (delay and off time).
            ...
            points: The number of pulse-measure points between the start and stop values of the
                pulse sweep (2 to 1e6).
            pulse_width: The time at the amplitude level for each pulse.
            meas_enable (optional): Enable or disable measurements at the top of each pulse.
            ...
            s_delay (optional): The amount of time that the instrument stays at bias level before
                each pulse (in seconds).
            off_time (optional): The amount of time that the instrument stays at bias level after
                each pulse (in seconds).
            count (optional): The number of pulse sweeps; default is 1.
            x_bias_limit (optional): The current or voltage limit for the defined bias level.
            x_pulse_limit (optional): The current or voltage limit for the defined pulse level.
            fail_abort (optional): Determines if the sweep is stopped immediately if a limit is
                exceeded; options are.
        ...
        try:
            ...
            self._device.write(  # type: ignore[union-attr]
                f"{self._cmd_syntax}.pulsesweeplinear({function_args})"
            )
        except AttributeError as error:
            msg = f"No TSPControl object was provided, unable to run the ``{self._cmd_syntax}.pulsesweeplinear()`` function."  # noqa: E501
            raise NoDeviceProvidedError(msg) from error

    # pylint: disable=too-many-arguments,too-many-locals
    def pulsesweeplinearstep(
        self,
        config_list_name: str,
        bias_level: str,
        start: float,
        ...
        Description:
            - This function sets up a linear source pulse sweep configuration list model with a
        ...
        Args:
            ...
            start: The voltage or current source level at which the pulse sweep starts.
            stop: The voltage or current source level at which the pulse sweep stops.
            step: The step size at which the source level will change; step size must be greater
                than 0; to calculate the number of source-measure points in a sweep, use the
                following formula.
            pulse_width: The time at the amplitude level for each pulse.
            meas_enable (optional): Enable or disable measurements at the top of each pulse.
            buffer_name (optional): The name of the reading buffer, which may be a default buffer
        ...
        try:
            ...
            self._device.write(  # type: ignore[union-attr]
                f"{self._cmd_syntax}.pulsesweeplinearstep({function_args})"
            )
        except AttributeError as error:
            msg = f"No TSPControl object was provided, unable to run the ``{self._cmd_syntax}.pulsesweeplinearstep()`` function."  # noqa: E501
            raise NoDeviceProvidedError(msg) from error
    ...
    def pulsesweeplist(
        ...
        Description:
            - This function sets up a pulse sweep based on a configuration list, which allows you to
              customize the sweep.
        ...
        Args:
            ...
            buffer_name (optional): The name of the reading buffer, which may be a default buffer
                (defbuffer1 or defbuffer2) or a user-defined buffer; defaults to defbuffer1.
            start_index (optional): The index in the configuration list where the pulse sweep
                starts; default is 1.
            count (optional): The number of pulse sweeps; default is 1.
            s_delay (optional): The amount of time that the instrument stays at bias level before
                each pulse (in seconds).
        ...
        try:
            ...
            self._device.write(  # type: ignore[union-attr]
                f"{self._cmd_syntax}.pulsesweeplist({function_args})"
            )
        except AttributeError as error:
            msg = f"No TSPControl object was provided, unable to run the ``{self._cmd_syntax}.pulsesweeplist()`` function."  # noqa: E501
            raise NoDeviceProvidedError(msg) from error
    ...
    def pulsesweeplog(
        ...
        try:
            ...
            self._device.write(  # type: ignore[union-attr]
                f"{self._cmd_syntax}.pulsesweeplog({function_args})"
            )
        except AttributeError as error:
            msg = f"No TSPControl object was provided, unable to run the ``{self._cmd_syntax}.pulsesweeplog()`` function."  # noqa: E501
            raise NoDeviceProvidedError(msg) from error
    ...
    def pulsetrain(
        ...
        Description:
            - This function defines a sequence of source pulses and creates a trigger model to
        ...
        Args:
            ...
            bias_level: Output level the instrument sources before the first pulse and returns to
                between pulses (delay and off time).
            pulse_level: The amplitude current or voltage from zero (not from the bias level).
            pulse_width: The time at the amplitude level for each pulse.
            ...
            x_bias_limit (optional): The current or voltage limit for the defined bias level.
            x_pulse_limit (optional): The current or voltage limit for the defined pulse level.
            fail_abort (optional): Determines if the pulse train is stopped immediately if a limit
                is exceeded; options are.
        ...
        try:
            ...
            self._device.write(  # type: ignore[union-attr]
                f"{self._cmd_syntax}.pulsetrain({function_args})"
            )
        except AttributeError as error:
            msg = f"No TSPControl object was provided, unable to run the ``{self._cmd_syntax}.pulsetrain()`` function."  # noqa: E501
            raise NoDeviceProvidedError(msg) from error
...
class SmuDigitizeRel(BaseTSPCmd):
    """The ``smu.digitize.rel`` command tree.

    Properties and methods:
        - ``.acquire()``: The ``smu.digitize.rel.acquire()`` function.
        - ``.enable``: The ``smu.digitize.rel.enable`` attribute.
        - ``.level``: The ``smu.digitize.rel.level`` attribute.
    """

    @property
    def enable(self) -> str:
        """Access the ``smu.digitize.rel.enable`` attribute.
        ...
        Usage:
            - Accessing this property will send the ``print(smu.digitize.rel.enable)`` query.
            - Setting this property to a value will send the ``smu.digitize.rel.enable = value``
              command.

        TSP Syntax:
            ```
            - smu.digitize.rel.enable = value
            - print(smu.digitize.rel.enable)
            ```
    ...
    @property
    def level(self) -> str:
        """Access the ``smu.digitize.rel.level`` attribute.

        Description:
            - This attribute contains the relative offset value.

        Usage:
            - Accessing this property will send the ``print(smu.digitize.rel.level)`` query.
            - Setting this property to a value will send the ``smu.digitize.rel.level = value``
              command.

        TSP Syntax:
            ```
            - smu.digitize.rel.level = value
            - print(smu.digitize.rel.level)
            ```
    ...
    def acquire(self) -> str:
        """Run the ``smu.digitize.rel.acquire()`` function.
...
class SmuDigitizeMathMxb(BaseTSPCmd):
    """The ``smu.digitize.math.mxb`` command tree.

    Properties and methods:
        - ``.bfactor``: The ``smu.digitize.math.mxb.bfactor`` attribute.
        - ``.mfactor``: The ``smu.digitize.math.mxb.mfactor`` attribute.
    """

    @property
    def bfactor(self) -> str:
        """Access the ``smu.digitize.math.mxb.bfactor`` attribute.

        Description:
            - This attribute specifies the offset, b, for the y = mx + b operation.

        Usage:
            - Accessing this property will send the ``print(smu.digitize.math.mxb.bfactor)`` query.
            - Setting this property to a value will send the
              ``smu.digitize.math.mxb.bfactor = value`` command.

        TSP Syntax:
            ```
            - smu.digitize.math.mxb.bfactor = value
            - print(smu.digitize.math.mxb.bfactor)
            ```
    ...
    @property
    def mfactor(self) -> str:
        """Access the ``smu.digitize.math.mxb.mfactor`` attribute.

        Description:
            - This attribute specifies the scale factor, m, for the y = mx + b math operation.

        Usage:
            - Accessing this property will send the ``print(smu.digitize.math.mxb.mfactor)`` query.
            - Setting this property to a value will send the
              ``smu.digitize.math.mxb.mfactor = value`` command.

        TSP Syntax:
            ```
            - smu.digitize.math.mxb.mfactor = value
            - print(smu.digitize.math.mxb.mfactor)
            ```
...
class SmuDigitizeMath(BaseTSPCmd):
    """The ``smu.digitize.math`` command tree.

    Properties and methods:
        - ``.enable``: The ``smu.digitize.math.enable`` attribute.
        - ``.format``: The ``smu.digitize.math.format`` attribute.
        - ``.mxb``: The ``smu.digitize.math.mxb`` command tree.
        - ``.percent``: The ``smu.digitize.math.percent`` attribute.
    """

    def __init__(self, device: Optional["TSPControl"], cmd_syntax: str) -> None:
        super().__init__(device, cmd_syntax)
        self._mxb = SmuDigitizeMathMxb(device, f"{self._cmd_syntax}.mxb")

    @property
    def enable(self) -> str:
        """Access the ``smu.digitize.math.enable`` attribute.
        ...
        Usage:
            - Accessing this property will send the ``print(smu.digitize.math.enable)`` query.
            - Setting this property to a value will send the ``smu.digitize.math.enable = value``
              command.

        TSP Syntax:
            ```
            - smu.digitize.math.enable = value
            - print(smu.digitize.math.enable)
            ```
    ...
    @property
    def format(self) -> str:
        """Access the ``smu.digitize.math.format`` attribute.
        ...
        Usage:
            - Accessing this property will send the ``print(smu.digitize.math.format)`` query.
            - Setting this property to a value will send the ``smu.digitize.math.format = value``
              command.

        TSP Syntax:
            ```
            - smu.digitize.math.format = value
            - print(smu.digitize.math.format)
            ```
    ...
    @property
    def mxb(self) -> SmuDigitizeMathMxb:
        """Return the ``smu.digitize.math.mxb`` command tree.
    ...
    @property
    def percent(self) -> str:
        """Access the ``smu.digitize.math.percent`` attribute.
        ...
        Usage:
            - Accessing this property will send the ``print(smu.digitize.math.percent)`` query.
            - Setting this property to a value will send the ``smu.digitize.math.percent = value``
              command.

        TSP Syntax:
            ```
            - smu.digitize.math.percent = value
            - print(smu.digitize.math.percent)
            ```
...
class SmuDigitizeLimitItemLow(BaseTSPCmd):
    """The ``smu.digitize.limit[r].low`` command tree.

    Properties and methods:
        - ``.value``: The ``smu.digitize.limit[r].low.value`` attribute.
    """

    @property
    def value(self) -> str:
        """Access the ``smu.digitize.limit[r].low.value`` attribute.

        Description:
            - This attribute specifies the lower limit for a limit test. (r = resistance in ohms)

        Usage:
            - Accessing this property will send the ``print(smu.digitize.limit[r].low.value)``
              query.
            - Setting this property to a value will send the
              ``smu.digitize.limit[r].low.value = value`` command.

        TSP Syntax:
            ```
            - smu.digitize.limit[r].low.value = value
            - print(smu.digitize.limit[r].low.value)
            ```
...
class SmuDigitizeLimitItemHigh(BaseTSPCmd):
    """The ``smu.digitize.limit[r].high`` command tree.

    Properties and methods:
        - ``.value``: The ``smu.digitize.limit[r].high.value`` attribute.
    """

    @property
    def value(self) -> str:
        """Access the ``smu.digitize.limit[r].high.value`` attribute.

        Description:
            - This attribute specifies the upper limit for a limit test. (r = resistance in ohms)

        Usage:
            - Accessing this property will send the ``print(smu.digitize.limit[r].high.value)``
              query.
            - Setting this property to a value will send the
              ``smu.digitize.limit[r].high.value = value`` command.

        TSP Syntax:
            ```
            - smu.digitize.limit[r].high.value = value
            - print(smu.digitize.limit[r].high.value)
            ```
...
class SmuDigitizeLimitItem(ValidatedDynamicNumberCmd, BaseTSPCmd):
    """The ``smu.digitize.limit[r]`` command tree.

    Properties and methods:
        - ``.audible``: The ``smu.digitize.limit[r].audible`` attribute.
        - ``.autoclear``: The ``smu.digitize.limit[r].autoclear`` attribute.
        - ``.clear()``: The ``smu.digitize.limit[r].clear()`` function.
        - ``.enable``: The ``smu.digitize.limit[r].enable`` attribute.
        - ``.fail``: The ``smu.digitize.limit[r].fail`` attribute.
        - ``.high``: The ``smu.digitize.limit[r].high`` command tree.
        - ``.low``: The ``smu.digitize.limit[r].low`` command tree.
    """

    def __init__(self, device: Optional["TSPControl"], cmd_syntax: str) -> None:
        super().__init__(device, cmd_syntax)
        self._high = SmuDigitizeLimitItemHigh(device, f"{self._cmd_syntax}.high")
        self._low = SmuDigitizeLimitItemLow(device, f"{self._cmd_syntax}.low")

    @property
    def audible(self) -> str:
        """Access the ``smu.digitize.limit[r].audible`` attribute.
        ...
        Usage:
            - Accessing this property will send the ``print(smu.digitize.limit[r].audible)`` query.
            - Setting this property to a value will send the
              ``smu.digitize.limit[r].audible = value`` command.

        TSP Syntax:
            ```
            - smu.digitize.limit[r].audible = value
            - print(smu.digitize.limit[r].audible)
            ```
    ...
    @property
    def autoclear(self) -> str:
        """Access the ``smu.digitize.limit[r].autoclear`` attribute.
        ...
        Usage:
            - Accessing this property will send the ``print(smu.digitize.limit[r].autoclear)``
              query.
            - Setting this property to a value will send the
              ``smu.digitize.limit[r].autoclear = value`` command.

        TSP Syntax:
            ```
            - smu.digitize.limit[r].autoclear = value
            - print(smu.digitize.limit[r].autoclear)
            ```
    ...
    @property
    def enable(self) -> str:
        """Access the ``smu.digitize.limit[r].enable`` attribute.

        Description:
            - This attribute enables or disables a limit test on the measurement from the selected
              digitize function. (r = resistance in ohms)

        Usage:
            - Accessing this property will send the ``print(smu.digitize.limit[r].enable)`` query.
            - Setting this property to a value will send the
              ``smu.digitize.limit[r].enable = value`` command.

        TSP Syntax:
            ```
            - smu.digitize.limit[r].enable = value
            - print(smu.digitize.limit[r].enable)
            ```
    ...
    @property
    def fail(self) -> str:
        """Access the ``smu.digitize.limit[r].fail`` attribute.

        Description:
            - This attribute queries the results of a limit test. (r = resistance in ohms)

        Usage:
            - Accessing this property will send the ``print(smu.digitize.limit[r].fail)`` query.

        TSP Syntax:
            ```
            - print(smu.digitize.limit[r].fail)
            ```
    ...
    @property
    def high(self) -> SmuDigitizeLimitItemHigh:
        """Return the ``smu.digitize.limit[r].high`` command tree.
    ...
    @property
    def low(self) -> SmuDigitizeLimitItemLow:
        """Return the ``smu.digitize.limit[r].low`` command tree.
    ...
    def clear(self) -> None:
        """Run the ``smu.digitize.limit[r].clear()`` function.

        Description:
            - This attribute clears the results of the limit test defined by Y. (r = resistance in
              ohms)

        TSP Syntax:
            ```
            - smu.digitize.limit[r].clear()
            ```
...
class SmuDigitize(BaseTSPCmd):
    """The ``smu.digitize`` command tree.

    Properties and methods:
        - ``.aperture``: The ``smu.digitize.aperture`` attribute.
        - ``.count``: The ``smu.digitize.count`` attribute.
        - ``.displaydigits``: The ``smu.digitize.displaydigits`` attribute.
        - ``.func``: The ``smu.digitize.func`` attribute.
        - ``.limit``: The ``smu.digitize.limit[r]`` command tree.
        - ``.math``: The ``smu.digitize.math`` command tree.
        - ``.range``: The ``smu.digitize.range`` attribute.
        - ``.read()``: The ``smu.digitize.read()`` function.
        - ``.readwithtime()``: The ``smu.digitize.readwithtime()`` function.
        - ``.rel``: The ``smu.digitize.rel`` command tree.
        - ``.samplerate``: The ``smu.digitize.samplerate`` attribute.
        - ``.sense``: The ``smu.digitize.sense`` attribute.
        - ``.unit``: The ``smu.digitize.unit`` attribute.
        - ``.userdelay``: The ``smu.digitize.userdelay[N]`` attribute.
    """

    def __init__(self, device: Optional["TSPControl"], cmd_syntax: str) -> None:
        super().__init__(device, cmd_syntax)
        self._limit: Dict[int, SmuDigitizeLimitItem] = DefaultDictPassKeyToFactory(
            lambda x: SmuDigitizeLimitItem(device, f"{self._cmd_syntax}.limit[{x}]")
        )
        self._math = SmuDigitizeMath(device, f"{self._cmd_syntax}.math")
        self._rel = SmuDigitizeRel(device, f"{self._cmd_syntax}.rel")
        self._userdelay: Dict[int, Union[str, float]] = DefaultDictDeviceCommunication(
            cmd_syntax=f"{self._cmd_syntax}.userdelay[{{key}}]",
            write_syntax=f"{self._cmd_syntax}.userdelay[{{key}}] = ",
            query_syntax=f"print({self._cmd_syntax}.userdelay[{{key}}])",
    ...
    def aperture(self) -> str:
        """Access the ``smu.digitize.aperture`` attribute.

        Description:
            - This attribute determines the aperture setting for the selected function.

        Usage:
            - Accessing this property will send the ``print(smu.digitize.aperture)`` query.
            - Setting this property to a value will send the ``smu.digitize.aperture = value``
              command.

        TSP Syntax:
            ```
            - smu.digitize.aperture = value
            - print(smu.digitize.aperture)
            ```
    ...
    @property
    def count(self) -> str:
        """Access the ``smu.digitize.count`` attribute.
        ...
        Usage:
            - Accessing this property will send the ``print(smu.digitize.count)`` query.
            - Setting this property to a value will send the ``smu.digitize.count = value`` command.

        TSP Syntax:
            ```
            - smu.digitize.count = value
            - print(smu.digitize.count)
            ```
    ...
    @property
    def displaydigits(self) -> str:
        """Access the ``smu.digitize.displaydigits`` attribute.
        ...
        Usage:
            - Accessing this property will send the ``print(smu.digitize.displaydigits)`` query.
            - Setting this property to a value will send the ``smu.digitize.displaydigits = value``
              command.

        TSP Syntax:
            ```
            - smu.digitize.displaydigits = value
            - print(smu.digitize.displaydigits)
            ```
    ...
    @property
    def func(self) -> str:
        """Access the ``smu.digitize.func`` attribute.

        Description:
            - This attribute determines which digitize function is active.

        Usage:
            - Accessing this property will send the ``print(smu.digitize.func)`` query.
            - Setting this property to a value will send the ``smu.digitize.func = value`` command.

        TSP Syntax:
            ```
            - smu.digitize.func = value
            - print(smu.digitize.func)
            ```
    ...
    @property
    def limit(self) -> Dict[int, SmuDigitizeLimitItem]:
        """Return the ``smu.digitize.limit[r]`` command tree.
    ...
    @property
    def math(self) -> SmuDigitizeMath:
        """Return the ``smu.digitize.math`` command tree.
    ...
    @property
    def range(self) -> str:
        """Access the ``smu.digitize.range`` attribute.
        ...
        Usage:
            - Accessing this property will send the ``print(smu.digitize.range)`` query.
            - Setting this property to a value will send the ``smu.digitize.range = value`` command.

        TSP Syntax:
            ```
            - smu.digitize.range = value
            - print(smu.digitize.range)
            ```
    ...
    @property
    def rel(self) -> SmuDigitizeRel:
        """Return the ``smu.digitize.rel`` command tree.
    ...
    def samplerate(self) -> str:
        """Access the ``smu.digitize.samplerate`` attribute.
        ...
        Usage:
            - Accessing this property will send the ``print(smu.digitize.samplerate)`` query.
            - Setting this property to a value will send the ``smu.digitize.samplerate = value``
              command.

        TSP Syntax:
            ```
            - smu.digitize.samplerate = value
            - print(smu.digitize.samplerate)
            ```
    ...
    @property
    def sense(self) -> str:
        """Access the ``smu.digitize.sense`` attribute.

        Description:
            - This attribute selects local (2-wire) or remote (4-wire) sensing for the selected
              digitize function.

        Usage:
            - Accessing this property will send the ``print(smu.digitize.sense)`` query.
            - Setting this property to a value will send the ``smu.digitize.sense = value`` command.

        TSP Syntax:
            ```
            - smu.digitize.sense = value
            - print(smu.digitize.sense)
            ```
    ...
    @property
    def unit(self) -> str:
        """Access the ``smu.digitize.unit`` attribute.
        ...
        Usage:
            - Accessing this property will send the ``print(smu.digitize.unit)`` query.
            - Setting this property to a value will send the ``smu.digitize.unit = value`` command.

        TSP Syntax:
            ```
            - smu.digitize.unit = value
            - print(smu.digitize.unit)
            ```
    ...
    @property
    def userdelay(self) -> Dict[int, Union[str, float]]:
        """Access the ``smu.digitize.userdelay[N]`` attribute.
        ...
        Usage:
            - Accessing an item from this property will send the
              ``print(smu.digitize.userdelay[N])`` query.
            - Setting an item from this property to a value will send the
              ``smu.digitize.userdelay[N] = value`` command.

        TSP Syntax:
            ```
            - smu.digitize.userdelay[N] = value
            - print(smu.digitize.userdelay[N])
            ```
...
class SmuContact(BaseTSPCmd):
    """The ``smu.contact`` command tree.

    Properties and methods:
        - ``.check()``: The ``smu.contact.check()`` function.
        - ``.checkall()``: The ``smu.contact.checkall()`` function.
        - ``.enable``: The ``smu.contact.enable`` attribute.
        - ``.threshold``: The ``smu.contact.threshold`` attribute.
    """

    @property
    def enable(self) -> str:
        """Access the ``smu.contact.enable`` attribute.

        Description:
            - This attribute indicates whether the contact check function is enabled or disabled on
              the instrument.

        Usage:
            - Accessing this property will send the ``print(smu.contact.enable)`` query.
            - Setting this property to a value will send the ``smu.contact.enable = value`` command.

        TSP Syntax:
            ```
            - smu.contact.enable = value
    ...
    def threshold(self) -> str:
        """Access the ``smu.contact.threshold`` attribute.

        Description:
            - This attribute sets the threshold value for contact resistance for the contact check
              status functions.

        Usage:
            - Accessing this property will send the ``print(smu.contact.threshold)`` query.
            - Setting this property to a value will send the ``smu.contact.threshold = value``
              command.

        TSP Syntax:
            ```
            - smu.contact.threshold = value
    ...
    def check(self) -> str:
        """Run the ``smu.contact.check()`` function.

        Description:
            - This function indicates whether one or more connections failed the contact check
              operation.
    ...
    def checkall(self) -> str:
        """Run the ``smu.contact.checkall()`` function.

        Description:
            - This function checks the high, low, and guard connections for excessive contact
              resistance.
        ...
        try:
            return self._device.query(  # type: ignore[union-attr]
                f"print({self._cmd_syntax}.checkall())"
            )
        except AttributeError as error:
            msg = f"No TSPControl object was provided, unable to run the ``{self._cmd_syntax}.checkall()`` function."  # noqa: E501
            raise NoDeviceProvidedError(msg) from error
...
class Smu(BaseTSPCmd):
    ...
    def __init__(self, device: Optional["TSPControl"] = None, cmd_syntax: str = "smu") -> None:
        super().__init__(device, cmd_syntax)
        self._contact = SmuContact(device, f"{self._cmd_syntax}.contact")
        self._digitize = SmuDigitize(device, f"{self._cmd_syntax}.digitize")
    ...
    @property
    def contact(self) -> SmuContact:
        """Return the ``smu.contact`` command tree.
    ...
    @property
    def digitize(self) -> SmuDigitize:
        """Return the ``smu.digitize`` command tree.
    ...
    @property
    def fastrangechange(self) -> str:
        """Access the ``smu.fastrangechange`` attribute.

        Description:
            - This attribute allows you to suppress checks that are normally done before range
              changes.

        Usage:
            - Accessing this property will send the ``print(smu.fastrangechange)`` query.
            - Setting this property to a value will send the ``smu.fastrangechange = value``
              command.

        TSP Syntax:
            ```
            - smu.fastrangechange = value
        ...
        try:
            if self._device.command_syntax_enabled:  # type: ignore[union-attr]
                return self._cmd_syntax + ".fastrangechange"
            return self._device.query(  # type: ignore[union-attr]
                f"print({self._cmd_syntax}.fastrangechange)"
            )
        except AttributeError as error:
            msg = f"No TSPControl object was provided, unable to access the ``{self._cmd_syntax}.fastrangechange`` attribute."  # noqa: E501
            raise NoDeviceProvidedError(msg) from error

    @fastrangechange.setter
    def fastrangechange(self, value: Union[str, float]) -> None:
        """Access the ``smu.fastrangechange`` attribute.
        ...
        try:
            if self._device.command_verification_enabled:  # type: ignore[union-attr]
                self._device.set_and_check(  # type: ignore[union-attr]
                    self._cmd_syntax + ".fastrangechange", value
                )
            else:
                self._device.write(  # type: ignore[union-attr]
                    f"{self._cmd_syntax}.fastrangechange = {value}"
                )
        except AttributeError as error:
            msg = f"No TSPControl object was provided, unable to access the ``{self._cmd_syntax}.fastrangechange`` attribute."  # noqa: E501
            raise NoDeviceProvidedError(msg) from error
    ...
    @property
    def priorityvoltage(self) -> str:
        """Access the ``smu.priorityvoltage`` attribute.

        Description:
            - This attribute changes the method the instrument uses to select the power supply rail
              that drives the output.

        Usage:
            - Accessing this property will send the ``print(smu.priorityvoltage)`` query.
            - Setting this property to a value will send the ``smu.priorityvoltage = value``
              command.

        TSP Syntax:
            ```
            - smu.priorityvoltage = value
            - print(smu.priorityvoltage)
            ```
        ...
        try:
            if self._device.command_syntax_enabled:  # type: ignore[union-attr]
                return self._cmd_syntax + ".priorityvoltage"
            return self._device.query(  # type: ignore[union-attr]
                f"print({self._cmd_syntax}.priorityvoltage)"
            )
        except AttributeError as error:
            msg = f"No TSPControl object was provided, unable to access the ``{self._cmd_syntax}.priorityvoltage`` attribute."  # noqa: E501
            raise NoDeviceProvidedError(msg) from error

    @priorityvoltage.setter
    def priorityvoltage(self, value: Union[str, float]) -> None:
        """Access the ``smu.priorityvoltage`` attribute.
        ...
        try:
            if self._device.command_verification_enabled:  # type: ignore[union-attr]
                self._device.set_and_check(  # type: ignore[union-attr]
                    self._cmd_syntax + ".priorityvoltage", value
                )
            else:
                self._device.write(  # type: ignore[union-attr]
                    f"{self._cmd_syntax}.priorityvoltage = {value}"
                )
        except AttributeError as error:
            msg = f"No TSPControl object was provided, unable to access the ``{self._cmd_syntax}.priorityvoltage`` attribute."  # noqa: E501
            raise NoDeviceProvidedError(msg) from error
