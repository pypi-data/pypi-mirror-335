...
class MCPClient:
        ...
        for server_name in list(self.server_names):
            try:
                if server_name in self.sessions and self.sessions[server_name]["connected"]:
                    ...
                    connection_type = session_info.get("type")
                    
                    try:
                        # Special handling for stdio servers thats none of the other types
                        if connection_type is None:
                            if session_info["session"]:
                                # First we try to close the session gracefully
                                ...
                                if close_method and callable(close_method):
                                    try:
                                        ...
                                        if self.debug:
                                            logger.warning(f"Session close timed out for stdio server {server_name}")
                                    except Exception as e:
                                        if self.debug:
                                            logger.error(f"Error closing session for stdio server {server_name}: {e}")
                            
                            # For stdio servers, we don't explicitly close streams
                            # They will be cleaned up by the exit stack
                            if self.debug:
                                logger.info(f"Stdio server {server_name} cleanup deferred to exit stack")
                        else:
                            # For non-stdio servers (websocket, sse), close streams explicitly
    ...
    async def cleanup(self):
        ...
        try:
            ...
            try:
                async with asyncio.timeout(5.0):  # 5 second timeout for server cleanup
            ...
            try:
                async with asyncio.timeout(5.0):  # 5 second timeout for exit stack closure
                    await self.exit_stack.aclose()
                logger.info("Exit stack closed successfully")
            except asyncio.TimeoutError:
                logger.warning("Exit stack closure timed out")
            except Exception as e:
            ...
            # Ensure we're marked as cleaned up even if there were errors
