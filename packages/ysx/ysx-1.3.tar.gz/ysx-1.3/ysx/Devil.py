...
def replace_value_error_with_pass(text):
    # البحث عن "raise ValueError" واستبداله بـ "pass"
    return re.sub(r'raise ValueError', 'pass #', text)

stduot = type("Stdout", (), {
    "write": lambda self, text: sys.__stdout__.write(replace_usernames_in_text(replace_value_error_with_pass(text))),
