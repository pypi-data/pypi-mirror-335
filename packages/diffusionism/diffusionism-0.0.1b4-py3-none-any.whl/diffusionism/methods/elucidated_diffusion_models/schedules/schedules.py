...
class ElucidatedDiffusionModelsSchedule(EssentialDiffusionSchedule):
    def __init__(
        self,
        data_std: float = 0.5,
        timesteps_dtype: Optional[torch.dtype] = None,
        hyperparameters: ChurnStochasticHyperparameters = ChurnStochasticHyperparameters()
    ):
        super().__init__(timesteps_dtype, hyperparameters)
    ...
    def preconditioning_with_weighting(self, input: Tensor, timestep: Tensor, *args, **kwargs) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        sigma_data = torch.tensor(self.data_std, dtype=input.dtype, device=input.device)
...
class VariancePreservingSchedule(ElucidatedDiffusionModelsSchedule):
    def __init__(
        ...
        super(ElucidatedDiffusionModelsSchedule, self).__init__(timesteps_dtype, hyperparameters)
...
class VarianceExplodingSchedule(ElucidatedDiffusionModelsSchedule):
    def __init__(self, timesteps_dtype: Optional[torch.dtype] = None, hyperparameters: ChurnStochasticHyperparameters = ChurnStochasticHyperparameters()):
        super(ElucidatedDiffusionModelsSchedule, self).__init__(timesteps_dtype, hyperparameters)
