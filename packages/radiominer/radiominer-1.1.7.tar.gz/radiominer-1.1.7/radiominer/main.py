...
def internet_available(host="1.1.1.1", port=53, timeout=3):
    """
    Prüft, ob die Internetverbindung verfügbar ist (Standard: Cloudflare DNS).
    """
    try:
...
class RadioRecorder:
    ...
    def __init__(self, stream_url, sender, segment_time=60, base_dir=None, poll_interval=5, whisper_model=WhisperModel.TURBO, quality="64k", record_only=False, transcribe_only=False, verbose=False, ffmpeg_path=None, run_once=False):    
        ...
        self.ffmpeg_process = None  # Ergänzt zur Verwaltung des ffmpeg-Prozesses
    ...
    def _terminate_ffmpeg_process(self):
        if self.ffmpeg_process and self.ffmpeg_process.poll() is None:
            self.logger.info("Beende laufenden FFmpeg-Prozess aufgrund von IP-Wechsel.")
            self.ffmpeg_process.terminate()
            try:
                self.ffmpeg_process.wait(timeout=10)
            except subprocess.TimeoutExpired:
                self.logger.warning("FFmpeg-Prozess erzwingend beendet.")
    ...
    def _record_segment(self):
        ...
        try:
            ...
            result_code = self.ffmpeg_process.returncode
            if result_code == 0:
                ...
                self.logger.warning("ffmpeg fehlgeschlagen für Segment: %s", temp_output_file)
                return None
        except subprocess.TimeoutExpired:
            self.logger.error("ffmpeg Timeout, beende Prozess...")
            self._terminate_ffmpeg_process()
            return None

    def record_stream(self):
        check_interval = 30  # Prüfe alle 30 Sekunden die Internetverbindung
        ...
        while self.running:
            ...
            if current_time - last_check > check_interval:
                if not internet_available():
                    self.logger.warning("Internetverbindung unterbrochen, FFmpeg wird neu gestartet.")
                    self._terminate_ffmpeg_process()
                    time.sleep(5)
