...
def run():
    ...
    try:
        # Use the correct relative path to access the startup.txt file
        startup_file_path = STATIC_DIR.joinpath("startup.txt")
        with open(startup_file_path, "r") as file:
            ascii_art = file.read()
        ascii_art = ascii_art.replace("{py_version}", py_version)
        print(ascii_art)
    except Exception as e:
        print("Starting ImgFixer ...")
        print(f"Error reading startup.txt: {e}")
