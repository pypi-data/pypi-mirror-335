...
class Transfer:
            ...
            if self.myDataType == self.DATA_TYPES["VALIDATED_SEISMIC_DATA_PH5"]:
                if not os.access(self.myDirectoryName + "/master.ph5", os.R_OK):
                    raise Exception(
                        ...
                        % (self.myDirectoryName + "/master.ph5")
                    )
    ...
    def send_data(self):
        ...
        logging.info("Transaction has %s files" % (filenumber))
        if filenumber > self.__CONFIG["limits"]["datafile max"][1]:
            raise Exception(
                "Transaction has %d files. Limit is set to %d. Please split your transaction in smaller chunks"
                % (filenumber, self.__CONFIG["limits"]["datafile max"][1])
            )
        ...
        if sizeGb > self.__CONFIG["limits"]["datasize max"][1]:
            raise Exception(
                "Transaction is %.2fGb big. Limit is set to %d. Please split your transaction in smaller chunks"
                % (sizeGb, self.__CONFIG["limits"]["datasize max"][1])
            )
