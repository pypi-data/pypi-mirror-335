...
def calculate_subnets(name: str, home_subnet: str = None) -> dict:
    subnet_num = 1
    if any(c.isdigit() for c in name):
        vpn_num = int("".join(filter(str.isdigit, name))[-3])
        if vpn_num <= 255/3:
    ...
    while (
        ...
        subnet_num += 3
    if subnet_num > 253:
        raise RuntimeError("Cannot find 3 subnets below 255")
    return {
        "docker_subnet": f"172.28.{subnet_num}.0/24",
        "trust_subnet": f"10.0.{subnet_num}.0",
        "guest_subnet": f"10.0.{subnet_num+1}.0",
        "home_subnet": home_subnet if home_subnet else f"10.0.{subnet_num+2}.0",
    }
...
def backup_vpn(docker: DockerManager, caddy_name: str, vpn_name: str) -> None:
    ...
    backup_cmd = (
        f"tar czf {backup_file} -C / opt/docker/volumes/{caddy_name} {vpn_path}"
    )
...
def _generate_vpn_context(
    ...
    home_subnet = config.get("home_subnet")
    subnets = calculate_subnets(name, home_subnet)
...
def update_vpn(docker: DockerManager, name: str, caddy_name: str, config: dict) -> None:
    ...
    try:
        ...
        with open(docker_compose_path) as f:
            for line in f:
                if "HOME_SUB:" in line:
                    config['home_subnet'] = line.split(":")[1].strip()
                    break
        context = _generate_vpn_context(
            docker, name, config, output_dir, admin_password
        )
...
def main():
    ...
    parser.add_argument("--homesubnet", help="Specify home subnet", default=None)
    ...
    try:
        ...
        if args.homesubnet:
            config["home_subnet"] = args.homesubnet
        ...
        if args.action == "create":
            ...
            print(f"Home subnet: {subnets['home_subnet']}")
