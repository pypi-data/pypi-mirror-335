...
def create_wrapper(wrapped, _, args, kwargs):
    ...
    """
    logging.debug("Ollama chat wrapper called")
    ...
    # Extract durations for logging
    prompt_eval_duration = getattr(response, 'prompt_eval_duration', 0) / 1000000  # convert ns to ms
    eval_duration = getattr(response, 'eval_duration', 0) / 1000000  # convert ns to ms
    total_duration = getattr(response, 'total_duration', 0) / 1000000  # convert ns to ms
    ...
    logging.debug("Metering thread started: %s", thread)
    return response
