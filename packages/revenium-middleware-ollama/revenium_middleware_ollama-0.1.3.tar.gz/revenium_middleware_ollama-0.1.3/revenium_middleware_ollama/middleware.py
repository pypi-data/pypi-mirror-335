...
def chat_wrapper(wrapped, instance, args, kwargs):
    ...
    logging.debug(f"Calling chat function with args: {args}, kwargs: {kwargs}")
    ...
    if is_streaming and isinstance(response, types.GeneratorType):
        ...
        logging.debug("Ollama chat response: %s", response)
...
def generate_wrapper(wrapped, instance, args, kwargs):
    ...
    Handles both streaming and non-streaming responses.
    """
    logging.debug("Ollama generate wrapper called")
    usage_metadata = kwargs.pop("usage_metadata", {}) if "usage_metadata" in kwargs else {}
    is_streaming = kwargs.get("stream", False)
    ...
    request_time_dt = datetime.datetime.now(datetime.timezone.utc)
    logging.debug(f"Calling generate function with args: {args}, kwargs: {kwargs}")
    ...
    if is_streaming and isinstance(response, types.GeneratorType):
        ...
        # Handle non-streaming response
        logging.debug("Ollama generate response: %s", response)
        handle_response(response, request_time_dt, usage_metadata)
        return response
...
def handle_response(response, request_time_dt, usage_metadata):
    ...
    async def metering_call():
        ...
        try:
            ...
            logging.debug("Arguments for create_completion: %s", completion_args)
