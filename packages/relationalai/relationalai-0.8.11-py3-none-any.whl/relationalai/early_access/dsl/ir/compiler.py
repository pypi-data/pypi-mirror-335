...
class Compiler:
    ...
    def to_args(self, ctx: CompilerContext, item:Atom) -> list[ir.Value]:
        return [
            self.to_var(ctx, arg)
            if not isinstance(arg, Wildcard)
            else ctx.to_project_away_var(self.to_type(role.root_unconstrained_type()))
            for arg, role in zip(item.args, item.relcomp._signature._types)
        ]
    ...
    def compile_atom(self, ctx:CompilerContext, atom:Atom) -> Union[ir.Construct, ir.Lookup, ir.Not]:
        ...
        if name.startswith('^'):
            ...
            args.extend(self.to_args(ctx, atom))
    ...
    @staticmethod
    def process_project_away_vars(ctx):
        if len(ctx.project_away_vars) != 0:
            clone = ctx.clone()
            clone.add(f.exists(list(ctx.project_away_vars), f.logical(list(ctx.items))))
            return clone
        return ctx
    ...
    def compile_rule(self, rule:Rule, name:str) -> ir.Task:
        ...
        for scalar_constraint in rule._sconstraints.values():
            ctx.add(self.compile_scalar_constraint(ctx, scalar_constraint))
        ctx = self.process_project_away_vars(ctx)
