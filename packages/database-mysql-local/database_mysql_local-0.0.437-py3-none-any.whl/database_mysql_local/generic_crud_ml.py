...
class GenericCRUDML(GenericCRUD, metaclass=MetaLogger, object=LOGGER_CRUD_ML_CODE_OBJECT):
    ...
    def get_values_dict_list_by_where(self, *, lang_code: LangCode = None,  # table_id: int,
        ...
        schema_name = schema_name or self.default_schema_name
        if schema_name == "test":
            view_name = f"{schema_name}_mysql_general_view"
        else:
            view_name = f"{schema_name}_view"

        column_name = column_name or self.default_column_name
        lang_code_str = self._get_lang_code_str(lang_code=lang_code)

        result = self.select_multi_dict_by_where(
            schema_name=schema_name, view_table_name=view_name, select_clause_value=select_clause_value,
    ...
    def merge_ml_entities(self, entity_id1: int, entity_id2: int, main_entity_ml_id: int = None):
        ...
        gcrml = GenericCRUDML(default_schema_name=schema_name, default_ml_table_name=generated_ml_table_name,is_test_data=True, default_table_name=generated_table_name, default_ml_view_table_name= generated_ml_view_name, is_main_column_name= 'is_main_title')
        ...
        ml_ids = gcrml.get_all_ml_ids_by_id(table_id=old_id,ml_column_name=generated_ml_column_name,order_by=DEFAULT_ORDER_BY, ml_table_name=generated_ml_table_name, column_name=generated_column_name, compare_view_name=generated_ml_view_name)

        ## get a list of existing langcodes for the id which is being merged into
        langcode_list = self.get_lang_codes_by_table_id(table_id=new_id, ml_view_table_name=generated_ml_view_name, schema_name=self.default_schema_name)

        for id in ml_ids:
            ## get the langcode of the ml id being merged. if it already exists in the id being merged into, then set this one to false.
            langcode = gcrml.select_one_value_by_column_and_value(select_clause_value='lang_code', column_name=generated_ml_column_name, column_value=id, view_table_name=generated_ml_view_name)
