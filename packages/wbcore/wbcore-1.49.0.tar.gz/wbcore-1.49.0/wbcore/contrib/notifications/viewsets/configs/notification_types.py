...
from wbcore.metadata.configs.display import Field, Formatting, FormattingRule, Legend, LegendItem, ListDisplay
...
class NotificationTypeSettingDisplayConfig(DisplayViewConfig):
    def get_list_display(self) -> ListDisplay:
        return ListDisplay(
            fields=[
                Field(key="locked", label="Locked", width=Unit.PIXEL(100)),
            ...
            formatting=[
                Formatting(
                    column="locked",
                    formatting_rules=[
                        FormattingRule(icon=WBIcon.LOCK.icon, condition=("==", True)),
                    ],
                ),
            ],
        )
