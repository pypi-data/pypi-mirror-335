...
def call_http_tool(tool_info: {}, arguments: {}):
    ...
    if protocol["methodType"].upper() == "POST":
        response = requests.post(url, headers=headers, json=data, verify='src/resource/_.aliyun-inc.com.pem')
    else:
        response = requests.get(url, params=params, headers=headers, verify='src/resource/_.aliyun-inc.com.pem')
