...
class CoreManager(Protocol):
    ...
    async def get_hwaccel_info(
        self, target_codec: Union[Literal["h264"], Literal["h265"]], full_hardware: Optional[bool] = None
    ) -> "FfmpegArgs": ...
...
class JsonSchemaNumber(JsonBaseSchema[Union[int, float]]):
    type: Literal["number"]
...
class JsonSchemaEnum(JsonBaseSchema[Union[str, list[str]]]):
    type: Literal["string"]
    enum: list[str]
