...
class MeshEden(Eden, RuntimeHook, Subscriber):
    ...
    def init_service(self) -> Service:
        ...
        service.attrs = {'mesh-group': System.environ().get_mesh_group()}
        service.urn = ''
        return service
    ...
    def new_registration(self, metadata: Resource, attachments: Dict[str, str]) -> Registration:
        attachments['mesh-subset'] = System.environ().get_mesh_subset()
        attachments['mesh-group'] = System.environ().get_mesh_group()
        ...
        registration.address = tool.anyone(System.environ().get_mesh_cruntime(), self.get_listened_address())
    ...
    async def subscribe(self, event: Event):
        ...
        for _, service in self.services.items():
            metadata.services.append(service.resource)
        metadata.references = []
        for _, reference in self.references.items():
            metadata.references.append(reference.resource)
