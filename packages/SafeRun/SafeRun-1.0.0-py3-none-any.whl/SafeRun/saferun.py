...
Note:
    - Some security features (e.g. resource limits, chroot, privilege dropping, seccomp) are only available on Unix-like systems.
    - On Windows these features are skipped.
"""
...
def run_in_sandbox(code, safe_globals, time_limit, memory_limit, chroot_dir):
    """
    Execute the given code in a sandboxed environment using the following techniques:
      - On Unix-like systems:
          * Resource limits (CPU time and memory) via the resource module.
          * Filesystem isolation using chroot.
          * Privilege dropping.
          * Optional syscall filtering using seccomp.
      - On Windows:
          * These features are not available, so only basic process isolation is used.
    
    Returns:
      The output message or traceback from the sandboxed execution.
    """
    def target(queue):
        try:
