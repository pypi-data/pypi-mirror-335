...
def add_cache_control_to_messages(messages: List[Dict], verbose: bool = False) -> List[Dict]:
    """
    ...
    Args:
        ...
        verbose: If True, print debug information when cache markers are added
    ...
    for message_idx, message in enumerate(messages):
            ...
            for content_idx, content_item in enumerate(message["content"]):
                ...
                if isinstance(content_item, dict) and content_item.get("type") == "text":
                    ...
                    if token_count_since_last_marker >= 1024 and markers_added < 5:
                        ...
                        if verbose:
                            print(f"[DEBUG] Added cache marker #{markers_added} to message at index {message_idx}, content item {content_idx}")
        ...
        # Process string content (convert to structured content with cache control if needed)
            ...
            if estimated_tokens >= 1024 and markers_added < 5:
                ...
                # Print debug information if verbose mode is enabled
                if verbose:
                    print(f"[DEBUG] Added cache marker #{markers_added} to message at index {message_idx}")
                    print(f"        Estimated tokens: {estimated_tokens}")
        
        processed_messages.append(processed_message)
    
    # Print summary if verbose mode is enabled and markers were added
    if verbose and markers_added > 0:
        print(f"[DEBUG] Total cache markers added: {markers_added}")
