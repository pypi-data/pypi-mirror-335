...
from codemie_tools.git.azure_devops.utils import get_repo_id
...
class AzureDevOpsToolkit(BaseToolkit):
    ...
    def get_toolkit(cls, configs: Dict[str, Any]):
        ...
        """
        repository_id = get_repo_id(configs["repo_link"])
        credentials = AzureDevOpsCredentials(
            organization_url=configs["organization_url"],
            project=configs["project"],
            token=configs["token"],
            base_branch=configs["base_branch"],
