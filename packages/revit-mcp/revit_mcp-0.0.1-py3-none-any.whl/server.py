...
def get_Revit_connection():
    ...
    if _Revit_connection is None:
        _Revit_connection = RevitConnection(host="localhost", port=8080)
...
def create_object(
    ...
    Parameters:
    - method: Object type (CreateWalls)
    - params: List of parameters for the wall creation

    Returns:
    A message indicating the created object ElementId.
    """
    try:
        # Validate parameters
        if params is None:
        ...
        elif not all(isinstance(param, dict) for param in params):
            raise ValueError("Invalid parameter format. Expected a list of dictionaries.")
