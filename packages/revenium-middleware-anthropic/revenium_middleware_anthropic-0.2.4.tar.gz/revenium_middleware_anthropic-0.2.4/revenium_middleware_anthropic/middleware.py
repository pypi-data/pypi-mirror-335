...
def stream_wrapper(wrapped, _, args, kwargs):
    ...
    """
    logging.debug("Anthropic client.messages.stream wrapper called")
    usage_metadata = kwargs.pop("usage_metadata", {})

    request_time_dt = datetime.datetime.now(datetime.timezone.utc)
    request_time = request_time_dt.strftime("%Y-%m-%dT%H:%M:%SZ")
    logging.debug(f"Calling wrapped stream function with args: {args}, kwargs: {kwargs}")
                ...
                async def metering_call():
                    try:
                        if shutdown_event.is_set():
                            logging.warning("Skipping metering call during shutdown")
                            return
                        logging.debug("Metering call to Revenium for stream completion %s", self.response_id)
                        result = client.ai.create_completion(
                            cache_creation_token_count=cache_creation_input_tokens,
                            cache_read_token_count=cache_read_input_tokens,
                            input_token_cost=0.0,
                        ...
                        logging.debug("Metering call result for stream: %s", result)
                    except Exception as e:
                        if not shutdown_event.is_set():
                            logging.warning(f"Error in metering call for stream: {str(e)}")
                            # Log the full traceback for better debugging
                            import traceback
                            logging.warning(f"Traceback: {traceback.format_exc()}")

                thread = run_async_in_thread(metering_call())
                logging.debug("Metering thread started for stream: %s", thread)

            except Exception as e:
                logging.warning(f"Error processing final message from stream: {str(e)}")
                import traceback
                logging.warning(f"Traceback: {traceback.format_exc()}")
