...
class Factor(BaseModel):
    ...
    def initialize(self, init_strategy: FactorInitialization):
    ...
    def normalize(self):
        if self.distribution == Distribution.Categorical:
            normalizer = self.values.sum(keepdims=True)
            zero_fill = 1 / np.prod(self.values.shape)

        elif self.distribution == Distribution.CategoricalConditional:
            normalizer = self.values.sum(axis=0, keepdims=True)
            zero_fill = 1 / self.values.shape[0]

        # If normalizer is zero, fill with uniform distribution
        self.values = np.nan_to_num(self.values / normalizer, nan=zero_fill)
...
class MarkovRandomField(VFG):
    ...
    def validate(self, raise_exceptions: bool = True):
        ...
        for factor_idx, factor in enumerate(self.factors):
            ...
            if factor.distribution != Distribution.Categorical:
                ...
                errors.extend(ConditionalDistributionInMRF(factor_idx, patch))
