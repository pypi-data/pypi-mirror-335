...
class Game:
    ...
    User is required to implement abstract methods _update_screen(), which is invoked within run()
    """
    ...
    def run(self, *event_handlers: Callable[[pygame.event.Event], None]):
        ...
        while self.running:
            ...
            if not self.screen_settings.no_screen:
                self._update_screen()
                pygame.display.flip()
            logger.debug(f"Handlers: {len(event_handlers)}")
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    self.running = False
                for handler in event_handlers:
                    logger.debug(f"handler given {handler}")
                    handler(event)
        pygame.quit()
