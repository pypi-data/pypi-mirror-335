...
class TokenManager:
    ...
    async def init(self, code, skip_error=False):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                response = await session.post("https://{}.amocrm.ru/oauth2/access_token".format(self.subdomain), json=data)
        except aiohttp.ClientError:
            logger.warning("can't init tokens")
            if not skip_error:
                raise
        else:
            if response.status != 200 and not skip_error:
                raise Exception((await response.json())["hint"])
            if response.status != 200 and skip_error:
                return
    ...
    async def _get_new_tokens(self) -> Tuple[str, str]:
        ...
        async with aiohttp.ClientSession() as session:
            response = await session.post("https://{}.amocrm.ru/oauth2/access_token".format(self.subdomain), json=body)
        if response.status == 200:
            data = await response.json()
            return data["access_token"], data["refresh_token"]
        raise EnvironmentError("Can't refresh token {}".format(await response.json()))
    ...
    async def get_access_token(self):
        ...
        if self._is_expire(token):
            token, refresh_token = await self._get_new_tokens()
