...
class _Link(_BaseField):
    ...
    async def on_get(self, data):
        return await self._manager.get(object_id=data)
...
class _ListData:
    ...
    async def __aiter__(self):
        for item in self._data:
            yield await self._manager.get(item["id"])

    async def append(self, value, main=False):
        return await self._links.link(for_entity=self._instance, to_entity=value, main=main)

    add = append

    async def remove(self, value):
        return await self._links.unlink(for_entity=self._instance, to_entity=value)
...
class _EmbeddedLinkField(_Link):
    ...
    async def on_get(self, data):
        if data:
            return await self._model.objects.get(data[0]["id"])
        return None

    async def on_set_instance(self, instance, value):
        if instance.id is None:
            raise exceptions.InitException("Create entity first")
        if value.id is None:
            await value.create()
        await self._links.link(instance, value)
        return [{"id": value.id}]
