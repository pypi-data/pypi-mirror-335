...
class CustomFieldModel(model.Model):
    ...
    @classmethod
    async def create_for(cls, instance, name, code=None, sort=None):
        return (
...
class BaseCustomField(fields._BaseField, metaclass=_FieldsRegisterMeta):
    ...
    async def _find(self, instance):
        async for field in CustomFieldModel.get_for(instance):
            if field.name == self._name or (self._code and self._code == field.code):
                return field
        return None

    async def _check(self, instance):
        if self._field_id:
            return
        field = await self._find(instance)
        if field:
    ...
    def __set__(self, instance, value):
        if instance is None or value is None:
            return
        if not hasattr(instance, "_pending_values"):
            instance._pending_values = {}
        instance._pending_values[self._name] = value
    
    async def apply(self, instance):
        if not hasattr(instance, "_pending_values"):
            return
        if self._name not in instance._pending_values:
            return
        value = instance._pending_values.pop(self._name)
        data = instance._data
        for _path in self._path:
            data = data.setdefault(_path, {self.name: []})
        _data = self._get_raw_field(data.get(self.name))
        if _data is None:
            await self._check(instance)
