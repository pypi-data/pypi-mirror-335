...
class _StatusesField(fields._BaseField):
    ...
    async def on_get_instance(self, instance, status_id):
        for status in (await instance.pipeline).statuses:
            if status.id == status_id:
                return status

    async def on_set_instance(self, instance, value):
        if isinstance(value, str):
            return (await Status.get_for(instance.pipeline).get(query=value)).id
