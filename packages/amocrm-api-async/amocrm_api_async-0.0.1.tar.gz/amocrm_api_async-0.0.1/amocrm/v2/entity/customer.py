...
class CustomersInteraction(GenericInteraction):
    ...
    async def enable(self, mode: str = "segments"):
        return await self.set(True, mode=mode)

    async def disable(self):
        return await self.set(False)

    async def set(self, enable: bool, mode: str = "segments"):
        data, _ = await self.request("patch", path="customers/mode", data={"is_enabled": enable, "mode": mode})
        return data
