...
async def render_models_file(init_token, enums=True):
    out = ""
    for model in MODELS:
    ...
    for model in MODELS:
        out += "\n\n"
        out += await render_model(model, enums)
    return out


async def render_model(model: Type[Model], enums=True):
    out = f"\nclass {model.__name__}(_{model.__name__}):"
    async for field in get_fields_for(model):
        out += "\n" + render_field(field, enums=enums)
    return out


def get_fields_for(model: Type[Model]) -> AsyncIterable[custom_field.CustomFieldModel]:
    return Manager(
        GenericInteraction(
...
def main():
    ...
    asyncio.run(gen(client_id, secret, subdomain, redirect_url, path, code, enums))
...
async def gen(client_id, secret, subdomain, redirect_url, path, code=None, enums=True):
    tokens.default_token_manager(
        client_secret=secret,
        client_id=client_id,
        subdomain=subdomain,
