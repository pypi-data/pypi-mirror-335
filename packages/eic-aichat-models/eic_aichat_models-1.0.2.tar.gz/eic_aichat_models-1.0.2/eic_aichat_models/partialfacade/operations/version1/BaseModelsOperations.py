...
class BaseModelsOperations(RestOperations):
    ...
    def register_routes(self, controller: RestController):
        ...
        controller.register_route('get', '/base_models', None,
                            self.get_base_models)

        controller.register_route('get', '/base_model/<id>', ObjectSchema(True)
                            .with_optional_property("base_model_id", TypeCode.String),
                            self.get_base_model_by_id)

        controller.register_route('post', '/base_model', ObjectSchema(True)
                            .with_required_property("body", BaseModelSchema()),
                            self.create_base_model)

        controller.register_route('put', '/base_model', ObjectSchema(True)
                            .with_required_property("body", BaseModelSchema()),
                            self.update_base_model)

        controller.register_route('delete', '/base_model/<id>', ObjectSchema(True)
