...
EVENT_URL_TEMPLATE = "https://earthquake.usgs.gov/earthquakes/feed/v1.0/detail/{eventid}.geojson"
...
class ShakeClone:
            ...
            for key in self.shakemap["contents"]:
                if "fault.txt" in key:
                    ...
                    if c2_lon:
                        ...
                        rupture_io = StringIO()
                        df.to_csv(
                            rupture_io, sep=" ", index=False, header=False
                        )
                        rupture_io.seek(0)
                    try:
                        rupture_dict = text_to_json(
                            rupture_io, new_format=True
                        )

                    except Exception as ve:
                        try:
                            rupture_dict = text_to_json(
                                rupture_io, new_format=False
                            )
                            has_coordinates = len(
                                rupture_dict["features"][0]["geometry"][
                                    "coordinates"
                                ][0]
                            )
    ...
    def get_event_dict(self, is_online_scenario=False):
        ...
        if self.source is not None:
            ...
            if is_online_scenario is True:
                ...
                netname = get_network_name(
                    self.event_json["properties"]["net"]
                )
                etype = "ACTUAL"
            edict = {
                "id": eventid,
    ...
    def get_history_dict(self):
        ...
        if self.shakemap is not None:
            ...
            versiondict = jdict["processing"]["shakemap_versions"][
                "map_data_history"
            ]
        return versiondict
