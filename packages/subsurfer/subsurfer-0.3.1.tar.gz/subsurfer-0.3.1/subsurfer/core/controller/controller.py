...
class SubSurferController:
    ...
    def save_results(self, results: Dict[str, Any], output_path: str) -> None:
        ...
        try:
            ...
            with open(output_path, "w", encoding="utf-8") as f:
                ...
                for subdomain in sorted(results['web_servers']):
                    f.write(f"{subdomain}\n")
                f.write("\n")
                
                # 웹 서비스 상세 정보
                if results['web_services']:
                    f.write("SubSurfer - webservice-details\n")
                    for url, analysis in sorted(results['web_services'].items()):
                        f.write(f"{url}: {analysis}\n")
                    f.write("\n")
                
                # 포트 스캔 결과
                if 'all_urls' in results and results['all_urls']:
                    f.write("SubSurfer - subdomain-webserver-portscan\n")
                    for subdomain, urls in sorted(results['all_urls'].items()):
                        for url, port in urls:
                            f.write(f"{url}:{port}\n")
                    f.write("\n")
                
                # 웹 서버는 아니지만 활성화된 서비스 목록
                if results['enabled_services']:
                    f.write("SubSurfer - subdomain-notweb-but-enable\n")
                    for subdomain in sorted(results['enabled_services']):
                        f.write(f"{subdomain}\n")
    ...
    async def collect_subdomains(self) -> Set[str]:
        ...
        if self.active:
            ...
            # 패시브 스캔만 실행
            return await self.passive_handler.collect()
