    ...
    def next(self):
        ...
        if is_live:
            ...
            if cur_bar_local_time_str not in self.cache:
                tdelta = timedelta(minutes=self.p.compression)
                if self.p.timeframe == bt.TimeFrame.Seconds:
                    tdelta = timedelta(seconds=self.p.compression)
        ...
        for line_name in self.lines.getlinealiases():
            ...
            if line[0] == float('-inf'):
                if len(self) > 1:
                    stdout_log(f"[CRITICAL], {self.__class__.__name__}, kline time: {cur_bar_local_time.astimezone().strftime('%Y-%m-%d %H:%M:%S')}, {line_name}[0] inherited from {line_name}[-1]: {line[-1]}")
                    line[0] = line[-1]
