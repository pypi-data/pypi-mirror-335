...
"The `ContentServicesApi` Python library offers a primary class designed to facilitate interaction with the Content Repository REST content_obj. This class encompasses a wide range of operations, including:
...
```bash
## From cmd.exe
pip install rocketcontent==1.0.2
```
...
The `ContentServicesApi` class is the primary interface for interacting with the Content Repository REST content_obj. It is instantiated by providing a YAML configuration file that contains essential settings for content_repocommunication. Once initialized, it exposes methods for performing various operations against the Content Repository REST content_obj.
...
* **Logging:**
    ...
    With `log_level="DEBUG"` in the configuration, all Smart Chat content_repointeractions are logged, including JSON payloads. Example log output:
