...
class Net(pl.LightningModule):
    def __init__(
        ...
        **evonet_kwargs,
        ...
        # self.num_classes_cover = len(classes_dict["cover"].keys())
        # self.num_classes_occlusion = len(classes_dict["occlusion"].keys())
        # self.num_classes_ecosystems = len(classes_dict["ecosystems"].keys())
        self.num_classes = (
        ...
        evonet_kwargs["activation_mlp"] = evonet_kwargs["activation_mlp"]   # "identity"
    ...
    def _loss(self, y_pred, y, y_weight, classes_weights_tensor):#, slicer=None):
        ...
        # classes_weights_tensor = self._classes_weights_tensor
    ...
    # def loss_component(self, y_pred, y, y_weight):
...
#         return self._loss(y_pred, y, y_weight_mask, self._slice_ecosystems)
    ...
    def predict(self, x, x_head, softmax=True):
        # from loguru import logger
        probs_raw = self.evonet.predict(x, x_head)
        probs_raw = probs_raw[np.newaxis, ...].astype(np.float32)
        y_pred_cover = self._slice_cover(probs_raw)
        y_pred_occlusion = self._slice_occlusion(probs_raw)
        y_pred_ecosystems = self._slice_ecosystems(probs_raw)

        # logger.info(f"y_pred_cover shape: {y_pred[0].shape}")
        if softmax:
            softmax = nn.Softmax(dim=1)
            y_pred_cover = softmax(torch.from_numpy(y_pred_cover)).numpy()
            y_pred_occlusion = softmax(torch.from_numpy(y_pred_occlusion)).numpy()
            y_pred_ecosystems = softmax(torch.from_numpy(y_pred_ecosystems)).numpy()
        return (y_pred_cover.astype(np.float32).squeeze(),
                y_pred_occlusion.astype(np.float32).squeeze(),
                y_pred_ecosystems.astype(np.float32).squeeze()
        )
    # self._slice_cover(pred), self._slice_occlusion(pred), self._slice_ecosystems(pred)
