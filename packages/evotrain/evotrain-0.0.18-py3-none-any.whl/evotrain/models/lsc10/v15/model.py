...
class Net(pl.LightningModule):
    def __init__(
        ...
        **evonet_kwargs,
        ...
        self.classes_weights_tensor_component['ecosystems'] = self.get_classes_weights_tensor(self.classes_weights_components_dict['ecosystems'], classes_dict['ecosystems'])
        self.classes_dict = classes_dict

        self.num_classes_components = {
            'cover': len(classes_dict["cover"].keys()),
            'occlusion': len(classes_dict["occlusion"].keys()),
            'ecosystems': len(classes_dict["ecosystems"].keys())
        }
            
        self.num_classes = (
            self.num_classes_components['cover']#num_classes_cover
            + self.num_classes_components['occlusion']#num_classes_occlusion
            + self.num_classes_components['ecosystems']#num_classes_ecosystems
        )
    ...
    def _slice_ecosystems(self, x):
        ...
        end = start + self.num_classes_components['ecosystems'] #lf.num_classes_ecosystems
        return x[:, start:end, ...]
    ...
    def loss(self, y_pred, y, y_weight_mask):
        ...
        loss_ecosystems = self._loss(y_pred[2],
                                     y[2],
                                     y_weight_mask[2],
                                    self.classes_weights_tensor_component['ecosystems'].cuda())
    ...
    def forward(self, x, x_head):
        ...
        y_pred = [self._slice_cover(y_pred), self._slice_occlusion(y_pred), self._slice_ecosystems(y_pred)]
    ...
    def predict(self, x, x_head):
        ...
        y_pred = [self._slice_cover(pred), self._slice_occlusion(pred), self._slice_ecosystems(pred)]
        softmax = nn.Softmax(dim=1)
        y_pred_cover = softmax(torch.from_numpy(y_pred[0])).numpy()
        y_pred_occlusion = softmax(torch.from_numpy(y_pred[1])).numpy()
        y_pred_ecosystems = softmax(torch.from_numpy(y_pred[2])).numpy()
        return y_pred_cover, y_pred_occlusion, y_pred_ecosystems
    ...
    def training_step(self, batch, batch_idx):
        ...
        y_pred = [y_pred_cover, y_pred_occlusion, y_pred_ecosystems]
    ...
    def validation_step(self, batch, batch_idx):
        # x, x_head, prob_cover, prob_occlusion, prob_eco, weight_cover, weight_occlusion, weight_eco
