...
from evotrain.models.lcm.roads.callbacks import InferencePreview
from evotrain.models.lcm.roads.dataset import get_train_val_dataloaders
from evotrain.models.lcm.roads.model import Net
...
def train(config):
    ...
    if checkpoints is not None and resume:
        ...
        model = Net(**model_config).load_from_checkpoint(
            checkpoints[-1],
        )
    else:
        checkpoint = None
    ...
    trainer_kwargs = dict(
        ...
        callbacks=[
            ModelCheckpoint(
                ...
                save_top_k=3,
                save_last=True,
                filename="epoch={epoch}-step={step}-iou={mean_iou_val:.4f}",
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Usage: python trainer.py <config_file_path>")
        sys.exit(1)
    ...
    logger.info(f"Config loaded from {config_fn}")
    logger.info(f"Starting training of model {config['trainer']['model_name']}")
