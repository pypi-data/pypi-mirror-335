...
def set_auth_for_url(url: Url | str) -> str:
    ...
    if not password:
        auth_parts = (
            ...
            or get_mamba_auth(parsed_url.netloc or "")
        )
        if auth_parts is not None:
            username, password = auth_parts
        if username or password:
            url = parsed_url._replace(auth=f"{username or ''}:{password or ''}").url

        if username and not password:
            logger.info(f"No password found for {url}")
        elif not username:
            logger.info(f"No username or password found for {url}")
