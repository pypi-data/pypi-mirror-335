...
3. 多中心场景按顺序优先原则标注（主要中心词+逗号分隔的次要中心词）""",
        ).export_yaml(practice_dir + "/central_word_extract.yaml")

    def run(self, question: str) -> CentralWord:

        _prompt = GeneralPromptBuilder.load("central_word_extract").get_instruction(
            question
        )
        res = self.llm_instance.chat(_prompt)
...
if __name__ == "__main__":
    ...
    CentralWordExtract.build_prompt()
