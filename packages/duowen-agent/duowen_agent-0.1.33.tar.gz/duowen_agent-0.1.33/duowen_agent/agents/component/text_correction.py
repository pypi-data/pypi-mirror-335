...
class Edits(BaseModel):
    ...
    # ctx: Tuple[str, str] = Field(
    #     ...,
    #     min_length=2,
    #     max_length=2,
    #     description="[前导0-3字符（空表示无前导）, 后续0-3字符（空表示无后续）]",
    # )
...
4. 修正结果必须与原始输入严格字面一致（包括标点符号的保留）""",
        ).export_yaml(practice_dir + "/text_correction.yaml")
    ...
    def run(
        ...
        **kwargs,
        ...
        _prompt = GeneralPromptBuilder.load("text_correction").get_instruction(question)
