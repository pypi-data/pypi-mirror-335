...
class PdfToJpg:
    ...
    def _get_pdf_buffer(self) -> bytes:
        ...
        if self.doc_info.get('doc_path'):
            ...
            return base64.b64decode(self.doc_info['doc_data'].split('base64,')[-1])
    ...
    def _convert_page(self, pdf: 'pyvips.Image', page_num: int) -> Dict[str, Any]:
        ...
        """
        page = pyvips.Image.pdfload_buffer(self._get_pdf_buffer(), page=page_num - 1)  # 0-based indexing in pyvips
        temp_dir = Config.get_temp_dir()
        print("temp_dir", temp_dir)
        temp_image_path = os.path.join(
            temp_dir,
            f"{self.doc_info['doc_id']}_page_{page_num}_{datetime.now().strftime('%Y%m%d%H%M%S')}.jpg"
        )
        ...
        return {
                'doc_uid': f"{self.doc_info['doc_id']}_page_{page_num}",
...
class PdfToPdf:
    ...
    def convert(self) -> Dict[str, Any]:
        ...
        try:
            ...
            reader = self._get_pdf_reader(pdf_buffer)
    ...
    def _get_pdf_reader(self, pdf_buffer: bytes) -> PdfReader:
        """
        Get the PDF reader object from the PDF buffer.

        Args:
            pdf_buffer (bytes): The PDF data in bytes.
    ...
    def _convert_page(self, page: 'PyPDF2.pdf.PageObject', page_num: int) -> Dict[str, Any]:
        ...
        """
        doc_uid = f"{self.doc_info['doc_id']}_page_{page_num}_{datetime.now().strftime('%Y%m%d%H%M%S')}"
