...
def create_resource_group(
    ...
    # --- PICK Command
    @bic_res_cmd.command(
        help=f"Return the value of a property from a {resource_title}"
    )
    ...
    def pick(ctx, property: str):
        ...
        property_value = get_property(resource, property)
        if isinstance(property_value, BaseModel):
            ctx.obj.response_handler.treat_single_resource(property_value, format="json")
        elif isinstance(property_value, List):
            if len(property_value) == 0:
                click.echo("[]")
            else:
                ctx.obj.response_handler.treat_list_resources(property_value, format="json")
