...
async def fetch_data():
    """
    Asynchronously fetch API data.
    
    Returns:
        API response data or None if request fails
    """
    try:
        print("Starting to fetch API data...", file=sys.stderr)
        async with aiohttp.ClientSession() as session:
            print(f"Sending POST request to {API_ENDPOINT}, params: {PARAMS}", file=sys.stderr)
            async with session.post(API_ENDPOINT, headers=HEADERS, json=PARAMS) as response:
                if response.status == 200:
                    data = await response.json()
                    print("API request successful", file=sys.stderr)
                    return data
                else:
                    print(f"❌ API request failed with status: {response.status}", file=sys.stderr)
                    try:
                        error_text = await response.text()
                        print(f"Response content: {error_text[:500]}", file=sys.stderr)
                    except Exception:
                        pass
                    return None
    except aiohttp.ClientError as e:
        print(f"❌ HTTP client error: {str(e)}", file=sys.stderr)
        return None
...
async def prepare():
    try:    
        ...
        if not data:
            print("❌ Error: API didn't return any data.", file=sys.stderr)
            return False
        if data.get('status') == 0:
            print("❌ Error: API status is 0. Please check if the API is working properly.", file=sys.stderr)
            return False
        if not data.get('plugins'):
            print("❌ Error: No plugins data in API response.", file=sys.stderr)
            return False
        ...
        try:
            # Execute the generated code in global namespace
            exec(func_template, globals())
            return True
        except Exception as e:
            print(f"❌ Error executing generated function template: {str(e)}", file=sys.stderr)
            print(f"First 100 characters of template: {func_template[:100]}...", file=sys.stderr)
            return False
            
    except json.JSONDecodeError as e:
        print(f"❌ JSON parsing error: {str(e)}", file=sys.stderr)
        print(f"Received non-JSON data, content: {data}", file=sys.stderr)
        return False
    except Exception as e:
...
async def run_server():
    ...
    if not success:
        print("Server preparation failed, exiting", file=sys.stderr)
        return  # Note: return instead of sys.exit()
    
    try:
        # This will block until the server exits
        print("Server preparation complete, starting MCP", file=sys.stderr)
        mcp.run()
    except Exception as e:
