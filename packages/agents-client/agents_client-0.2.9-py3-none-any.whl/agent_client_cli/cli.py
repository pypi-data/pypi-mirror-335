...
def send(
    ...
    with console.status("Sending operation..."):
        try:
            response = send_operation(
                operation, params_dict, api_key, agent_id, base_url=get_base_url()
            )
...
def upload(
    file_path: str = typer.Argument(
        ..., help="Path where the file should be saved on the server"
    ),
        ...
        with open(local_file, "rb") as f:
            with console.status(f"Uploading file {local_file}..."):
                response = upload_file(
                    api_key, agent_id, file_path, f, base_url=get_base_url()
                )
...
def stream(
    ...
    async def run_stream():
        ...
        try:
            async for message in connect_websocket(
                agent_id, api_key, base_url=get_base_url()
            ):
...
def setup():
    ...
    open_browser = typer.confirm(
        "Would you like to open the token creation page in your browser?", default=True
    )
    ...
    if token_option == "F":
        ...
        token_file = typer.prompt("Enter the path to your downloaded token file")
        try:
            with open(token_file, "r") as f:
        ...
        auth_token = typer.prompt("Paste your authentication token", hide_input=True)

    # Validate token format
    if not auth_token or ":" not in auth_token:
        console.print(
            "[bold red]Invalid token format. Expected format: user_id:salt:token[/bold red]"
        )
        raise typer.Exit(1)
    ...
    if set_base_url:
        base_url = typer.prompt("Enter the API base URL", default=API_BASE_URL)
        config = load_config()
...
def config():
    ...
    if update:
        ...
        if setting == "api_base_url":
            ...
            console.print(
                f"[bold green]Updated api_base_url to:[/bold green] {new_value}"
            )

        elif setting == "clear_token":
            confirm = typer.confirm(
                "Are you sure you want to clear your saved auth token?", default=False
            )
...
def call_info_api(action, agent_id=None):
    ...
    payload = {"auth_token": token, "action": action}
    ...
    try:
        ...
        if response.status_code == 401:
            ...
            console.print(
                "Please run 'agents-cli setup' to configure a new auth token."
            )
        else:
...
def info():
    ...
    console.print(
        "    [dim]Example: agents-cli agent-info 2f86289c-33c6-40d7-bfa6-cda143dcc094[/dim]"
    )
    ...
    console.print(
        "  [cyan]agents-cli send <operation> <params_json> [--api-key KEY] [--agent-id ID][/cyan]"
    )
    console.print("    Send an operation to an agent")
    console.print(
        '    [dim]Example: agents-cli send message \'{"text":"Hello"}\' --api-key mykey --agent-id myagent[/dim]'
    )

    console.print(
        "  [cyan]agents-cli upload <server_path> <local_file> [--api-key KEY] [--agent-id ID][/cyan]"
    )
    console.print("    Upload a file to an agent")
    console.print(
        "    [dim]Example: agents-cli upload /data/file.txt ./local.txt --api-key mykey --agent-id myagent[/dim]"
    )
    ...
    console.print(
        "    [dim]Example: agents-cli stream --api-key mykey --agent-id myagent[/dim]"
    )
...
def usage():
    ...
    console.print(
        f"[bold green]Total Cost:[/bold green] {response.get('total_cost', 0)}"
    )
    console.print(
        f"[bold cyan]Average Cost Per Hour:[/bold cyan] {response.get('avg_cost_per_hr', 0)}"
    )
    console.print(
        f"[bold blue]Average Cost Per Day:[/bold blue] {response.get('avg_cost_per_day', 0)}"
    )
    console.print(
        f"[bold magenta]Average Cost Per Week:[/bold magenta] {response.get('avg_cost_per_week', 0)}"
    )
    ...
    if agents_usage:
        ...
        for agent in agents_usage:
            table.add_row(
                agent.get("agent_name", "Unknown"), str(agent.get("credits_used", 0))
            )
...
@app.command()
def create_key(name: str = typer.Option(None, help="Name for the new API key")):
    """
    ...
    if name is None:
        name = typer.prompt(
            "Enter a name for the new API key", default="CLI Generated Key"
        )

    base_url = get_base_url()
    url = f"{base_url}/createapikey"

    payload = {"auth_token": token, "key_name": name}
    ...
    with console.status("Creating new API key..."):
        try:
            ...
            save_to_file = typer.confirm(
                "Would you like to save this key to a file?", default=False
            )
            if save_to_file:
...
def link(
    action: str = typer.Argument(
        ..., help="The resource to get a link for (e.g., dashboard, billing, docs)"
    )
):
    """
    ...
    try:
        ...
        # Outside the status context manager, so the spinner stops here
        console.print(f"[bold green]Link for {action}:[/bold green] {link_url}")

        # Ask if they want to open it - this is now outside the spinner context
        open_link = typer.confirm("Open this link in your browser?", default=False)
        if open_link and link_url:
            try:
