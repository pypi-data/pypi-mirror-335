...
class Bottle(object):
    ...
    # Added June 3rd, 2024 - Zenen
    def endpoint(self, endpoint_name, api, **options):
        """ Maps a solar endpoint to a designated API """

        path = f'<path:re:.*>{endpoint_name}'
        planets = []
        for app in self._mounts:
            if getattr(app, 'data') is None:
                continue

            app_path = app.data.get('path')
            planets.append(app_path)

        options['planets'] = planets
               
        return self.route(path, ["GET","POST","DELETE","PATCH"], callback=api, **options)
...
class BaseTemplate(object):
    ...
    extensions = ['tpl', 'html', 'thtml', 'stpl', 'ms']
