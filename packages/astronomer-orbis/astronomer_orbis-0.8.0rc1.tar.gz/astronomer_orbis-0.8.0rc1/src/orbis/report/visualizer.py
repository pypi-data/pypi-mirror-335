...
class Visualizer:
    ...
    def generate_combined_graph(self, dataframes: list[pl.DataFrame], metrics: list[Metric]) -> Figure:
        ...
        for metric, df in zip(metrics, dataframes):
            ...
            if "small" in metric.metric_identifier:
                ...
                fig.add_trace(go.Scatter(x=df["Time Stamp"], y=df["Value"], mode="lines", name=name, line=dict(color=COLORS[0], width=1)))
                ...
                fig.add_trace(go.Scatter(x=df["Time Stamp"], y=df["Value"], mode="lines", name=name, line=dict(color=COLORS[1], width=1)))
        ...
        if total_stats and total_dfs:
            ...
            fig.add_trace(go.Scatter(x=df["Time Stamp"], y=df["Mean"], mode="lines", name="Mean", line=dict(color="grey", width=2, dash="dash")))
            fig.add_trace(go.Scatter(x=df["Time Stamp"], y=df["Median"], mode="lines", name="Median", line=dict(color="magenta", width=2, dash="dashdot")))
    ...
    def _add_trace(self, fig: go.Figure, df: pl.DataFrame, metric: Metric, idx: int):
        if metric.metric_name == "Tasks Trend":
            ...
            fig.add_trace(go.Scatter(x=df["Time Stamp"], y=df["Value"], mode="lines", name=name, line=dict(color=COLORS[idx], width=1)))
            ...
            fig.add_trace(go.Scatter(x=df["Time Stamp"], y=df["Value"], mode="lines", name=name, line=dict(color=color, width=1)))
            ...
            fig.add_trace(go.Scatter(x=df["Time Stamp"], y=df["Value"], mode="lines", name=name, line=dict(color=COLORS[idx % len(COLORS)], width=1)))
        else:
    ...
    def _add_mean_median_traces(self, fig: go.Figure, df: pl.DataFrame):
        ...
        for name, color, dash in trace_configs:
            try:
                fig.add_trace(go.Scatter(x=df["Time Stamp"], y=df[name], mode="lines", name=name, line=dict(color=color, width=2, dash=dash)))
            except Exception as e:
