...
{Fore.light_steel_blue}if 'b' returns to previous menu, try '#b' to return to barcode input, in ListMode@$LOCATION_FIELD, 'e' does the same{Style.reset}''')
                ...
                with db.Session(db.ENGINE) as session:
                    ...
                    for num,i in enumerate(results):
                        ...
                        msg+=f"{Fore.light_magenta} |-|{Fore.light_green} Total = {Fore.light_sea_green}{total}|-|{Fore.light_magenta}Price({i.Price}){Fore.medium_violet_red}*{Fore.light_slate_blue}Total({total}):{i.Price*total}{Style.reset}"
                        print(msg)
                    msg=f"{Fore.light_green}Total Product Value:{Fore.slate_blue_1}{master_total}{Style.reset}"
                    print(msg)
