    ...
    for ablockname, antbool in srcup.items():
        ...
        if any(srcupalways[ablockname].values()):
            ...
            if len(ant_can) >= len(o.stations):
                ...
                rprint(" [dim](always observable by everyone but ", end='')
                rprint(','.join([ant for ant in o.stations.station_codenames if ant not in ant_can]), end='')
                rprint(")[/dim]")
        else:
...
def cli():
    ...
    if args.list_bands:
        ...
        for aband in obs.freqsetups.bands:
            ...
            rprint("[dim]  Observable with ", end='')
            rprint({', '.join([nn for nn, n in _NETWORKS.items() \
                                             if aband in n.observing_bands])}, end='')
            rorint("[/dim]")
