...
class PlaywrightNavigateParams(BaseModel):
    url: str = Field(..., description="需要导航的URL地址")
    timeout: int = Field(default=30000, description="导航超时时间(毫秒)")
    wait_until: str = Field(default="load", description="导航等待条件可选的值：load, domcontentloaded, networkidle, commit")
        ...
        async def playwright_navigate(params: PlaywrightNavigateParams):
            ...
            async def highlight_interactive_elements():
                ...
                try:
                    ...
                    return f"已标注 {element_count} 个可交互元素"
                    
                except Exception as e:
                    raise ValueError(f"标注交互元素失败: {e}")
