...
class Meeting:
    ...
    def start_recording(self, recording_config: Optional[RecordingConfig] = None):
        ...
        """
        logging.debug(f"recording starting {self.__id}")
        self.__room_client.start_recording(recording_config)

    def stop_recording(self):
        """Stop recording the meeting."""
        logging.debug(f"recording stopping {self.__id}")
        self.__room_client.stop_recording()

    def start_transcription(self, transcription_config: Optional[TranscriptionConfig] = None):
        """
    ...
    def upload_base64(self, base64_data, token, file_name):
        ...
        if not base64_data or not token or not file_name:
            raise ValueError(
                "Please provide base64_data, token, and file_name")
    ...
    def __listen(self):
        ...
        def on_speaker_changed(data):
            ...
            self.__emit(VideoSDKEvents.EV_SPEAKER_CHANGED.value,
                        self.__active_speaker_id)
        ...
        def on_participant_joined(data):
            ...
            self.__emit(
                VideoSDKEvents.EV_PARTICIPANT_JOINED.value, participant)
        ...
        def on_participant_left(data):
            ...
            if id in self.__participants:
                ...
                self.__emit(
                    VideoSDKEvents.EV_PARTICIPANT_LEFT.value, participant)
        ...
        def on_transcription_state_changed(data):
            ...
            self.__emit(
                VideoSDKEvents.EV_TRANSCRIPTION_STATE_CHANGED.value, data)
