...
class PubSub:
    ...
    def __init__(self, room_client: RoomClient):
        ...
        logging.debug("PubSub initialized")
    ...
    async def subscribe(self, pubsub_config: PubSubSubscribeConfig):
        ...
        """
        logging.debug(f"Subscribing to topic: {pubsub_config.get('topic')}")
        ans = await self.__room_client.async_handle_pubsub_subscribe(pubsub_config)
        if ans is not None:
            self.__topic_cb[pubsub_config.get(
                'topic')] = pubsub_config.get("cb")
        return ans
    ...
    async def publish(self, pubsub_config: PubSubPublishConfig):
        ...
        """
        logging.debug(f"Publishing to topic: {pubsub_config.get('topic')}")
