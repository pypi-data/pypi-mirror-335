...
def install_dependencies():
    ...
    if config.get("BEEKEEPER_ENVIRONMENT") in ["dev", "tst", "stg"]:
        wheel_url = f"https://stusebkstoragesdk.blob.core.windows.net/escrowaisdk/dist/{config.get('BEEKEEPER_ENVIRONMENT')}/EscrowAICI-0.1.5-py3-none-any.whl"
...
def encrypt_and_upload(secrets):
    ...
    subprocess.run(
        ["rsync", "-av", "--exclude", ".git", "--exclude", "temp", ".", folder_path],
        check=True,
    )

    for notebook in Path(folder_path).rglob("*.ipynb"):
        notebook.unlink()
