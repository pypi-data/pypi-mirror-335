...
def _create_dataset_or_fail(import_job: ImportJob, dry_run: bool):
    ...
    try:
        ...
        return import_format.create_dataset(data)
...
def _run_import_job(import_job: ImportJob, dry_run: bool = True) -> None:
    ...
    try:
        dataset = _create_dataset_or_fail(import_job, dry_run)
    except Exception:
        return

    change_job_status(import_job, "import", "2/5 Processing import data", dry_run)

    class Resource(model_config.resource):  # type: ignore
    ...
    _finish_import(import_job, dry_run, resource, result, skip_diff)
