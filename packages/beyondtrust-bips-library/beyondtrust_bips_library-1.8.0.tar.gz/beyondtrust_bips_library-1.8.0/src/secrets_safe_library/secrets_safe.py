...
class SecretsSafe(APIObject):
    ...
    def __init__(
        ...
        self._schema = {
            ...
            "password": {"type": "string", "maxlength": 256, "nullable": True},
    ...
    def get_secret_by_path(
        ...
        if self._validator.validate(attributes, update=True):
            ...
            endpoint = f"/secrets-safe/secrets?{params}"

            utils.print_log(
                self._logger,
                f"Calling get_secret_by_path endpoint: {endpoint}",
                logging.DEBUG,
            )
    ...
    def _get_file_by_id_req(self, secret_id):
        ...
        endpoint = f"/secrets-safe/secrets/{secret_id}/file/download"

        utils.print_log(
            self._logger,
            f"Calling _get_file_by_id_req endpoint: {endpoint}",
            logging.DEBUG,
        )
        response = self._run_get_request(endpoint)
        return response
    ...
    def get_secret_by_id(self, secret_id: str) -> dict:
        ...
        endpoint = f"/secrets-safe/secrets/{secret_id}"

        utils.print_log(
            self._logger,
            f"Calling get_secret_by_id endpoint: {endpoint}",
            logging.DEBUG,
        )
    ...
    def list_secrets(
        ...
        endpoint = f"/secrets-safe/secrets?{query_string}"

        utils.print_log(
            self._logger, f"Calling list_secrets endpoint: {endpoint}", logging.DEBUG
        )
    ...
    def create_secret(
        ...
        attributes = {"description": description, "password": password, "notes": notes}
        ...
        if partial_endpoint == "secrets/file":
            utils.print_log(
                self._logger,
                f"Calling _run_post_file_request: {endpoint}",
                logging.DEBUG,
            )
            ...
            utils.print_log(
                self._logger,
                f"Calling _run_post_request: {endpoint}",
                logging.DEBUG,
            )
    ...
    def delete_secret_by_id(self, secret_id: str) -> None:
        ...
        utils.print_log(
            self._logger,
            f"Calling delete_secret_by_id endpoint: {endpoint}",
            logging.DEBUG,
        )
    ...
    def update_secret(
        ...
        endpoint = f"/secrets-safe/secrets/{secret_id}/{partial_endpoint}"

        if file_path:
            utils.print_log(
                self._logger,
                f"Calling _run_put_file_request: {endpoint}",
                logging.DEBUG,
            )
            response = self._run_put_file_request(
                endpoint,
                req_body,
                file_path=file_path,
            )
            ...
            utils.print_log(
                self._logger,
                f"Calling _run_put_request: {endpoint}",
                logging.DEBUG,
            )
