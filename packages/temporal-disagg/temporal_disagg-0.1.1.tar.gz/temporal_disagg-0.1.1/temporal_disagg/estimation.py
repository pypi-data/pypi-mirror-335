...
class TempDisModel(TempDisBase):
    ...
    def __init__(self,
        ...
        if self.method not in self.valid_methods:
            raise ValueError(f"El método '{self.method}' no está soportado. Métodos válidos: {self.valid_methods}")
    ...
    def predict(self):
        ...
        if self.ensemble:
            df_predicted = self.ensemble_prediction(df_predicted, self.conversion, {k: v for k, v in all_methods.items() if k != self.method})
