...
class NocodbClient:
    ...
    async def synchronize_records(self, records: list[NocodbModel], **mapper_kwargs):
        ...
        for absent_external_id in absent_external_ids:
            absent_record = existing_lookup_map[absent_external_id]
            self.logger.info(f"Removing absolete record: {absent_record['Id']}")
            await self.delete_table_record(table_id, absent_record['Id'])
            continue
