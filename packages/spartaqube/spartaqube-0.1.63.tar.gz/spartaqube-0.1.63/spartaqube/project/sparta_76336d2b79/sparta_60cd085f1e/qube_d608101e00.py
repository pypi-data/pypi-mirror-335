...
from project.sparta_3bc15131c6.sparta_57b0e04fde.qube_5a839bac4d import sparta_c99e2a0b36
from project.sparta_3bc15131c6.sparta_57b0e04fde.qube_b4fff0626d import sparta_5f9759fc60
...
from project.sparta_3bc15131c6.sparta_ef73db400d.qube_a300615bb6 import sparta_5d910cb206,sparta_00fc6ac630,get_api_key_async
class NotebookWS(AsyncWebsocketConsumer):
	...
	async def notebook_permission_code_exec(A,json_data):from project.sparta_3bc15131c6.sparta_b8050354d7 import qube_2eeaed24d7 as B;return await coreNotebook.notebook_permission_code_exec(json_data)
		...
		if len(t)>0:
			...
			if E=='init-socket'or E=='reconnect-kernel'or E=='reconnect-kernel-run-all':
				...
				try:
					...
					AP=json.loads(A['execCodeInput']);x=N.join([A[Y]for A in AP]);U=W(x,A);U+=N+L;await B.sender_kernel_obj.send_zmq_request(sender_dict={D:P,J:U},b_send_websocket_msg=_B);V=sparta_c99e2a0b36(x);y=A['guiInputVarName'];AQ=A['guiOutputVarName'];AR=A[AH];n=[y,AQ,AR];X=[]
					for S in n:
				...
				try:
					z=M;d=M
					if m in A:e=A[m];A0=sparta_5f9759fc60(e);z=A0['assign_state_variable'];d=A0['assign_code']
					AS=json.loads(A['execCodeOutput']);A1=N.join([A[Y]for A in AS]);U=d+N;U+=z+N;U+=A1;await B.sender_kernel_obj.send_zmq_request(sender_dict={D:P,J:U},b_send_websocket_msg=_B);V=sparta_c99e2a0b36(A1)
					try:V.append(A[m][l])
				...
				try:
					...
					f+=N+g;logger.debug('cmd to execute');logger.debug('cmd_to_exec');logger.debug(f);await B.sender_kernel_obj.send_zmq_request({D:P,J:f});V=sparta_c99e2a0b36(g)
				except Exception as H:C=json.dumps({F:-1,D:E,T:str(H)});await B.send(text_data=C);return
