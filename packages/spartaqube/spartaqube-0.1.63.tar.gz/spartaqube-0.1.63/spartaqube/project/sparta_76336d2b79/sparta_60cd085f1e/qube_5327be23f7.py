...
class HotReloadLivePreviewWS(AsyncWebsocketConsumer):
	...
	def build_dist(D):
		J='main.js';I='frontend';H='Darwin';G='x86_64';B=platform.system();C=platform.machine();K=sparta_ab2d4b6ec5()['project/core/developer'];L=os.path.join(K,'esbuild')
	...
	async def receive(A,text_data):
		D=text_data
		if len(D)>0:
			E=json.loads(D);B=sparta_b0cac1454c(E['projectPath']);A.project_path=B;C=[os.path.join(B,'backend/logs'),os.path.join(B,'frontend/dist')];C=[sparta_b0cac1454c(A)for A in C]
			if A.monitor_task and not A.monitor_task.done():
				A.monitor_task.cancel()
				try:await A.monitor_task
