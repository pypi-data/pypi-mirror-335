...
class ApiWebsocketWS(WebsocketConsumer):
	...
	def receive(A,text_data):
		...
		if len(E)>0:
			B=json.loads(E);J=B.get('isRunMode',False);K=sparta_b0cac1454c(B[I]);F=os.path.join(os.path.dirname(K),'backend');sys.path.insert(0,F);import websockets as C;importlib.reload(C);G=B['service'];H=B.copy();del B[I];L=os.path.join(F,'logs','output.log')
			if J:D=C.sparta_1ccbc38365(G,H,A.user);A.send(json.dumps(D))
			else:
				with OutputRedirector(A,L):
					try:D=C.sparta_1ccbc38365(G,H,A.user);A.send(json.dumps(D))
