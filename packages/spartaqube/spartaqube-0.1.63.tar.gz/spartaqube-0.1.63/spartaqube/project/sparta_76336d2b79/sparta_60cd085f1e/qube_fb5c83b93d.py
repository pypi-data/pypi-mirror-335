...
class PipInstallWS(WebsocketConsumer):
	...
	def receive(C,text_data):
		...
		if len(F)>0:
			G=json.loads(F);H=G['pipInstallCmd'].strip();L=G['env_name'];M=qube_d70bb96a2a.sparta_67a8966c74(L);N=H.replace('pip',M)
			if not sparta_71f37c0980(H):A={E:-1,'errorMsg':'Invalid syntax'};B=json.dumps(A);C.send(text_data=B);return
