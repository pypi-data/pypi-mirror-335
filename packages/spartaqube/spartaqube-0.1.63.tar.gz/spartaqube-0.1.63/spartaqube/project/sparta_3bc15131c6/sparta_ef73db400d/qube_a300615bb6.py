...
from project.sparta_3bc15131c6.sparta_0d2990c769.qube_1cd9f39e09 import sparta_813aa1a49c,sparta_211bae8a6b
from project.logger_config import logger
def sparta_bf9ce77410():keygen_fernet='spartaqube-api-key';key=keygen_fernet.encode(_B);key=hashlib.md5(key).hexdigest();key=base64.b64encode(key.encode(_B));return key.decode(_B)
def sparta_9abc7afdbb():keygen_fernet='spartaqube-internal-decoder-api-key';key=keygen_fernet.encode(_B);key=hashlib.md5(key).hexdigest();key=base64.b64encode(key.encode(_B));return key.decode(_B)
def sparta_3c8b4622fc(f,str_to_encrypt):data_to_encrypt=str_to_encrypt.encode(_B);token=f.encrypt(data_to_encrypt).decode(_B);token=base64.b64encode(token.encode(_B)).decode(_B);return token
def sparta_a67547b2c5(api_token_id):
	if api_token_id=='public':
		try:return User.objects.filter(email='public@spartaqube.com').all()[0]
		except:return
	try:
		f_private=Fernet(sparta_9abc7afdbb().encode(_B));api_key=f_private.decrypt(base64.b64decode(api_token_id)).decode(_B).split('@')[1];user_profile_set=UserProfile.objects.filter(api_key=api_key,is_banned=_D).all()
...
def sparta_00fc6ac630(user_obj):
...
def sparta_5d910cb206(user_obj,domain_name):api_key=sparta_00fc6ac630(user_obj);random_nb=str(randint(0,1000));data_to_encrypt=f"apikey@{api_key}@{random_nb}";f_private=Fernet(sparta_9abc7afdbb().encode(_B));private_encryption=sparta_3c8b4622fc(f_private,data_to_encrypt);data_to_encrypt=f"apikey@{domain_name}@{private_encryption}";f_public=Fernet(sparta_bf9ce77410().encode(_B));public_encryption=sparta_3c8b4622fc(f_public,data_to_encrypt);return public_encryption
def sparta_acec9400fc(json_data,user_obj):api_key=sparta_00fc6ac630(user_obj);domain_name=json_data['domain'];public_encryption=sparta_5d910cb206(user_obj,domain_name);return{_A:1,'token':public_encryption}
def sparta_3fac67ba62(json_data,user_obj):userprofile_obj=UserProfile.objects.get(user=user_obj);api_key=str(uuid.uuid4());userprofile_obj.api_key=api_key;userprofile_obj.save();return{_A:1}
def sparta_a0b64ade8a():plot_types=sparta_813aa1a49c();plot_types=sorted(plot_types,key=lambda x:x['Library'].lower(),reverse=_D);return{_A:1,'plot_types':plot_types}
def sparta_8e2bfb0e21(json_data):logger.debug('DEBUG get_plot_options json_data');logger.debug(json_data);plot_type=json_data['plot_type'];plot_input_options_dict=sparta_211bae8a6b(plot_type);plot_input_options_dict[_A]=1;return plot_input_options_dict
def sparta_70610bec25(code):
	tree=ast.parse(code)
...
def sparta_35b341103f(json_data,user_obj):
	A='errorMsg';user_code_example=json_data['userCode'];resp=_E;error_msg=''
	try:
		logger.debug('EXECUTE API EXAMPLE DEBUG DEBUG DEBUG');api_key=sparta_00fc6ac630(user_obj);core_api_path=sparta_ab2d4b6ec5()['project/core/api'];ini_code='import os, sys\n';ini_code+=f'sys.path.insert(0, r"{str(core_api_path)}")\n';ini_code+='from spartaqube import Spartaqube as Spartaqube\n';ini_code+=f"Spartaqube('{api_key}')\n";user_code_example=ini_code+'\n'+user_code_example;exec(user_code_example,globals());last_expression_str=sparta_70610bec25(user_code_example)
...
def sparta_3f28e9f735(json_data,user_obj):
	...
	return{_A:-1}
def sparta_3e7f12b97d(json_data,user_obj):widget_id=json_data['widgetId'];return qube_e27ca60edf.sparta_3e7f12b97d(user_obj,widget_id)
def sparta_976fb3087b(json_data,user_obj):
	api_service=json_data['api_service']
	if api_service=='get_status':output=sparta_a195f15066()
	elif api_service=='get_status_ws':return sparta_3ad2c5156e()
	elif api_service=='get_connectors':return sparta_d1cefd5272(json_data,user_obj)
	elif api_service=='get_connector_tables':return sparta_e6db6b31a5(json_data,user_obj)
	elif api_service=='get_data_from_connector':return sparta_8a658bb620(json_data,user_obj)
	elif api_service=='get_widgets':output=sparta_5e1bb2636b(user_obj)
	elif api_service=='has_widget_id':return sparta_518429127b(json_data,user_obj)
	elif api_service=='get_widget_data':return sparta_d500dfd9c1(json_data,user_obj)
	elif api_service=='get_plot_types':return sparta_813aa1a49c()
	return{_A:1,_F:output}
def sparta_a195f15066():return 1
def sparta_d1cefd5272(json_data,user_obj):
	A='db_connectors';keys_to_retain=['connector_id',_C,'db_engine'];res_dict=qube_e27ca60edf.sparta_bd7cdb3cef(json_data,user_obj)
	if res_dict[_A]==1:res_dict[A]=[{k:d[k]for k in keys_to_retain if k in d}for d in res_dict[A]]
	return res_dict
def sparta_e6db6b31a5(json_data,user_obj):res_dict=qube_e27ca60edf.sparta_14229cf97f(json_data,user_obj);return res_dict
def sparta_8a658bb620(json_data,user_obj):res_dict=qube_e27ca60edf.sparta_402787c56f(json_data,user_obj);return res_dict
def sparta_5e1bb2636b(user_obj):return qube_e27ca60edf.sparta_67d3369ad0(user_obj)
def sparta_518429127b(json_data,user_obj):return qube_e27ca60edf.sparta_4766ab7476(json_data,user_obj)
def sparta_d500dfd9c1(json_data,user_obj):return qube_e27ca60edf.sparta_11db3e854e(json_data,user_obj)
def sparta_65edd23ef1(json_data,user_obj):date_now=datetime.now().astimezone(UTC);session_id=str(uuid.uuid4());pickled_data=json_data['data'];NewPlotApiVariables.objects.create(user=user_obj,session_id=session_id,pickled_variables=pickled_data,date_created=date_now,last_update=date_now);return{_A:1,'session_id':session_id}
def sparta_43975e3200():return sparta_813aa1a49c()
def sparta_3c8f3255f7():cache.clear();return{_A:1}
def sparta_3ad2c5156e():
	global is_wss_valid;is_wss_valid=_D
	try:
		api_path=sparta_ab2d4b6ec5()['api']
...
def sparta_571eb7eda3(json_data,user_obj):
	...
	if not user_obj.is_anonymous:
		api_get_widgets=sparta_5e1bb2636b(user_obj)
...
def sparta_74f7e4c3c8(notebook_id):
...
def sparta_705fb5ae47(dashboard_id):
