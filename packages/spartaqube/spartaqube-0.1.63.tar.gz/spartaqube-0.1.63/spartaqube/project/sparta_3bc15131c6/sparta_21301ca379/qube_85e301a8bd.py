...
def sparta_b10ae816cb():B='db-conn';A=B.encode(_A);A=hashlib.md5(A).hexdigest();A=base64.b64encode(A.encode(_A));return A.decode(_A)
def sparta_0db65b9c7e(password_to_encrypt):A=password_to_encrypt;A=A.encode(_A);C=Fernet(sparta_b10ae816cb().encode(_A));B=C.encrypt(A).decode(_A);B=base64.b64encode(B.encode(_A)).decode(_A);return B
def sparta_4972cf6edd(password_e):B=Fernet(sparta_b10ae816cb().encode(_A));A=base64.b64decode(password_e);A=B.decrypt(A).decode(_A);return A
def sparta_a199e22d72():
