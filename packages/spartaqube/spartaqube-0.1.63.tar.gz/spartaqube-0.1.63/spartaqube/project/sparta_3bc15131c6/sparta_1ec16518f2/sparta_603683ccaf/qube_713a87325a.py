...
def sparta_8702da6ba3(type='candlestick'):B='Example to plot a simple candlestick chart with lightweight chart';A=_E;C={_F:_I,_G:_J,_H:_K};D=_M;return[{_A:f"{type.capitalize()}",_B:B,_C:'',_D:f"""{A}
...
plot_example"""}]
def sparta_9315ce97d4(type='line2'):A=_E;B={_F:_I,_G:_J,_H:_K};C=_M;return[{_A:f"{type.capitalize()}",_B:f"Example to plot a simple {type} chart with lightweight chart",_C:'',_D:f"""{A}
import yfinance as yf
spartaqube_obj = Spartaqube()
# Fetch the data for Apple (ticker symbol: AAPL)
apple_price_df = yf.Ticker(\"AAPL\").history(period=\"1y\")
...
plot_example"""}]
def sparta_891afc83f8():return sparta_9315ce97d4('line2')
def sparta_29f5438706():return sparta_9315ce97d4('bar2')
def sparta_87f9aadeca():return sparta_9315ce97d4('area2')
def sparta_ae593a6981():return sparta_9315ce97d4('lollipop2')
def sparta_9f2b75e444():B='Example to plot a simple baseline chart with lightweight chart';A=_E;D={_F:_I,_G:_J,_H:_K};C='{\n    "baseline": [\n      \t{\n          "defaultBaselinePrice": 200,\n        },\n    ]\n    }';type='ts_baseline';return[{_A:f"{type.capitalize()}",_B:B,_C:'',_D:f"""{A}
import yfinance as yf
spartaqube_obj = Spartaqube()
# Fetch the data for Apple (ticker symbol: AAPL)
apple_price_df = yf.Ticker(\"AAPL\").history(period=\"1y\")
...
plot_example"""}]
def sparta_6555eb9bb3():A=_E;B={_F:_I,_G:_J,_H:_K};C=_M;type='performance';return[{_A:f"{type.capitalize()}",_B:f"Example to plot a simple {type} chart with lightweight chart",_C:'',_D:f"""{A}
import yfinance as yf
spartaqube_obj = Spartaqube()
# Fetch the data for Apple (ticker symbol: AAPL)
apple_price_df = yf.Ticker(\"AAPL\").history(period=\"1y\")
