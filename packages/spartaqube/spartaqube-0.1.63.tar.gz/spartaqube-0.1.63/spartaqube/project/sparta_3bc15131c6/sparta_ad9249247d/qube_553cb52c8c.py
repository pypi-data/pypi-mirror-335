...
from project.sparta_3bc15131c6.sparta_438eb06364 import qube_879159c153 as qube_879159c153
def sparta_49d6f73134(json_data,userObj):
...
def sparta_3a674a78ba(json_data,user_obj):
	...
	if M==0:
		...
		if N.count()==1:
			F=N[0];G=qube_c9db55e3c2.sparta_e2c6ddbe2b(A);C=[A.userGroup for A in G]
		...
		if O.count()==1:
			K=O[0];G=qube_c9db55e3c2.sparta_e2c6ddbe2b(A);C=[A.userGroup for A in G]
	...
	return{_B:-1}
def sparta_1a46235acd(json_data,user_obj):
	F=json_data;A=user_obj;G=[];K=[];L=int(F[_F])
	if L==0:
		S=F[_G];M=Portfolio.objects.filter(ptf_id=S,is_delete=_A).all()
		if M.count()==1:
			D=M[0];H=qube_c9db55e3c2.sparta_e2c6ddbe2b(A);C=[A.userGroup for A in H]
		...
		if N.count()==1:
			E=N[0];H=qube_c9db55e3c2.sparta_e2c6ddbe2b(A);C=[A.userGroup for A in H]
...
def sparta_5ee2705596(json_data,user_obj):
	Y='Your are not allowed to share this object';X='errorMsg';R='member';K=json_data;B=user_obj;L=datetime.now().astimezone(UTC);P=qube_c9db55e3c2.sparta_e2c6ddbe2b(B);M=[A.userGroup for A in P];U=K['member2ShareArr'];I=int(K[_F])
	...
	if I==0:
		...
		if V.count()==1:
			C=V[0];P=qube_c9db55e3c2.sparta_e2c6ddbe2b(B);M=[A.userGroup for A in P]
		...
		if W.count()==1:
			D=W[0];P=qube_c9db55e3c2.sparta_e2c6ddbe2b(B);M=[A.userGroup for A in P]
	...
	return{_B:1}
def sparta_c7d92eb026(json_data,user_obj):
