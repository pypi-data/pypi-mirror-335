...
def sparta_1f74d23353():return str(uuid.uuid4())
def sparta_a55f8f6723(app_id):
	B=coreApps.get_app_folder_default_path();A=os.path.join(B,app_id)
	if not os.path.exists(A):os.makedirs(A)
	return A
def sparta_5eab48091d(project_path):
	A=project_path
	try:
		if not os.path.exists(A):os.makedirs(A)
		return{_A:1}
...
def sparta_1070f930c9(json_data,userObj):
	A=json_data;logger.debug('CREATE RESOURCE');logger.debug(A);C=A[_B];D=sparta_5eab48091d(C)
	if D[_A]==-1:return D
	...
	try:
		...
		elif not os.path.exists(B):sparta_3cb7189564(B)
	...
	return{_A:1}
def sparta_3a03ad39a1(json_data,userObj):
	...
	return{_A:1}
def sparta_6acd351024(json_data,user_obj,file_obj_to_upload):
	D=file_obj_to_upload;B=json_data;A=B[_B];E=sparta_5eab48091d(A)
	if E[_A]==-1:return E
	...
	I={_A:1};return I
def sparta_065b06176d(json_data,userObj):A=json_data[_B];B=qube_732e1b1f9c.get_folder_and_files_recursively_from_path(A);C={_A:1,'folderStructure':B};return C
def sparta_a89369d351(json_data,userObj):
	A=json_data
	try:
...
def sparta_cab38ee843(json_data,userObj):
	...
	return{_A:1}
def sparta_36fcf5c36e(json_data,userObj):
	...
	return{_A:1}
def sparta_db55e9d081(json_data,userObj):
...
def sparta_f26f00cefc(json_data,userObj):
	...
	return{_A:-1}
def sparta_96a1d5b9ee(json_data,userObj):
