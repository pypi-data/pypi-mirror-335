...
def sparta_52ce380875(json_data,user_obj):
	...
	for A in K:
		B=sparta_367581573a(model_to_dict(A));Q=int(A.typeObject)
		if not A.is_seen:F+=1
		for R in P:B.pop(R,None)
		C=User.objects.get(id=A.user_from.id);G=C.first_name+' '+C.last_name;H=A.date_created.astimezone(UTC);B[N]=G;B[O]=sparta_f841f4baaa(H)
		if Q==0:0
	...
	for A in L:
		if not A.is_seen:F+=1
		B=sparta_367581573a(model_to_dict(A));C=User.objects.get(id=A.user_from.id);G=C.first_name+' '+C.last_name;H=A.dateCreated.astimezone(UTC);B[N]=G;B['type_object']=-2;B[O]=sparta_f841f4baaa(H);E.append(B)
	E=sorted(E,key=lambda obj:obj['dateCreated'],reverse=I);return{'res':1,'resNotifications':E,'nbNotificationNotSeen':F}
def sparta_f841f4baaa(dateCreated):
	...
	return str(A)
def sparta_02ac7054af(json_data,user_obj):
