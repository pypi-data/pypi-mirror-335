...
def sparta_31a034600d(json_data,userObj):
	B=json_data[_C]
	if int(B)==1:
	...
	try:G,H=sparta_e68aaea3df(A,B);D={_A:1,_D:H,_E:G,_B:A}
	except:D={_A:-1,_F:_G}
	return D
def sparta_0ccd45d7a1(json_data,userObj):
	A=json_data
	try:D=A[_B];E=A[_C];B=os.path.dirname(D);F,G=sparta_e68aaea3df(B,E);C={_A:1,_D:G,_E:F,_B:B}
	except:C={_A:-1,_F:_G}
	return C
def sparta_068bdacb50(json_data,userObj):
	A=json_data
	try:B=A[_B];D=A[_C];E,F=sparta_e68aaea3df(B,D);C={_A:1,_D:F,_E:E,_B:B}
	except:C={_A:-1,_F:_G}
	return C
def sparta_e68aaea3df(currentPath,typeexplorer=None):
...
def sparta_ab1fbdd614(currentPath):
