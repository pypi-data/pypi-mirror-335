...
def sparta_7d8d6d46a2(user_obj):
	...
	return B
def sparta_2893f5b9a6(project_path,has_django_models=_A):
	C=has_django_models;B=project_path
	if not os.path.exists(B):os.makedirs(B)
	G=B;D=os.path.join(sparta_ab2d4b6ec5()[_a],_K,_b)
	for E in os.listdir(D):
		...
		if os.path.isdir(A):
			...
			shutil.copy2(A,F)
	return{_L:B}
def sparta_235e09607b(json_data,user_obj):
	F=json_data;B=user_obj;A=F[_H];A=sparta_b0cac1454c(A);G=Notebook.objects.filter(project_path=A).all()
	if G.count()>0:
		C=G[0];H=sparta_7d8d6d46a2(B)
	...
	try:
		...
		L=F['hasDjangoModels'];M=sparta_2893f5b9a6(A,L);A=M[_L];return{_B:1,_L:A}
	except Exception as E:return{_B:-1,_E:f"Failed to create folder: {str(E)}"}
def sparta_afb12076c9(json_data,user_obj):A=json_data;A['bAddGitignore']=_A;A['bAddReadme']=_A;return qube_696f9e8f80.sparta_11cf456865(A,user_obj)
def sparta_2ccfb768be(json_data,user_obj):
	L='%Y-%m-%d';K='Recently used';E=user_obj;G=sparta_7d8d6d46a2(E)
...
def sparta_4612ad5472(json_data,user_obj):
	...
	if not B.is_anonymous:
		...
		if G.count()==1:
			A=G[G.count()-1];E=A.notebook_id;I=sparta_7d8d6d46a2(B)
		...
		J=H.get('modalPassword',_D);logger.debug(f"DEBUG DEVELOPER VIEW TEST >>> {J}");C=sparta_64424211ef(E,B,password_notebook=J);logger.debug('MODAL DEBUG DEBUG DEBUG notebook_access_dict');logger.debug(C)
...
def sparta_d233745a83(json_data,user_obj):
	I=user_obj;A=json_data;logger.debug('Save notebook');logger.debug(A);logger.debug(A.keys());N=A['isNew']
	if not N:return sparta_2fa22dd2b2(A,I)
	C=datetime.now().astimezone(UTC);J=str(uuid.uuid4());G=A[_Y];E=_D
	if G:E=A[_j];E=qube_e3b4c78a63.sparta_5ff3a13eb1(E)
	O=A[_Z];P=A[_I];Q=A[_Q];D=A[_H];D=sparta_b0cac1454c(D);R=A[_k];S=A[_l];G=A[_Y];T=A[_m];U=A.get('notebookVenv',_D);B=A[_P]
	if len(B)==0:B=A[_I]
	K=slugify(B);B=K;L=1
	...
	if F is not _D:
		try:
			F=F.split(',')[1];V=base64.b64decode(F);D=sparta_ab2d4b6ec5()[_o];M=os.path.join(D,_p,_q,_K);os.makedirs(M,exist_ok=_A);H=str(uuid.uuid4());W=os.path.join(M,f"{H}.png")
			with open(W,'wb')as X:X.write(V)
		except:pass
	Y=Notebook.objects.create(notebook_id=J,name=P,slug=B,description=Q,is_expose_notebook=R,is_public_notebook=S,has_password=G,password_e=E,is_exec_code_display=T,lumino_layout=O,project_path=D,notebook_venv=U,thumbnail_path=H,date_created=C,last_update=C,last_date_used=C,spartaqube_version=sparta_c4aba70c92());Z=ShareRights.objects.create(is_admin=_A,has_write_rights=_A,has_reshare_rights=_A,last_update=C);NotebookShared.objects.create(notebook=Y,user=I,share_rights=Z,is_owner=_A,date_created=C);return{_B:1,_M:J}
def sparta_2fa22dd2b2(json_data,user_obj):
	...
	if I.count()==1:
		A=I[I.count()-1];H=A.notebook_id;M=sparta_7d8d6d46a2(G)
		...
		if N:
			...
			if E is not _D:
				...
				try:
					b=sparta_ab2d4b6ec5()[_o];S=os.path.join(b,_p,_q,_K);os.makedirs(S,exist_ok=_A)
...
def sparta_159bd89cf5(json_data,user_obj):
	...
	return{_B:1}
def sparta_79d25ef163(json_data,user_obj):
	...
	return{_B:1}
def sparta_64424211ef(notebook_id,user_obj,password_notebook=_D):
			...
			if A.is_public_notebook:
				...
				if not L:logger.debug('no password');return{_B:1,_J:A}
					...
					if F is _D:logger.debug('empty passord provided');return{_B:2,_E:'Require password',_J:A}
						...
						try:
							if qube_e3b4c78a63.sparta_44317619db(A.password_e)==F:return{_B:1,_J:A}
	...
	return{_B:-1,J:2}
def sparta_bf74319cda(json_data,user_obj):A=sparta_b0cac1454c(json_data[_H]);return sparta_2ee9aaa788(A)
def sparta_a20a655b5f(json_data,user_obj):A=sparta_b0cac1454c(json_data[_H]);return sparta_404ff110f6(A)
def sparta_8aa61709b2(json_data,user_obj):
	...
	if D.count()==1:
		A=D[D.count()-1];F=A.notebook_id;G=sparta_7d8d6d46a2(C)
		...
		if E.count()>0:
			...
			if I.is_admin or I.has_write_rights:H=_A
		if H:K=sparta_b0cac1454c(B['fullPath']);A.main_ipynb_fullpath=K;A.save()
	return{_B:1}
async def notebook_permission_code_exec_DEPREC(json_data):
	...
	try:
		...
		if await D.acount()>0:
			...
			H=qube_55479fa538.sparta_c6b20dd766(A);I=H['cells']
			for B in I:
...
async def notebook_permission_code_exec(json_data):
	...
	return''
def sparta_4122d97ed5(json_data,user_obj):0
from django.core.management import call_command
from io import StringIO
def sparta_aa948b1ae2(project_path,python_executable=_u):
...
def sparta_48e3dc86d4():
	...
	return B
def sparta_582b8e2320(json_data,user_obj):
	D=sparta_b0cac1454c(json_data[_H]);A=D;B=os.path.join(sparta_ab2d4b6ec5()[_a],_K,_b);C=os.path.join(A,_O)
...
def sparta_4afc522e8b(json_data,user_obj):
	A=sparta_b0cac1454c(json_data[_H]);A=os.path.join(A,_O);G,C,D=sparta_aa948b1ae2(A);B=_A;E=1;F=''
	if D:
...
def sparta_e2ab1a3e3a(project_path,python_executable=_u):
...
def sparta_58cc4c1e2b(json_data,user_obj):
	A=sparta_b0cac1454c(json_data[_H]);A=os.path.join(A,_O);B,C=sparta_e2ab1a3e3a(A);D=1;E=''
	if not B:D=-1;E=C
	return{_B:D,'res_migration':B,_y:C,_E:E}
def sparta_2b8288be1d(json_data,user_obj):return{_B:1}
def sparta_40ca38543a(json_data,user_obj):return{_B:1}