...
def sparta_fe44cda07b(webSocket,json_data,user_obj):
...
def sparta_11cf456865(json_data,user_obj):
	...
	return{_A:1,'output':'\n'.join(F)}
def sparta_f512edda50(json_data,user_obj):
	F=user_obj;B=json_data;logger.debug('sqEditorGitAddRemoteOrigin json_data');logger.debug(B);C=B[_C];G=sparta_7f5ba658e2(C)
	if G:
		H=B['remoteUrl'];I=B['remoteName'];A=git.Repo(C);sparta_e6f1c8f5f5(A,F);J=A.create_remote(I,url=H);A=git.Repo(C);sparta_e6f1c8f5f5(A,F)
		for D in A.remotes:D.fetch()
		for D in A.remotes:
			if J==D:
				E=D.refs
				if len(E)>0:K=E[len(E)-1];L=os.path.dirname(os.path.realpath(__file__));os.chdir(C);sparta_17668676dc(K);os.chdir(L)
	return{_A:1}
def sparta_aa612483f5(json_data,user_obj):
	A=json_data;logger.debug('git_load_available_track_remote json_data');logger.debug(A);B=A[_C];F=sparta_7f5ba658e2(B)
	if F:
		G=A[_H];C=git.Repo(B);sparta_e6f1c8f5f5(C,user_obj);D=[]
		for E in C.remotes:
			if G==E.config_reader.get(_J):
				for H in E.refs:D.append({_I:H.name})
	return{_A:1,'available_branches':D}
def sparta_7c0faccd89(json_data,user_obj):
	A=json_data;logger.debug('*******************************************');logger.debug('git_set_track_remote json_data');logger.debug(A);B=A[_C];D=sparta_7f5ba658e2(B)
	if D:G=A[_H];E=A['remoteBranchToTrack'];C=git.Repo(B);sparta_e6f1c8f5f5(C,user_obj);H=C.head.ref.name;F=os.path.dirname(os.path.realpath(__file__));os.chdir(B);sparta_17668676dc(E);os.chdir(F)
	return{_A:1}
@sparta_d7c9d95fb6
def sparta_f6f6e38c6e(json_data,user_obj):return{_A:1}
def sparta_cfbeb5cadf(json_data,user_obj):
	B=json_data[_C];C=sparta_7f5ba658e2(B)
	if C:A=git.Repo(B);sparta_e6f1c8f5f5(A,user_obj);D=A.active_branch;E=D.tracking_branch().remote_name;F=A.remote(name=E);F.pull(allow_unrelated_histories=_B)
	return{_A:1}
def sparta_b068679eb1(json_data,user_obj):
	B=json_data[_C];C=sparta_7f5ba658e2(B)
	if C:A=git.Repo(B);sparta_e6f1c8f5f5(A,user_obj);D=A.active_branch;E=D.tracking_branch().remote_name;F=A.remote(name=f"{E}");F.push()
	return{_A:1}
def sparta_bd98f6ffaf(json_data,user_obj):
	A=json_data[_C];C=sparta_7f5ba658e2(A)
	if C:
		B=git.Repo(A);sparta_e6f1c8f5f5(B,user_obj)
		for D in B.remotes:D.fetch()
	return{_A:1}
def sparta_c075aeb18b(json_data,user_obj):A=json_data[_C];B=sparta_7f5ba658e2(A);return{_A:1,_L:B}
def sparta_dcaf9f1369(json_data,user_obj):
	...
	def B(folder_path):
		j='remotes_arr';i='commits_ahead_arr';h='commits_behind_arr';g='is_ahead';f='is_behind';W='branch';H=folder_path;P=sparta_7f5ba658e2(H);logger.debug(f"is_git_repository > {H}");logger.debug(P)
		if P:
			A=git.Repo(H);F=[];Z=[]
...
def sparta_8f999d737d(json_data,user_obj):
	H='path';G='file';C=json_data[_C];I=sparta_7f5ba658e2(C)
	if I:
		B=git.Repo(C);sparta_e6f1c8f5f5(B,user_obj);D=[]
	...
	return{_A:1}
def sparta_39e202cf22(json_data,user_obj):
	B=json_data;C=B[_C];D=B['gitMsg'];E=sparta_7f5ba658e2(C)
	if E:A=git.Repo(C);sparta_e6f1c8f5f5(A,user_obj);A.git.add(all=_B);A.git.commit('-m',D)
	return{_A:1}
@sparta_d7c9d95fb6
def sparta_6c0bb9e776(json_data,user_obj):return{_A:1}
def sparta_ce557fb991(json_data,user_obj):
	A=json_data;logger.debug('Delete Remoete');logger.debug(A);C=A[_C];E=sparta_7f5ba658e2(C)
	if E:
		B=git.Repo(C);sparta_e6f1c8f5f5(B,user_obj);F=A[_H]
		for D in B.remotes:
			if F==D.config_reader.get(_J):B.delete_remote(D);break
	return{_A:1}
def sparta_d358e4b6a6(json_data,user_obj):return{_A:1}
def sparta_7db24b3464(json_data,user_obj):
	D=json_data;E=D[_C];C=D['newBranchName'];F=sparta_7f5ba658e2(E)
	if F:
		A=git.Repo(E);sparta_e6f1c8f5f5(A,user_obj);B=[A for A in A.branches if A.name==C]
	...
	return{_A:-1,_D:_G}
def sparta_0a75858db0(json_data,user_obj):
	A=json_data;B=A[_C];D=A['branch2Checkout'];E=sparta_7f5ba658e2(B)
	if E:
		C=git.Repo(B);sparta_e6f1c8f5f5(C,user_obj)
		try:C.git.checkout(D);return{_A:1}
		except Exception as F:return{_A:-1,_D:str(F)}
	return{_A:-1,_D:_G}
def sparta_99556b0d6e(json_data,user_obj):
	B=json_data;C=B[_C];D=B['branch2Merge'];F=sparta_7f5ba658e2(C)
	if F:
		A=git.Repo(C);sparta_e6f1c8f5f5(A,user_obj);E=A.head.ref.name
	...
	return{_A:-1,_D:_G}
def sparta_e595df1639(json_data,user_obj):
	D=json_data;A=D[_C];B=D['branch2Delete'];F=sparta_7f5ba658e2(A)
	if F:
		E=git.Repo(A);sparta_e6f1c8f5f5(E,user_obj);G=E.head.ref.name
	...
	return{_A:-1,_D:_G}
def sparta_615d06b721(json_data,user_obj):
	F='diff_output';A=json_data;logger.debug('sqEditorGitLoadFilesDiff');logger.debug(A);B=A[_C];G=A['filePath'];H=A['fileType'];C=sparta_7f5ba658e2(B);logger.debug(_L);logger.debug(C)
	if C:D=git.Repo(B);sparta_e6f1c8f5f5(D,user_obj);E=D.git.diff();logger.debug(F);logger.debug(E);return{_A:1,F:E}
	return{_A:-1,_D:_G}