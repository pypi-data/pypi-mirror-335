...
def sparta_7d8d6d46a2(user_obj):
	...
	return B
def sparta_30dc60a806():B=sparta_235d77fd71();A=os.path.join(B,'sq_venv');os.makedirs(A,exist_ok=_C);return A
def sparta_67a8966c74(env_name):
	A=env_name;B=sparta_30dc60a806()
	if sys.platform=='win32':C=os.path.join(B,A,'Scripts','pip.exe')
	else:C=os.path.join(B,A,'bin','pip')
	return C
def sparta_e1053a671f(json_data,user_obj):A=sparta_30dc60a806();B=[B for B in os.listdir(A)if os.path.isdir(os.path.join(A,B))];return{_A:1,'available_venvs':B}
def sparta_d1d37335d8(json_data,user_obj):
	B=sparta_30dc60a806();A=json_data[_D];C=os.path.join(B,A)
	try:
		subprocess.run([sys.executable,'-m','venv',C],check=_C);D=['cloudpickle']
		for E in D:
			F=sparta_67a8966c74(A);G=f"pip install {E}";H=G.replace('pip',F);I=subprocess.Popen(H,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=_C)
...
def sparta_7326f77f9f(json_data,user_obj):
	...
	L=qube_9d39f4b22f.sparta_1c67ca485f(C,B);return{_A:1}
def sparta_ac2fa6c638(json_data,user_obj):
	B=user_obj;E=json_data[_H];C=Dashboard.objects.filter(dashboard_id__startswith=E,is_delete=_B).all()
	if C.count()==1:
		A=C[C.count()-1];E=A.dashboard_id;F=sparta_7d8d6d46a2(B)
	...
	return{_A:1}
def sparta_c134ee8ea6(json_data,user_obj):
	C=user_obj;A=json_data;D=A[_H];E=A[_J]
	if str(D)!=_F:sparta_ac2fa6c638(A,C)
	if str(E)!=_F:sparta_e46db117be(A,C)
	F=sparta_30dc60a806();G=A[_D];H=os.path.join(F,G)
...
def sparta_2faee59bed(json_data,user_obj):
	B=json_data[_D];C=sparta_67a8966c74(B);A=[]
	try:
...
def sparta_3d46570cf3(env_name,project_path):
	A=os.path.join(project_path,'requirements.txt');B=sparta_67a8966c74(env_name)
	try:
		with open(A,'w')as C:subprocess.run([B,'freeze'],stdout=C)
		return{_A:1}
	except Exception as D:return{_A:-1,_E:str(D)}
def sparta_2059baaa39(json_data,user_obj):
	...
	if D.count()==1:
		A=D[D.count()-1];F=A.developer_id;G=sparta_7d8d6d46a2(C)
	...
	L=qube_9d39f4b22f.sparta_1c67ca485f(B,C);return{_A:1}
def sparta_e46db117be(json_data,user_obj):
	B=user_obj;E=json_data[_J];C=Developer.objects.filter(developer_id__startswith=E,is_delete=_B).all()
	if C.count()==1:
		A=C[C.count()-1];E=A.developer_id;F=sparta_7d8d6d46a2(B)
	...
	return{_A:1}
def sparta_4fdb5a74fe(json_data,user_obj):
...
def sparta_140d6def9c(json_data,user_obj):
	...
	return{_A:1}
def sparta_65e01b91fd(json_data,user_obj):
	C=json_data;B=user_obj;from project.sparta_3bc15131c6.sparta_e0bb3dc4e4 import qube_9d39f4b22f as J;F=C[_L];D=Kernel.objects.filter(kernel_manager_uuid__startswith=F,is_delete=_B).all()
	if D.count()==1:
		A=D[D.count()-1];F=A.kernel_manager_uuid;G=sparta_7d8d6d46a2(B)
	...
	M=J.sparta_1c67ca485f(C,B);return{_A:1}
def sparta_cc55b55457(json_data,user_obj):
	B=user_obj;E=json_data[_L];C=Kernel.objects.filter(kernel_manager_uuid__startswith=E,is_delete=_B).all()
	if C.count()==1:
		A=C[C.count()-1];E=A.kernel_manager_uuid;F=sparta_7d8d6d46a2(B)
	...
	return{_A:1}
def sparta_bae1dad41a(json_data,user_obj):
	B=user_obj;A=json_data;logger.debug(_O);logger.debug(A);D=A[_H];E=A[_J];F=A[_K];G=A[_L]
	if str(D)!=_F:return sparta_2ca2877858(A,B)
	if str(E)!=_F:return sparta_f8c69db518(A,B)
	if str(F)!=_F:return sparta_475b2378db(A,B)
	if str(G)!=_F:return sparta_5cec827839(A,B)
	H=A[_D];C=A[_G]
	if A[_P]:C=os.path.join(C,_M)
	return sparta_3d46570cf3(H,C)
def sparta_2ca2877858(json_data,user_obj):
	...
	if D.count()==1:
		...
		if E.count()>0:
			...
			if I.is_admin or I.has_write_rights:H=_C
		if H:return sparta_3d46570cf3(J,K)
	return{_A:1}
def sparta_f8c69db518(json_data,user_obj):
	...
	if E.count()==1:
		A=E[E.count()-1];G=A.developer_id;H=sparta_7d8d6d46a2(C)
		...
		if I:return sparta_3d46570cf3(K,D)
	return{_A:1}
def sparta_475b2378db(json_data,user_obj):
	...
	if D.count()==1:
		...
		if H:return sparta_3d46570cf3(J,K)
	return{_A:1}
def sparta_5cec827839(json_data,user_obj):
	A=json_data;from project.sparta_3bc15131c6.sparta_e0bb3dc4e4 import qube_9d39f4b22f as B;C=A[_L];D=A[_D];E=A[_G];F=B.sparta_88e68c3ab9(user_obj,C)
	if F is _I:return sparta_3d46570cf3(D,E)
	return{_A:1}
def sparta_33dfb5849e(json_data,user_obj):
	B=json_data;logger.debug(_O);logger.debug(B);E=B[_D];C=os.path.join(sparta_30dc60a806(),E);logger.debug('venv_path');logger.debug(C);A=sparta_b0cac1454c(B[_G])
