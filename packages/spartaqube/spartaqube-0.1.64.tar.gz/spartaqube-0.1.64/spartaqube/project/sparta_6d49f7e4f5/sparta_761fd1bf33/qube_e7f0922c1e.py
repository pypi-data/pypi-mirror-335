...
def sparta_aeacc0e972(json_data,user_obj):
	...
	for A in K:
		B=sparta_202fbce0ad(model_to_dict(A));Q=int(A.typeObject)
		if not A.is_seen:F+=1
		for R in P:B.pop(R,None)
		C=User.objects.get(id=A.user_from.id);G=C.first_name+' '+C.last_name;H=A.date_created.astimezone(UTC);B[N]=G;B[O]=sparta_34040cf9de(H)
		if Q==0:0
	...
	for A in L:
		if not A.is_seen:F+=1
		B=sparta_202fbce0ad(model_to_dict(A));C=User.objects.get(id=A.user_from.id);G=C.first_name+' '+C.last_name;H=A.dateCreated.astimezone(UTC);B[N]=G;B['type_object']=-2;B[O]=sparta_34040cf9de(H);E.append(B)
	E=sorted(E,key=lambda obj:obj['dateCreated'],reverse=I);return{'res':1,'resNotifications':E,'nbNotificationNotSeen':F}
def sparta_34040cf9de(dateCreated):
	...
	return str(A)
def sparta_05f45497f3(json_data,user_obj):
