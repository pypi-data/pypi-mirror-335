...
def sparta_cfece85c36(path):A=Path(path).resolve();return str(A).replace('\\','\\\\')
def sparta_28f823a3b9():
	B='virtual_env_name';C=hasattr(sys,'real_prefix')or hasattr(sys,'base_prefix')and sys.base_prefix!=sys.prefix
	if C:A=sys.prefix;D=os.path.basename(A);return{_A:True,B:D,_B:sparta_cfece85c36(A)}
	else:return{_A:False,B:None,_B:None}
def sparta_4a060ddb2f(bat_file_path,shortcut_path,icon_path):
...
def sparta_44cd16bf45(shell_script_path,shortcut_name,icon_path):
...
def sparta_e468b7bc0d(shortcut_name,shell_script_path,icon_path):
...
def sparta_2902446992(json_data,user_obj):
	X='SpartaQube Launcher';W='SpartaQube.sh';V='SpartaQube.lnk';U='favicon512.ico';T='Icon';S='images';R='assets';L='errorMsg';F='res';M=qube_b950b30269.sparta_f6a5acfc61(json_data,user_obj)
	if M[F]==1:
		Y=M['token'];B=sparta_3acf216d77()['api'];A=sparta_cfece85c36(os.path.join(B,'spartaqube_exec.py'))
		if os.path.exists(A):
			try:os.remove(A)
...
"""
		...
		G=sparta_cfece85c36(os.path.join(B,'spartaqube_launcher.py'))
		if os.path.exists(G):
			try:os.remove(G)
			except Exception as D:return{F:-1,L:str(D)}
		J=sparta_28f823a3b9();logger.debug('env_info');logger.debug(J)
		if J[_A]:E=J[_B];a=sparta_cfece85c36(os.path.join(f"{E}",'bin','python')if os.name!='nt'else os.path.join(f"{E}",'Scripts','python.exe'));N=sparta_cfece85c36(os.path.join(f"{E}",'bin/activate')if os.name!='nt'else os.path.join(E,'Scripts/activate.bat'));O=f'''
import os
import sys
import subprocess
import platform
...
'''
		...
		P=sparta_3acf216d77()['spartaqube_path']
		if conf_settings.IS_DEV:H=sparta_cfece85c36(os.path.join(P,'static',R,S,T,U))
		else:H=sparta_cfece85c36(os.path.join(P,'staticfiles',R,S,T,U))
		Q=platform.system()
		if Q==_C:
			C=sparta_cfece85c36(os.path.join(B,'SpartaQube.bat'));K=sparta_cfece85c36(os.path.join(B,V))
			if os.path.exists(K):
				try:os.remove(K)
				except Exception as D:return{F:-1,L:str(D)}
			sparta_4a060ddb2f(C,K,H);b=os.path.join(os.path.expanduser('~'),'Desktop');c=os.path.join(b,V);sparta_4a060ddb2f(C,c,H)
		elif Q=='Darwin':C=sparta_cfece85c36(os.path.join(B,W));sparta_44cd16bf45(shell_script_path=C,shortcut_name=X,icon_path=H)
		else:C=sparta_cfece85c36(os.path.join(B,W));sparta_e468b7bc0d(shell_script_path=C,shortcut_name=X,icon_path='/path/to/your/icon.png')
	return{F:1}