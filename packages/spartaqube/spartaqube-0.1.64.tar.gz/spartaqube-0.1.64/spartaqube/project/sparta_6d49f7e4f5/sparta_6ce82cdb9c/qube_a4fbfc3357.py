...
from project.sparta_d3863a192d.sparta_3c800a8540.qube_a8d0eb6c62 import sparta_c8b38dec53,sparta_39ab508c7e,sparta_b01aa358eb
from project.sparta_6d49f7e4f5.sparta_03569db088.qube_9fc342787d import sparta_94162d2399
from project.logger_config import logger
def sparta_55c27bc53f(json_data,user_obj):
...
def sparta_97ad2ca98f(message,typeCase=0,companyName=_G):
	...
	if C.count()>0:
		...
		A.send()
def sparta_b054b9bbd7(json_data,userObj):
	...
	return B
def sparta_64ccc202bc(json_data,userObj):
	...
	return A
def sparta_c15e131c96(json_data,userObj):
	D=json_data;F=D['old_spartaqube_code'];G=D['new_spartaqube_code']
	if not sparta_94162d2399(F):return{_A:-1,_B:'Invalid current code'}
...
def sparta_d835bc29fe():B='spartaqube-reset-password';A=B.encode(_D);A=hashlib.md5(A).hexdigest();A=base64.b64encode(A.encode(_D));return A.decode(_D)
def sparta_c031fa549f(json_data):
	A=json_data;C=A[_F];E=A[_H];B=A[_I];F=A[_R]
	if not sparta_b01aa358eb(E):return{_A:-1,_B:_J}
...
def sparta_185ff49548(json_data):
	A=json_data;E=A[_E];B=A[_F];F=A[_H];G=sparta_c8b38dec53(E)
	if G[_A]!=1:return{_A:-1,_B:_T}
	if not sparta_39ab508c7e(F):return{_A:-1,_B:_J}
...
def sparta_917a122841(request,json_data):
	A=json_data;F=A[_E];D=A[_F];G=A[_H];H=A[_U];E=A[_I];I=A[_R];J=sparta_c8b38dec53(F)
	if J[_A]!=1:return{_A:-1,_B:_T}
	if not sparta_39ab508c7e(G):return{_A:-1,_B:_J}
