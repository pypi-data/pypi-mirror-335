...
from project.sparta_6d49f7e4f5.sparta_0c58483439 import qube_abc0b74f03 as qube_abc0b74f03
def sparta_527c17774a(json_data,userObj):
...
def sparta_ac94d132fd(json_data,user_obj):
	...
	if M==0:
		...
		if N.count()==1:
			F=N[0];G=qube_6ff33ab51c.sparta_03fbfe1583(A);C=[A.userGroup for A in G]
		...
		if O.count()==1:
			K=O[0];G=qube_6ff33ab51c.sparta_03fbfe1583(A);C=[A.userGroup for A in G]
	...
	return{_B:-1}
def sparta_27fbf4f458(json_data,user_obj):
	F=json_data;A=user_obj;G=[];K=[];L=int(F[_F])
	if L==0:
		S=F[_G];M=Portfolio.objects.filter(ptf_id=S,is_delete=_A).all()
		if M.count()==1:
			D=M[0];H=qube_6ff33ab51c.sparta_03fbfe1583(A);C=[A.userGroup for A in H]
		...
		if N.count()==1:
			E=N[0];H=qube_6ff33ab51c.sparta_03fbfe1583(A);C=[A.userGroup for A in H]
...
def sparta_4b76434aad(json_data,user_obj):
	Y='Your are not allowed to share this object';X='errorMsg';R='member';K=json_data;B=user_obj;L=datetime.now().astimezone(UTC);P=qube_6ff33ab51c.sparta_03fbfe1583(B);M=[A.userGroup for A in P];U=K['member2ShareArr'];I=int(K[_F])
	...
	if I==0:
		...
		if V.count()==1:
			C=V[0];P=qube_6ff33ab51c.sparta_03fbfe1583(B);M=[A.userGroup for A in P]
		...
		if W.count()==1:
			D=W[0];P=qube_6ff33ab51c.sparta_03fbfe1583(B);M=[A.userGroup for A in P]
	...
	return{_B:1}
def sparta_23b10d240b(json_data,user_obj):
