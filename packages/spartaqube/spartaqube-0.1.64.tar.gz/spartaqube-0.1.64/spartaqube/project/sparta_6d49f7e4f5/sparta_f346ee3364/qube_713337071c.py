...
def sparta_366b4f8cf1(user_obj):
	...
	return B
def sparta_fa741cca94(project_path,has_django_models=_A):
	C=has_django_models;B=project_path
	if not os.path.exists(B):os.makedirs(B)
	G=B;D=os.path.join(sparta_3acf216d77()[_a],_K,_b)
	for E in os.listdir(D):
		...
		if os.path.isdir(A):
			...
			shutil.copy2(A,F)
	return{_L:B}
def sparta_c8083a70eb(json_data,user_obj):
	F=json_data;B=user_obj;A=F[_H];A=sparta_cfece85c36(A);G=Notebook.objects.filter(project_path=A).all()
	if G.count()>0:
		C=G[0];H=sparta_366b4f8cf1(B)
	...
	try:
		...
		L=F['hasDjangoModels'];M=sparta_fa741cca94(A,L);A=M[_L];return{_B:1,_L:A}
	except Exception as E:return{_B:-1,_E:f"Failed to create folder: {str(E)}"}
def sparta_24b98d40ef(json_data,user_obj):A=json_data;A['bAddGitignore']=_A;A['bAddReadme']=_A;return qube_b9d3b4b6c9.sparta_73e2cbc7f4(A,user_obj)
def sparta_1552c135bd(json_data,user_obj):
	L='%Y-%m-%d';K='Recently used';E=user_obj;G=sparta_366b4f8cf1(E)
...
def sparta_e4358fa4d8(json_data,user_obj):
	...
	if not B.is_anonymous:
		...
		if G.count()==1:
			A=G[G.count()-1];E=A.notebook_id;I=sparta_366b4f8cf1(B)
		...
		J=H.get('modalPassword',_D);logger.debug(f"DEBUG DEVELOPER VIEW TEST >>> {J}");C=sparta_93e3fdd3de(E,B,password_notebook=J);logger.debug('MODAL DEBUG DEBUG DEBUG notebook_access_dict');logger.debug(C)
...
def sparta_43d709cf79(json_data,user_obj):
	I=user_obj;A=json_data;logger.debug('Save notebook');logger.debug(A);logger.debug(A.keys());N=A['isNew']
	if not N:return sparta_5c314b8d4c(A,I)
	C=datetime.now().astimezone(UTC);J=str(uuid.uuid4());G=A[_Y];E=_D
	if G:E=A[_j];E=qube_a2a748f1f2.sparta_50840022e6(E)
	O=A[_Z];P=A[_I];Q=A[_Q];D=A[_H];D=sparta_cfece85c36(D);R=A[_k];S=A[_l];G=A[_Y];T=A[_m];U=A.get('notebookVenv',_D);B=A[_P]
	if len(B)==0:B=A[_I]
	K=slugify(B);B=K;L=1
	...
	if F is not _D:
		try:
			F=F.split(',')[1];V=base64.b64decode(F);D=sparta_3acf216d77()[_o];M=os.path.join(D,_p,_q,_K);os.makedirs(M,exist_ok=_A);H=str(uuid.uuid4());W=os.path.join(M,f"{H}.png")
			with open(W,'wb')as X:X.write(V)
		except:pass
	Y=Notebook.objects.create(notebook_id=J,name=P,slug=B,description=Q,is_expose_notebook=R,is_public_notebook=S,has_password=G,password_e=E,is_exec_code_display=T,lumino_layout=O,project_path=D,notebook_venv=U,thumbnail_path=H,date_created=C,last_update=C,last_date_used=C,spartaqube_version=sparta_00b65c604c());Z=ShareRights.objects.create(is_admin=_A,has_write_rights=_A,has_reshare_rights=_A,last_update=C);NotebookShared.objects.create(notebook=Y,user=I,share_rights=Z,is_owner=_A,date_created=C);return{_B:1,_M:J}
def sparta_5c314b8d4c(json_data,user_obj):
	...
	if I.count()==1:
		A=I[I.count()-1];H=A.notebook_id;M=sparta_366b4f8cf1(G)
		...
		if N:
			...
			if E is not _D:
				...
				try:
					b=sparta_3acf216d77()[_o];S=os.path.join(b,_p,_q,_K);os.makedirs(S,exist_ok=_A)
...
def sparta_7a95bf11a2(json_data,user_obj):
	...
	return{_B:1}
def sparta_3960eeebee(json_data,user_obj):
	...
	return{_B:1}
def sparta_93e3fdd3de(notebook_id,user_obj,password_notebook=_D):
			...
			if A.is_public_notebook:
				...
				if not L:logger.debug('no password');return{_B:1,_J:A}
					...
					if F is _D:logger.debug('empty passord provided');return{_B:2,_E:'Require password',_J:A}
						...
						try:
							if qube_a2a748f1f2.sparta_bb4d8efa9a(A.password_e)==F:return{_B:1,_J:A}
	...
	return{_B:-1,J:2}
def sparta_1ddd686cb6(json_data,user_obj):A=sparta_cfece85c36(json_data[_H]);return sparta_05b64f9262(A)
def sparta_a6f09fdbd1(json_data,user_obj):A=sparta_cfece85c36(json_data[_H]);return sparta_09daf7b195(A)
def sparta_0fa8524839(json_data,user_obj):
	...
	if D.count()==1:
		...
		if E.count()>0:
			...
			if I.is_admin or I.has_write_rights:H=_A
		if H:K=sparta_cfece85c36(B['fullPath']);A.main_ipynb_fullpath=K;A.save()
	return{_B:1}
async def notebook_permission_code_exec_DEPREC(json_data):
	...
	try:
		...
		if await D.acount()>0:
			...
			H=qube_f2c5d89efa.sparta_deafe1d3a3(A);I=H['cells']
			for B in I:
...
async def notebook_permission_code_exec(json_data):
	...
	return''
def sparta_4419fb3ee3(json_data,user_obj):0
from django.core.management import call_command
from io import StringIO
def sparta_5c2e27c007(project_path,python_executable=_u):
...
def sparta_dee882c21e():
	...
	return B
def sparta_711256a734(json_data,user_obj):
	D=sparta_cfece85c36(json_data[_H]);A=D;B=os.path.join(sparta_3acf216d77()[_a],_K,_b);C=os.path.join(A,_O)
...
def sparta_b2de379298(json_data,user_obj):
	A=sparta_cfece85c36(json_data[_H]);A=os.path.join(A,_O);G,C,D=sparta_5c2e27c007(A);B=_A;E=1;F=''
	if D:
...
def sparta_e6a91d8abb(project_path,python_executable=_u):
...
def sparta_aba344bdc2(json_data,user_obj):
	A=sparta_cfece85c36(json_data[_H]);A=os.path.join(A,_O);B,C=sparta_e6a91d8abb(A);D=1;E=''
	if not B:D=-1;E=C
	return{_B:D,'res_migration':B,_y:C,_E:E}
def sparta_627f42295f(json_data,user_obj):return{_B:1}
def sparta_53f69dadce(json_data,user_obj):return{_B:1}