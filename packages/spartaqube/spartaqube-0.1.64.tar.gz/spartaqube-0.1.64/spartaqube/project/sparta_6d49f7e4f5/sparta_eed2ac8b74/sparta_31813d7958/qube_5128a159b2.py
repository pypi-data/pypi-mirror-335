...
def sparta_414d4944c8():A=_F;type='notebook';return[{_A:f"{type.capitalize()}",_B:'Example to display a python Notebook',_C:'',_D:f"""{A}
...
plot_example"""}]
def sparta_55f1bacc55():A=_F;B="{'reportType': 1}";C="{'reportType': 2}";D="{'reportType': 3}";type='quantstats';return[{_A:f"{type.capitalize()} basic report",_B:_E,_C:'',_D:f"""{A}
...
plot_example"""}]
def sparta_d10c886299():A=_F;type='dataframe';return[{_A:f"{type.capitalize()}",_B:_E,_C:'',_D:f"""{A}
import yfinance as yf
spartaqube_obj = Spartaqube()
# Fetch the data for Apple (ticker symbol: AAPL)
apple_price_df = yf.Ticker(\"AAPL\").history(period=\"1y\")
