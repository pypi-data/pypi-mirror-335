...
def sparta_dccf491b11(json_data,user_obj,group_id=None):
	...
	F=UserGroup.objects.create(name=E,group_id=C,user_creator=B,date_created=A,last_update=A);UserGroupUser.objects.create(user_group=F,user=B,b_admin=_B,date_created=A,last_update=A);sparta_5504ff1c84(G,F,A,B);H={_A:1,_C:C};return H
def sparta_5504ff1c84(membersArr,user_group_obj,dateCreation,userMe):
...
def sparta_ae595e060a(user_obj):A=user_obj;B=UserGroupUser.objects.filter(is_delete=0,user=A,b_admin=_B,user_group__is_delete=0).all();C=UserGroupUser.objects.filter(is_delete=0,user=A,b_admin=_D,user_group__is_delete=0).all();D={_E:B,_F:C};return D
def sparta_03fbfe1583(user_obj):return UserGroupUser.objects.filter(is_delete=0,user=user_obj,user_group__is_delete=0)
def sparta_ba514d2e6d(json_data,user_obj):A=sparta_ae595e060a(user_obj);B=[{_C:A.user_group.groupId,_G:A.user_group.name}for A in A[_E]];C=[{_C:A.user_group.groupId,_G:A.user_group.name}for A in A[_F]];return{_A:1,_E:B,_F:C}
def sparta_5eee488bed(json_data,user_obj):
	...
	B={_A:-1};return B
def sparta_153c189d0d(json_data,user_obj):
	...
	if D.count()>0:
		...
		if H.count()>0:A.name=F;A.save();I=datetime.now();sparta_5504ff1c84(G,A,I,C)
	J={_A:1};return J
def sparta_5b41ad8dbb(json_data,user_obj):
	...
	I={_A:1};return I
def sparta_5bfc398d34(json_data,user_obj):
	...
	D={_A:1};return D
def sparta_1dcdff0503(json_data,user_obj):
	...
	H={_A:1};return H
def sparta_9706a0c49b(json_data,user_obj):
