...
def sparta_12d4428936(json_data,user_obj):
	B=user_obj;A=json_data[_E];A=sparta_cfece85c36(A);F=Dashboard.objects.filter(project_path=A).all()
	if F.count()>0:
	...
	try:
		if not os.path.exists(A):os.makedirs(A)
		J=sparta_1b5f5434d0(A);L=J[_N];M=J[_O];return{_C:1,'main_ipynb_filename':L,'main_ipynb_fullpath':M}
	except Exception as E:return{_C:-1,_D:f"Failed to create folder: {str(E)}"}
def sparta_98a51f28e5(json_data,user_obj):
...
def sparta_8b0cbfbac5(user_obj,json_data):
...
def check_perform_project_explorer_action_dashboard(user_obj,dashboard_id,project_path):
	...
	if len(C)>0:
		...
		if D.count()==1:
			A=D[D.count()-1];C=A.dashboard_id;F=sparta_366b4f8cf1(B)
...
def check_perform_project_explorer_action_developer(user_obj,developer_id,project_path):
	...
	if len(C)>0:
		...
		if D.count()==1:
			A=D[D.count()-1];C=A.developer_id;F=sparta_366b4f8cf1(B)
...
def check_perform_project_explorer_action_notebook(user_obj,notebook_id,project_path):
	...
	if len(C)>0:
		...
		if D.count()==1:
			A=D[D.count()-1];C=A.notebook_id;F=sparta_366b4f8cf1(B)
	...
	return{_C:1}
def sparta_e3f4b725ee(main_path,test_path):B=test_path;A=main_path;A=Path(A).resolve();B=Path(B).resolve();return A in B.parents or A==B
def sparta_a65575fdf4(json_data,user_obj):
	A=json_data;logger.debug('Create resources');logger.debug(A);G=sparta_8b0cbfbac5(user_obj,A)
	if G[_C]==-1:return G
	C=A[_E];C=sparta_cfece85c36(C);F=A['createResourceName'];I=A['createType']
	try:
		B=os.path.join(C,F)
		if int(I)==1:
			D=A[_H];E=sparta_cfece85c36(D)
			if len(D)>0:
				if not sparta_e3f4b725ee(C,E):return{_C:-1,_D:_L}
				B=os.path.join(E,F)
			...
			if len(D)>0:
				if not sparta_e3f4b725ee(C,E):return{_C:-1,_D:_L}
				B=os.path.join(E,F)
			if not os.path.exists(B):sparta_660353d55e(B)
	...
	return{_C:1}
def sparta_89df7af16a(json_data,user_obj):
	C=json_data;logger.debug('Rename resources');logger.debug(C);H=sparta_8b0cbfbac5(user_obj,C)
	if H[_C]==-1:return H
	D=C[_H];I=C['editName'];E=C[_E];E=sparta_cfece85c36(E);J=int(C['renameType'])
	if J==1:
		B=D;F=os.path.dirname(D);A=os.path.join(F,I);B=sparta_cfece85c36(B);A=sparta_cfece85c36(A)
		if E in A:
			try:os.rename(B,A)
	...
	return{_C:1}
def sparta_38737834ec(json_data,user_obj):
	A=json_data;logger.debug('*'*100);logger.debug('MOVE drag & drop resources');logger.debug(A);G=sparta_8b0cbfbac5(user_obj,A)
	if G[_C]==-1:return G
	D=A[_E];D=sparta_cfece85c36(D);B=A['folderLocation'];B=sparta_cfece85c36(B);logger.debug('folder_location >>>>> ');logger.debug(B);J=A[_P];K=A[_Q]
	for H in J:
		L=H[_I];I=H[_M];E=os.path.join(L,I);C=os.path.join(B,I);E=sparta_cfece85c36(E);C=sparta_cfece85c36(C)
		if D in C:
			try:logger.debug(f"Move from\n{E}\nto\n{C}");shutil.move(E,C)
			except Exception as M:logger.debug('Exception move 1');logger.debug(M)
	for N in K:
		F=N[_I];F=sparta_cfece85c36(F)
		if D in B:
			try:shutil.move(F,B)
			except:pass
	return{_C:1}
def sparta_0aac3e6ca2(json_data,user_obj):
	A=json_data;logger.debug('Delete resource');logger.debug(A);E=sparta_8b0cbfbac5(user_obj,A)
	if E[_C]==-1:return E
	C=A[_E];C=sparta_cfece85c36(C);F=int(A['typeDelete']);G=A[_H];B=sparta_cfece85c36(G)
	if not sparta_e3f4b725ee(C,B):return{_C:-1,_D:_L}
	if F==1:
		try:os.rmdir(B)
		except:
	...
	return{_C:1}
def sparta_25fa50abdb(json_data,user_obj):
	B=json_data;logger.debug('Delete multiple resources');logger.debug(B);G=sparta_8b0cbfbac5(user_obj,B)
	if G[_C]==-1:return G
	I=B[_P];J=B[_Q];C=B[_E];C=sparta_cfece85c36(C)
	for D in I:
		K=D[_M];H=D[_I];E=os.path.join(H,K);E=sparta_cfece85c36(E)
		if C in H:
			try:logger.debug(f"File to delete: {E}");os.remove(E)
			except Exception as F:return{_C:-1,_D:str(F)}
	for D in J:
		A=D[_I];A=sparta_cfece85c36(A)
		if C in A:
	...
	return{_C:1}
def sparta_05eb96beda(json_data,user_obj):
	A=json_data;C=sparta_8b0cbfbac5(user_obj,A)
	if C[_C]==-1:return C
	B=A[_E];E=A[_H];D=sparta_cfece85c36(E);B=sparta_cfece85c36(B)
	if B in D:return{_C:1,_J:D}
	return{_C:-1}
def sparta_bf1a5c5a19(json_data,user_obj):
	...
	return{_C:-1}
def sparta_da93193950(json_data,user_obj):
	A=json_data;B=A[_E];C=sparta_8b0cbfbac5(user_obj,A)
	if C[_C]==-1:return C
	F='app'
	def D(zf,folder):
		C=folder
	...
	return{_C:-1}
def sparta_9a807e9753(json_data,user_obj,file_obj):
	F=file_obj;C=json_data;logger.debug('**********************************');logger.debug('upload_resource');logger.debug(C);E=C[_E];G=sparta_8b0cbfbac5(user_obj,C)
	if G[_C]==-1:return G
	B=C[_H];E=sparta_cfece85c36(E)
	if len(B)==0:B=E
	else:B=sparta_cfece85c36(B)
	D=C[_I];logger.debug(f"path_folder >> {D}")
	if len(D)>0:
		A=os.path.join(B,D);A=sparta_cfece85c36(A)
		if not os.path.exists(A):os.makedirs(A)
	else:
		A=os.path.join(B,D);A=sparta_cfece85c36(A)
		if not os.path.exists(A):os.makedirs(A)
	...
	J={_C:1};return J
def sparta_8a52275282(fileName):
	A=['pdf','png','jpg','jpeg'];B=fileName.split('.')[-1].lower()
	if B in A:return _G
	return _F
def sparta_141d294314(full_path):
	...
	return B
def sparta_897dc54071(json_data,user_obj):
	A=json_data;E=A[_E];B=sparta_8b0cbfbac5(user_obj,A)
	if B[_C]==-1:return B
	C=sparta_cfece85c36(A[_J]);D=qube_f2c5d89efa.sparta_deafe1d3a3(C);return{_C:1,'ipynb_dict':json.dumps(D)}
def sparta_c994301ce7(json_data,user_obj):
	R='cells';M='ipynb';K='file_extension';J='file_content';I='cm_mode';H='is_previewable';G='is_handled';D=json_data;S=D[_M];B=D[_J];B=sparta_cfece85c36(B);N=D[_E];N=sparta_cfece85c36(N);O=sparta_8b0cbfbac5(user_obj,D)
	if O[_C]==-1:return O
	E=_F;P=_F;F='';A='';C=S.split('.')[-1].lower();T=sparta_0c301e466e()
	for Q in T:
		try:
	...
	if E:
		...
		if sparta_8a52275282(B):C=B.split('.')[-1];V=sparta_141d294314(B);return{_C:1,G:_G,H:E,I:F,J:A,K:C,'resource':V}
		C=B.split('.')[-1]
		if C==M:
