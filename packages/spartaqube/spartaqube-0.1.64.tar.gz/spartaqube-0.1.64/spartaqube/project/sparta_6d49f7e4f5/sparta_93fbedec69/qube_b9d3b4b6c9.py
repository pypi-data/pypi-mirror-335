...
def sparta_7a90e5aa73(path):
	try:A=git.Repo(path).git_dir;return _B
	except git.exc.InvalidGitRepositoryError:return _E
def sparta_2ce5093461(json_data,user_obj):
	A=json_data['notebookProjectId'];B,C=qube_b51aa9d54d.get_notebookProjectObj(A,user_obj)
	if B is not _F:return _B
	return _E
def sparta_c79580ed26(remoteBranchToTrack):A=Popen(f"git branch -u {remoteBranchToTrack}",stdout=PIPE,stderr=subprocess.STDOUT,bufsize=1,universal_newlines=_B,shell=_B);B=A.stdout.readline();logger.debug('realtime_output 1');logger.debug(B);A=Popen(f"git config push.default upstream",stdout=PIPE,stderr=subprocess.STDOUT,bufsize=1,universal_newlines=_B,shell=_B);logger.debug('realtime_output 2');logger.debug(B)
def sparta_17116247cd(func):
	def A(json_data,user_obj):
		B=user_obj;A=json_data
		if not sparta_2ce5093461(A,B):return{_A:-1,_D:_K}
		return func(A,B)
	return A
def sparta_1edc5beb8b(func):
	def A(webSocket,json_data,user_obj):
		B=user_obj;A=json_data
		if not sparta_2ce5093461(A,B):return{_A:-1,_D:_K}
		return func(webSocket,A,B)
	return A
def sparta_03febe282a(repo,user_obj):
...
def sparta_81e22ba834(webSocket,json_data,user_obj):
...
def sparta_73e2cbc7f4(json_data,user_obj):
	...
	return{_A:1,'output':'\n'.join(F)}
def sparta_679dcd3e40(json_data,user_obj):
	F=user_obj;B=json_data;logger.debug('sqEditorGitAddRemoteOrigin json_data');logger.debug(B);C=B[_C];G=sparta_7a90e5aa73(C)
	if G:
		H=B['remoteUrl'];I=B['remoteName'];A=git.Repo(C);sparta_03febe282a(A,F);J=A.create_remote(I,url=H);A=git.Repo(C);sparta_03febe282a(A,F)
		for D in A.remotes:D.fetch()
		for D in A.remotes:
			if J==D:
				E=D.refs
				if len(E)>0:K=E[len(E)-1];L=os.path.dirname(os.path.realpath(__file__));os.chdir(C);sparta_c79580ed26(K);os.chdir(L)
	return{_A:1}
def sparta_e47120834f(json_data,user_obj):
	A=json_data;logger.debug('git_load_available_track_remote json_data');logger.debug(A);B=A[_C];F=sparta_7a90e5aa73(B)
	if F:
		G=A[_H];C=git.Repo(B);sparta_03febe282a(C,user_obj);D=[]
		for E in C.remotes:
			if G==E.config_reader.get(_J):
				for H in E.refs:D.append({_I:H.name})
	return{_A:1,'available_branches':D}
def sparta_7f743e20b8(json_data,user_obj):
	A=json_data;logger.debug('*******************************************');logger.debug('git_set_track_remote json_data');logger.debug(A);B=A[_C];D=sparta_7a90e5aa73(B)
	if D:G=A[_H];E=A['remoteBranchToTrack'];C=git.Repo(B);sparta_03febe282a(C,user_obj);H=C.head.ref.name;F=os.path.dirname(os.path.realpath(__file__));os.chdir(B);sparta_c79580ed26(E);os.chdir(F)
	return{_A:1}
@sparta_17116247cd
def sparta_a72c5d81fa(json_data,user_obj):return{_A:1}
def sparta_79810c64ac(json_data,user_obj):
	B=json_data[_C];C=sparta_7a90e5aa73(B)
	if C:A=git.Repo(B);sparta_03febe282a(A,user_obj);D=A.active_branch;E=D.tracking_branch().remote_name;F=A.remote(name=E);F.pull(allow_unrelated_histories=_B)
	return{_A:1}
def sparta_e9ef2c20fc(json_data,user_obj):
	B=json_data[_C];C=sparta_7a90e5aa73(B)
	if C:A=git.Repo(B);sparta_03febe282a(A,user_obj);D=A.active_branch;E=D.tracking_branch().remote_name;F=A.remote(name=f"{E}");F.push()
	return{_A:1}
def sparta_d53a8ad12f(json_data,user_obj):
	A=json_data[_C];C=sparta_7a90e5aa73(A)
	if C:
		B=git.Repo(A);sparta_03febe282a(B,user_obj)
		for D in B.remotes:D.fetch()
	return{_A:1}
def sparta_6a15599a6c(json_data,user_obj):A=json_data[_C];B=sparta_7a90e5aa73(A);return{_A:1,_L:B}
def sparta_16de10f5ef(json_data,user_obj):
	...
	def B(folder_path):
		j='remotes_arr';i='commits_ahead_arr';h='commits_behind_arr';g='is_ahead';f='is_behind';W='branch';H=folder_path;P=sparta_7a90e5aa73(H);logger.debug(f"is_git_repository > {H}");logger.debug(P)
		if P:
			A=git.Repo(H);F=[];Z=[]
...
def sparta_7a823bc102(json_data,user_obj):
	H='path';G='file';C=json_data[_C];I=sparta_7a90e5aa73(C)
	if I:
		B=git.Repo(C);sparta_03febe282a(B,user_obj);D=[]
	...
	return{_A:1}
def sparta_4d7fbf17b5(json_data,user_obj):
	B=json_data;C=B[_C];D=B['gitMsg'];E=sparta_7a90e5aa73(C)
	if E:A=git.Repo(C);sparta_03febe282a(A,user_obj);A.git.add(all=_B);A.git.commit('-m',D)
	return{_A:1}
@sparta_17116247cd
def sparta_fa7c9906b6(json_data,user_obj):return{_A:1}
def sparta_86ca5b6c91(json_data,user_obj):
	A=json_data;logger.debug('Delete Remoete');logger.debug(A);C=A[_C];E=sparta_7a90e5aa73(C)
	if E:
		B=git.Repo(C);sparta_03febe282a(B,user_obj);F=A[_H]
		for D in B.remotes:
			if F==D.config_reader.get(_J):B.delete_remote(D);break
	return{_A:1}
def sparta_fea9c4d6c5(json_data,user_obj):return{_A:1}
def sparta_b26c7420d3(json_data,user_obj):
	D=json_data;E=D[_C];C=D['newBranchName'];F=sparta_7a90e5aa73(E)
	if F:
		A=git.Repo(E);sparta_03febe282a(A,user_obj);B=[A for A in A.branches if A.name==C]
	...
	return{_A:-1,_D:_G}
def sparta_856aea0335(json_data,user_obj):
	A=json_data;B=A[_C];D=A['branch2Checkout'];E=sparta_7a90e5aa73(B)
	if E:
		C=git.Repo(B);sparta_03febe282a(C,user_obj)
		try:C.git.checkout(D);return{_A:1}
		except Exception as F:return{_A:-1,_D:str(F)}
	return{_A:-1,_D:_G}
def sparta_c8eb6da8b4(json_data,user_obj):
	B=json_data;C=B[_C];D=B['branch2Merge'];F=sparta_7a90e5aa73(C)
	if F:
		A=git.Repo(C);sparta_03febe282a(A,user_obj);E=A.head.ref.name
	...
	return{_A:-1,_D:_G}
def sparta_b27cab0939(json_data,user_obj):
	D=json_data;A=D[_C];B=D['branch2Delete'];F=sparta_7a90e5aa73(A)
	if F:
		E=git.Repo(A);sparta_03febe282a(E,user_obj);G=E.head.ref.name
	...
	return{_A:-1,_D:_G}
def sparta_bb7b3f2029(json_data,user_obj):
	F='diff_output';A=json_data;logger.debug('sqEditorGitLoadFilesDiff');logger.debug(A);B=A[_C];G=A['filePath'];H=A['fileType'];C=sparta_7a90e5aa73(B);logger.debug(_L);logger.debug(C)
	if C:D=git.Repo(B);sparta_03febe282a(D,user_obj);E=D.git.diff();logger.debug(F);logger.debug(E);return{_A:1,F:E}
	return{_A:-1,_D:_G}