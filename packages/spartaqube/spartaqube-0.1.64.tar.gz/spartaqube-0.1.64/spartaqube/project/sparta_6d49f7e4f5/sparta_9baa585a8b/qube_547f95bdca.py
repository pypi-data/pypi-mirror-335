...
def sparta_5a8c89e200(json_data,userObj):
	B=json_data[_C]
	if int(B)==1:
	...
	try:G,H=sparta_e22d9cc54f(A,B);D={_A:1,_D:H,_E:G,_B:A}
	except:D={_A:-1,_F:_G}
	return D
def sparta_6f164eb1fd(json_data,userObj):
	A=json_data
	try:D=A[_B];E=A[_C];B=os.path.dirname(D);F,G=sparta_e22d9cc54f(B,E);C={_A:1,_D:G,_E:F,_B:B}
	except:C={_A:-1,_F:_G}
	return C
def sparta_e7c833aab4(json_data,userObj):
	A=json_data
	try:B=A[_B];D=A[_C];E,F=sparta_e22d9cc54f(B,D);C={_A:1,_D:F,_E:E,_B:B}
	except:C={_A:-1,_F:_G}
	return C
def sparta_e22d9cc54f(currentPath,typeexplorer=None):
...
def sparta_9f9baf431b(currentPath):
