...
def sparta_f93092ce0d():C='/';B='\\';D=os.path.dirname(os.path.abspath(__file__)).replace(B,C);A=os.path.dirname(D).replace(B,C);A=os.path.dirname(A).replace(B,C);A=os.path.dirname(A).replace(B,C);sys.path.append(A);os.environ.setdefault('DJANGO_SETTINGS_MODULE','spartaqube_app.settings');os.environ['DJANGO_ALLOW_ASYNC_UNSAFE']='true';django.setup()
def sparta_af70265ffb():
	...
	for B in F:
		if B.slug is None:
			...
			B.slug=A;B.save()
if __name__=='__main__':sparta_f93092ce0d();qube_a1945e1861()