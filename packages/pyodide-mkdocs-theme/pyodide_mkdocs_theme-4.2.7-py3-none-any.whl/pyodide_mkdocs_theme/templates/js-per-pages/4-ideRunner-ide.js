...
class IdeHistoryManager extends IdeAceManager {
  ...
  openHistoryModal(){
    ...
    if(!this.validations.length || $('#'+histId).length) return;
    ...
    // Force focus once mounted: this activates focusout, in case the user never enters the panel.
    jHist.appendTo(this.global.find('button[btn_kind=check]')).trigger('focus')
  }
    ...
    this.global.find("button").each(function(){
      ...
      switch(kind){
        ...
        case 'check':     ideThis.updateValidationBtnColor()
                          ...
                          callback = ideThis.validateFactory() ; break
