...
class ClangReplConfig:
    ...
    def get_c_include():
        if ClangReplConfig.USER_DEFINED_BIN_PATH is not None:
            return os.path.join(ClangReplConfig.USER_DEFINED_BIN_PATH,'..', ClangReplConfig.get_platform_path(), 'include')
        else:
    ...
    def get_dynlib_dir():
        ...
        if ClangReplConfig.PLATFORM_NAME_ENUM == PlatformPath.Platform.Linux:
            abs_path.append(
                os.path.join('/lib',
                             PlatformPath.PATH[ClangReplConfig.PLATFORM_NAME_ENUM.value][ClangReplConfig.PLATFORM_BIT.value]))
