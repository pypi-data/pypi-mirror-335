...
def autoplay(seed, fig, num_players, policy, log=False):
    g = initgame(str(uuid4()), fig, num_players, seed)
    s = getstate(g)
    try:
...
def initgame(uuid, fig, num_players, seed, actions=[]):
    rng = getrng(seed)

    if getsettingvalue(fig, 'action_route_discard'):
        initial_to_play = list(range(1, s.game.num_players + 1))
    else:
        initial_to_play = [getfirstplayeridx(rng, num_players)]
