...
class Game(PClass):
    ...
    def __todict__(self):
        return {
            ...
            "seed": self.seed,
            "actions": [x.__todict__() for x in self.actions],
        }
...
def autoplay(seed, fig, num_players, policy, log=False):
    g = Game(uuid=str(uuid4()), fig=fig, num_players=num_players, seed=seed, rng=getrng(seed), actions=[])
    s = getstate(g)
    try:
...
def getinitialstate(g, f):
    route_deck = shuffledeck(getnumroutecards(f), g.rng)
    unit_deck = shuffledeck(gettotaldeckcards(f), g.rng)
...
@dispatch(Game)
def getstate(g):
    return getstate(g, False)


@dispatch(Game, bool)
def getstate(g, log):
    return getstate(g, g.actions, log)


@dispatch(Game, list, bool)
def getstate(g, actions, log):
    return reduce(lambda state, action: getnextstate(state, action, log), actions, getinitialstate(g))
...
def getnextaction(s, policy):
    ...
    action_spec = legal_actions[s.game.rng.randint(0, len(legal_actions) - 1)]
    ...
    if action_spec.action_name == "CLAIM_POINT":
        point = action_spec.points[s.game.rng.randint(0, len(action_spec.points) - 1)]
        return AltAction(
...
def getfirstplayeridx(g):
    return g.rng.randint(0, g.num_players-1)
