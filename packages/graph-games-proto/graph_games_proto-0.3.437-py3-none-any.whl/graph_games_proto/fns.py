...
class FrozenLinkPath(PClass):
    ...
    def __json__(self):
        return {
            ...
            "segments": [x.__json__() for x in self.segments],
        }
...
class FrozenPath:
    ...
    def __json__(self):
        return {
            ...
            "path": self.path.__json__(),
        }


@dataclass(frozen=True)
class FrozenRouteDataclass:
    num: int
    uuid: UUID
    point_a_uuid: UUID
    point_b_uuid: UUID
...
class FrozenBoardConfigDataclass:
    ...
    routes: List[FrozenRouteDataclass]
...
class FrozenBoardConfig(PClass):
    ...
    def __json__(self):
        return {
            "routes": [x.__json__() for x in self.routes],
            "deck_units": [x.__json__() for x in self.deck_units],
            "settings": [x.__json__() for x in self.settings],
            "points": [x.__json__() for x in self.points],
            "clusters": [x.__json__() for x in self.clusters],
            "board_paths": [x.__json__() for x in self.board_paths],
        }
...
class Fig(PClass):
    ...
    def __json__(self):
        return {
            ...
            "board_config": self.board_config.__json__(),
        }
...
class Game(PClass):
    ...
    def __json__(self):
        return {
            ...
            "fig": self.fig.__json__(),
            "seed": self.seed,
            "actions": [x.__json__() for x in self.actions],
        }
...
class PlayerInfo(PClass):
    ...
    def __json__(self):
        return {
            ...
            "final_score": self.final_score.__json__() if self.final_score else None,
        }
...
class State(PClass):
    ...
    def __json__(self):
        return {
            "game_idx": self.game.uuid,
            "action_history": [x.__json__() for x in self.action_history],
            "to_play": self.game.num_players,
            "num_route_cards": len(self.route_cards),
            "num_unit_cards": len(self.unit_cards),
            "faceup_spots": list(self.faceup_spots),
            "most_clusters_player_idxs": self.most_clusters_player_idxs,
            "num_route_discards": len(self.route_discards),
            "num_unit_discards": len(self.unit_discards),
            "player_hands": [x.__json__() for x in self.player_hands],
...
class PlayerScore(PClass):
    ...
    def __json__(self):
        return {
            "breakdown": [x.__json__() for x in self.breakdown], 
            "total": self.total,
        }
...
class PublicPlayerInfo(PClass):
    ...
    def __json__(self):
        return {
            "": self.final_score.__json__() if self.final_score else None,
            "longest_road": self.longest_road,
            "longest_road_len": self.longest_road_len,
            "num_pieces": self.num_pieces,
            ...
            "paths": self.paths,
            "route_statuses": [x.__json__() for x in self.route_statuses],
            "score": self.score,
        }
...
class PublicState(PClass):
    ...
    def __json__(self):
        return {
            ...
            "winners": self.winners,
            "terminal": self.terminal,
            "captured_points": [x.__json__() for x in self.captured_points],
            "captured_segments": [x.__json__() for x in self.captured_segments],
        }
    # fig::Fig
    # captured_segments::Vector{CapturedSegment}
    # captured_points::Vector{CapturedPoint}
...
def autoplay(fig, num_players, policy, log=False):
    g = Game(uuid=str(uuid4()), fig=fig, num_players=num_players, seed=random.randint(0, 2**32-1), actions=[])
    s = getstate(g)
    try:
...
def getnextstate(s, action):
    ...
    if next.last_to_play == action.player_idx:
        ...
        terminal_two_or_less_pieces = getsettingvalue(s.game.fig, 'terminal_two_or_less_pieces')
        terminal_first_cluster = getsettingvalue(s.game.fig, 'terminal_first_cluster')
        terminal_no_available_points = getsettingvalue(s.game.fig, 'terminal_no_available_points')
        if terminal_two_or_less_pieces and player_hand.num_pieces <= 2:
            ...
            and len(getunavailablepoints(next)) == len(next.game.fig.board_config.points)
        ):
            next = next.set(last_to_play=action.player_idx, terminal=True)
            next = calcwinners(next)
...
def getpublicscore(s, player_idx):
    ...
    if getsettingvalue(s, 'path_scoring'):
        ...
        if not path_lens:
            return 0
        addends.append(sum(s.game.fig.path_scores[len] for len in path_lens))
    
    if getsettingvalue(s, 'cluster_scoring'):
        clusters = s.game.fig.board_config.clusters
...
def getprivatescore(s, hand):
    ...
    # Path scores
    if getsettingvalue(s, 'path_scoring'):
        path_scores = s.game.fig.path_scores
        for len in getplayerpathlens(s, player_idx):
            breakdown.append(ScoreItem(
                code_idx=getscorecodeidx(s.game.fig, 'PATH'),
                amount=path_scores[len],
            ))

    # Bonus: most clusters
    if getsettingvalue(s, 'most_clusters_bonus'):
        bonus_most_clusters_score = getsettingvalue(s.game.fig, 'bonus_most_clusters_score')
        if player_idx in s.most_clusters_player_idxs:
            breakdown.append(ScoreItem(
                code_idx=getscorecodeidx(s.game.fig, 'MOST_CLUSTERS'),
                amount=bonus_most_clusters_score,
            ))

    # Longest road
    if not getsettingvalue(s, 'disable_longest_path_bonus'):
        longest_path_score = getsettingvalue(s.game.fig, 'longest_path_score')
        if player_idx in s.longest_road_player_idxs:
            breakdown.append(ScoreItem(
                code_idx=getscorecodeidx(s.game.fig, 'LONGEST_ROAD'),
                amount=longest_path_score,
            ))

    # Completed routes
    if getsettingvalue(s, 'route_scoring'):
        routes = s.game.fig.board_config.routes
        for route_idx in hand.route_cards:
            route_score = routes[route_idx].score
            amount = route_score if route_idx in hand.completed_routes else -1 * route_score
            breakdown.append(ScoreItem(
                code_idx=getscorecodeidx(s.game.fig, 'ROUTE'),
                amount=amount
            ))
    ...
    if getsettingvalue(s, 'cluster_scoring'):
        ...
        if cluster_scores:
            breakdown.append(ScoreItem(
                code_idx=getscorecodeidx(s.game.fig, 'CLUSTER'),
                amount=sum(cluster_scores)
            ))
...
def assertunitcardsaccountedfor(s):
    total_num_unit_cards = gettotaldeckcards(s.game.fig)
    total_found = getunitcardstotalfound(s)
...
def assertroutecardsaccountedfor(s):
    total_num_route_cards = getnumroutecards(s.game.fig)
...
def getcapturedsegments(s):
    ...
    board_paths = s.game.fig.board_config.board_paths
    captured_segments = []
    for player_num, player_hand in enumerate(public_player_hands):
        for path_num in player_hand.paths:
            link_path = board_paths[path_num].path
