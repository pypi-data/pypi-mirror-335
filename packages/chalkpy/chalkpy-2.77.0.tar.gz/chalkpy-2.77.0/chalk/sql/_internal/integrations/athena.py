...
class AthenaSourceImpl(BaseSQLSource):
    ...
    def __init__(
        ...
        *,
        ...
        self.aws_region = aws_region or load_integration_variable(integration_name=name, name=_ATHENA_AWS_REGION_NAME)
        self.aws_access_key_id = aws_access_key_id or load_integration_variable(
            integration_name=name, name=_ATHENA_AWS_ACCESS_KEY_ID_NAME
        )
        self.aws_access_key_secret = aws_access_key_secret or load_integration_variable(
            integration_name=name, name=_ATHENA_AWS_ACCESS_KEY_SECRET_NAME
        )
        self.s3_staging_dir = s3_staging_dir or load_integration_variable(
            integration_name=name, name=_ATHENA_S3_STAGING_DIR_NAME
        )
        self.role_arn = role_arn or load_integration_variable(integration_name=name, name=_ATHENA_ROLE_ARN_NAME)
        self.schema_name = schema_name or load_integration_variable(
            integration_name=name, name=_ATHENA_SCHEMA_NAME_NAME
        )
        self.catalog_name = catalog_name or load_integration_variable(
            integration_name=name, name=_ATHENA_CATALOG_NAME_NAME
        )
        self.work_group = work_group or load_integration_variable(integration_name=name, name=_ATHENA_WORK_GROUP_NAME)
