...
class DatabricksSourceImpl(BaseSQLSource):
    ...
    def __init__(
        ...
        del sql
        self.host = host or load_integration_variable(name=_DATABRICKS_HOST_NAME, integration_name=name)
        self.http_path = http_path or load_integration_variable(name=_DATABRICKS_HTTP_PATH_NAME, integration_name=name)
        self.access_token = access_token or load_integration_variable(
            name=_DATABRICKS_TOKEN_NAME, integration_name=name
        )
        self.db = db or load_integration_variable(name=_DATABRICKS_DATABASE_NAME, integration_name=name)
        self.port = (
            int(port)
            if port is not None
            else load_integration_variable(name=_DATABRICKS_PORT_NAME, integration_name=name, parser=int)
        )
