...
class SpannerSourceImpl(BaseSQLSource):
    ...
    def __init__(
        ...
        self.project_id = project or load_integration_variable(integration_name=name, name=_SPANNER_PROJECT_NAME)
        self.instance_id = instance or load_integration_variable(integration_name=name, name=_SPANNER_INSTANCE_NAME)
        self.database_id = database or load_integration_variable(integration_name=name, name=_SPANNER_DATABASE_NAME)
        self.credentials_base64 = credentials_base64 or load_integration_variable(
            integration_name=name, name=_SPANNER_CREDENTIALS_BASE64_NAME
        )
        self.emulator_host = emulator_host or load_integration_variable(
            integration_name=name, name=_SPANNER_EMULATOR_HOST_NAME
        )
