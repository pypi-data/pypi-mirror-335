...
class TrinoSourceImpl(BaseSQLSource, TableIngestMixIn, SQLSourceWithTableIngestProtocol):
    ...
    def __init__(
        ...
        del trino
        self.name = name
        self.host = host or load_integration_variable(integration_name=name, name=_TRINO_HOST_NAME)

        self.port = (
            int(port)
            if port is not None
            else load_integration_variable(integration_name=name, name=_TRINO_PORT_NAME, parser=int)
        )
        self.catalog = catalog or load_integration_variable(integration_name=name, name=_TRINO_CATALOG_NAME)
        self.schema = schema or load_integration_variable(integration_name=name, name=_TRINO_SCHEMA_NAME)
        self.user = user or load_integration_variable(integration_name=name, name=_TRINO_USER_NAME)
        self.password = password or load_integration_variable(integration_name=name, name=_TRINO_PASSWORD_NAME)
