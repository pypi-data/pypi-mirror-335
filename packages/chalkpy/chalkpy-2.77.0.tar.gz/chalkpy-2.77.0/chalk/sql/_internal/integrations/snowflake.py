    ...
    def __init__(
        ...
        *,
        ...
        self.account_identifier = account_identifier or load_integration_variable(
            integration_name=name, name=_SNOWFLAKE_ACCOUNT_ID_NAME
        )
        self.warehouse = warehouse or load_integration_variable(integration_name=name, name=_SNOWFLAKE_WAREHOUSE_NAME)
        self.user = user or load_integration_variable(integration_name=name, name=_SNOWFLAKE_USER_NAME)
        self.password = password or load_integration_variable(integration_name=name, name=_SNOWFLAKE_PASSWORD_NAME)
        self.db = db or load_integration_variable(integration_name=name, name=_SNOWFLAKE_DATABASE_NAME)
        self.schema = schema or load_integration_variable(integration_name=name, name=_SNOWFLAKE_SCHEMA_NAME)
        self.role = role or load_integration_variable(integration_name=name, name=_SNOWFLAKE_ROLE_NAME)
        self.private_key_b64 = private_key_b64 or load_integration_variable(
            integration_name=name, name=_SNOWFLAKE_PRIVATE_KEY_B64_NAME
        )
