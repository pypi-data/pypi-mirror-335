...
class MySQLSourceImpl(BaseSQLSource, TableIngestMixIn, SQLSourceWithTableIngestProtocol):
    def __init__(
        ...
        del pymysql
        self.name = name
        self.host = host or load_integration_variable(name=_MYSQL_HOST_NAME, integration_name=name)
        self.port = (
            int(port)
            if port is not None
            else load_integration_variable(name=_MYSQL_TCP_PORT_NAME, integration_name=name, parser=int)
        )
        self.db = db or load_integration_variable(name=_MYSQL_DATABASE_NAME, integration_name=name)
        self.user = user or load_integration_variable(name=_MYSQL_USER_NAME, integration_name=name)
        self.password = password or load_integration_variable(name=_MYSQL_PWD_NAME, integration_name=name)
