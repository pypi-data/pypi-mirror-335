        ...
        *,
        ...
        super(KafkaSource, self).__init__(
            ...
            or load_integration_variable(
                name=_KAFKA_BOOTSTRAP_SERVER_NAME, integration_name=name, parser=comma_whitespace_split
            ),
            topic=topic or load_integration_variable(name=_KAFKA_TOPIC_NAME, integration_name=name),
            ssl_keystore_location=ssl_keystore_location
            or load_integration_variable(name=_KAFKA_SSL_KEYSTORE_LOCATION_NAME, integration_name=name),
            client_id_prefix=client_id_prefix
            or load_integration_variable(name=_KAFKA_CLIENT_ID_PREFIX_NAME, integration_name=name)
            or KafkaSource.__fields__["client_id_prefix"].default,
            group_id_prefix=group_id_prefix
            or load_integration_variable(name=_KAFKA_GROUP_ID_PREFIX_NAME, integration_name=name)
            or KafkaSource.__fields__["group_id_prefix"].default,
            security_protocol=security_protocol
            or load_integration_variable(name=_KAFKA_SECURITY_PROTOCOL_NAME, integration_name=name)
            or KafkaSource.__fields__["security_protocol"].default,
            sasl_mechanism=sasl_mechanism
            or load_integration_variable(name=_KAFKA_SASL_MECHANISM_NAME, integration_name=name)
            or KafkaSource.__fields__["sasl_mechanism"].default,
            sasl_username=sasl_username
            or load_integration_variable(name=_KAFKA_SASL_USERNAME_NAME, integration_name=name),
            sasl_password=sasl_password
            or load_integration_variable(name=_KAFKA_SASL_PASSWORD_NAME, integration_name=name),
            name=name,
            late_arrival_deadline=late_arrival_deadline,
            dead_letter_queue_topic=dead_letter_queue_topic,
            ssl_ca_file=ssl_ca_file or load_integration_variable(name=_KAFKA_SSL_CA_FILE_NAME, integration_name=name),
        )
