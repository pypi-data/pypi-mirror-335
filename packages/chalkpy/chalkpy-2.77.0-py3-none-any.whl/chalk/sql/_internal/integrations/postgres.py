...
class PostgreSQLSourceImpl(BaseSQLSource, TableIngestMixIn, SQLSourceWithTableIngestProtocol):
    ...
    def __init__(
        ...
        self.name = name
        self.host = host or load_integration_variable(integration_name=name, name=_PGHOST_NAME)
        self.port = (
            int(port)
            if port is not None
            else load_integration_variable(integration_name=name, name=_PGPORT_NAME, parser=int)
        )
        self.db = db or load_integration_variable(integration_name=name, name=_PGDATABASE_NAME)
        self.user = user or load_integration_variable(integration_name=name, name=_PGUSER_NAME)
        self.password = password or load_integration_variable(integration_name=name, name=_PGPASSWORD_NAME)
