...
class RedshiftSourceImpl(BaseSQLSource):
    ...
    def __init__(
        ...
        self.host = host or load_integration_variable(name=_REDSHIFT_HOST_NAME, integration_name=name)
        self.db = db or load_integration_variable(name=_REDSHIFT_DB_NAME, integration_name=name)
        self.user = user or load_integration_variable(name=_REDSHIFT_USER_NAME, integration_name=name)
        self.password = password or load_integration_variable(name=_REDSHIFT_PASSWORD_NAME, integration_name=name)
        self.unload_iam_role = unload_iam_role or load_integration_variable(
            name=_REDSHIFT_UNLOAD_IAM_ROLE_NAME, integration_name=name
        )
        self.port = (
            int(port)
            if port is not None
            else load_integration_variable(name=_REDSHIFT_PORT_NAME, integration_name=name, parser=int)
        )
        ...
        self._s3_bucket = s3_bucket or load_integration_variable(
            name=_REDSHIFT_UNLOAD_S3_BUCKET_NAME, integration_name=name
        )
