...
class ImperativeTestCase(_TestCase):
    ...
    def __str__(self) -> str:
        return f"TestCase {self.__class__.__name__} which has {self.get_tests()} {"tests" if self.get_tests() > 1 else "test"}"
...
class DeclarativeTestCase(_TestCase):
    ...
    def __str__(self) -> str:
        return f"Tester {self.name} which has {self.get_tests()} {"tests" if self.get_tests() != 1 else "test"}"
