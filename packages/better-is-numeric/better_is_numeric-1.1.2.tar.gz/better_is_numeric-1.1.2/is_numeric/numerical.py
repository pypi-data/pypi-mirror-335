...
def is_numeric(string:str, flags:set[str]=None):
    ...
    if NM_ALLOW_LEADING_ZERO in flags:
        regex.write("[0-9]+") # allow one or more digits. must be digits
    else:
        regex.write("([1-9][0-9]*|0)") # allows either: a non-zero digit followed by any (including zero) amount of any digits OR a single zero
    if NM_ALLOW_DECIMALS in flags:
        regex.write("(\\.[0-9]+)?") # allow zero or one instances of: a decimal point followed by one or more digits
