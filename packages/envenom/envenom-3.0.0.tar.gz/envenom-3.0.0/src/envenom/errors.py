...
class FileNotFound(Exception):
    ...
    path: Path
    original_error: FileNotFoundError = field(kw_only=True, compare=False)
...
class FileAccessDenied(Exception):
    ...
    path: Path
    original_error: PermissionError = field(kw_only=True, compare=False)
...
class ConfigurationInvalid[T](ConfigurationError):
    """
    Value for a specific field is invalid.
    """

    value: str
    parser: Parser[T]
    original_error: ValueError = field(kw_only=True, repr=False, compare=False)
    ...
    class From[FT]:
        ...
        def __call__(self, e: ValueError) -> ConfigurationError:
            return ConfigurationInvalid(
                self.var_name, self.var_value, self.parser, original_error=e
            )
...
class ConfigurationFileUnreadable(ConfigurationError):
    ...
    path: Path
    original_error: FileReadError = field(kw_only=True, repr=False)
    ...
    class From:
        ...
        def __call__(self, e: FileReadError) -> ConfigurationError:
            return ConfigurationFileUnreadable(self.var_name, e.path, original_error=e)
