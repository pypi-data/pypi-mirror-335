...
from s1_cns_cli.cli.registry import CodeTypeSubParser, HttpMethod, TIME_FORMAT, SUPPORTED_FRAMEWORKS, \
    ...
    DOWNLOAD_IAC_CACHE_API, PUBLISH_ISSUES_API, InvalidInputException, MissingRequiredFlagsException, \
    SUPPORTED_GIT_PROVIDERS, \
    OUTPUT_FORMAT, SEVERITY_MAP, ScanNotEnabledException, DEFAULT_IAC_CACHE_UPDATE_FREQUENCY, \
    ScanFailedException
...
def evaluate_rego(input_json, rego, cache_directory):
    ...
    try:
        ...
        if result.stderr:
            ...
            raise ScanFailedException(err_msg)
...
def validate_iac_command(args):
    ...
    if args.provider != "" and args.provider not in SUPPORTED_GIT_PROVIDERS:
        raise InvalidInputException(f"Provider not supported. Supported providers: {', '.join(SUPPORTED_GIT_PROVIDERS)}")

    if args.repo_full_name != "" and "/" not in args.repo_full_name:
        raise InvalidInputException("Please provide repository full name(owner/repoName)")

    if args.repo_url:
        for provider in SUPPORTED_GIT_PROVIDERS:
            if provider.lower() in args.repo_url.lower():
                return
        raise InvalidInputException("Git provider not supported yet. Please contact SentinelOne CNS support.")
