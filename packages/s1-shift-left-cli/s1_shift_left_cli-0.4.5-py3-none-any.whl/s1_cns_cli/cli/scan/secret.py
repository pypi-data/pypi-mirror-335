...
from s1_cns_cli.cli.registry import CodeTypeSubParser, BASELINE_FILE, MissingRequiredFlagsException, \
    ...
    LogColors, PUBLISH_ISSUES_API, InvalidInputException, SUPPORTED_GIT_PROVIDERS, OUTPUT_FORMAT, \
...
def secret_pre_evaluation(args):
    ...
    if args.repo_url:
        if all(provider.lower() not in args.repo_url.lower() for provider in SUPPORTED_GIT_PROVIDERS):
            raise InvalidInputException(f"provider not supported yet. Please contact SentinelOne CNS support.")
