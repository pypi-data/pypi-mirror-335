...
$root.dlc.v4.Graph = class Graph {
    ...
    static decode(reader, position) {
        ...
        // QAIS: Identifying IO tensors and creating new node for the same
        $.tensors.forEach(tensor => {
            const isInput = tensor.location === 0;
            const isOutput = tensor.location === 1;
            if (isInput || isOutput) {
                const newNode = new $root.dlc.v4.Node();
...
$root.dlc.v4.TensorData = class TensorData {
    ...
    static decode(reader, position) {
        ...
        $.name = reader.string_(position, 4, null);
        $.bytes = reader.typedArray(position, 6, Uint8Array);
        $.data_index = reader.int64_(position, 8, 0); // QAIS: Pull index from TensorData table
        return $;
    }
};
