...
// QAIS: Import QAIS modules
import qsubgraph from './qais/qsubgraph.js';
import dagreLayout from './qais/dagre-layout.js';
...
grapher.Graph = class {
    ...
    // QAIS: Allow nodes to be set externally
    set nodes(nodes) {
        this._nodes = nodes;
    }
    ...
    // QAIS: Allow edges to be set externally
    set edges(edges) {
        this._edges = edges;
    }
    ...
    // QAIS: Use web worker to run the dagre layout algorithm
    async layout() {
        await dagreLayout(this, this._layout);
        for (const key of this.nodes.keys()) {
            const entry = this.node(key);
            if (this.children(key).length == 0) {
                const node = entry.label;
                ...
                // QAIS: Render markers indicating hidden edges in the subgraph
