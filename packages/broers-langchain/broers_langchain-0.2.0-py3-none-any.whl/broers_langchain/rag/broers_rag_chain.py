...
from .broers_rag_tool import BroersRAGTool
...
class BroersRetrievalQA(Chain):
    ...
    broers_rag_tool: BroersRAGTool = Field(default_factory=BroersRAGTool,
    ...
    def from_llm(
        ...
        **kwargs: Any,
        ...
        broers_rag_tool = BroersRAGTool(vector_store=vector_store,
                                          ...
                                          **kwargs)
        return cls(
            broers_rag_tool=broers_rag_tool,
            callbacks=callbacks,
    ...
    def _call(
        ...
        if self.return_source_documents:
            answer, docs = self.broers_rag_tool.run(
                question,
            ...
            answer = self.broers_rag_tool.run(question, return_only_outputs=True)
    ...
    async def _acall(
        ...
        if self.return_source_documents:
            answer, docs = await self.broers_rag_tool.arun(question, return_only_outputs=False)
            return {self.output_key: answer, 'source_documents': docs}
        else:
            answer = await self.broers_rag_tool.arun(question, return_only_outputs=True)
            return {self.output_key: answer}
