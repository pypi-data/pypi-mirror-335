...
from broers_langchain.retrievers import EnsembleRetriever
from broers_langchain.vectorstores import ElasticKeywordsSearch, Milvus
from broers_langchain.rag.init_retrievers import (
    BaselineVectorRetriever,
    KeywordRetriever,
    MixRetriever,
    SmallerChunksVectorRetriever,
)
from broers_langchain.rag.scoring.ragas_score import RagScore
from broers_langchain.rag.extract_info import extract_title
from broers_langchain.rag.utils import import_by_type, import_class


class BroersRagPipeline:
    ...
    def __init__(self, yaml_path) -> None:
        ...
        if self.params['elasticsearch'].get('extract_key_by_llm', False):
            extract_key_prompt = import_class(f'broers_langchain.rag.prompts.EXTRACT_KEY_PROMPT')
    ...
    def retrieval_and_rerank(self, question, collection_name, max_content=100000):
        ...
        if self.params['post_retrieval']['with_rank'] and len(docs):
            if not hasattr(self, 'ranker'):
                ...
                rerank_object = import_class(f'broers.rag.rerank.{rerank_type}')
    ...
    def question_answering(self):
        ...
        if 'prompt_type' in self.params['generate']:
            ...
            prompt = import_class(f'broers_langchain.rag.prompts.{prompt_type}')
        else:
            prompt = None
...
if __name__ == '__main__':
    ...
    rag = BroersRagPipeline(args.params)
