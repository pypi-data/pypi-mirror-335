...
async def log_token_usage(
    ...
    logging.debug("Calling client.ai.create_completion with args: %s", completion_args)
...
def create_wrapper(wrapped, _, args, kwargs):
    ...
    if stream:
        ...
        logging.debug("Added include_usage to stream_options for accurate token counting in streaming response")
    ...
    if stream:
        ...
        logging.debug("Handling streaming response")
        ...
        logging.debug("Handling non-streaming response: %s", response)
    ...
    class StreamWrapper:
        ...
        def _process_chunk(self, chunk):
            ...
            if hasattr(chunk, 'usage') and chunk.usage and (not chunk.choices or len(chunk.choices) == 0):
                logging.debug(f"Found usage data in final chunk: {chunk.usage}")
        ...
        def _log_usage(self):
            ...
            if self.final_usage:
                ...
                logging.warning("No usage data found in streaming response!")
            ...
            if self.response_id:
                ...
                thread = run_async_in_thread(metering_call())
                logging.debug("Streaming metering thread started: %s", thread)
