        ...
        if capable_tools:
            ...
            try:
                ...
                if tools is not None and len(tools) > 0:
                    ...
                    # Check if messages contain tool results (role: tool)
                    has_tool_results = any(msg.get("role") == "tool" for msg in messages) if messages else False
                    
                    if has_tool_results:
                        # Convert tool results to a format the model can understand
                        adapted_messages = self.parser.convert_toolcalls_results(messages)
                        
                        # Call the model with the adapted messages
                        return super().chat(
                            model=model,
