...
class Analyzer:
    ...
    def login(        
        ...
        :rtype: None
        """
        status_code = self.__client._login(user=user, account=account, endpoint=endpoint, 
                                           endpoint_path=endpoint_path, role=role, snowflake_account_url=snowflake_account_url, 
                                           lifetime=lifetime, renewal_delay=renewal_delay, attempts=attempts, verbose=verbose)
        if status_code==200:
