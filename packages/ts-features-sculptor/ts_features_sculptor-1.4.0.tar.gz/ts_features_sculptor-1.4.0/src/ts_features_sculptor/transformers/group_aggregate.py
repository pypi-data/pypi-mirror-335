...
class GroupAggregate(BaseEstimator, TransformerMixin, TimeValidator):
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        for func, func_name in zip(self._agg_funcs, self._agg_names):
            cum_agg = enriched_df[self.feature_col] \
                .expanding(min_periods=1).agg(func)
            new_column_name = (
                f"{self.out_feature_prefix}_{self.feature_col.lower()}_"
                f"{func_name.lower()}"
            )
            enriched_df[new_column_name] = cum_agg.shift(1)
