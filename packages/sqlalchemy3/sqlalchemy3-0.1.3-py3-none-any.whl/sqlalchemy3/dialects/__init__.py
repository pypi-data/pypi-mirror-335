...
def _auto_fn(name: str) -> Optional[Callable[[], Type[Dialect]]]:
    ...
    try:
        if dialect == "mariadb":
            ...
            # look much nicer.
            module = __import__(
                "sqlalchemy3.dialects.mysql.mariadb"
            ).dialects.mysql.mariadb
            return module.loader(driver)  # type: ignore
        else:
            module = __import__("sqlalchemy3.dialects.%s" % (dialect,)).dialects
...
registry = util.PluginLoader("sqlalchemy3.dialects", auto_fn=_auto_fn)

plugins = util.PluginLoader("sqlalchemy3.plugins")
