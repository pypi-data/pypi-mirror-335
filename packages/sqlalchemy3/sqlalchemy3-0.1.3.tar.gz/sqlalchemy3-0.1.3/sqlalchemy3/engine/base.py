...
class Connection(ConnectionEventsTarget, inspection.Inspectable["Inspector"]):
    ...
    def execute(
        ...
        *,
        ...
        """
        执行SQL语句构造并返回engine.CursorResult。

        :param statement: 要执行的语句。这总是一个同时存在于_expression中的对象。子句element和_expression可执行的层次结构，包括：
        ...
        :param parameters: 参数，这些参数将绑定到语句中。
            这可以是一个参数名称到值的字典，也可以是一个可变的字典序列（例如列表）。
            当传递字典列表时，底层语句执行将使用cursor.executemany()方法。
            当传递单个字典时，将使用cursor.execute()方法。
        :param execution_options: 执行选项的可选字典，它将与语句执行相关联。
            该字典可以提供_engine.Connection.execution_options接受的选项子集。
        :return: _engine.Result对象
        """
        distilled_parameters = _distill_params_20(parameters)
        try:
