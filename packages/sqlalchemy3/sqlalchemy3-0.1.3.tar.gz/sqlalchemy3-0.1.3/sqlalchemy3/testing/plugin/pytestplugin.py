...
def pytest_addoption(parser):
    group = parser.getgroup("sqlalchemy3")
...
def _log_sqlalchemy_info(session):
    import sqlalchemy
    from sqlalchemy3 import __version__
    from sqlalchemy3.util import has_compiled_ext
    from sqlalchemy3.util._has_cython import _CYEXTENSION_MSG
    ...
    if has_compiled_ext():
        from sqlalchemy3.engine import _util_cy
...
def pytest_sessionstart(session):
    from sqlalchemy3.testing import asyncio
...
class XDistHooks:
    def pytest_configure_node(self, node):
        from sqlalchemy3.testing import provision
        from sqlalchemy3.testing import asyncio
...
def pytest_pycollect_makeitem(collector, name, obj):
    if inspect.isclass(obj) and plugin_base.want_class(name, obj):
        from sqlalchemy3.testing import config
...
def _pytest_fn_decorator(target):
    ...
    from sqlalchemy3.util.langhelpers import format_argspec_plus
    from sqlalchemy3.util.compat import inspect_getfullargspec
...
class PytestFixtureFunctions(plugin_base.FixtureFunctions):
    ...
    def combinations(self, *arg_sets, **kw):
        ...
        """
        from sqlalchemy3.testing import exclusions
