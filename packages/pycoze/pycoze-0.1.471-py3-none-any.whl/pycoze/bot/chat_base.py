...
def mark_json_error(e, content):
    """标记JSON解析错误的位置
    Args:
        e: JSONDecodeError异常
        content: 原始JSON内容
    Returns:
        标记错误位置的字符串，并用 **ERROR** 标注错误位置
    """
    # 确保上下文长度至少为前后各50个字符
    error_start = max(0, e.pos - 50)
    error_end = min(len(content), e.pos + 50)
    error_context = content[error_start:error_end]
    
    # 计算错误在上下文中的位置
    error_pos_in_context = e.pos - error_start
    
    # 确保错误位置在上下文中
    if error_pos_in_context < 0 or error_pos_in_context >= len(error_context):
        # 如果错误位置超出上下文范围，返回整个上下文并标记错误位置
        return (
            f"Error position is at the end of the context:\n"
            f"{error_context}**ERROR_HERE**"
        )
    
    # 标记错误位置
    marked_error = (
        error_context[:error_pos_in_context] +
        '**ERROR_HERE**' + error_context[error_pos_in_context] + '**ERROR_HERE**' +
        error_context[error_pos_in_context + 1:]
    )
            ...
            if len(response) == 2:
                if (
                    ...
                    try:
                        ...
                        # 使用函数标记错误位置
                        marked_error = mark_json_error(e, cleaned_content)
                        json_err =  f"JSON parsing error\n\nError type: {type(e).__name__}\nError message: {e.msg}\nPosition: line {e.lineno}, column {e.colno}\n\nError context:\n{marked_error}\n\nPlease think and check and correct the JSON format, then try again."
                        conversation_history.append(
                            {
                                "role": "user",
                                "content": json_err,
                    ...
                    conversation_history.append(
                        {"role": "system", "content": "[Tool Result Begin]\n"+ result+ "\n[Tool Result End]"}
                    )
