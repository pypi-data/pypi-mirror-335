...
class IndexCache:
    ...
    async def _get_index(self, select_file: Path) -> VectorStoreIndex:
        ...
        chunked_documents = convert_to_llama_docs(text_splitter.split_documents(langchain_docs))
...
class FolderDataSource(DataSourceBase):
    ...
    async def select_file(self, query: Query) -> Optional[Path]:
        ...
        if cache_key in self._file_selection_cache:
            logger.trace("Using cached file selection for company: {}, date: {}", query.company, query.date)
        ...
        logger.trace("Cached file selection for company: {}, date: {}", query.company, query.date)
