...
async def generate_search_keywords(query: Query) -> List[str]:
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", """You are a search expert. Given a query about a company's financial information, 
...
async def evaluate_search_results(query: Query, candidates: List[Entity], search_keywords: List[str]) -> Tuple[bool, Optional[str]]:
    ...
    """
    prompt = ChatPromptTemplate.from_messages([
        ("system", """You are a financial information evaluator. Given search results and the original query,
...
async def summarize_results(query: Query, candidates: List[Entity]) -> Tuple[Entity, Entity]:
    ...
    """
    prompt = ChatPromptTemplate.from_messages([
        ("system", """You are a compay's financial report information summarizer. Given multiple search results,
    ...
    return (
        Entity(value=summary["value"], reference=ref_text),
        reference_entity
    )
...
async def search_entity(
    ...
    try:
        ...
        while attempt < max_attempts:
            logger.info(f"Search attempt {attempt + 1} with keywords: {search_keywords}")
            ...
            # Evaluate results
            is_sufficient, new_keywords = await evaluate_search_results(query, all_candidates, search_keywords)
            
            if is_sufficient:
                # Summarize the results to get the best answer
                result, reference_entity = await summarize_results(query, all_candidates)
                logger.info(
