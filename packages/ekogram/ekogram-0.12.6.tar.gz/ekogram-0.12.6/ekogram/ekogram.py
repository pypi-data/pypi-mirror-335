...
class Location:
    ...
    def __repr__(self) -> str:
        ...
        return (f"Location(latitude={self.latitude}, longitude={self.longitude}, "
                f"horizontal_accuracy={self.horizontal_accuracy}, live_period={self.live_period}, "
                f"heading={self.heading}, proximity_alert_radius={self.proximity_alert_radius})")
...
class Bot:
    ...
    def reply_media_group(self, chat_id: Union[int, str] = None, media: list = None, disable_notification: bool = None) -> Optional[list]:
        ...
        if chat_id is None:
            raise ValueError("chat_id не должен быть None")
        if media is None or not isinstance(media, list):
            raise ValueError("media должен быть непустым списком объектов InputMedia")
