...
#  @brief   This file contains all the functions for MCP2515 communicaton
...
#class Mcp2515(implements(CanInterface)):
class Mcp2515:
    def __init__(self, channel = 'can0', bustype = 'socketcan', baudrate = CanBusBaudRate.RATE_1000):
        self._channel = channel
        self._bustype = bustype
        self._baudrate = baudrate.value * 1000
        logging.error(channel + " " + bustype)
    ...
    def canopen_transmit(self, address: int, _object: int,  sub_index: int, informatrion_to_send: list) -> Tuple[bool, Error]:
        ...
        msg = can.Message(
            ...
            data=[0x22,
                _object.to_bytes(2,byteorder='big')[1],  # LSB Object Index
                _object.to_bytes(2,byteorder='big')[0],  # MSB Object Index
                sub_index,                                   # Sub Index
        ...
        if      ( information_received.arbitration_id == (0x580 + address)                   # Check COB-ID
            ...
            and ( information_received.data[1] == _object.to_bytes(2,byteorder='big')[1] )    # Check Object Index(LSB)
            and ( information_received.data[2] == _object.to_bytes(2,byteorder='big')[0] )):  # Check Object Index(MSB)
