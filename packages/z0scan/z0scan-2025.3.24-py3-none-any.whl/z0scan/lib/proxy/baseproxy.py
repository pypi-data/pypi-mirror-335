...
class CAAuth(object):
    ...
    def _gen_ca(self, again=False):
        ...
        self.cert.get_subject().O = 'JiuZero'
        self.cert.get_subject().CN = 'Z0SCAN'
...
class ProxyHandle(BaseHTTPRequestHandler):
        ...
        try:
            ...
            if request:
                ...
                if not self._is_replay() and response:
                    ...
                    req = FakeReq(url, request._headers, method, request.get_body_data().decode('utf-8', 'ignore'))
    ...
    def _proxy_to_ssldst(self):
        ...
        self.proxy_connect()
        # 进行SSL包裹
        self._proxy_sock = SSLContext.wrap_socket(self._proxy_sock)
